...
public abstract class Instance implements IXInstance, IInstance {
    ...
    protected OpdBaseComponent graphicalRepresentation;
    ...
    public Instance(OpdKey pKey, OpdProject project) {
        ...
        project.getSystemStructure().put(this);
        ...
        bypassDistance = (int) Math.round(BYPASS_DISTANCE * factor);
    }

    public OpdBaseComponent getGraphicalRepresentation() {
    ...
    public boolean isMainInstance() {
        if (getOpd().getMainInstance() != null) {
            return getOpd().getMainInstance().getUUID() == this.getUUID();
        } else {
            return false;
        }
    }
