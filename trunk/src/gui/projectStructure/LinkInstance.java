...
import gui.opdGraphics.draggers.*;
...
public class LinkInstance extends Instance implements MoveUpdatable,
    ...
    private ArrayList<LinkInstance> linkedSons = new ArrayList<LinkInstance>();


    public ArrayList<LinkInstance> getLinkedSons() {
        return linkedSons;
    }

    public void setLinkedSons(ArrayList<LinkInstance> linkedSons) {
        this.linkedSons = linkedSons;
    }
    ...
    private void _setBAckGroundColor(Color backgroundColor) {
    ...
    private void _setColorByReasource(double res) {
        if ((this.getEntry().getLogicalEntity() instanceof OpmEffectLink) &&
                (Configuration.getInstance().getProperty("graphics.effect.colorbyresource").equalsIgnoreCase("yes"))) {

            if (res != LinkEntry.DEFAULT_RESOURCE_CONSUMPTION) {

                Color minus = Configuration.getInstance().getColorFromProperty("graphics.effect.minus");
                Color plus = Configuration.getInstance().getColorFromProperty("graphics.effect.plus");

                if (res > 0) {
                    _setBAckGroundColor(plus);
                    _setBorderColor(plus);

                    return;
                }

                if (res < 0) {
                    _setBAckGroundColor(minus);
                    _setBorderColor(minus);
    ...
    public void setColorByResource() {
        LinkEntry entry = (LinkEntry) getEntry();
        double res = entry.getResourceConsumption();
        _setColorByReasource(res);

        //do the same to all instances
        Enumeration<Instance> loopEnum = getEntry().getInstances(true);
        while (loopEnum.hasMoreElements()) {
            LinkInstance loopInstance = (LinkInstance) loopEnum.nextElement();
            loopInstance._setColorByReasource(res);
        }
    ...
    public void _setBorderColor(Color borderColor) {
