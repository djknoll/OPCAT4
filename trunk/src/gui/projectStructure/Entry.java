...
public abstract class Entry implements IXEntry, IEntry {
    ...
    public boolean hasTextRepresentation() {
        return (getTextRepresentation() != null) &&
                !(getTextRepresentation().trim().equalsIgnoreCase("")) &&
                !(getTextRepresentation().trim().equalsIgnoreCase("none"));
    }
    ...
    public Enumeration<Instance> getInstances(boolean includeClipBoard) {
        return getInstancesArray(includeClipBoard).elements();
    }

    public Vector<Instance> getInstancesArray(boolean includeClipBoard) {
    ...
    public ArrayList<Instance> getAllInstanceInOPD(Opd opd) {
        ArrayList<Instance> ret = new ArrayList<Instance>();

        for (Enumeration e = this.getInstances(false); e.hasMoreElements(); ) {
            Instance currInstance = (Instance) e.nextElement();
            if (currInstance.getKey().getOpdId() == opd.getOpdId()) {
                ret.add(currInstance);
            }
        }
        return ret;
    ...
    public void setProperty(String key, String value) {
        ...
        processTextRepresentation(key, value);
    }

    private void processTextRepresentation(String key, String value) {

        if (key == null) return;

        boolean setToValue = (this instanceof ProcessEntry) && key.equalsIgnoreCase(Configuration.getInstance().getProperty("graphics.default.process.tooltip.property"));
        if (!setToValue)
            setToValue = (this instanceof ObjectEntry) && key.equalsIgnoreCase(Configuration.getInstance().getProperty("graphics.default.object.tooltip.property"));
        //if(!setToValue)  setToValue = (this instanceof StateEntry) && key.equalsIgnoreCase(Configuration.getInstance().getProperty("graphics.default.state.tooltip.property"));
        //if(!setToValue)  setToValue = key.equalsIgnoreCase(Configuration.getInstance().getProperty("graphics.default.other.tooltip.property"));

        if (setToValue && (value != null)) setTextRepresentation(value);
     ...
     *
     * @return a NAME with the entity description
     */
    ...
    public void loadProperties(Element properties) {
        ...
        getMyProps()._loadProperties_(properties);
        ...
        for (Object key : this.getMyProps().keySet()) {
            processTextRepresentation(key.toString(), getMyProps().get(key).toString());
        }
    ...
    public void ShowURL() {
        ...
        if (!(this.getUserURL().equalsIgnoreCase("none") || this.getUserURL()
            ...
            String[] urls = this.getUserURL().split(";");
            for (String url : urls) {
    ...
    public void ShowInstances(Vector opmThings) {
        ...
        for (int i = 0; i < opmThings.size(); i++) {
            DataAbstractItem opmEnt = (DataAbstractItem) opmThings.get(i);
    ...
    public String getMetaData(MetaLibrary meta) {

        String data = "";
        String[] colsToShow = Configuration.getInstance().getProperty("graphics.show.metadata.colstoshow").split(",");
        boolean headFromColHead = Boolean.valueOf(Configuration.getInstance().getProperty("graphics.show.metadata.headfromcolumnhead"));
        String[] headOfColsToShow = Configuration.getInstance().getProperty("graphics.show.metadata.headerofcolstoshow").split(",");
        String headSep = Configuration.getInstance().getProperty("graphics.show.metadata.headersep");
        boolean showColomName = Boolean.valueOf(Configuration.getInstance().getProperty("graphics.show.metadata.showheader"));
        ...
        try {
            ...
            while (roleIter.hasNext()) {
                ...
                //if(role.getLibraryId() != meta.getID() ) continue; ;
                DataAbstractItem metaItem = (DataAbstractItem) role.getThing();
                Vector vecData = metaItem.getAllData();
                for (int i = 0; i < colsToShow.length; i++) {
                    String head = "";
                    try {
                        if (showColomName) {
                            if (headFromColHead) {
                                head = meta.getDataProject().getHeaders().get(Integer.valueOf(colsToShow[i])) + headSep;
                            } else {
                                head = vecData.get(Integer.valueOf(headOfColsToShow[i])) + headSep;
                            }
                        }
                    } catch (Exception ex) {
                        OpcatLogger.error(ex, false);
                    }
                    data = (data.trim().equalsIgnoreCase("") ? "" : data + "\n") + head + vecData.get(Integer.valueOf(colsToShow[i])); //+ (i == colsToShow.length - 1 ? "" : "\n");
                }
        ...
        try {
            boolean showtimes = Boolean.valueOf(Configuration.getInstance().getProperty("graphics.show.metadata.showtimes"));
            String timesLabel = Configuration.getInstance().getProperty("graphics.show.metadata.timeslabel");

            if (showtimes) {
                boolean drawMin = false;
                boolean drawMax = false;
                if (getLogicalEntity() instanceof OpmProcess) {
                    OpmProcess p = (OpmProcess) getLogicalEntity();
                    if (!p.getMinTimeActivation().equalsIgnoreCase(OpmProcess.DEFAULT_MIN_TIME_ACTIBVATION))
                        drawMin = true;

                    if (!p.getMaxTimeActivation().equalsIgnoreCase(OpmProcess.DEFAULT_MAX_TIME_ACTIBVATION))
                        drawMax = true;

                    if (drawMax || drawMin) {
                        String maxString = p.getMaxTimeActivationString();
                        String minString = p.getMinTimeActivationString();

                        data = (data.trim().equalsIgnoreCase("") ? "" : data + "\n") + timesLabel + headSep + (drawMax ? "[" + maxString + ":" : " [:") + (drawMin ? minString + "]" : "]");
                    }
                }
            }
    ...
    public Color getMetaColor(MetaLibrary meta) {
        ...
        Color color = Configuration.getInstance().getColorFromProperty("graphics.default.metacolor"); //Color.BLACK;
    ...
    public PlanarImage getRoleIcon() {
        ...
        while (roles.hasMoreElements()) {
            ...
            if (role.getThing() instanceof OpmEntity) {
                ...
                if (role.getOntology().getStructure() != null) {
