...
import extensionTools.Testing.TestingSystem;
...
public class Opcat2 {
    ...
    private final static String opmTypes = FileControl.getInstance().getOPCATDirectory() + fileSeparator + "codeGenerator" + fileSeparator + "opl2Java" + fileSeparator + "opmTypes";
    ...
    /**
     * will not remove the gui just wil not update it.
     *
     * @param show
     */
    public static void setShowHideExtensionTools(boolean show) {
        Opcat2.showHideExtensionTools.setSelected(show);
    }
    ...
    public void minimizeOPLPane() {
        desktopTools.setDividerLocation(desktopTools.getMaximumDividerLocation());
    }
    ...
    private void checkInstalation() throws OpcatException {
        try {
            ...
            OpcatLogger.info("Starting Opcat Session", false);
            OpcatLogger.info("Show gui is " + Configuration.getInstance().getProperty("gui.show").toString(), false);
            OpcatLogger.info("Java home = " + System.getProperty("java.home"), false);
            OpcatLogger.info("Java version = " + System.getProperty("java.version"), false);
            OpcatLogger.info("Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024 + "/Kb", false);
            OpcatLogger.info("Total Java Memory = " + Runtime.getRuntime().totalMemory() / 1024 + "/Kb", false);
            OpcatLogger.info("Opcat Directory = " + FileControl.getInstance().getOPCATDirectory(), false);
            OpcatLogger.info("Requirement feature file path = " + Features.getReqFilePath(), false);
            OpcatLogger.info("Requirement feature exists = " + new Boolean(Features.hasReq()).toString(), false);
            OpcatLogger.info("CSV Export feature file path = " + Features.getCSVExportDefFilePath(), false);
            OpcatLogger.info("CSV Export feature exists = " + new Boolean(Features.hasCSV()).toString(), false);
            OpcatLogger.info("CSV Loading feature file path = " + Features.CSVThingsLoadingPath(), false);
            OpcatLogger.info("CSV Loading feature exists = " + new Boolean(Features.hasCSVThingsLoading()).toString(), false);
            OpcatLogger.info("Desecret feature file path = " + Features.DESecretOptionPath(), false);
            OpcatLogger.info("Desecret feature exists = " + new Boolean(Features.hasDeSecretOption()).toString(), false);


            //SwingUtilities.invokeLater(new Runnable() {
            //    @Override
            //    public void run() {
            OpcatLogger.info("Trying database connection ", false);
            Connection cn = null;
            try {
                cn = OpcatDatabaseConnection.getInstance().getConnection();
            } catch (Exception ex) {
                OpcatLogger.error(ex, false);
            }
            try {
                if ((cn != null) && (!cn.isClosed())) {
                    OpcatLogger.info("Connected to database ", false);
                } else {
                    OpcatLogger.info("Connection to database failed. is database up ?  ", false);
                }
            ...
            Object[] keys = (Object[]) Configuration.getInstance().getProperties().keySet().toArray();
            Arrays.sort(keys);
            for (int i = 0; i < keys.length; i++) {
                try {
                    OpcatLogger.info(keys[i] + "\t=\t" + Configuration.getInstance().getProperty(keys[i].toString()), false);
                } catch (Exception ex) {
    ...
    public Opcat2() {
        ...
        this.newProjectActionA = new NewProjectAction("New...",
                StandardImages.NEW);
        this.loadExampleActionA = new LoadExampleAction("", StandardImages.System_Icon);
        ...
        if (!FileControl.getInstance().isGUIOFF()) {
        ...
        // add OPL Pane
        if (Boolean.valueOf(Configuration.getInstance().getProperty("show_opl"))) {
            this.extensionToolsPane.add("OPL Generator", new JScrollPane(oplPane));
        }

        int high = Integer.valueOf(
                Configuration.getInstance().getProperty(
                        "graphics.default.lowerpanelminhighet")).intValue();
        extensionToolsPane.setMinimumSize(new Dimension(extensionToolsPane.getWidth(), high));
        ...
        KeyListener escapeListener = new KeyAdapter() {
            ...
            public void keyReleased(KeyEvent e) {
                if ((e.getKeyCode() == KeyEvent.VK_ESCAPE) && (currentProject != null) && (currentProject.getCurrentOpd() != null)) {
                    if (!StateMachine.isWaiting()) {
                        StateMachine.reset();
                        currentProject.getCurrentOpd().getOpdContainer().getDrawingArea().setCursor(
    ...
    private JMenuBar _constructMenuBar() {
        ...
        File[] examples = (new File(FileControl.getInstance().getOPCATDirectory() + FileControl.fileSeparator + "Examples" + fileSeparator)).listFiles(new CustomFileFilter("opz"));

        if (examples == null) {
            examples = (new File("Examples" + fileSeparator)).listFiles(new CustomFileFilter("opz"));
        }
        ...
        if (this.configuration != null) {
            try {
                ListIterator<LastFileEntry> lastFiles = this.configuration.getLastUsedFiles();
                ...
                while (lastFiles.hasNext() && (i < 4)) {
                    ...
                    if (last != null) {
                        i++;
                        LoadPrevious loadPrevious = new LoadPrevious(i + "  " + last.getFileNameWithoutPath(),
        ...
        // // {
        // NAME name = themes[k]; // lfs[k].getName();
        ...
        showMessagesConsole.addActionListener(new OpcatMessagesShowConsoleAction(
                "Messages Console",
    ...
    public JToolBarButton getPrintButton() {
        return printButton;
    }
    ...
    private JToolBar _constructStandardToolBar() {
        ...
        JToolBarButton vision = null;
        String address = Configuration.getInstance().getProperty("admin.opcat.VisionName");
        if (address != null) {
            vision = new JToolBarButton(this.openVision, "Vision");
        }
        ...
        if (vision != null) {
            //this.standardToolBar.add(vision);
        }
        this.standardToolBar.add(new JToolBar.Separator());
    ...
    private void _setBorders4ToolBar(JToolBar bar) {
        for (int i = 0; i < bar.getComponentCount(); i++) {
            if (bar.getComponentAtIndex(i) instanceof JToolBarButton) {
                ((JToolBarButton) bar.getComponentAtIndex(i)).setInitialBorder();
            } else if (bar.getComponentAtIndex(i) instanceof JToolBarToggleButton) {
                JToolBarToggleButton tButton = (JToolBarToggleButton) bar.getComponentAtIndex(i);
    ...
    private JToolBar _constructTestingToolBar() {
        ...
        int maxPanelsSize = 14 + (int) Math.max(f.getStringBounds("Forward",
                new FontRenderContext(null, true, false)).getWidth(), f.getStringBounds("Backward",
        ...
        this.testingToolBar.add(new JToolBarButton(this.anPlay, "Play/Continue"));
        this.testingToolBar.add(new JToolBarButton(this.anBackward, "Backward"));
        ...
        this.testingToolBar.add(new JToolBarButton(this.anActivate, "Activate"));
    ...
    private JSplitPane _constructdesktopToolsPane(Component top,
        ...
        if (Boolean.valueOf(Configuration.getInstance().getProperty("show_opl"))) {
            ...
            sp.setDividerLocation((int) (myFrame.getHeight()));
        }
            ...
            Thread runner = new Thread() {
                ...
                public void run() {
                    try {
                        ...
                        String fileName = Opcat2.this.myFileChooser.getSelectedFile().getPath();
                        ...
                        os.write(currentProject.getOplXmlScript().toString().getBytes());
                        os.close();
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                LibrariesLoadingWindow importedLibraries = new LibrariesLoadingWindow(
                        Opcat2.getCurrentProject().getMetaManager(), Opcat2.getCurrentProject(), false, false, Opcat2.getFrame());
    ...
    Action projectMetaLibrariesAction = new AbstractAction("Templates",
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                LibrariesLoadingWindow importedLibraries = new LibrariesLoadingWindow(
                        Opcat2.getCurrentProject().getMetaManager(), Opcat2.getCurrentProject(), false, true, Opcat2.getFrame());
                boolean ret = importedLibraries.showDialog();
                if (ret) {
                    Vector<MetaLibrary> updated = importedLibraries.getUpdatedLibs();
    ...
    Action viewZoomInAction = new AbstractAction("Zoom in",
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            int previousSize = ((Integer) config.getProperty("CurrentSize")).intValue();
            int normalSize = ((Integer) config.getProperty("NormalSize")).intValue();
    ...
    Action showExtensionToolsAction = new AbstractAction("Show OPL",
        ...
        public void actionPerformed(ActionEvent e) {
            if (!showHideExtensionTools.isSelected()) {
                ...
                Opcat2.this.desktopTools.setDividerLocation((int) (myFrame.getHeight() * 0.63));
    ...
    Action go2TestingTEstMoreThenOneAnimationScreen = new AbstractAction(
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Opcat2.this.diagramPanel.add(Opcat2.this.anStatusLabel,
                    BorderLayout.SOUTH);
            MetaLibrary meta = (MetaLibrary) currentProject.getMetaManager().getMetaLibraries().nextElement();
            OpdProject project = (OpdProject) meta.getProjectHolder();
            project.showOPD(((Opd) project.getSystemStructure().getOpds().nextElement()).getOpdId());
    ...
    public Action closeTesting = new AbstractAction("Close",
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            boolean tempLifeSpan = TestingSettings.getInstance().isSHOW_LIFE_SPAN();
            boolean tempREsource = TestingSettings.getInstance().isSHOW_RESOURCE();
            ...
            Opcat2.this.anSystem.getTestingPanel().RemoveFromExtensionToolsPanel();
    ...
    Action openVision = new AbstractAction("Open model in Vision", StandardImages.VISION) {
        ...
        public void actionPerformed(ActionEvent e) {
            if ((currentProject == null) || (currentProject.getPath() == null)) return;
            String address = Configuration.getInstance().getProperty("admin.opcat.VisionName") + ":" + Configuration.getInstance().getProperty("admin.opcat.VisionPort") + "/" + Configuration.getInstance().getProperty("admin.opcat.VisionContext") + "/main.html";
            if (address != null) {
                SVNURL svn = OpcatMCManager.getInstance().getFileURL(new File(currentProject.getPath()));
                if (svn != null) {
                    String user = Configuration.getInstance().getProperty("admin.opcat.VisionUserID");
                    URI url = URI.create("http://" + address + "#userID=" + user + "&entityID=" + svn.getPath().hashCode());
                    try {
                ...
                try {
                    ...
                    if (isWholeProjectCompiled) {
                        ...
                        if (currentOpd == null) {
                            ...
                            s = currentProject.getOPL(true, currentOpd.getOpdId());
                        }
                    }

                    FileOutputStream file = new FileOutputStream(
                            Opcat2.this.myFileChooser.getSelectedFile().getAbsolutePath());
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            Thread runner = new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if (!opmDir.exists()) {
                            ...
                            for (int i = 0; i < files.length; i++) {
                                if (!files[i].getAbsolutePath().endsWith(".svn")) {
                        ...
                        JOptionPane.showMessageDialog(myFrame,
                                "Code Generator Error: I/O error in types copying\\" + e.getMessage(), "Error",
                                JOptionPane.ERROR_MESSAGE);
                    } catch (Exception ex) {
                        JOptionPane.showMessageDialog(
                                myFrame,
                    ...
                    try {
                        engine.loadLanguage(new FileInputStream(FileControl.getInstance().getOPCATDirectory() + fileSeparator + "codeGenerator" + fileSeparator + "opl2Java.xml"));
                        engine.setOutputPREProcessor(new FileInputStream(
                                FileControl.getInstance().getOPCATDirectory() + fileSeparator + "codeGenerator" + fileSeparator + "transform.java.xsl"));
                        engine.translateOPDS(new ByteArrayInputStream(
                                currentProject.getOplXmlScript().toString().getBytes()), selectedDirectory, true,
                                System.out);
                        ...
                        StateMachine.setWaiting(false);
                        Opcat2.this.setCursor4All(Cursor.DEFAULT_CURSOR);
                        JOptionPane.showMessageDialog(myFrame, fnfe.getMessage(), "Error",
                                JOptionPane.ERROR_MESSAGE);
                    } finally {
                        Opcat2.this.hideWaitMessage();
                        StateMachine.setWaiting(false);
...
//            }
//            JPanel panel = null;
//            NAME str = e.getActionCommand();
//
//            ExtensionToolInfo eti = (ExtensionToolInfo) Opcat2.this.extensionsTable.get(str);
//
//            if (eti != null) {
...
//                        panel = ((OpcatExtensionToolX) eti.getExtension()).execute((IXSystem) currentProject);
//                    } else {
//                        panel = ((OpcatExtensionTool) eti.getExtension()).execute((ISystem) currentProject);
//                    }
//                } catch (Exception ex) {
//                    ex.printStackTrace();
//                }
...
//                }
//            } else {
//                JOptionPane.showMessageDialog(myFrame, "No '" + str + "' Extension Tool could be found.", "Opcat2 - Error",
//                        JOptionPane.ERROR_MESSAGE);
//            }
//        }
//    };
...
//                    tp.setBorder(BorderFactory.createEmptyBorder(0, 10,
//                            10, 10));
...
//                            BorderLayout.SOUTH);
//                    Opcat2.this.aboutDlg.setDefaultCloseOperation(WindowConstants.DISPOSE_ON_CLOSE);
//
...
//                    Opcat2.this.aboutDlg.setLocation(fX + Math.abs(pWidth / 2 - Opcat2.this.aboutDlg.getWidth() / 2), fY + Math.abs(pHeight / 2 - Opcat2.this.aboutDlg.getHeight() / 2));
//
...
//    };
    ...
    private void _createSplashScreen() {
        ...
        this.splashLabel = new SplashLabel(copyrightString, MiscImages.SPLASH.getImage());
        ...
        JLabel licenseLabel = new JLabel(Opcat2.getLicense().getLicenseCaption());
    ...
    public static JFrame getFrame() {
        if (myFrame == null) return new JFrame();
        return myFrame;
    }
    ...
    public static void updateStructureChange(int kindOfChange) {
        ...
        if ((currentProject == null) || (kindOfChange == Opcat2.GRAPHICAL_CHANGE) || StateMachine.isAnimated()) {
            return;
        }
            ...
            if (Boolean.valueOf(Configuration.getInstance().getProperty("show_opl"))) {
                ...
                try {
                    if (isWholeProjectCompiled) {
                        ...
                        Opd currentOpd = currentProject.getCurrentOpd();
                        if ((currentOpd == null) || (currentOpd.getOpdId() == OpdProject.CLIPBOARD_ID)) {
                            oplPane.setText(EMPTY_HTML);
                        } else {
                            oplPane.setText(currentProject.getOPL(true, currentOpd.getOpdId()));
                        }
                    }

                    if (oplPane.getText().length() > 0)
                        oplPane.setCaretPosition(Math.min(currentPosition, oplPane.getText().length() - 1));
        ...
        SwingUtilities.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                if (currentProject != null) {
                    for (Enumeration e = currentProject.getComponentsStructure().getOpds(); e.hasMoreElements(); ) {
    ...
    public void handleWaitScreen(JWindow theScreen, String message,
        ...
        theScreen.pack();
        theScreen.setLocation(myFrame.getWidth() / 2 - theScreen.getSize().width / 2, myFrame.getHeight() / 2 - theScreen.getSize().height / 2);
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            com.jidesoft.utils.Lm.verifyLicense("Opcat Systems", "Opcat", "Wc765:2AtS:8FBrjFz2kFhvWhVWwnlv2");
            String look = Configuration.getInstance().getProperty("system.lookandfeel.class").toString();
        ...
        if ((op != null) && (args.length > 0)) {
            ...
            if (args.length == 2) {
                try {
                    ...
                    if (idString.indexOf("uuid") >= 0) {
                        String uuid = args[1].substring(4, args[1].length());
                        id = Integer.parseInt(String.valueOf(op.getFileControl().getCurrentProject().getEntry(UUID.fromString(uuid)).getId()));
                    } else if (idString.indexOf("name") >= 0) {
                        String uuid = args[1].substring(4, args[1].length());
                        id = 0;
                        name = uuid;
                    } else {
                    ...
                    if (id > 0) {
                        entry = op.getFileControl().getCurrentProject().getSystemStructure().getEntry(id);
                    } else {
                        ArrayList<Entry> entries = op.getFileControl().getCurrentProject().getSystemStructure().getEntry(name);
                        if (entries.size() > 0) {
                            entry = entries.get(0);
                        }
                    }
                    if (entry != null) {
                        OpcatExposeShowEntity show = new OpcatExposeShowEntity(
                                "none", null, entry.getInstances(false).nextElement());
    ...
    public void clearValidationWindow() {
        int indexOfValidation = this.extensionToolsPane.indexOfTab(Validator.VALIDATION_TAG_NAME);
    ...
    public void validate() {
        ...
        this.extensionToolsPane.setIconAt(this.extensionToolsPane.indexOfTab(Validator.VALIDATION_TAG_NAME),
    ...
    public void closeCleanUp() {
        ...
        if (currentProject != null) {
            OpcatLogger.info("Before Opcat cleanup, Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024 + "/Kb", false);
            OpcatLogger.info("Before Project close " + currentProject.getName() + ", Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024 + "/Kb", false);
            currentProject.close();
            //System.gc();
            OpcatLogger.info("After Project close, Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024 + "/Kb", false);
        } else {
            OpcatLogger.info("After Opcat cleanup, Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024 + "/Kb", false);
            return;
        }
        ...
        System.gc();
        OpcatLogger.info("After Opcat cleanup, Free Java Memory = " + Runtime.getRuntime().freeMemory() / 1024, false);
     ...
     *
     * @param title The title of the windows ("Error")
     * @param text  The text of the message ("Code generator cannot work, becuase
     *              there is not enough memory")
     * @param type  The type of the message, according to JOptionPane vocabulary
     *              (JOptionPane.ERROR_MESSAGE)
     */
    ...
    public ILicense loadLicense() {
        ...
        try {
            FileInputStream fis = new FileInputStream(FileControl.getInstance().getOPCATDirectory() + FileControl.fileSeparator + "license.lic");
            props.load(fis);
    ...
    /**
     * @param waitScreen The waitScreen to set.
     */
    ...
    /**
     * @param projectWasOpened The projectWasOpened to set.
     */
    ...
    /**
     * @param metaWaitScreen The metaWaitScreen to set.
     */
    ...
    /**
     * @param myFileChooser The myFileChooser to set.
     */
     ...
     * user.
     *
     * @param isUndo If the parameter is <code>true</code>, then the operation will
    ...
    public static void setLoadedfilename(String loadedfilename) {
        Opcat2.loadedfilename = loadedfilename;
    }

    public void showMetaData(MetaLibrary ont, DataProject metaData) {
        reqActionA.showMeta(ont, metaData);
    }


//    public static PerformanceInfiniteProgressPanel getGlassPane() {
//        PerformanceInfiniteProgressPanel glassPane = new PerformanceInfiniteProgressPanel(false);
//        myFrame.setGlassPane(glassPane);
//        return glassPane;
//    }

    public static void startWait() {
        if (myFrame.getGlassPane().isVisible()) return;
        PerformanceInfiniteProgressPanel glassPane = new PerformanceInfiniteProgressPanel(false);
    ...
    public static PerformanceInfiniteProgressPanel getGlassPane(String text) {
        myFrame.setGlassPane(null);
        PerformanceInfiniteProgressPanel glassPane = new PerformanceInfiniteProgressPanel(false);
        myFrame.setGlassPane(glassPane);
        return glassPane;
    }
