...
public class RSVNPopup extends RDefaultPopup {
    ...
    public RSVNPopup(BaseView view, OpdProject prj) {
        ...
        if (userObject instanceof OpcatMCDirEntry) {
            ...
            add(new SvnCheckOutAction(entry, true, "Checkout & lock",
                    SvnImages.ACTION_CHECKOUT));
            add(new JSeparator());
            // }

            if (entry.getKind() == SVNNodeKind.DIR) {
                add(new SvnCheckOutAction(entry, false, "Checkout unlocked", null));
                add(new JSeparator());
            }
            // if (entry.getKind() == SVNNodeKind.FILE) {

            add(new SvnRepoDeleteAction(entry, "Delete", null));
            add(new SvnRepoUnlockAction(entry, "Unlock", null));
            add(new JSeparator());
            // }
