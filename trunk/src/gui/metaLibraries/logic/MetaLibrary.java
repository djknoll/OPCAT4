...
public class MetaLibrary implements Comparable {
    ...
    private void lazyInit() {
        if (cachedID == -1) {
            String id = dataProject.getID();
            cachedID = id.hashCode();
            cachedStrID = id;
        }
    }
    ...
    private static ProcessInstance insertProcessInstance(Role insertedRole,
        ...
        if (!keepConnected || instancesofRoleInOPD.size() == 0) {
            ...
            if (Boolean.valueOf(Configuration.getInstance().getProperty("expose.copy.properties"))) {
                ((Instance) newInstance).getEntry().getMyProps().copyFrom(((Entry) roleThing).getMyProps(), true);
                /**
                 * a patch
                 */
                //((Instance) newInstance).getEntry().getMyProps().put("process.rpg.Order", "0");
            }
    ...
    private static StateInstance insertStateInstance(Role insertedRole,
        ...
        if (keepConnected && instancesofRoleInOPD.size() > 0) {
            ...
            if (Boolean.valueOf(Configuration.getInstance().getProperty("expose.copy.properties"))) {
                ((Instance) newObjectInstance).getEntry().getMyProps().copyFrom(((Entry) object).getMyProps(), true);
                /**
                 * a patch
                 */
                //((Instance) newObjectInstance).getEntry().getMyProps().put("object.rpg.Order", "0");
            }
    ...
    private static ObjectInstance insertObjectInstance(Role insertedRole,
        ...
        if (!keepConnected || instancesofRoleInOPD.size() == 0) {
            ...
            if (Boolean.valueOf(Configuration.getInstance().getProperty("expose.copy.properties"))) {
                ((Instance) newInstance).getEntry().getMyProps().copyFrom(((Entry) object).getMyProps(), true);
                /**
                 * a patch 
                 */
                //((Instance) newInstance).getEntry().getMyProps().put("object.rpg.Order", "0");
            }
     ...
     * @param targetOpd
     * @param keepConnected
     * @param relativeInstance if adding a process the this is used also as the parent of the
    ...
    public boolean load(OpdProject project) {
        if (dataProject == null) {
            this.setState(MetaLibrary.STATE_LOAD_FAILED);
            return false;
        }
        dataProject.load(project);

        try {
            if (dataProject.getStatus().isLoadFail()) {
                this.setState(MetaLibrary.STATE_LOAD_FAILED);
                if (!isHidden())
                    OpcatLogger.error(dataProject.getStatus()
                            .getFailReason());
            } else {
    ...
    public boolean load() {
        ...
        try {
            ...
            //load the roles from project
            if (Opcat2.getCurrentProject() != null) {
                ArrayList<Role> roles = this.getRolesInProject(Opcat2.getCurrentProject());
                for (Role role : roles) {
                    role.load(this);
                }
            }
