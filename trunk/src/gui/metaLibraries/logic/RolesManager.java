...
public class RolesManager {
    ...
    public boolean removeRole(Role role) {
        if (this.ontoRoles != null) {
            return this.ontoRoles.remove(role);
        }
    ...
    public Vector<Role> getNotLoadedRolesVector(int ontoType) {
        ...
        while (iter.hasNext()) {
            Role role = (Role) iter.next();
            if (!role.isLoaded()) {
                // if ((role.getOntology().getIDType() == ontoType )) {
                selectedRoles.add(role);
                // }
            }
        }
    ...
    public void loadOntRoles(MetaLibrary ont) {

        if (ont == null) return;

        Enumeration<Role> roles = getRoles();
        while (roles.hasMoreElements()) {
            Role role = roles.nextElement();
            try {
                if (ont.getID() == role.getLibraryId()) {
                    try {
                        if (!(role.isLoaded())) role.load(ont);
