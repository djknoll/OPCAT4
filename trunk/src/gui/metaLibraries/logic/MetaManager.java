...
public class MetaManager implements Cloneable {
     ...
     * the project.
     */
    private Vector<MetaLibrary> metaLibraries = new Vector<MetaLibrary>();
    ...
    public MetaLibrary getMetaLibrary(long id) {
        MetaLibrary meta;
        for (MetaLibrary metaLibrary : this.metaLibraries) {
            meta = metaLibrary;
            if (meta.getID() == id) {
                return meta;
            }
    ...
    public MetaLibrary getMetaLibrary(String name) {
        MetaLibrary meta;
        for (MetaLibrary metaLibrary : this.metaLibraries) {
            meta = metaLibrary;
            if (meta.getName().toLowerCase().equalsIgnoreCase(name)) {
                return meta;
            }
        }
        return null;
...
//     * Retruns a <code>MetaLibrary</code> according to a given id.
//     *
//     * @param path The ID of the library that should be retrieved.
//     * @return The MetaLibrary or <code>null</code> if was not found.
//     */
//    public MetaLibrary getMetaLibraryByPath(NAME path) {
//        MetaLibrary meta;
...
//    }
     ...
     * @throws MetaException no MetaLibrary exist with the given path.
     */
     ...
     *
     * @param id A path to the MetaLibrary to be marked.
     ...
     *
     * @param lib The <code>MetaLibrary</code> that should be cleaned out the
     *            project.
     ...
     * @param doRemove  If set to <code>true</code>, the method removes all the found
     ...
     *
     * @param dataReferenceType The reference type (<code>TYPE_FILE</code> or
     *                          <code>TYPE_URL</code>).
    ...
    public void setAllLibstoReferenced() {
        ...
        while (iter.hasNext()) {
            ...
            if ((meta.getState() != MetaLibrary.STATE_REMOVED) && (meta.getState() != MetaLibrary.STATE_LOADED))
    ...
    public void refreshMetaRoles(OpdProject myProject, MetaLibrary meta) {
        Enumeration locenum = myProject.getComponentsStructure().getElements();
        // Entry thing;
        Enumeration allRoles;
        Role role;
        ...
        try {
            while (locenum.hasMoreElements()) { // if (role.isLoaded()) {
                ...
                while (allRoles.hasMoreElements()) {
                    role = (Role) allRoles.nextElement();
                    // Handling only unloaded roles
                    if ((role.getState() == Role.UNLOADED && (role.getLibraryId() == meta.getID()))) {
                        role.load(meta);
                    }
                }
                // }
    ...
    private void loadLibraries() {
        ...
        for (int i = 0; i < this.metaLibraries.size(); i++) {
            ...
            if (metaLib.getState() != MetaLibrary.STATE_LOADED) {
                if (!metaLib.load()) {
                    dead.add(metaLib);
                }
            }
