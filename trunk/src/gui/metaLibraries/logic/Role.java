...
public class Role implements IXRole, IRole, Comparable {
    ...
    public Role(long _thingID, MetaLibrary onto, boolean loadMeta) throws MetaException {
        if (onto == null) {
            throw new MetaException("Meta library was passed null to Role", "");
        }
        this.define(_thingID, onto.getID());
        ...
        if (onto != null) {
            if ((onto.getType() == MetaLibrary.TYPE_POLICY)) {
                ...
                try {
                    ...
                    OpcatLogger
                            .error("Library strcture is passed to null for Role loading");
                    OpcatLogger.error(npe);
                }
                ...
                if (entry != null) {
                    ...
                    OpcatLogger.error("Entry not found in library , Searched for " + this.getThingId() + " in " + onto.getName());
                }
            }
    ...
    public ArrayList connectedThings(OpdProject opdProject) {
        ...
        while (iter.hasNext()) {
            ...
            if ((entry instanceof Entry)) {
                ...
                //        .getLogicalEntity();
                RolesManager roleMang = ((Entry) entry).getLogicalEntity().getRolesManager();
                if (roleMang.getRolesCollection().size() > 0) {
                    Iterator rolesIter = Collections.list(roleMang.getLoadedRoles()).iterator();
                    // .getLoadedRolesVector(ontoType).elements()).iterator();
                    while (rolesIter.hasNext()) {
                        Role role = (Role) rolesIter.next();
                        //if(!role.isLoaded()) continue;
                        // if (role.getThing().getDataInstance() != null) {
                        DataAbstractItem tempReq = role.getThing();

                        try {
                            long roleProjectID = role.getOntology().getID();
                            if (roleProjectID == getOntology().getID()) {
                                if (tempReq.getId() == getThing().getId()) {
                                    things.add(((Entry) entry).getLogicalEntity());
                                }
                            }
                        } catch (Exception ex) {
                            OpcatLogger.error(ex);
    ...
    public String connectedThingstoString(OpdProject opdProject) {
        ArrayList connectedThings = connectedThings(opdProject);
        String names = "";
        for (int j = 0; j < connectedThings.size(); j++) {
            names = names + ((DataAbstractItem) connectedThings.get(j)).getName().replace("\n", " ");
