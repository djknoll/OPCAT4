...
//            //if (reqs.getSourceType().getType() == DataCreatorType.DATA_TYPE_OPCAT_FILE_OPZ) {
//            //    OpdProjectFileLoader fl = (OpdProjectFileLoader) ((DataProject) holder).getLoader().getLoader();
//            //    return fl.getProject();
//            //}
//            return this.holder;
//        }
//        return null;
    ...
    public Collection getDataComponents() {
        ...
        if (this.type == MetaLibrary.TYPE_CLASSIFICATION) {
            ...
            if (reqs.getSourceType().getType() == DataCreatorType.DATA_TYPE_OPCAT_FILE_OPZ) {
                OpdProjectFileLoader fl = (OpdProjectFileLoader) ((DataProject) holder).getLoader().getLoader();
                ArrayList ret = new ArrayList();
                MainStructure oStructure = fl.getProject().getComponentsStructure();
                Iterator iter = Collections.list(oStructure.getElements())
                        .iterator();
                while (iter.hasNext()) {
                    Entry ent = (Entry) iter.next();
