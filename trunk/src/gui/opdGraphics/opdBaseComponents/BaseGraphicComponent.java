...
public abstract class BaseGraphicComponent extends JLayeredPane implements
        MouseListener, MouseMotionListener, Connectable, Movable {

    /**
     *
     */

    public static final boolean DEFAULT_SENT_TO_BACK = false;
    public static final boolean DEFAULT_IS_PRESENTED = true;
    public static final boolean DEFAULT_ZOOMED_IN = false;
    ...
    private boolean isPresented = DEFAULT_IS_PRESENTED;

    private Color currentForColor = null;

    DataColors colors = new DataColors();

    boolean sentToBAck = DEFAULT_SENT_TO_BACK;
     ...
     * </p>
     *
     * @param pProject OpdProject</code>, project this component is added
     *                 to.
     */
//    public BaseGraphicComponent(OpdProject pProject, OpdKey myInstance) {
//        this.lines = new Hashtable();
//        this.draggers = new Hashtable();
//        this.updateListeners = new Hashtable();
//        this.objects2Move = new Hashtable();
//        this.setLayout(null);
//        this.zoomedIn = DEFAULT_ZOOMED_IN;
//        this.myProject = pProject;
//        this.addKeyListener(new BaseKeyListner());
//    }
    public BaseGraphicComponent(OpdProject pProject) {
        ...
        //some constants for calculations of class sons
        config = this.myProject.getConfiguration();
        currentSize = ((Integer) config.getProperty("CurrentSize")).doubleValue();
        normalSize = ((Integer) config.getProperty("NormalSize")).doubleValue();
        factor = currentSize / normalSize;
        selOffset = Math.round((float) (SELECTION_OFFSET * factor));
        dSelOffset = 2 * selOffset;
        borderSensitivity = (int) Math.round((currentSize / normalSize)
                * BORDER_SENSITIVITY);
...
//    }
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (this.isDragged) {
            ...
            for (Enumeration e1 = ht.keys(); e1.hasMoreElements(); ) {
                ...
                if (bgc instanceof OpdThing) {
                    OpdThing thing = (OpdThing) bgc;
                    ((ThingInstance) thing.getInstance()).setChangedLocation(true);
                }
     ...
     * system was changed.
     *
     * @param yn</code> <code>true</code> to select the component
    ...
    private void bringToFront() {
        ...
        tempContainer = (JLayeredPane) getParent();
        if (tempContainer != null) {
            tempContainer.moveToFront(BaseGraphicComponent.this);
        }
    }
    ...
    private void sendToBack() {
        ...
        tempContainer = (JLayeredPane) getParent();
        if (tempContainer != null) {
            tempContainer.moveToBack(BaseGraphicComponent.this);
        }
...
//    }
    ...
    public void changeToMetaColor(Color color) {
...
class BaseKeyListner implements KeyListener {
