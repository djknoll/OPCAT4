...
public class OpdObject extends OpdThing {
     ...
     *
     * @param pOpdId         long</code>, uniqe identifier of the OPD this
     *                       component is added to.
     * @param pEntityInOpdId long</code>, uniqe identifier of the
     *                       component within OPD it is added to.
     * @param pProject       OpdProject</code>, project this component is added
     *                       to.
     * @param pEntry         ThingEntry</code>, the entry of the thing, this
    ...
    public void paintComponent(Graphics g) {
        ...
        this._drawCurrentMeaserUnit(g2);
    ...
    public boolean isInAdjacentArea(int x, int y) {
        ...
        double proximity = 12.0 * currentSize / normalSize;
     ...
     * </p>
     *
     * @param yn boolean</code>, if <code>true</code> autoarrang is on, if
    ...
    private void _drawCurrentMeaserUnit(Graphics2D g2) {
        if (((OpmObject) this.myEntity).getMesurementUnitInitialValue() == ((OpmObject) this.myEntity).getMesurementUnitCurrentValue()) {
            return;
        }

        String noi = String.valueOf(((OpmObject) this.myEntity).getMesurementUnitCurrentValue());

        int stringX, stringY;

        Font currentFont = (Font) config.getProperty("MeasureFont");
        currentFont = currentFont
                .deriveFont((float) (currentFont.getSize2D() * factor));
        g2.setColor(Configuration.getInstance().getColorFromProperty("graphics.fonts.MeasureFont.color"));
        g2.setFont(currentFont);
        this.currentMetrics = this.getFontMetrics(currentFont);
    ...
    public void setupNextScreen() {
        ...
        for (Enumeration en = this.getEntry().getInstances(false); en
                .hasMoreElements(); ) {
