...
public abstract class OpdLine extends JComponent implements MouseListener,
    ...
    private Color beforeEventBAckgroundColor;
    ...
    private  double lineSensitivity ;
     ...
     *
     * @param pEdge1 Connectable</code>, one edge of Opd Link a subclass
    ...
    public OpdLine(Connectable pEdge1, RelativeConnectionPoint cPoint1,
        ...
        this.textColor = Configuration.getInstance().getColorFromProperty("graphics.default.LineTextColor");
        this.lineColor = Configuration.getInstance().getColorFromProperty("graphics.default.LineColor");
    ...
    currentSize = ((Integer) config.getProperty("CurrentSize"))
            .doubleValue();

    normalSize = ((Integer) config.getProperty("NormalSize"))
            .doubleValue();

    shiftLine = (currentSize / normalSize) * MIN_SIZE;

    lineSensitivity = LINE_SENSITIVITY * (currentSize / normalSize);
     ...
     *
     * @param yn boolean, if <code>true</code> arranges the line,
     ...
     *
     * @param pLineColor Color</code>, the new color the line will be
     *                   rendered with.
     */
     ...
     *
     * @param pTextColor Color, the new color the text will be
     *                   rendered with.
     */
...
//                .doubleValue();
     ...
     *
     * @param me Connectable the Connectable on one edge of the
     *           link.
    ...
    public void mouseEntered(MouseEvent e) {
        Entry myEntry = myProject.getSystemStructure().getEntry(myEntity.getId());
        if ((myEntry != null) && myEntry.hasURL()) {
            this.beforeEventBAckgroundColor = this.getLineColor();
            this.beforeEventToolTip = this.getToolTipText();
            this.setToolTipText("Press Ctrl+Left-Mouse to activate URL");
            this.setLineColor((Color) this.myProject.getConfiguration().getProperty("UrlColor"));
    ...
    public void mouseExited(MouseEvent e) {
        if (this.colorChanged) {
            ...
            this.setLineColor(this.beforeEventBAckgroundColor);
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        Entry myEntry = myProject.getSystemStructure().getEntry(myEntity.getId());
        if ((myEntry != null) && e.isControlDown()) {
            myEntry.ShowURL();
            return;
        }
    }
