...
public class OpcatExposeAdvisorImpl {
    ...
    public void implementAdviceTree(ConnectionEdgeInstance root,
                                    boolean connected, int level) throws Exception {
        ...
        for (OpcatExposeAdvice i : advices) {
            for (OpcatExposeInterfaceItem j : i.getProperties()) {
                ...
                if (ret != null) {
                    if (level > 0) {
                        int myLevel = level - 1;
                        implementAdviceTree((ConnectionEdgeInstance) ret, connected, myLevel);
                    }
                }
            }
        }
     ...
     *
     * @param interfaceItem      - the son as defined in the remote model represented as an
     *                           {@link OpcatExposeInterfaceItem}
     * @param connected          keep the inserted connected to the source model
     * @param sourceInstance     source {@link ConnectionEdgeInstance} which is a pointer to a
     *                           remote Instance.
     * @param insideMainInstance if the source instance is inside a main instance then insert
     *                           outside or innside
     * @return
     */
            ...
            if (addRole) {
                ...
                if (((OpmEntity) interfaceItem.getRole().getThing())
                    ...
                    if ((interfaceItem.getRole().getOntology()
                            .getReferenceType() == DataCreatorType.REFERENCE_TYPE_PRIVATE_POLICY_FILE)
                            || (interfaceItem.getRole().getOntology()
                            .getReferenceType() == DataCreatorType.REFERENCE_TYPE_TEMPLATE_POLICY_FILE)) {
