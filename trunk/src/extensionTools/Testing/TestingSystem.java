...
public class TestingSystem implements Testing {
    ...
    public TestingSystem(IXSystem mySys, TestingStatusBar guiPanel) {
        ...
        this.testingPanel.AddToExtensionToolsPanel(true);
        ...
        if (testingTimer == null) {
            ...
            if (lifeSpanGrid == null) {
                ...
                if (!TestingSettings.getInstance().isSHOW_LIFE_SPAN()) {
                    ...
                    lifeSpanGrid.AddToExtensionToolsPanel(true);
                }
            ...
            if (resourceGrid == null) {
                ...
                if (!TestingSettings.getInstance().isSHOW_RESOURCE()) {
                    ...
                    resourceGrid.AddToExtensionToolsPanel(true);
                }
    ...
    public synchronized void testingStop() {
        ...
        if (Opcat2.getCurrentProject() != null) {
            Enumeration en = Opcat2.getCurrentProject().getSystemStructure().getAllElements();
            while (en.hasMoreElements()) {
                Entry entr = (Entry) en.nextElement();
                if (entr.getLogicalEntity() instanceof OpmObject) {
                    ((OpmObject) entr.getLogicalEntity()).setMesurementUnitCurrentValue(((OpmObject) entr.getLogicalEntity()).getMesurementUnitInitialValue());
                }
            }
        }
    ...
    private void treatProcessActivationConsequences(TestingEntry aProcess,
        ...
        en = activatedProcess.getInstances(false);
        for (; en.hasMoreElements();) {
    ...
    private void treatObjectActivationConsequences(TestingEntry aObject) {
        ...
        hasElements = activatedObject.getInstances(false).hasMoreElements();
        if (!(hasElements)) {
            return;
        ...
        IXObjectInstance objIns = (IXObjectInstance) (activatedObject
                .getInstances(false).nextElement());
        boolean hasActiveState = this.hasActiveState(activatedObject);

        if (!hasActiveState) {
            StateEntry chosen = ((ObjectEntry) activatedObject).chooseState();
            if (chosen != null) this.activate(this.findEntryById(chosen.getId()));
        }
                        ...
                        if (!isActivation) {
                            ...
                            if (testingEntry.isMeasurementUnitExeption() != TestingEntry.NoResourceExeption) {
                                ...
                                if (testingEntry.getSavedState() != null) {
                                    ...
                                    StateEntry exeption = (maxState == null ? (minState == null ? null : minState) : maxState);
                                    if (exeption != null) {
                                        if ((object.chooseState() != null) && (activeState != null) && (exeption.getId() == activeState.getEntry().getId())) {
                                            activate(getTestingElementByIXEntryID(object.chooseState().getId()));
                                        }
                                    }
                                }
                                testingEntry.setSavedState(null);
    ...
    private void treatChildProcessTerminationConsequences(
        ...
        if (aProcess.getParents().hasMoreElements()) {
            Enumeration en = iProcess.getInstances(false);
            for (; en.hasMoreElements();) {
                IXProcessInstance activatedProcessInstance = (IXProcessInstance) en
                        .nextElement();
                ...
                if (parentProcessInstance != null) {
                    TestingEntry aParentProcess = this
                            .findEntryById(parentProcessInstance.getIXEntry()
                                    .getId());
                    if (aParentProcess != null) { //aParentProcess.isActive(currentEvent.getExecutionId())) {
                        if (this.checkProcessBeforeTerminate(aParentProcess,
                            ...
                            if (!this.activateLevel(nextLevelChildren,
                                ...
                                // parent process
                                if (aParentProcess != null) { // (aParentProcess.getCurrentEvent(currentEvent
                                    //.getExecutionId()) != null) {
                                    ...
                                    // deactivate parent
                                    TestingEvent termEvent;
                                    if (aParentProcess
                                            .getCurrentEvent(currentEvent
                                                    .getExecutionId()) == null) {
                                        termEvent = new TestingEvent();
                                    } else {
                                        termEvent = new TestingEvent(
                                                aParentProcess
                                                        .getCurrentEvent(currentEvent
    ...
    private TestingGridPanel InitTestingGridPanel() {
        ...
        locPanel = new TestingGridPanel(cols, this.mySys);
        locPanel.doLayout();
        return locPanel;
    }
    ...
    public Vector<TestingEntry> getTestingElements() {
