...
class TestingEntry extends Object {
    ...
    public void animate(TestingEvent aEvent) {
        ...
        Enumeration eTemp = this.iEntry.getInstances(false);
    ...
    public void pauseTesting() {
        Enumeration en = this.iEntry.getInstances(false);
        for (; en.hasMoreElements();) {
            if (TestingAuxiliary.isLink(this.iEntry)) {
    ...
    // private Vector getChildrenByClass(NAME className) {
    // Vector<Entry> results = new Vector<Entry>();
    // if (!TestingAuxiliary.isProcess(this.iEntry)) {
    // return results;
    // }
    ...
    public void setMeasurementUnitAcommulatedValue(
        ...
        if (getIXEntry() instanceof IXObjectEntry) {
            ObjectEntry entry = (ObjectEntry) getIXEntry();
            ((OpmObject) entry.getLogicalEntity()).setMesurementUnitCurrentValue(measurementUnitAcommulatedValue);
        }
    }
