...
import messages.OpcatMessagesConstants.*;
 ...
 * @author raanan
 */
public class OpcatMCManager extends Observable implements ISVNEventHandler {

    private final String gridArraySeperator = "$";
    public static final String WORKING_COPY = Configuration.getInstance().getProperty("models_directory");
    public static final String server = Configuration.getInstance().getProperty("MCserver");
    public static final String svnAccessType = Configuration.getInstance().getProperty("MCaccesstype");
    private static final String svnURL = Configuration.getInstance().getProperty("MCaccesstype") + "://" + Configuration.getInstance().getProperty("MCserver");
    private static final String repo = svnURL + "/" + Configuration.getInstance().getProperty("MCrepository");

    public static String getSvnURL() {
        return svnURL;
    }

    private static String TEMPLATES_RELATIVE_PATH = Configuration.getInstance().getProperty("MCTemplates");
    public static String TEMPLATES_DIR = WORKING_COPY + FileControl.fileSeparator + Configuration.getInstance().getProperty("MCTemplates");
    public static String CATEGORIES_DIR = WORKING_COPY + FileControl.fileSeparator + Configuration.getInstance().getProperty("MCTemplates") + FileControl.fileSeparator + Configuration.getInstance().getProperty("MCCategories");
    ...
    public File doExportToTempDir(String encodedPath)
            throws Exception {

        File ret = null;

        String decoded = SVNURL.parseURIEncoded(
                getSvnURL() + encodedPath).toDecodedString();
        ...
        File destination;
        destination = new File(getOpcatMCTemporeryDirectory()
                .getPath());
        //+ FileControl.fileSeparator + fileName);


        boolean overwrite = Boolean.valueOf(Configuration.getInstance().getProperty("mc.export.overwrite.tmp"));
        ...
        //OpcatLogger.info("Tring to export  " + ret.getName() + " into " + destination.getPath(), false);
        getInstance().doExport(encodedPath, SVNRevision.HEAD,
                destination);
    ...
    public static void doSilentLogin(String username, String password) {
        OpcatMCManager.onlineUpdate();
        OpcatUser user = OpcatUser.getCurrentUser();
        user.setName(username);
        user.setPassword(password);
        user.setStatus(new OpcatUserStatus(OpcatUserStatus.LOGIN_OK));
        getInstance();
    }
    ...
    public static File getOpcatMCTemporeryDirectory() throws IOException {
        ...
        ret.delete();
        File opcat = new File(ret.getParentFile().getPath() + FileControl.fileSeparator + "opcat.tmp");
        opcat.mkdirs();
        return opcat;
    }

    public ArrayList<SVNLogEntry> getVersions(File file) {
        ArrayList<SVNLogEntry> logEntries = new ArrayList<SVNLogEntry>();
        //OpcatMCDirEntry entry = getEntry(file);
        SVNURL url = getFileURL(file);
        if (url != null) {
            try {
                String relativePath = SVNPathUtil.getPathAsChild(repository.getRepositoryRoot(true).toDecodedString(), url.toDecodedString());
                relativePath = (relativePath == null ? "" : relativePath);
                logEntries = new ArrayList<SVNLogEntry>(repository.log(new String[]{relativePath}, null, 0, -1, true, true));
            } catch (Exception ex) {
                handleException(ex, "Get File Revisions", file.getName());
            }
        }
        return logEntries;
    }

    public ArrayList<SVNLogEntry> getVersions(SVNURL url) {
        ArrayList<SVNLogEntry> logEntries = new ArrayList<SVNLogEntry>();
        try {
            String relativePath = SVNPathUtil.getPathAsChild(repository.getRepositoryRoot(true).toDecodedString(), url.toDecodedString());
            relativePath = (relativePath == null ? "" : relativePath);
            ...
            handleException(ex, "Get File Revisions", url.toDecodedString());
        }
        return logEntries;
    }
    ...
    public SVNURL getFileURL(File file) {
        ...
        try {
            ...
            if (e.getErrorMessage().getErrorCode() == SVNErrorCode.WC_NOT_DIRECTORY) {
                if (!(file.getPath().startsWith(WORKING_COPY))) {
                    //no need for exeption if this is not a working copy file
                    return null;
                }
            }
            handleException(e, "Get File URL", file.getName());
        } catch (Exception ex) {
            handleException(ex, "Get File URL", file.getName());
        }
...
//    public File doCheckOutCategories() {
//        forceConnection();
//        File file = new File(CATEGORIES_DIR);
//        if (!file.exists()) {
//            doCheckOut(CATEGORIES_RELATIVE_PATH, SVNRevision.HEAD, false,
//                    file, SVNDepth.INFINITY);
//        } else {
//            Cleanup(file);
//            doUpdate(file);
...
//    }
    ...
    private void initMessages() {
        ...
        fileGrid.init("Number" + gridArraySeperator + "Type" + gridArraySeperator + "Name" + gridArraySeperator + "Message" + gridArraySeperator + "Author" + gridArraySeperator + "Revision");

        adminGrid.init("Number" + gridArraySeperator + "Transaction Type" + gridArraySeperator + "Message Type" + gridArraySeperator + "Message" + gridArraySeperator + "Information");
        ...
        try {
            ...
            for (OpcatMCDirEntry entry : entries) {
                if (entry.getKind() == SVNNodeKind.FILE) {
                    ...
                    if (lock != null && (lock.getOwner() != null) && lock.getOwner().equalsIgnoreCase(
                        ...
                        String msg = "Can not unlock, " + (lock == null ? "File not locked"
                                : (lock.getOwner() == null ? "could not get owner of lock"
                                : lock.getOwner() + " is owner of lock"));
    ...
    public void doUpdate(File file) {
        try {
            ...
            if (isConnectedToerver()) {
                ...
                ((DefaultSVNOptions) manager.getDiffClient().getOptions()).setConflictHandler(new OpcatMCConflictHandler());
    ...
    public void doLocalDelete(File file, boolean force) {
        try {
            ...
            if (lock == null) {
                ...
                addFileMsg("Could not delete file, " + file.getName() + (lock.getOwner() == null ? "owner is empty" : lock.getOwner() + " is owner of lock"), "Delete", file.getName(), SVNRevision.WORKING.getNumber(),
                        "Delete file");
            }
    ...
    public void doRemoteDelete(OpcatMCDirEntry entry, String msg) {
        try {
            ...
            if (force) {
                doRemoteUnlock(entry.getURL(), msg, true);
                manager.getCommitClient().doDelete(new SVNURL[]{entry.getURL()}, msg);
                return;
            }
            ...
            if (entry.getKind() == SVNNodeKind.DIR) {
                DefaultMutableTreeNode node = getOneLevelRepositoryTree(entry,
                        new OpcatFileNameFilter(true, true));
            ...
            if (lock == null) {
                ...
                addFileMsg("Could not delete file, " + (lock.getOwner() == null ? "owner is empty" : lock.getOwner() + " is owner of lock"), "Lock", entry);
            }
    ...
    public void doCheckOut(String repoPath, File location) {
        try {
            manager.getUpdateClient().doCheckout(getRepoURL().appendPath(repoPath, false), location, SVNRevision.HEAD, SVNRevision.HEAD, SVNDepth.INFINITY, false);
        } catch (Exception ex) {
    ...
    private SVNURL setRepo() {
        try {
            ...
            manager = SVNClientManager.newInstance(options, repository.getAuthenticationManager());
    ...
    private void handleException(Exception ex, String name, String filename,
        ...
        if (ex instanceof SVNAuthenticationException) {
            if (showMsg) {
                addAdminMsg("Error" + gridArraySeperator + ex.getMessage() + gridArraySeperator + "Authentication Error", name);
            }
        ...
        if (ex instanceof SVNException) {
            ...
            if (svnEx.getErrorMessage().getErrorCode().getCode() == SVNErrorCode.WC_NOT_DIRECTORY.getCode()) {
            ...
            if (showMsg) {
                addAdminMsg("Error" + gridArraySeperator + error.getMessage() + gridArraySeperator + "Error Code " + error.getErrorCode().getCode() + ":" + error.getErrorCode().getDescription(), name);
            }
            ...
            for (OpcatMCDirEntry entry : entries) {
                if (showMsg) {
                    addFileMsg(svnEx.getErrorMessage().getErrorCode().getDescription(), name, entry);
                }
            }
        ...
        if (showMsg) {
            addAdminMsg("Error" + gridArraySeperator + ex.getMessage() + gridArraySeperator + "    ", name);
        }
    ...
    public static void onlineUpdate() {
        if (!online) {
            OpcatMCTimerOnlineTask task = new OpcatMCTimerOnlineTask();
            task.run();
        }
    ...
    public static boolean connect() {
        ...
        OpcatMCManager.onlineUpdate();
        return getInstance().forceConnection();
    }
    ...
    private boolean connectToRepo() {
        try {
            ...
            if (nodeKind == SVNNodeKind.NONE) {
                OpcatLogger.error("There is no entry at '" + tgtURL + "'.");
                addAdminMsg("Error" + gridArraySeperator + "Server Root is Empty" + gridArraySeperator + "Connecting", "Connection");
                return false;
            } else if (nodeKind == SVNNodeKind.FILE) {
                OpcatLogger.error("The entry at '" + tgtURL + "' is a file while a directory was expected.");
                addAdminMsg("Error" + gridArraySeperator + "Server Root is Corrupted" + gridArraySeperator + "while connecting", "Connection");
                return false;
            }
            ...
            manager = SVNClientManager.newInstance(options, repository.getAuthenticationManager());
            manager.setEventHandler(this);
            manager.setIgnoreExternals(true);
    ...
    public DefaultMutableTreeNode getOneLevelRepositoryTree(
        ...
        try {
            ...
            if (startingDirEntry == null) {
                root = new DefaultMutableTreeNode(getRepoURL());
            } else {
     ...
     *
     * @param relativePath starting point getFileListFromRepo("") returns all Files in repository
     * @throws SVNException
     */
    public LinkedList<OpcatMCDirEntry> getFileListFromRepo(String relativePath) {
        ...
        try {
            ...
            return getFileListFromRepo(root);
     ...
     * get all files from root down without the directories
     *
     * @param root starting point getFileListFromRepo("") returns all Files in repository
     * @throws SVNException
     */
    ...
    public SVNNodeKind getNodeKind(SVNURL url) {
        try {
            String repositoryRootPath = repository.getRepositoryRoot(true).getPath();

            String currentPath = (url != null ? url.getPath() : repositoryRootPath);

            String path = SVNPathUtil.getRelativePath(repositoryRootPath,
                    currentPath);
            path = (path == null ? "/" : "/" + path);
            OpcatMCDirEntryHandler handler = new OpcatMCDirEntryHandler(null);
            repository.getDir(path, repository.getLatestRevision(), null, handler);

            LinkedList<OpcatMCDirEntry> entries = handler.getEntries();

            return entries.get(0).getKind();
    ...
    private void addEntries(DefaultMutableTreeNode root,
        ...
        try {
            ...
            String currentPath = (rootSvnEntry != null ? rootSvnEntry.getURL().getPath() : tgtURL.getPath());
            ...
            while (iterator.hasNext()) {
                ...
                if (entry.getKind() == SVNNodeKind.DIR) {
                    ...
                    if ((filter != null) && filter.accept(null, entry.getName())) {
                        root.add(node);
                    } else {
                        root.add(node);
                    }
    ...
    public Collection<SVNFileRevision> doLog(SVNURL url, String name) {
        try {
            ...
            String relativePath = SVNPathUtil.getPathAsChild(repository.getRepositoryRoot(true).toDecodedString(), url.toDecodedString());
            relativePath = (relativePath == null ? "" : relativePath);
            return repository.getFileRevisions(relativePath,
                    (Collection) null, 0, repository.getLatestRevision());
    ...
    private File doCheckOut(OpcatMCDirEntry entry, SVNRevision revision,
        ...
        try {
            ...
            manager.getUpdateClient().setIgnoreExternals(true);
    ...
    public long doExport(String encodedRelativePath, SVNRevision revision,
                         File destination) {
        try {
            ...
            //OpcatLogger.info("Export : exporting " + url.getURIEncodedPath(), false);
            //OpcatLogger.info("Export : exporting " + url.getURIEncodedPath() + ", overwrite = " + overwrite, false);
            return manager.getUpdateClient().doExport(url, destination, revision,
                    revision, null, true, SVNDepth.INFINITY);
            //OpcatLogger.info("Export : Exported OK , overwrite = " + overwrite, false);
        } catch (Exception ex) {
            //OpcatLogger.error(ex);
            handleException(ex, "Export", null);
            return -2;
        }
    }

    public void doRemoteUnlock(SVNURL url, String msg, boolean force) {
        try {
            OpcatMCDirEntryHandler handler = new OpcatMCDirEntryHandler(null,
                    true);
            ...
            for (OpcatMCDirEntry entry : handler.getEntries()) {
                try {
                    ...
                    handleException(ex, "Repository Unlock", entry.getURL().getPath());
                }
            }
        } catch (SVNException e) {
            handleException(e, "Repository Unlock", null);
        }
    }
    ...
    public File doCheckOut(OpcatMCDirEntry entry, SVNRevision revision,
        ...
        try {
            ...
            SVNInfo info = manager.getWCClient().doInfo(url, revision, revision);
            ...
            //
            // } else {
            File ret = doCheckOut(SVNPathUtil.removeHead(SVNPathUtil.removeHead(url.getPath())), revision, locLock, dst.getFile(), depth);
            // }

            if (entry.getKind() == SVNNodeKind.FILE) {

                File file = new File(dst.getFile().getPath() + FileControl.fileSeparator + entry.getName());
    ...
    public SVNLock getEntryLock(OpcatMCDirEntry entry) {
        try {
            if (entry.getKind() == SVNNodeKind.FILE) {
                String myPath = SVNPathUtil.getRelativePath(entry.getRepositoryRoot().getPath(), entry.getURL().getPath());
                if (!forceConnection()) {
                    return null;
                }
    ...
    public void setRepositoryProperty(OpcatMCDirEntry entry, String property,
        ...
        try {
            ...
            handleException(ex, "Creating Property " + property, entry.getName());
        }
    }
        ...
        try {
            ...
            for (OpcatMCDirEntry entry : fileEntries) {
                if (entry.getKind() == SVNNodeKind.FILE) {
                    ...
                    if ((lock != null) && (lock.getOwner() != null) && !(lock.getOwner().equalsIgnoreCase(OpcatUser.getCurrentUser().getName()))) {
                        addFileMsg(
                                "Could not lock file, " + (lock.getOwner() == null ? "owner is empty"
                                        : lock.getOwner() + " is owner of lock"),
                                "Lock", entry);
    ...
    public boolean doAdd(File wcPath) {
        boolean ret = false;
        try {
    ...
    public void doMakeDirectory(SVNURL parent, String newName) {
        try {
            ...
            manager.getCommitClient().doMkDir(
                    new SVNURL[]{url},
                    OpcatUser.getCurrentUser().getName() + " : Created " + url.getPath());
    ...
    private LinkedList<OpcatMCDirEntry> getWCEntries(File file, SVNDepth depth)
        ...
        OpcatMCDirEntryHandler handler = new OpcatMCDirEntryHandler(null);
        manager.getLogClient().setIgnoreExternals(true);
     ...
     * @param removeLock  - remove lock (if-exists) after commit.
     * @param deleteFiles - not used
     * @return
     */
    public boolean doCommit(OpcatFile file, String msg, boolean removeLock,
        ...
        try {
            ...
            SVNCommitPacket packet = manager.getCommitClient().doCollectCommitItems(new File[]{file}, true, true,
                    SVNDepth.INFINITY, changelists);

            if (packet.getCommitItems().length == 0) {
                addAdminMsg("Information" + gridArraySeperator + "Nothing to commit" + gridArraySeperator + "Commit packet is empty   ", "Commit");
                return false;
            }
            ...
            if (info.getNewRevision() < 0) {
                ...
                while (error != null) {
                    addAdminMsg("Error" + gridArraySeperator + error.getMessage() + gridArraySeperator + "Fail code " + error.getErrorCode(), "Commit");
                    error = error.getChildErrorMessage();
                }
                ret = false;
            } else {
    ...
    private void addFileMsg(String msg, String type, OpcatMCDirEntry entry) {
        ...
        msg = entry.getName() + gridArraySeperator + msg + gridArraySeperator + entry.getAuthor() + gridArraySeperator + entry.getRevision();

        fileMsg.add(msg);
        fileGrid.addRow(fileMsg.size() + gridArraySeperator + type + gridArraySeperator + msg);
    }

    private void addFileMsg(String msg, String type, String name,
                            long revision, String author) {
        msg = msg.replaceAll("svn", "OPCAT MC");
        msg = name + gridArraySeperator + msg + gridArraySeperator + author + gridArraySeperator + revision;
    ...
    private void addAdminMsg(String msg, String type) {
        msg = msg.replaceAll("svn", "OPCAT MC");
        adminMsg.add(msg);
        adminGrid.addRow(adminMsg.size() + gridArraySeperator + type + gridArraySeperator + msg);
    }
    ...
    public void handleEvent(SVNEvent event, double progress)
        ...
        try {
            ...
            if ((event != null) && (event.getFile() != null) && (isIgnored(event.getFile().getName()) || isIgnored(event.getFile().getParentFile().getPath()) || isIgnored(event.getFile().getPath()))) {
                return;
            }
            ...
            if (action == SVNEventAction.COMMIT_MODIFIED) {
                ...
                addFileMsg("Sending Changed File ", "Commit", event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                ...
                addFileMsg("Commit Completed", "Commit", null, repository.getLatestRevision(), OpcatUser.getCurrentUser().getName());
                ...
                for (File file : processed.keySet()) {
                    OPCAT_MESSAGES_OP op = (OPCAT_MESSAGES_OP) processed.get(file);
                    String subject = file.getName();
                    OPCAT_MESSAGES_TYPE type = OPCAT_MESSAGES_TYPE.USER;
                    OPCAT_MESSAGES_SUBSYSTEMS subsystem = OPCAT_MESSAGES_SUBSYSTEMS.FILES_COMMIT;
                    ...
                    OpcatMessage opcatMessage = new OpcatMessage(OpcatUser.getCurrentUser().getName(), OpcatUser.getCurrentUser().getName(), type, type, subsystem,
                ...
                addFileMsg("Transmitting file data...", "Commit", event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                ...
                if (SVNProperty.isBinaryMimeType(mimeType)) {
                    addFileMsg("Adding  (bin)", "Commit", event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                } else {
                ...
                notifyObservers("SVN Processed " + event.getFile().getName() + ", " + action.toString());
                ...
                if (contentsStatus == SVNStatusType.CHANGED) {
                    ...
                    addFileMsg("CONFLICTED", "Update", event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                ...
                addFileMsg("UPDATE EXTERNAL", "Update", event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                ...
                addFileMsg("Update Completed", "Update", null, repository.getLatestRevision(), OpcatUser.getCurrentUser().getName());
                ...
                for (File file : processed.keySet()) {
                    ...
                    if (file.exists()) {
                        SVNLock lock = getRemoteLock(file);
                        if ((lock != null) && !lock.getOwner().equalsIgnoreCase(
                                OpcatUser.getCurrentUser().getName())) {
                    ...
                    String subject = file.getName();
                    OPCAT_MESSAGES_TYPE type = OPCAT_MESSAGES_TYPE.USER;
                    OPCAT_MESSAGES_SUBSYSTEMS subsystem = OPCAT_MESSAGES_SUBSYSTEMS.FILES_CHECKOUT;
                    OPCAT_MESSAGES_SEVIRITY sevi = OPCAT_MESSAGES_SEVIRITY.INFORMATION;
                    OPCAT_MESSAGES_SYSTEMS system = OPCAT_MESSAGES_SYSTEMS.FILES;
                    OpcatMessage opcatMessage = new OpcatMessage(OpcatUser.getCurrentUser().getName(), OpcatUser.getCurrentUser().getName(), type, type, subsystem,
                            subsystem, system, sevi, op, subject, message);
                ...
                addFileMsg("Unlock Failed", "Unlock",
                        event.getFile().getName(), -1, OpcatUser.getCurrentUser().getName());
                return;
            } else if (action == SVNEventAction.UNLOCKED) {
                addFileMsg("Unlocked", "Unlock", event.getFile().getName(), -1,
                        OpcatUser.getCurrentUser().getName());
                processed.put(event.getFile(), OPCAT_MESSAGES_OP.FILES_UNLOCKED);
                ...
                if (event.getErrorMessage() != null) {
                    ...
                    addAdminMsg("Error" + gridArraySeperator + "Unknown error " + gridArraySeperator + "  ", "Repository Handler");
                }
            }
        } catch (Exception ex) {
