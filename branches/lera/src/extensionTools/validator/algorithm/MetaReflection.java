...
public class MetaReflection extends Reflection	{
	...
	public void addPair(IThingEntry thing) throws ValidationException {
		VRole vrole = new VRole(this.libraryID, thing.getId());
	...
	protected Set getSpecializationRoles(VRole vrole)	{
		...
		IThingEntry thing = (IThingEntry)this.model.getISystemStructure().getIEntry(vrole.getThingID());
		...
		while (connections.hasMoreElements())	{
			...
			if (rel.getRelationType() == OpcatConstants.SPECIALIZATION_RELATION) {
				IThingEntry specialization =(IThingEntry)this.model.getISystemStructure().getIEntry(
													rel.getDestinationId());
				VRole role = new VRole(this.libraryID, specialization.getId());
				set.add(role);
