 ...
 * that were within the container are placed outside the container TODO: When
 * dragging things into unfolded things, then the name of the main entity in the
...
public class MergeCommand {
	 ...
	 *             Thrown when the dragged thing has inzooming or unfolding
	 *             levels
	 */
	public boolean commitMerge(ThingInstance draggedInstance,
			ThingInstance targetInstance) throws OpcatApiException {
	...
	/**
	 * Handles copying of zoomin levels between the draffed thing and the target
	 * thing.
	 * 
	 * @param draggedInstance
	...
	protected void mergeUnfoldedLevels(ThingInstance draggedInstance,
		...
		try {
			this.commitMerge(oldMainInstance, newMainInstance);
	...
	protected void mergeRelations(ThingInstance draggedInstance,
		...
		while (relatedInstances.hasMoreElements()) {
			...
			if (tmpImstance instanceof LinkInstance) {
				...
				Instance otherSideInstance = null;
				if (linkInstance.getSource().getEntry().getId() == draggedEntry
						.getId()) {
					if(linkInstance.getDestinationInstance() instanceof StateInstance ) {
						otherSideInstance = (StateInstance) linkInstance
						.getDestinationInstance();
					} else {
					otherSideInstance = (ThingInstance) linkInstance
							.getDestinationInstance();
					}
				} else if (linkInstance.getSourceIXInstance() instanceof ThingInstance) {
					otherSideInstance = (ThingInstance) linkInstance
							.getSourceIXInstance();
				}
				if (otherSideInstance != null) {
					Enumeration e ;					
					if(otherSideInstance instanceof  StateInstance ) {
						e= ( (StateInstance) otherSideInstance).getRelatedInstances() ; 
					} else {
						e = ((ThingInstance) otherSideInstance).getRelatedInstances() ; 
					}
				...
				if (linkInstance.getSource().getEntry().getId() == draggedEntry
						.getId()) {
					this.currentProject.addLink(targetInstance, linkInstance
	...
	public boolean mergeOpd(Opd opdToMerge) {
		if (this.mergedOpds.contains(opdToMerge)) {
			return true;
		}
		this.mergedOpds.add(opdToMerge);
		Enumeration e = this.currentProject.getSystemStructure()
		...
		while (e.hasMoreElements()) {
			...
			if (a instanceof ObjectInstance) {
				a.setBorderColor((Color) this.currentProject.getConfiguration()
			...
			if (a instanceof ThingInstance) {
				Instance target = this.currentProject.getCurrentOpd()
						.getOpenReuseTargetInheritedInstance(a);
				if (target != null) {
					try {
						this.commitMerge((ThingInstance) a,
				...
				if (thi.isZoomedIn()) {
					...
					if (te.getZoomedInOpd() != null) {
						this.mergeOpd(te.getZoomedInOpd());
					}
				}
			}
