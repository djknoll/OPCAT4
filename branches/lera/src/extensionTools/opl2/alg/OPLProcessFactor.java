...
IEntry destination = this.builder.myStructure.getIEntry(rel.getDestinationId());
...
try {
  ...
  sentence.getProcessInheritanceSentence().add(this.buildProcInhSentence(rel));
}
catch (Exception e) {
  throw e;
}
...
String fatherName = this.builder.myStructure.getIEntry(rel.getSourceId()).
    getName();
try {
  ...
  protected ProcessUniDirectionalRelationSentence buildProcUniDirSentence(
    ...
    String fatherName = this.builder.myStructure.getIEntry(rel.getSourceId()).
        getName();
    String destination = this.builder.myStructure.getIEntry(rel.getDestinationId()).
        getName();
    try {
    ...
    try {
      ...
      if (rule == OPLGeneral.DESTINATION) {
        ...
        if((!OPLGeneral.isEmpty(rel.getForwardRelationMeaning())) && (!OPLGeneral.isEmpty(rel.getBackwardRelationMeaning())) )
         {
           this.fillProcBiDirSentence(sentence, destination, fatherName, meaning);
      ...
      this.fillProcBiDirSentence(sentence, fatherName, destination, meaning);
  ...
  protected ProcessTimeoutSentence buildProcessTimeoutSentence(ILinkEntry link, String path) throws Exception{
    ...
    IProcessEntry proc =(IProcessEntry)this.builder.myStructure.getIEntry(link.getSourceId());
...
//What is logical relation?????
  protected ConditionClause buildConditionClause(ILinkEntry link) throws
      Exception {
    IStateEntry state = (IStateEntry) this.builder.myStructure.getIEntry(link.
        getSourceId());
...
//What is logical relation?????
  protected EffectClause buildEffectClause(ILinkEntry link) throws Exception {
    IObjectEntry obj = (IObjectEntry) this.builder.myStructure.getIEntry(link.
        getSourceId());
...
//What is logical relation?????
  protected ResultClause buildResultClause(ILinkEntry link) throws Exception {
    IEntry entry = this.builder.myStructure.getIEntry(link.getDestinationId());
    String stateName = "";
    IObjectEntry obj;
    if (entry instanceof IStateEntry) {
      IStateEntry state = (IStateEntry) entry;
