...
class TestingEventsScheduler extends Hashtable {
	...
	public void scheduleEvent(TestingEvent aEvent) {
		TestingAuxiliary.debug(this, "scheduleEvent getSchedulingStep = "
				+ aEvent.getSchedulingStep());
	...
	public void removeAllScheduledEvents(long schedulingStep, long executionId) {
		TestingAuxiliary.debug(this, "Starting removeAllScheduledEvents");
			...
			for (int i = v.size() - 1; i >= 0; i--) {
				...
				if (aEvent.getSchedulingStep() > schedulingStep) {
					if ((executionId == 0)
						...
						TestingAuxiliary.debug(this, "remove event for step "
								+ aEvent.getStep());
				...
				if (aEvent.getCancelStep() > schedulingStep) {
					if ((executionId == 0)
						...
						TestingAuxiliary.debug(this,
								"restore cancelled event for step "
										+ aEvent.getStep());
		...
		TestingAuxiliary.debug(this, "After removeAllScheduledEvents");
		this.print();
	}
	...
	public void print() {
		...
		for (; en.hasMoreElements();) {
			Vector v = (Vector) en.nextElement();
			if (!v.isEmpty()) {
				TestingAuxiliary.debug(this, "Step: "
						+ ((TestingEvent) v.firstElement()).getStep());
			}
			// print the events stored inside teh vector
			for (int i = 0; i < v.size(); i++) {
				TestingEvent aEvent = (TestingEvent) v.get(i);
				if (aEvent.getEventType() == TestingSettings.ACTIVATION_EVENT) {
					TestingAuxiliary.debug(this, "#" + i
							+ ": Activation event");
				} else {
					TestingAuxiliary.debug(this, "#" + i
							+ ": Termination event");
				}
				TestingAuxiliary.debug(this, " was issued on "
						+ aEvent.getSchedulingStep());
			}
		}
	}
			...
			if (key.longValue() >= currentStep) {
				...
				for (int i = 0; i < v.size(); i++) {
					...
					if ((aEvent.getExecutionId() == executionId)
							&& (aEvent.getEventType() != TestingSettings.TERMINATION_EVENT)) {
						TestingAuxiliary.debug(this,
								"found future event, step " + aEvent.getStep());
						return aEvent;
					}
				}
			}
