 ...
 */
  ...
  */
  private void procsInRootOpd(IOpd rootIOpd,Vector orderProcVec)
  {
    ...
    list=this.MyStructure.getInstancesInOpd(rootIOpd.getOpdId());   //all IInstances in the root Opd;
    while (list.hasMoreElements()){
      ...
      if (inst instanceof IProcessInstance){    //if it is a process instance
        ...
        if((!this.hasIncomIvoc((IProcessEntry)procInst.getIEntry()))&&(!this.hasIncomAggr((IProcessEntry)procInst.getIEntry()))) {
 ...
 */
  ...
  */
    ...
    while (enumLoc.hasMoreElements()){
      ...
      while (childList.hasMoreElements()){
        ...
        if (inst instanceof IProcessInstance){
          ...
          if((!this.hasIncomIvoc(child))&&(!this.hasIncomAggr(child))) {
  ...
  private void createLists(Vector sortedProcVec,Vector orderProcVec){
    ...
    if (!enumLoc.hasMoreElements()){//if there are no more elements in enum-->no more procs
      list.addLast(procInst1.getIEntry());//adding the last proc to the last list
      orderProcVec.addElement(list);
    }

    while(enumLoc.hasMoreElements()){//if there are more procs in sorted vec
      list.addLast(procInst1.getIEntry());//adding the proc to the list
      procInst2 = (IProcessInstance)enumLoc.nextElement();//next proc
      if(this.isParalel(procInst1,procInst2)){//if proc1 and proc 2 are paralel-->in the same list
        ...
        if (!enumLoc.hasMoreElements()){//if there are no more elemnts in sorted vec
          list.addLast(procInst2.getIEntry());
          orderProcVec.addElement(list);
        }
        else{
 ...
 */
  private void createInvoceVec(String headProcId,IProcessEntry proc,Vector invocedProcVec,Vector transitionsVec,Vector TransitionPrint){
    ...
    while(locenum.hasMoreElements()){
      ...
      if (link.getLinkType() == OpcatConstants.INVOCATION_LINK){
        procInvoc=(IProcessEntry)this.MyStructure.getIEntry(link.getDestinationId());
        if(headProcId.compareTo(""+procInvoc.getId()+".5")!= 0){
          if(!this.branchIsNeeded(procInvoc,condVec,exVec,exProcVec)){//if branch is not needed
          ...
          this.createInvoceVec(headProcId,procInvoc,invocedProcVec,transitionsVec,TransitionPrint);
        }
      }
    }
  ...
  private void findConditonExeptionLink(IProcessEntry proc,Vector condLinkVec,Vector exptLinkVec,Vector exptLinkProcVec){
    ...
    while (locenum.hasMoreElements()){
      ...
      if (link.getLinkType()==OpcatConstants.CONDITION_LINK){
        absEntry=this.MyStructure.getIEntry(link.getSourceId()); //source of the link;
 ...
 */
  ...
  private boolean branchIsNeeded(IProcessEntry proc,Vector condLinkVec,Vector exptLinkVec,Vector exptLinkProcVec){
    ...
    this.findConditonExeptionLink(proc,condLinkVec,exptLinkVec,exptLinkProcVec);//creates vecs which contains all links of type exeption/conditon and they from state

    if ((condLinkVec.isEmpty())&&(exptLinkVec.isEmpty())&&(exptLinkProcVec.isEmpty())) {
  ...
  private void addSignalEvents(IProcessEntry proc,Vector signalEventVec,Vector condLinkVec,Vector exptLinkVec,Vector exptLinkProcVec){
    ...
    if (!condLinkVec.isEmpty()){//has condition links
      ...
      while (locenum.hasMoreElements()){
        ...
        state=((IStateEntry)(this.MyStructure.getIEntry(link.getSourceId())));
    ...
    if (!exptLinkProcVec.isEmpty()){//has exeption link to a process
      ...
      while (locenum.hasMoreElements()){
        link=(ILinkEntry)locenum.nextElement();//exeption link
        exptProc = ((IProcessEntry)(this.MyStructure.getIEntry(link.getSourceId())));
 ...
 */
  private InOutStructure  createInOutStructureProc(String headProcId,IProcessEntry proc,int forkIndex,LinkedList nextList,InOutStructure forkStructure,Vector transitionVec,String lastId,Vector TransitionPrint,Vector BranchPrint,Vector signalEventVec,Vector branchVec){
    ...
    if(this.branchIsNeeded(proc,condLinkVec,exptLinkVec,exptLinkProcVec)){
      branchStruct = this.createBranch(lastId,headProcId,proc,procStructure,transitionVec,TransitionPrint,BranchPrint);
      if (forkStructure==null){
        branchVec.addElement(branchStruct);
        branchVec.addElement(""+proc.getId()+".5.triggerOut");
      }
      this.addSignalEvents(proc,signalEventVec,condLinkVec,exptLinkVec,exptLinkProcVec);
      branchNeeded=true;
    }
    else
 ...
 */
  private InOutStructure createBranchInvoc(String headProcId,IProcessEntry invocProc,InOutStructure invocStructure,Vector transitionVec,Vector TransitionPrint,Vector BranchPrint,Vector branchVec){
    ...
    while(locenum.hasMoreElements()){
      link = (ILinkEntry)locenum.nextElement();
      if(link.getLinkType() == OpcatConstants.INVOCATION_LINK){
        srcProc = (IProcessEntry)this.MyStructure.getIEntry(link.getSourceId());
 ...
 */
  private InOutStructure createInvocProcStructure(String headProcId,IProcessEntry invocProc,Vector branchVec,Vector transitionVec,Vector TransitionPrint,Vector BranchPrint,Vector signalEventVec){
    ...
    if(this.branchIsNeeded(invocProc,condLinkVec,exptLinkVec,exptLinkProcVec)){
      this.createBranchInvoc(headProcId,invocProc,invocStructure,transitionVec,TransitionPrint,BranchPrint,branchVec);
      this.addSignalEvents(invocProc,signalEventVec,condLinkVec,exptLinkVec,exptLinkProcVec);
    }
 ...
 */
  ...
  try{
    ...
    if (join==null){//in current list one proc
      procStructure=this.createInOutStructureProc(headProc,proc,forkIndex,nextList,fork,transitionsVec,prevId,TransitionPrint,BranchPrint,signalEventVec,newBranchVec);//structure created for the proc
    } else {
		procStructure=this.createInOutStructureProc(headProc,proc,forkIndex,nextList,fork,transitionsVec,prevId,TransitionPrint,BranchPrint,signalEventVec,branchVec);//structure created for the proc
	}
    this.createInvoceVec(headProc,proc,invocedProcVec,transitionsVec,TransitionPrint);//creating a vector of all invoced process from proc
    ...
    if (invocedProcVec.isEmpty()){//no invoced process at all
      ...
      if (join==null){//one proc in the list-->no join outside
        if (!newBranchVec.isEmpty()){
          ...
          newBranch = (InOutStructure)locenum.nextElement();//new branch
        ...
        if (nextList==null){//it is the last list-->final state creation
          ...
          if (nextList.size()==1){//in next list only one process -->connecting directly to it
            ...
            Vector exVec = new Vector(2,2);
            Vector exProcVec = new Vector(2,2);
            if(this.branchIsNeeded(nextproc,condVec,exVec,exProcVec)) {
    ...
    //printing
    this.printAct(proc,procStructure,level,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec);
    ...
    while (invocEnum.hasMoreElements()){
      ...
      invocStructure = this.createInvocProcStructure(headProc,invocProc,branchVec,transitionsVec,TransitionPrint,BranchPrint,signalEventVec);//creating structure for invocProc
      if (!invocStructure.getOutVec().hasMoreElements()){//no invocations outgoing from the invocProc
        ...
        if (join==null){//need to check if new join needed
          if(this.joinNeeded(invocedProcVec)){//need to create new join
            needNewJoin=true;
            ...
            if (nextList==null){//this is the last proc-->need to be connected to a final state
              ...
              if (nextList.size()==1){
                IProcessEntry nextProc=((IProcessEntry)nextList.getFirst());
                if (this.branchIsNeeded(nextProc,vec1,vec2,vec1)){//if for new next proc needed branch
      ...
      this.printAct(invocProc,invocStructure,level,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec);
    ...
    try{
      ...
      if (!procVec.isEmpty()){     //if there are elemnets in the procVec sturcture-->Diagram Begins!-->initial State
        ...
        if(initialNeeded){
          ...
          this.printInitialState(headProcId,list,forkIndex,transitionsVec,TransitionPrint,InitialPrint,branchID);
      ...
      while((locenum.hasMoreElements())||(flag)){  //list out of vector
        if (flag) {
			flag=false;
		}
        if (locenum.hasMoreElements()){
        ...
        if (listSize>1){//fork and join needed
          ...
          while (!list.isEmpty()){//while there are things in list
            ...
            list.removeFirst();
            this.procTreatment(proc,headProcId,nextList,level,forkStructure,joinStructure,prevId,joinIndex,forkIndex,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec,branchVec);
          }
        ...
        if (listSize==1){//there is one process in the list
          proc = (IProcessEntry)list.getFirst();
          list.removeFirst();
          prevId = this.procTreatment(proc,headProcId,nextList,level,null,null,prevId,joinIndex,forkIndex,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec,branchVec);
    ...
    try{
      ...
      if(( this.isInzoomed(proc) || this.isUnfolded(proc) )&&(level>1)){
        ...
        if (this.isInzoomed(proc)){
          ...
          this.createActForProc(""+proc.getId()+"",proc.getName(),true,(level-1),orderProcVec,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec);
        }
        else
          if (this.isUnfolded(proc)){
            this.getUnfoldedProcs(proc,unfoldProcVec);//needd to be implemented
...
//--------------------------------------------
      ...
      if(opA == 1){         //from top level down
        ...
        if (!procVec.isEmpty()) {
			this.createActForProc("00.555","RootOpd",true,level,procVec,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec);//define  id=00.555 as id of the top level
		}
      }
      ...
      if(opA == 2){         //from proc down
        ...
        while(locenum.hasMoreElements()){
          ...
          if (!procVec.isEmpty()){
            this.createActForProc(""+proc.getId()+".5",proc.getName(),initialNeeded,level,procVec,transitionsVec,actDiagramsVec,ActivityPrint,ForkPrint,JoinPrint,InitialPrint,FinalPrint,BranchPrint,TransitionPrint,signalEventVec);
            initialNeeded = false;
          }
        }
      }
