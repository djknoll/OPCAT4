...
public class LoaderVer2 implements LoaderI {
	...
	public OpdProject load(JDesktopPane parentDesktop, Document document,
		...
		try {
			...
			// Added by Eran Toch (2/12/04). The root OPD is shown only after
			// all
	...
	protected void _loadEntityAttr(Entry entity, Element xmlEntity)
		...
		/*
		 * TODO: this is the place to load the url from the file. this is done
		 * here to save the need to create a new file loader and saver.
		 */
		try {
	...
	protected ThingInstance _loadVisualThing(Element xmlThing, Opd currentOpd,
			ThingInstance parent, int relationWithParent) throws JDOMException {
		...
		if (tData.getChild("VisualProcess") != null) {
			...
			// /
			/**
			 * compatability with older versions of new existence states.
			 */
//			try {
//				ObjectInstance obj = (ObjectInstance) myThing;
//				ObjectEntry myEntry = (ObjectEntry) obj.getEntry();
//
//				boolean addExistenceState = !(myEntry
//						.isStateEntryExists(OpmState.EXISTENT_STATE_NAME))
//						&& !(myEntry.getStateHash().size() > 0);
//				boolean addNonExixtenceState = !myEntry
//						.isStateEntryExists(OpmState.NONEXISTENT_STATE_NAME);
//
//				if (addExistenceState) {
//					StateEntry state = myProject.addState((OpdObject) obj
//							.getThing());
//					state.setName(OpmState.EXISTENT_STATE_NAME);
//					((StateInstance) state.getInstances().nextElement())
//							.setVisible(false);
//				}
//				if (addNonExixtenceState) {
//					StateEntry state = myProject.addState((OpdObject) obj
//							.getThing());
...
//				}
//				/**
//				 * now add the states to each instance of the object
//				 */
//				Iterator iter = Collections.list(myEntry.getInstances())
//						.iterator();
//
//				while (iter.hasNext()) {
//					ObjectInstance objIns = (ObjectInstance) iter.next();
//
//					boolean insHasMoreStates = objIns.getStateInstances()
//							.hasMoreElements();
//					boolean isStateExists = objIns
//							.isStateInstaceExists(OpmState.EXISTENT_STATE_NAME);
//
//					addExistenceState = !(isStateExists) && !(insHasMoreStates);
//
//					addNonExixtenceState = !objIns
//							.isStateInstaceExists(OpmState.NONEXISTENT_STATE_NAME);
//
//					if (addExistenceState) {
//						long stateInOpdId = currentOpd
//								._getFreeEntityInOpdEntry();
//						OpdKey stateKey = new OpdKey(currentOpd.getOpdId(),
//								stateInOpdId);
//
//						StateEntry sEntry = myEntry
//								.getState(OpmState.EXISTENT_STATE_NAME);
//
//						StateInstance tmpStateInstance = new StateInstance(
//								sEntry, stateKey, (OpdObject) objIns
//										.getConnectionEdge(), myProject);
//
...
//
//						StateEntry sEntry = myEntry
//								.getState(OpmState.NONEXISTENT_STATE_NAME);
//
//						StateInstance tmpStateInstance = new StateInstance(
//								sEntry, stateKey, (OpdObject) objIns
//										.getConnectionEdge(), myProject);
...
//			}
	...
	protected FundamentalRelationInstance _loadVisualFundamentalRelation(
		...
		if (destinationInstance == null) {
			...
			debug.Print(sourceEntry, "Source Entry - " + sourceEntry.getName(),
					"1");
			debug.Print(destinationEntry, "Destination Entry - "
					+ destinationEntry.getName(), "1");
			debug.Print(sourceInstance.getOpd(), "Opd "
					+ sourceInstance.getOpd().getName() + ", ID - "
					+ sourceInstance.getOpd().getOpdId(), "1");
			debug.Print(sourceInstance, "Source Instance Key"
					+ sourceInstance.getKey(), "1");
			debug.Print(xmlRelation, "XML - " + xmlRelation.getText(), "1");
			return null;
		}
	...
	protected void _loadVisualLink(Element xmlLink, Opd currentOpd)
		...
		if (destinationInstance == null) {
			Debug debug = Debug.getInstance();
			debug.Print(sourceEntry, "Source  " + sourceEntry.getName(), "1");
			debug.Print(destinationEntry, "Dest " + destinationEntry.getName(),
					"1");
			debug.Print(destinationEntry, "Dest in OPD ID "
