...
public class LoaderVer3 extends LoaderVer2 {
	...
	protected HashMap _loadScenarios(List scenarios) {
		...
		for (Iterator i = scenarios.iterator(); i.hasNext();) {
			...
			for (Iterator j = types.iterator(); j.hasNext();) {
				...
				scenHash.put(type, typeHash);
			}
			prjScenarios.put(scenName, scenHash);
		}
		return prjScenarios;
	}
	...
	protected void _loadMetaLibraries(List metaLibs, MetaManager man) {
		for (Iterator i = metaLibs.iterator(); i.hasNext();) {
			...
			if (dataType == null) {
				lib.setType(LibraryTypes.MetaLibrary); // support old versions of the
												// meta library infra
			} else {
	...
	protected void _loadThingAttr(ThingEntry thing, Element xmlThing)
		...
		Attribute visibility = xmlThing.getAttribute("areInstancesVisible");
		if (visibility == null){
			thing.setInstancesVisible(true);
		}else{
			thing.setInstancesVisible(visibility.getBooleanValue());
		}
	...
	protected void _loadLogicalProcess(Element xmlProcess) throws JDOMException {
		...
		Element xmlPreds = xmlProcess.getChild("Predecessors");
		if(xmlPreds!=null){
			List preds = xmlPreds.getChildren("Predecessor");
			for(int i=0; i< preds.size();i++){
				Element currPred = (Element)preds.get(i);
				Predecessor pr = new Predecessor();
				pr.setType(currPred.getAttributeValue("type"));
				pr.setProcessID(new Long(currPred.getAttributeValue("predID")).longValue());
				myProcess.addPredecessor(pr);
			}
		}
		// end reuse comment
	...
	protected void _loadConnectionEdgeAttr(ConnectionEdgeInstance cEdge,
		...
		super._loadConnectionEdgeAttr(cEdge, xmlConEdge);
		org.jdom.Attribute visibility = xmlConEdge.getAttribute("isVisible");
		if (visibility == null){
			cEdge.setVisible(true);
		}else{
			cEdge.setVisible(visibility.getBooleanValue());
		}
