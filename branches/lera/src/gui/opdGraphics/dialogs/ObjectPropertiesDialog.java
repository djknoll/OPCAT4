	...
	public ObjectPropertiesDialog(
		...
		if (!isCreation) {
			...
			if (StateMachine.isAnimated()){
				showTabs = BaseGraphicComponent.SHOW_INSTANCES;
			}
		} else {
			this.showTabs =
				BaseGraphicComponent.SHOW_ALL_TABS
					 - BaseGraphicComponent.SHOW_INSTANCES;
			showButtons =
		...
		if ((this.showTabs & BaseGraphicComponent.SHOW_2) != 0) {
			/*
			 * TODO: this is the keys and indices tab. i removed this as it has no real
			 * porpose and use. we can add this later when we create a real DB
			 * creation tools. 
			 * raanan 
			 * 
			 */
		...
		if ((this.showTabs & BaseGraphicComponent.SHOW_INSTANCES) != 0
				&& !myEntry.isInstance()) {
			this.tabs.add("Instances", new InstancesPanel(myEntry, myProject));
		}
		...
		KeyListener escapeAndEnterListener = new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				...
				if ((e.getKeyCode() == KeyEvent.VK_ENTER)
					&& !ObjectPropertiesDialog.this.description.hasFocus() /* && !type.hasFocus()*/
					) {
		...
		this.setLocation(
			fX + Math.abs(pWidth / 2 - this.getWidth() / 2),
			fY + Math.abs(pHeight / 2 - this.getHeight() / 2));
	...
	private void _initVariables() {
		try {
			if ((this.showTabs & BaseGraphicComponent.SHOW_1) != 0) {
				...
				this.initialValue = new JTextField(this.object.getInitialValue());
				...
				this.scope.setSelectedIndex(
					Character.digit((this.object.getScope()).charAt(0), 3));
				// end general tab
			...
			if ((this.showTabs & BaseGraphicComponent.SHOW_2) != 0) {
				...
				this.indexOrder =
					new JTextField(
						(new Integer(this.object.getIndexOrder())).toString());
				 ...
				 * raanan
				 */
				this.description = new JTextArea(this.object.getDescription(), 8, 30);
...
//						JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
				...
				//description
				this.generaldescription = new JTextArea(this.object.getDescription(), 8, 30);
				//generaldescription = new JTextArea("NOT IMPLEMENTED YET, do not enter anything here", 8, 30);
				this.generaldescriptionScrollPane =
					new JScrollPane(
...
//	}
	...
	private JPanel _constructMiscTab() {
		...
		ObjectEntry sampleEntry =
			new ObjectEntry(new OpmObject(0, "Sample"), this.myProject);
		...
		this.sampleObject.setBackgroundColor(this.opdObject.getBackgroundColor());
...
//			}
			...
			if (!this.object.equals(this.url.getText())) {
				this.object.setUrl(
					this.url.getText().trim());
			}
			
			if (!this.object.equals(this.generaldescription.getText())) {
				this.object.setDescription(this.generaldescription.getText().trim()) ; 
			}
		...
		if ((this.showTabs & BaseGraphicComponent.SHOW_2) != 0) {
			...
			try {
				this.object.setIndexOrder(
					(Integer.decode(this.indexOrder.getText())).intValue());
			} catch (NumberFormatException e) {
				JOptionPane.showMessageDialog(
					this,
					"Index Order is not an integer",
			...
			/*
			 * TODO: this should be returned to keys description when key are returned
			 */
		...
		//states
		if ((this.showTabs & BaseGraphicComponent.SHOW_3) != 0 && !isCreation) {
			this.opdObject.setStatesAutoarrange(this.statesPanel.isAutoarrange());
			this.opdObject.setStatesDrawingStyle(this.statesPanel.getStatesView());
		...
		//by Eran Toch
		this.object.getRolesManager().setRoles(this.rolesTab.getRoles());
		...
		object.updateDefaultURL() ; 
		return true;
	}

	private void _updateObjectsTypes(String newName) {
		for (Enumeration e = this.myProject.getComponentsStructure().getElements();
			e.hasMoreElements();
			) {
	...
	public void commitLockForEdit() {
		...
		this.initialValue.setEditable(false);
		this.indexOrder.setEditable(false);
		...
		this.isKey.setEnabled(false);
		this.isPersistent.setEnabled(false);
		this.objTypePanel.setEnabled(false);
		this.statesPanel.setEnabled(false);
		//

		Component[] array = this.statesPanel.getComponents();
		int i = 0;
	...
	Action positionAction = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			ObjectPropertiesDialog.this.addHelpON = ObjectPropertiesDialog.this.additionHelpON.isSelected() ; 			
			(ObjectPropertiesDialog.this).sampleObject.repaint();
		...
		public void actionPerformed(ActionEvent e){
			...
			if (newRef != null) {
				...
				if (type == MetaLibrary.TYPE_FILE ) {
					ObjectPropertiesDialog.this.url.setText("file:///".concat(path));
				}
				else {
