...
import gui.procDependencies.*;
import gui.procDependencies.gui.DependenciesProcessPropertiesTab;
 ...
 */
public class ProcessPropertiesDialog extends JDialog implements
		ComponentListener {
	 ...
	 * A JPanel containing roles selection.
	 * 
	 * @author Valeria Perelman
	 */
	private DependenciesProcessPropertiesTab dependenciesTab;
	 ...
	 *            <code>projectStructure.ThingEntry</code> class, entry of
	 *            this OPD Process in project dynamic structure.
	...
	public ProcessPropertiesDialog(OpdProcess pProcess, ProcessEntry pEntry,
		...
		if ((this.showTabs & BaseGraphicComponent.SHOW_5) != 0) {
			this.tabs.add("Dependencies", this._constructDependenciesTab());
		}
	...
	private JPanel _constructDependenciesTab(){
		this.dependenciesTab = new DependenciesProcessPropertiesTab(this.process, this.myProject);
		return this.dependenciesTab;
	}
	...
	private boolean _updateProcessData() {
		...
		if (!process.canBeGeneric() && !this.rolesTab.getRoles().isEmpty()
		...
		// end roles
		this.process.addPredecessors(this.dependenciesTab.getPredecessors());
	...
	class resourceListner implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			LibraryLocationDialog addLocation = new LibraryLocationDialog(
					Opcat2.getFrame(), "", MetaLibrary.TYPE_FILE);
