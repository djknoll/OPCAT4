...
public class StyledLine implements MoveUpdatable {
	...
	private boolean isForwardAnimated;
	...
	public synchronized void animate(long time, boolean isForwardAnimated) {
		if (this.animated) {
			if (time == 0) {
				this.totalTime = 0;
				this._stpTesting();
				...
				animate(0, !isForwardAnimated); 	// Workaround for animation problem
			}
		...
		this.animated = true;
		this.isForwardAnimated = isForwardAnimated;

		if (this.style == INVOCATION) {
			((OpdInvocationLine) this.lines.get(0)).setAnimated(true);
			((OpdInvocationLine) this.lines.get(0)).repaint();
			this.testingTimer = new javax.swing.Timer((int) time, this.stTesting);
