...
//	protected Hashtable multiMoveComponents;
  ...
  public static final int SHOW_INSTANCES = 256;
  public static final int SHOW_ALL_TABS = SHOW_1|SHOW_2|SHOW_3|SHOW_4|SHOW_5|SHOW_6|SHOW_7|SHOW_MISC|SHOW_INSTANCES;
...
//	}
	...
	public void repaintLines()
	{
		...
		{
		  return !(this.lines.isEmpty() && this.draggers.isEmpty() && this.updateListeners.isEmpty());
		}
	...
	public void mouseReleased(MouseEvent e)
	{
		...
		if (StateMachine.isAnimated()){
			if (e.isPopupTrigger() || e.isMetaDown()){
				this.showAnimationPopupMenu(e.getX(), e.getY());
			}
			return;
		}
...
//								 new Point(getX(), getY()), new Dimension(getWidth(), getHeight())));
			...
			for(Enumeration e1 = ht.keys(); e1.hasMoreElements();)
			{
				...
				undoAction.addAction(new UndoableMoveResizeComponent(bgc, p, d,
									 new Point(bgc.getX(), bgc.getY()), new Dimension(bgc.getWidth(), bgc.getHeight()), this.myProject));

			}

			for (Enumeration locenum = this.objects2Move.keys() ; locenum.hasMoreElements();)
			{
				...
				undoAction.addAction(new UndoableMoveResizeComponent(t, new Point((int)oldSize.getX(), (int)oldSize.getY()), new Dimension((int)oldSize.getWidth(), (int)oldSize.getHeight()),
					  new Point(t.getX(), t.getY()), new Dimension(t.getWidth(), t.getHeight()), this.myProject));
			}
		...
		for (Enumeration locenum = this.updateListeners.elements();locenum.hasMoreElements();)
		{
		...
		if (this.resize)
		{
			...
			switch (this.whichResize)
			{
				case OpcatConstants.N_BORDER:
				   ...
				   rHeight = this.initialY+this.initialHeight-( this.getY()-this.tempY+e.getY());
				   rX = this.initialX;
				   rY = this.getY()-this.tempY+e.getY();
				   break;
				case OpcatConstants.NE_BORDER:
				   rWidth = this.initialX+this.initialWidth-(this.getX()+this.tempX-e.getX());
				   rHeight = this.initialY+this.initialHeight-( this.getY()-this.tempY+e.getY());
				   rX = this.initialX;
				   rY = this.getY()-this.tempY+e.getY();
				   break;
			...
			boolean insideContainer = (this.tempContainer.contains(rX, rY) && this.tempContainer.contains(rX+rWidth, rY)
						&& this.tempContainer.contains(rX, rY+rHeight) && this.tempContainer.contains(rX + rWidth, rY + rHeight));
			...
			if ((rHeight>this.getMinimumSize().getHeight()) && insideContainer)
			{
	...
	public void setMoveable(boolean moveable)
	{
		...
		public void moveComp(MouseEvent e, int initX, int initY)
		{
			...
			if (tempContainer.contains(rX, rY) && tempContainer.contains(rX+this.getWidth(), rY)
				&& tempContainer.contains(rX, rY+this.getHeight()) && tempContainer.contains(rX + this.getWidth(), rY + this.getHeight()))
			{
				this.setLocation(this.getX()-initX+e.getX(), this.getY()-initY+e.getY());
				this.repaintLines();
			}
			return;
		}
...
*/
	...
	protected void showAnimationPopupMenu(int pX, int pY){}
