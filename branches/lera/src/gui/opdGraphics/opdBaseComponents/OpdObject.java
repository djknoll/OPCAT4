...
import gui.opdGraphics.popups.AnimatedObjectPopup;
 ...
 * In addition it can contain <a href = "OpdState.html"><code>OpdState</code></a>s
 * in two modes: autoarranged and free placed.
 */
public class OpdObject extends OpdThing {
	...
	public void paintComponent(Graphics g) {
		...
		if (this.isShadow()) {
			...
			g2.fillRect(shadowOffset, shadowOffset, this.getWidth() - shadowOffset
					- 1 - dSelOffset, this.getHeight() - shadowOffset - 1
					- dSelOffset);
		}
	 ...
	 * href = "../opmEntities/OpmObject.html"><code>OpmObject</code></a>
	 * this <code>OpdObject</code> represents.
	 */
	...
	public boolean isInAdjacentArea(int x, int y) {
		...
		if (!((x > (int) -proximity) && (y > (int) -proximity)
				&& (x < this.getActualWidth() + (int) proximity) && (y < this.getActualHeight()
				+ (int) proximity))) {
			return false;
		}
	...
	public boolean inResize(int pX, int pY) {
		...
		if (!((pX > borderSensitivity) && (pY > borderSensitivity)
				&& (pX < this.getActualWidth() - borderSensitivity) && (pY < this.getActualHeight()
				- borderSensitivity))) {
			return true;
		} else {
			return false;
	 ...
	 * </p>
	 * 
	 * @param <code>yn boolean</code>, if <code>true</code> autoarrang is on,
	 *            if <code>false</code> autoarrange off
	 */
	...
	public Point getAbsoluteConnectionPoint(RelativeConnectionPoint pPoint) {
		...
		case OpcatConstants.E_BORDER:
			...
			retPoint.setLocation(this.getActualWidth() - 2, this.getActualHeight()
					* pPoint.getParam());
			break;
	...
	private void drawText(Graphics2D g2) {
		...
		if ((this.myObjectEntry.getStateHash().size() == 0) || (this.statesDrawingStyle == 0)) {
			textShift = 0;
		} else {
			textShift = 0;
			Component components[] = this.getComponents();
		...
		if (this.getTextPosition().equals("C") || this.getTextPosition().equals("E")
		...
		if (this.getTextPosition().equals("N") || this.getTextPosition().equals("NE")
		...
		if (this.getTextPosition().equals("S") || this.getTextPosition().equals("SE")
		...
		//
		if (this.getTextPosition().equals("C") || this.getTextPosition().equals("S")
		...
		if (this.getTextPosition().equals("NE") || this.getTextPosition().equals("E")
		...
		if (this.getTextPosition().equals("NW") || this.getTextPosition().equals("W")
				|| this.getTextPosition().equals("SW")) {
			stringX = 9;
	...
	private void _drawNoOfInstances(Graphics2D g2) {
		...
		if (StateMachine.isAnimated()){
			val = ((OpmThing) this.myEntity).getNumberOfAnimatedInstances();
			if (val == 0) {
				return;
			}
		}else{
			val = ((OpmThing) this.myEntity).getNumberOfInstances();
			if (val == 1) {
				return;
			}
		}
		...
		stringX = this.getActualWidth() - 7 - this.currentMetrics.stringWidth(noi);
	...
	private void _addState() {
		...
		OpdObject sampleObjOpd = new OpdObject(sampleObjEntry, this.myProject, 0, 0);
    ...
    public void showAnimationPopupMenu(int pX, int pY) {
        JPopupMenu jpm = new AnimatedObjectPopup(this.myProject, this.getInstance());
