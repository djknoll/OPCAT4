...
public abstract class OpdConnectionEdge extends OpdBaseComponent {
	...
	private boolean levelColored = false;
	...
	public synchronized void animateUrl(long time) {
		...
		if (((url != null) || !url.equalsIgnoreCase("none"))
				&& (url.endsWith("jpg") || url.endsWith("bmp")
						...
						|| url.endsWith("wmv") || url.endsWith("wma") || url
						.endsWith("amr"))) {
			try {
				BrowserLauncher2.openURL(this.getEntity().getUrl());
			} catch (Exception e) {
				Debug debug = Debug.getInstance();
				debug.Print(this, e.getMessage(), "testing");
			}
		}
	}
	...
	public synchronized void animate(long time) {
		if (this.animated) {
			if (time == 0) {
				...
				stopTesting(0);  // Workaround for animation problem
			}
		}
	...
	public synchronized void stopTesting(long time) {
		...
		if (time == 0) {
			...
			animate(0);  // Workaround for animation problem
		}
		...
		public synchronized void actionPerformed(java.awt.event.ActionEvent e) {
			...
			if (timeLeft <= 0) {
				...
				if (edge instanceof OpdObject) {
					...
					//System.out.println("orign Color " + edge.originalColor);
	...
	public void changeToLevelColor(int level, int maxLevel) {
		...
		levelColored = true;
		if(maxLevel < colors.size()) {
			setBackgroundColor((Color)colors.get(level));
			return ; 
		} 
		
		int red = 255 - level * (255 / maxLevel);
		int green = level * (255 / maxLevel);
		int blue = 100;
		Color col = new Color(red, green, blue);
		setBackgroundColor(col);
	}

	public void restoreFromLevelColor() {
		if ((currentForColor != null) && (levelColored))
