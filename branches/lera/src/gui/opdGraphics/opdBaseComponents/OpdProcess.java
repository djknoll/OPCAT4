...
import gui.opdGraphics.popups.AnimatedProcessPopup;
...
public class OpdProcess extends OpdThing
{
	...
	private static final int BREAKPOINT_SIZE = 8;
	...
	public	OpdProcess(ThingEntry pEntry, OpdProject pProject, long pOpdId, long pEntityInOpdId)
	{
		...
		this.borderColor = (Color)pProject.getConfiguration().getProperty("ProcessColor");
		isBreakPoint = false;
	}
	...
	public void paintComponent(Graphics g)
	{
		...
		if (this.isShadow())
		{
			...
			g2.fillOval(shadowOffset, shadowOffset, this.getWidth()-shadowOffset-1-dSelOffset, this.getHeight()-shadowOffset-1-dSelOffset);
		}
		

		g2.setColor(this.backgroundColor);
		g2.fillOval(selOffset, selOffset,  this.getActualWidth()-1-dSelOffset, this.getActualHeight()-1-dSelOffset);
		...
		g2.drawOval(selOffset, selOffset,  this.getActualWidth()-1-dSelOffset, this.getActualHeight()-1-dSelOffset);
		...
		if (this.isBreakPoint && StateMachine.isAnimated()){
			g2.setColor(Color.red.brighter());
			g2.fillOval(getActualWidth() - BREAKPOINT_SIZE * 2,
						(getActualHeight() - BREAKPOINT_SIZE) /2,
	...
	{
		ProcessPropertiesDialog pd = new ProcessPropertiesDialog(this, (ProcessEntry)this.myEntry, this.myProject, false);
		pd.showDialog();
	}
	...
	{
		if ((pX<0) || (pX>this.getActualWidth()) || (pY<0) || (pY>this.getActualHeight())) {
			return false;
		}
	...
	public  Point getAbsoluteConnectionPoint(RelativeConnectionPoint pPoint)
	{
		...
		if (pPoint.getParam() > 1) {
			return new Point(this.getActualWidth()-2, this.getActualHeight()/2);
		}
		...
		switch (pPoint.getSide())
		{
			...
			case OpcatConstants.E_BORDER:
				...
				retPoint.setLocation(this.getActualWidth(), this.getActualHeight()*pPoint.getParam());
				break;
	...
	private void drawText(Graphics2D g2)
	{
        ...
        if (((OpmThing)this.myEntity).getScope().equals(OpcatConstants.PRIVATE))
        {
            head = "- ";
        }

        if (((OpmThing)this.myEntity).getScope().equals(OpcatConstants.PROTECTED))
        {
            head = "# ";
        }

		StringTokenizer st = new StringTokenizer(head+this.myEntity.getName(), "\n");
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.N_BORDER, 0.25));
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.N_BORDER, (aw-sw)/(2*aw)));
			stringX = (int)tempPoint.getX();
			stringY = (int)(tempPoint.getY() + 3);
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.N_BORDER, 0.75));
			...
			while (st.hasMoreTokens())
			{
				...
				ats.addAttribute(TextAttribute.FONT, currentFont);
				dY = dY + this.currentMetrics.getAscent();
				g2.drawString(ats.getIterator(), stringX+(int)(sw-this.currentMetrics.stringWidth(tString)), stringY+dY);
			}
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.S_BORDER, 0.25));
			stringX = (int)tempPoint.getX();
			stringY = (int)(tempPoint.getY() - 6 - this.currentMetrics.getAscent()*numOfRows);
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.S_BORDER, (aw-sw)/(2*aw)));
		...
		{
			tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.S_BORDER, 0.75));
			stringX = (int)(tempPoint.getX() - sw);
			stringY = (int)(tempPoint.getY() - 6 - this.currentMetrics.getAscent()*numOfRows);
	...
	{
		String role = ((OpmThing)this.myEntity).getRole();
		if(role == null)
		{
			return;
		}
		...
		Point tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.N_BORDER, 0.2));
	...
	private void _drawNoOfInstances(Graphics2D g2)
	{
		...
		Point tempPoint = this.getAbsoluteConnectionPoint( new RelativeConnectionPoint(OpcatConstants.S_BORDER, 0.8));
		stringY = (int)(tempPoint.getY() - 3*factor);
		stringX = (int)(tempPoint.getX() - this.currentMetrics.stringWidth(noi));
	...
	public void fitToContent()
	{
		...
		Font currentFont = (Font)this.myProject.getConfiguration().getProperty("ThingFont");
		currentFont = currentFont.deriveFont((float)(currentFont.getSize2D()*factor));
		this.currentMetrics = this.getFontMetrics(currentFont);
		...
		this.setActualSize((int)width, (int)height);
	}
	...
	public void drawSelection(Graphics2D g)
	{
		...
		g.fillRect(this.getActualWidth()/2-selOffset,0,dSelOffset,dSelOffset);
		...
		g.fillRect(0,this.getActualHeight()/2-selOffset,dSelOffset,dSelOffset);
		...
		g.drawRect(this.getActualWidth()/2-selOffset,0,dSelOffset,dSelOffset);
		p = this.getAbsoluteConnectionPoint(new RelativeConnectionPoint(OpcatConstants.N_BORDER, 0.8));
		g.drawRect((int)p.getX()+1-selOffset, (int)p.getY()+1-selOffset, dSelOffset,dSelOffset);
		g.drawRect(0,this.getActualHeight()/2-selOffset,dSelOffset,dSelOffset);
		p = this.getAbsoluteConnectionPoint(new RelativeConnectionPoint(OpcatConstants.S_BORDER, 0.2));
		g.drawRect((int)p.getX()+1-selOffset, (int)p.getY()-1-selOffset, dSelOffset,dSelOffset);
    ...
    public void showAnimationPopupMenu(int pX, int pY) {
        JPopupMenu jpm = new AnimatedProcessPopup(this.myProject, this.getInstance());
