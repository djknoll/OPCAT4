...
public class ProjectSummaryAction extends EditAction {
	...
	public ProjectSummaryAction() {
		super("Project Summary", StandardImages.PROPERTIES);
	}
							...
							while (roleIter.hasNext()) {
								...
								for (i = metaColIndexStart; i <= metaColIndexEnd; i++) {
									if (meta.getProjectHolder() instanceof MetaDataProject) {
										if (((String) colNames.elementAt(i))
												.equalsIgnoreCase(((MetaDataProject) meta
														.getProjectHolder())
														.getType())) {
											break;
										}
								...
								if ((addedRow[i] != null)
									...
									addedRow[i] = role.getThingName().replaceAll("\n"," ");
								}
							}
	...
	private void initGrid() {
		...
		Iterator iter = Collections.list(
				edit.getCurrentProject()
						.getMetaLibraries(LibraryTypes.MetaData)).iterator();
		if (iter.hasNext()) {
			while (iter.hasNext()) {
				MetaLibrary meta = (MetaLibrary) iter.next();
				if (meta.getState() == MetaLibrary.STATE_LOADED) {
					colNames.add(((MetaDataProject) meta.getProjectHolder())
							.getType());
				}
			}
		}
		Object[] types = edit.getCurrentProject().getModelTypes();
		...
		metaColIndexEnd = colNames.size() - 4;
		this.results = new GridPanel(colNames.size(), colNames);
