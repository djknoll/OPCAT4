...
*/
											  ...
											  OpdProject project,Container cn)
	{
		...
		this.destinationDragger = this.addTextDragger(this.destination , pPoint,this.logicalRelation.getDestinationCardinality());
...
//                cn.add(destinationDragger.getOpdCardinalityLabel(),JLayeredPane.MODAL_LAYER);
		...
		this.getGraphicalRelationRepresentation().getRelation().addUpdateListener(this);
		updateVisibility();

	}
	
	public void updateVisibility(){
		setVisible(destination.isVisible());
	}
	...
	{
		  RelativeConnectionPoint centerPoint = new RelativeConnectionPoint(BaseGraphicComponent.CENTER, 0);

		  this.lines[0] = this.createLine(this.myRepresentation.getRelation(), new RelativeConnectionPoint(OpcatConstants.S_BORDER, 0.5) ,  this.points[0] , centerPoint);
		  this.lines[1] = this.createLine(this.points[0], centerPoint,  this.points[1] , centerPoint);
		  this.lines[2] = this.createLine(this.points[1], centerPoint,  this.points[2] , centerPoint);
		  this.lines[3] = this.createLine(this.points[2], centerPoint,  this.points[3] , centerPoint);
		  this.lines[4] = this.createLine(this.points[3], centerPoint,  this.destinationDragger , centerPoint);
...
//	  cn.add(points[3], JLayeredPane.PALETTE_LAYER);
		  ...
		  int x1 = rel.getX() + rel.getWidth()/2 - this.points[0].getWidth()/2 - 5;
		  int y1 = rel.getY() + rel.getHeight();

		  int x2 = this.destinationDragger.getX() + this.destinationDragger.getWidth()/2 - this.points[0].getWidth()/2;
...
//       	  points[3].addMouseMotionListener(points[3]);
	...
	{
		OpdSimpleLine gLine;
		gLine = new OpdSimpleLine(edge1, pPoint1, edge2, pPoint2, this.logicalRelation,
									 this.getKey(), this.myProject);
...
//	container.repaint();
   ...
   {
       return this.myRepresentation.getSourceDragger().getRelativeConnectionPoint();
   }
   ...
   {
       this.myRepresentation.getSourceDragger().setRelativeConnectionPoint(point);
   }
   ...
   {
       return (IXConnectionEdgeInstance)this.myRepresentation.getSourceInstance();
   }
   ...
   {
       return (IConnectionEdgeInstance)this.myRepresentation.getSourceInstance();
   }
...
*/
	...
	{
		return this.destination.getEntry().getInstance(this.destination.getOpdKey());
	}
...
*/
	public OpmFundamentalRelation getLogicalRelation()
	{
		...
		public void setTextColor(Color textColor)
		{
		  ...
		  this.destinationDragger.getOpdCardinalityLabel().setTextColor(textColor);
		...
		public void setVisible(boolean isVisible)
		{
			...
			this.myRepresentation.updateVisibility();
		}
		...
		public void add2Container(Container cn)
		{
		  ...
		  for (int i = 0; i < this.lines.length; i++)
		  {
			cn.add(this.lines[i], new Integer(JLayeredPane.PALETTE_LAYER.intValue()-1));
		  }

		  cn.add(this.destinationDragger, new Integer(JLayeredPane.PALETTE_LAYER.intValue()+1));
		  this.destination.addDragger(this.destinationDragger);
		  cn.add(this.destinationDragger.getOpdCardinalityLabel(), JLayeredPane.MODAL_LAYER);
		...
		public void update()
		{
			this.destinationDragger.setText(this.logicalRelation.getDestinationCardinality());
			this.destination.repaintLines();
			for (int i = 0; i < this.points.length; i++)
			{
