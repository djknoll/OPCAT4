...
*/
	...
	public void setBackgroundColor(Color backgroundColor)
	{
		...
		public void setVisible(boolean isVisible)
		{
		  ...
		  Enumeration instances = getRelatedInstances();
		  while (instances.hasMoreElements()){
		  	Object currInstance = instances.nextElement();
		  	if (currInstance instanceof FundamentalRelationInstance){
		  		((FundamentalRelationInstance)currInstance).updateVisibility();
		  	}
		  }
		}
		...
		public Enumeration getRelatedInstances()
		{
			...
			for (Enumeration e1 = myEntry.getAllRelatedEntities(); e1.hasMoreElements();)
			{
				...
				Entry relEntry = this.myProject.getComponentsStructure().getEntry(relEntity.getId());
				...
				for (Enumeration e2 = relEntry.getInstances(); e2.hasMoreElements();)
				{
					...
					{
						LinkInstance li = (LinkInstance)relInstance;
						if (li.getSource().getOpdKey().equals(this.key) || li.getDestination().getOpdKey().equals(this.key))
						{
					...
					if (relInstance instanceof GeneralRelationInstance)
					{
						...
						if (gri.getSource().getOpdKey().equals(this.key) || gri.getDestination().getOpdKey().equals(this.key))
						{
					...
					if (relInstance instanceof FundamentalRelationInstance)
					{
						...
						if (fri.getGraphicalRelationRepresentation().getSource().getOpdKey().equals(this.key)
								|| fri.getDestination().getOpdKey().equals(this.key))
						{
