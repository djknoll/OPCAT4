...
public class ObjectEntry extends ThingEntry implements IObjectEntry,
	...
	public ObjectEntry(OpmObject pObject, OpdProject project, boolean instancesVisible) {
		super(pObject, project, instancesVisible);
		this.stateHash = new Hashtable();
	}

	public ObjectEntry(OpmObject pObject, ThingEntry parentThing,
			OpdProject project) {
		super(pObject, parentThing, project, true);
	}
	
	public ObjectEntry(OpmObject pObject, ThingEntry parentThing,
			OpdProject project, boolean instancesVisible) {
		super(pObject, parentThing, project, instancesVisible);
		this.stateHash = new Hashtable();
	}
	...
	*/
	...
	public int getNumberOfInstances(){
		Enumeration relations = getRelationBySource();
		int numberOfInstances = 0;
		while (relations.hasMoreElements()){
			RelationEntry currRelation = (RelationEntry)relations.nextElement();
	...
	/**
	* Determines whether the given ObjectEntry is instance (objects related by instantiation relation).
	* @return true if ObjectEntry is instance.
	*/
	
	public boolean isInstance(){
		Enumeration relations = getRelationByDestination();
		while (relations.hasMoreElements()){
			RelationEntry currRelation = (RelationEntry)relations.nextElement();
			if (currRelation.getRelationType() == OpcatConstants.INSTANTINATION_RELATION){
				return true;
