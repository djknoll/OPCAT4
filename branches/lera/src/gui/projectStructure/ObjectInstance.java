...
public class ObjectInstance extends ThingInstance implements IObjectInstance, IXObjectInstance
{
    ...
    public ObjectInstance(ObjectEntry myEntry, OpdKey myKey, Container container, OpdProject project, boolean bringStates)
    {
        ...
        {
            this.myConnectionEdge = new OpdObject(myEntry, this.myProject, myKey.getOpdId(), myKey.getEntityInOpdId());
        }

        myConnectionEdge.setVisible(myEntry.isInstancesVisible());
        ...
        if (bringStates)
        {
            ...
            for(Enumeration e = myEntry.getStateEnum(); e.hasMoreElements();)
            {
                ...
                {
                    tmpOpd = this.myProject.getComponentsStructure().getOpd(this.myConnectionEdge.getOpdId());
                }
                long stateInOpdId = tmpOpd._getFreeEntityInOpdEntry();
                OpdKey stateKey = new OpdKey(tmpOpd.getOpdId(), stateInOpdId);
                StateEntry sEntry = (StateEntry)(this.myProject.getComponentsStructure().getEntry(lState.getId()));
                StateInstance tmpStateInstance = new StateInstance(sEntry,stateKey, (OpdObject)this.myConnectionEdge, this.myProject);                
