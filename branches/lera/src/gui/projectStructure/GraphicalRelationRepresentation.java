...
public class GraphicalRelationRepresentation implements MoveUpdatable {
	...
	/**
	 * Creates GraphicalRelationRepresentationEntry that holds all information
	 * about specified pRelation ("triangle").
	 * 
	 * @param pSource
	 *            OpdConnectionEdge which is source for all fundamental relation
	 *            connected via this "triangle".
	 * @param pSourceDragger
	 *            AroundDragger which is connected to the source.
	 * @param pLine
	 *            OpdLine which connects sourceDragger to the "triangle".
	 * @param pRelation
	 *            OpdFundamentalRelation that represents graphically "triangle"
	 *            itself.
	 */
	...
	{
		this.graphicalInstances = new Hashtable();
		...
		this.sourceDragger = this.addDragger(this.source, pPoint1, this.myContainer);
	...
	private void createLines(Container cn) {
		...
		this.lines[0] = this.createLine(this.sourceDragger, centerPoint, this.points[0],
				centerPoint);
		...
		this.lines[4] = this.createLine(this.points[3], centerPoint, this.relation,
				new RelativeConnectionPoint(OpcatConstants.N_BORDER, 0.5));
	...
	private void createPoints(Container cn) {
		...
		if (this.source.getY() + this.source.getWidth() < this.relation.getY()) {
			...
			int diff = (this.relation.getY() - this.source.getY() - this.source.getWidth()) / 2;
	...
	/**
	 * Returns OpdConnectionEdge which is source for all fundamental relation
	 * connected via this "triangle".
	 */
	 ...
	 *         graphical instances of this GraphicalRelationRepresentationEntry.
	 *         Returns false and does nothing if specified key already exist in
	 *         the data structure.
	 */
	...
	private boolean areThereVisibleInstances(){
		Iterator iter = graphicalInstances.values().iterator();
		while (iter.hasNext()){
			
			if (((FundamentalRelationInstance)iter.next()).isVisible()){
				return true;
			}
		}
		return false;
	...
	public void updateVisibility(){
		setVisible(areThereVisibleInstances());
	}
	/**
	 ...
	 * GraphicalRelationRepresentationEntry. Use the Enumeration methods on the
	 * returned object to fetch the Instances sequentially
	 * 
