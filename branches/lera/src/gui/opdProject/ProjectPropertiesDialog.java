 ...
 * Afterwards, the information is processed by the <code>Documents</code>
 * tool.
 * 
...
public class ProjectPropertiesDialog extends JDialog {
	...
	private void jbInit() throws Exception {
		...
		this.informations[2] = new GenInfoEntry("Possible_Users_for_the_System",
				"Possible Users", this.currentProject
		...
		this.informations[10] = new GenInfoEntry("Client", "Client", this.currentProject
				.getInfoValue("Client"));
		...
		this.date = new JTextField(this.currentProject.getCreationDate().toString());
		this.date.setEditable(false);
		
		Object[] types = currentProject.getModelTypes() ;		
		...
		this.tabs.add(this.importedLibraries, " Meta-Libraries ");
	 ...
	 * the last entry value in the <code>informations</code> array.
	 * Afterwards, the method changes the <code>textarea</code> input field,
	...
	void doOK() {
		...
		Opcat2.getFrame().setTitle("Opcat II - " + this.name.getText());
		...
		for (int i = 0; i < this.informations.length; i++) {
			...
			if ((entry != null) & (entry.getKey() != null)
					&& (entry.getValue() != null)) {
				this.currentProject.setInfoValue(entry.getKey(), entry.getValue());
			}
		}
		this.okPressed = true;
		this.dispose();
