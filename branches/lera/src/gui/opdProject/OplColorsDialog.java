...
public class OplColorsDialog extends JDialog {
	...
	private final String sizes[] = { "Smallest", "Smaller", "Medium", "Larger",
			"Largest" };
	...
	private void jbInit() throws Exception {
		...
		this.border1 = BorderFactory.createEtchedBorder(Color.white, new Color(142,
				142, 142));
		...
		this.sizesBox.addActionListener(new OplColorsDialog_sizesBox_actionAdapter(
				this));
		...
		this.okButton.addActionListener(new OplColorsDialog_okButton_actionAdapter(
				this));
		...
		this.boldBox.addActionListener(new OplColorsDialog_boldBox_actionAdapter(
				this));
	...
	private void myInit() {
		for (int i = 0; i < this.sizes.length; i++) {
			this.sizesBox.addItem(this.sizes[i]);
		}

		this.colorScheme.getSchemeFrom(this.originalScheme);

		this.sizesBox.setSelectedIndex(this.colorScheme.getTextSize());
	...
	void elementsList_valueChanged(ListSelectionEvent e) {
		ColorAttribute att = (ColorAttribute) this.elementsList.getSelectedValue();
		if (att == null) {
			return;
		}
	...
	void sizesBox_actionPerformed(ActionEvent e) {
		this.colorScheme.setTextSize(this.sizesBox.getSelectedIndex());
	}
