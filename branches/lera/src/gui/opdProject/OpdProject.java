...
public class OpdProject implements ISystem, IXSystem {
	...
	private HashMap scen = new HashMap();
	...
	public OpdProject(JDesktopPane parent, long pId) {
		...
		this.modelTypes.put("4", "Component Catalog");
	...
	public void arrangeOPDs(OpdLayout layout){
		Dimension desktopSize = parentDesktop.getSize();
		int numOfRows = layout.getNumOfRows();
		if (numOfRows <= 0){
			return;
		}
		int rowHeight = desktopSize.height / numOfRows;
		Iterator<ArrayList<Long>> rows = layout.getRows();
		int rowNum = 0;
		while (rows.hasNext()){
			ArrayList<Long> currRow = rows.next();
			int rowWidth = desktopSize.width / currRow.size();
			for (int i = 0; i < currRow.size(); i++){
				Opd currOpd = getOpdByID(currRow.get(i));
				currOpd.getOpdContainer().setBounds(i * rowWidth, rowNum * rowHeight, rowWidth, rowHeight);
			}
			rowNum++;
		}
		
		Enumeration opds = mainComponentsStructure.getAllOpds();
		while (opds.hasMoreElements()){
			Opd currOpd = (Opd)opds.nextElement();
			currOpd.getOpdContainer().setVisible(layout.isInLayout(currOpd.getOpdId()));
		}
	}
	...
	public void unfolding() {
		...
		if (myEntry.isEnvironmental()
			...
			while (iter.hasNext()) {
				...
				try {
					...
					Debug debug = Debug.getInstance();
					debug.Print(role.getOntology(), ex.getLocalizedMessage(),
							"1");
				}
			}
			return;
		...
		int retValue = JOptionPane.NO_OPTION;
		...
		this.currentOpd = this.unfolding(myInstance,
	...
	public IXObjectInstance addObject(String name, int x, int y, long opdId) {
		return addObject(name,  x,  y,  opdId, true);
	}
	...
	public UndoInfo addInstance4Object(IXObjectEntry parent, boolean isVisible){
		return addInstance4Object(parent, -1, isVisible);
	}
	...
	public UndoInfo addInstance4Object(IXObjectEntry parent, long instanceId, boolean isVisible){
		...
		ObjectEntry newObject = null;
		Enumeration instances = parent.getInstances();
		while (instances.hasMoreElements()){
			ObjectInstance currInstance = (ObjectInstance)instances.nextElement();
			Container container = currInstance.getConnectionEdge().getParent();
			
			if (newObject != null){
				instanceId = newObject.getId();
			}else{
				if (instanceId != -1){  // Treatment for reserved Ids
					if (getComponentsStructure().getEntry(instanceId) != null){
						throw new IllegalArgumentException("Entry with passed instanceId already exists");
					}
					
					ObjectEntry myObject;
					if (container instanceof OpdThing) {
						ThingEntry parentEntry = (ThingEntry) ((OpdThing) container)
								.getEntry();
						myObject = new ObjectEntry(new OpmObject(instanceId, ""), parentEntry, this, isVisible);
					} else {
						myObject = new ObjectEntry(new OpmObject(instanceId, ""), this, isVisible);
					}
					
					getComponentsStructure().put(instanceId, myObject);
				}
			}
			

			ObjectInstance newInstance = addObject(currInstance.getX() + 100, currInstance.getY() + 100, 
					currInstance.getConnectionEdge().getParent() , currInstance.getOpd().getOpdId(), instanceId, -1, true, isVisible);

			if (newObject == null){
				newObject = (ObjectEntry)newInstance.getEntry();
				Enumeration states = parent.getStates();
				while (states.hasMoreElements()){
					StateEntry currState = (StateEntry)states.nextElement();
					StateEntry newState = (StateEntry)addState("", newInstance, false);
					OpmState parentState = (OpmState)((Entry)currState).getLogicalEntity();
					((OpmState)newState.getLogicalEntity()).copyPropsFrom(parentState);
					newState.updateInstances();
				}
				
				OpmObject parentEntity = (OpmObject)((Entry)parent).getLogicalEntity();
				((OpmObject)newObject.getLogicalEntity()).copyPropsFrom(parentEntity);
				newObject.setName(parent.getName()+" #"+newObject.getId());
				newObject.updateInstances();
			}
			
			((ObjectInstance) newInstance).getConnectionEdge().fitToContent();
			
			undoAction.addAction(new UndoableAddObject(this, newInstance));
			undoAction.addAction(new UndoableAddFundamentalRelation(this, 
					(FundamentalRelationInstance)addRelation(currInstance, newInstance, OpcatConstants.INSTANTINATION_RELATION, false)));
			
			break; // at this moment Opcat doen't know add more that one instance representation
		}
		
		return new UndoInfo(newObject, undoAction);
	}
	
	public IXObjectInstance addObject(String name, int x, int y, long opdId, boolean isVisible) {
		...
		IXObjectInstance newIns = this.addObject(x, y,
				tempOpd.getDrawingArea(), -1, -1, false, isVisible);
	...
	public IXObjectInstance addObject(String name, int x, int y,
		...
		return addObject(name, x, y, parentInstance, true);
	}
	public IXObjectInstance addObject(String name, int x, int y,
			IXThingInstance parentInstance, boolean isVisible) {
	...
	public IXRelationInstance addRelation(IXConnectionEdgeInstance source,
		...
		return addRelation(source, destination, relationType, true);
	}
	
	public IXRelationInstance addRelation(IXConnectionEdgeInstance source,
			IXConnectionEdgeInstance destination, int relationType, boolean showRelation) {
		...
		if (showRelation && ((this.currentOpd == null)
	...
	public CheckResult switchLinkEdge(LinkInstance instance,
		...
		LinkInstance newInstance = this.addLink(source, sourcePoint,
				destination, destinationPoint, cn, Constants
	...
	public IXStateEntry addState(String name, IXObjectInstance parentObject) {
		return addState(name, parentObject, true);
	}
	public IXStateEntry addState(String name, IXObjectInstance parentObject, boolean showState) {
		long opdId = parentObject.getKey().getOpdId();

		if (showState && ((this.currentOpd == null) || (this.currentOpd.getOpdId() != opdId))) {
			this.showOPD(opdId);
		}
				...
				newLi = this.addLink(newSrc.getConnectionEdge(), origLi
						...
						newDst.getConnectionEdge(), origLi
								...
								.getType4Link((OpmProceduralLink) origLi
										.getEntry().getLogicalEntity()), origLi
								.getEntry().getId(), -1);
						...
						.addRelation(
								...
								Constants
										.getType4Relation((OpmStructuralRelation) origFri
												.getEntry().getLogicalEntity()),
								newSrc.getConnectionEdge().getParent(), origFri
										.getEntry().getId(), -1);
	...
	public AbstractUndoableEdit deleteEntry(IXConnectionEdgeEntry entry){
		...
		while (instances.hasMoreElements()){
			ConnectionEdgeInstance currInstance = (ConnectionEdgeInstance)instances.nextElement();
			Enumeration related = currInstance.getRelatedInstances();
			while (related.hasMoreElements()){
				Instance relatedInstance = (Instance)related.nextElement();
				undoAction.addAction(deleteInstance(relatedInstance));
			}
	...
	public ObjectInstance addObject(int pX, int pY, Container container,
		...
		return addObject(pX, pY, container,
				entityId, entityInOpdId, bringStates, true); 
	}
	
	public ObjectInstance addObject(int pX, int pY, Container container, 
			long entityId, long entityInOpdId, boolean bringStates, boolean isVisible) {
		return addObject(pX, pY, container, -1,
				entityId, entityInOpdId, bringStates, isVisible);
	}	
	...
	public ObjectInstance addObject(int pX, int pY, Container container, long opdId,
			long entityId, long entityInOpdId, boolean bringStates, boolean isVisible) {
		...
		if (entityId == -1) {
			...
			if (container instanceof OpdThing) {
				ThingEntry parentEntry = (ThingEntry) ((OpdThing) container)
						.getEntry();
				myEntry = new ObjectEntry(lObject, parentEntry, this, isVisible);
			} else {
				myEntry = new ObjectEntry(lObject, this, isVisible);
			}
		...
		if (opdId == -1){
			opdId = this.currentOpd.getOpdId();
		}
		
		OpdKey key = new OpdKey(opdId, objectInOpdId);
...
//		} 
	...
	public ObjectInstance addObject(Opd destOpd, int pX, int pY,
		...
		ObjectInstance myInstance = addObject(pX, pY, container, entityId, entityInOpdId, bringStates) ;
				...
				while (sourceEnum.hasMoreElements() && !cancel) {
					...
					while (destEnum.hasMoreElements() && !cancel) {
						...
						if (this.currentOpd.isContaining(locSourceInstance)
								&& this.currentOpd
										.isContaining(locDestInstance)
								&& !this.currentOpd.isConnected(
										locSourceInstance, locDestInstance, li)) {
	...
	public HashMap getScen() {
		return scen;
	}

	public void setScen(HashMap scen) {
		this.scen = scen;
	}
