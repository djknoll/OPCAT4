...
public class Repository extends JPanel implements ChangeListener {
    ...
    public Repository(OpdProject prj, Opcat2 opcat2) {
        ...
        if (this.project == null) {
            ...
            this.opdThingsView = new OpdThingsView("root"); // this is not
            // visible
            DefaultTreeModel model = (DefaultTreeModel) this.opdThingsView.getModel();
    ...
    public void changeView(int i) {
        ...
        if ((currentView != null) && (currentView.getModel().getRoot() != null)) {
            this.currentView.expandedDescendants = this.currentView.getExpandedDescendants(new TreePath(currentView.getModel().getRoot()));
        }
    ...
    public void rebuildTrees(boolean current) {
        ...
        if (!(currentView.isCanViewWitoutProject()) && (this.project == null)) {
            for (Enumeration<BaseView> e = this.trees.elements(); e.hasMoreElements();) {
        ...
        if (current) {
            if (this.project == null) {
                ...
                this.currentView.rebuildTree(this.project.getComponentsStructure(), this.project);
            }
        } else {
            for (Enumeration<BaseView> e = this.trees.elements(); e.hasMoreElements();) {
                BaseView view = (BaseView) e.nextElement();
