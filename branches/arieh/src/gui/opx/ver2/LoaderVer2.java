...
public class LoaderVer2 implements LoaderI {
    ...
    protected void _loadEntityAttr(Entry entity, Element xmlEntity)
        ...
        entity.getLogicalEntity().getMyProps().loadProperties(xmlEntity.getChild("OPMProperties"));
...
//        }
    ...
    protected FundamentalRelationInstance _loadVisualFundamentalRelation(
        ...
        if (sourceEntry == null) {
            OpcatLogger.error("Source Entry - NULL, ID In XMl file = "
                    + cPart.getAttribute("sourceId").getLongValue());
            OpcatLogger.error("OPD - " + currentOpd.getName());
            OpcatLogger.error("XML - " + xmlRelation.toString());
            return null;
        }
        ...
        if (destinationEntry == null) {
            OpcatLogger.error("Source Entry - " + sourceEntry.getName());
            OpcatLogger.error("Destination Entry - NULL, ID In XMl file = "
                    + xmlRelation.getAttribute("destinationId"));
            OpcatLogger.error("OPD - " + currentOpd.getName());
            OpcatLogger.error("XML - " + xmlRelation.toString());
            return null;
        ...
        if (sourceInstance == null) {
            OpcatLogger.error("Source Entry - " + sourceEntry.getName());
            OpcatLogger.error("Destination Entry - "
                    + destinationEntry.getName());
            OpcatLogger.error("Source Instance = NULL");
            OpcatLogger.error("OPD - " + currentOpd.getName());
            OpcatLogger.error("XML - " + xmlRelation.toString());
            return null;
        }
        ...
        if (destinationInstance == null) {
            ...
            OpcatLogger.error("Destination Entry - "
                    + destinationEntry.getName());
            OpcatLogger.error("Source Instance Key "
    ...
    protected void _loadVisualLink(Element xmlLink, Opd currentOpd)
        ...
        ConnectionEdgeInstance destinationInstance = (ConnectionEdgeInstance) destinationEntry
                .getInstance(new OpdKey(currentOpd.getOpdId(), lineAttr
                        .getAttribute("destinationInOpdId").getLongValue()));
        if (destinationInstance == null) {
            OpcatLogger.error("Source  " + sourceEntry.getName());
            OpcatLogger.error("Dest " + destinationEntry.getName());
            OpcatLogger.error("Dest in OPD ID "
