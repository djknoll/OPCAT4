...
public class LoaderVer3 extends LoaderVer2 {
    ...
    protected void _loadMetaLibraries(List<Element> metaLibs, MetaManager man) {
        for (Iterator<Element> i = metaLibs.iterator(); i.hasNext();) {
            ...
            } else if (dataReferenceType == DataCreatorType.REFERENCE_TYPE_CATEGORY_FILE) {
                String originalPath = (prop.getChildText("OriginalPath") != null ? prop.getChildText("OriginalPath")
                        : loadingPath);
...
//        }
    ...
    protected void _loadLogicalRelation(Element xmlRelation)
        ...
        OpmConnectionEdge source = (OpmConnectionEdge) this.myProject.getComponentsStructure().getEntry(
                xmlRelation.getAttribute("sourceId").getLongValue()).getLogicalEntity();
        OpmConnectionEdge destination = (OpmConnectionEdge) this.myProject.getComponentsStructure().getEntry(
                xmlRelation.getAttribute("destinationId").getLongValue()).getLogicalEntity();

        OpmStructuralRelation opmRel = Constants.createRelation(xmlRelation.getAttribute("relationType").getIntValue(), relId, source,
                destination);
