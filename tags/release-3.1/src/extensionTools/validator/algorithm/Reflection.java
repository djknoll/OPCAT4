...
public abstract class Reflection {
	 ...
	 */
	protected abstract void addPair(IConnectionEdgeEntry thing) throws ValidationException;
	 ...
	 * @return
	 */
	protected abstract MultipleApplicant createApplicant(IConnectionEdgeEntry thing) throws ValidationException;
	...
	protected ConstraintSet findConstraints(Finder vThing, IConnectionEdgeEntry thingEntry)	
		throws ValidationException
	{
		Vector neighbors = vThing.findNeighbors(thingEntry, this.model.getISystemStructure());
		ConstraintSet set = new ConstraintSet();
		for (int i=0; i<neighbors.size(); i++)	{
			Neighbor neighbor = (Neighbor)neighbors.get(i);
			try	{
				MultipleApplicant applicant = this.createApplicant(neighbor.getConnectedThing());
				Constraint constraint =
				this.createConstraintObject(
