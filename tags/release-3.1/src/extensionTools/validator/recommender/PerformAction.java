...
public class PerformAction extends AbstractAction {
	...
	private IConnectionEdgeInstance instance;
	...
	public void actionPerformed(ActionEvent arg0) {
		try {
			IConnectionEdgeEntry roleThing;
			if (offence == null) {
				roleThing = (IConnectionEdgeEntry) this.neighbor
						.getConnectedThing();

			} else {
				roleThing = (IConnectionEdgeEntry) ((OpdProject) this.metaLib
			...
			ArrayList<Instance> instances = opd.getInstancesInOPDOfRole(role);
			if (instances.size() > 0) {
			...
			if (!connectToExisting) {
				...
				if (!(instance instanceof StateInstance) && (instance.getIOPD().getMainIInstance() != null)
					...
					if (ret == JOptionPane.NO_OPTION) {
						...
						newInstance = MetaLibrary
								.insertConnectionEdge(role, Opcat2
										.getCurrentProject().getCurrentOpd(),
										true,
										(ConnectionEdgeInstance) instance,
										parent);
					} else {
						newInstance = MetaLibrary.insertConnectionEdge(role,
								...
								true, (ConnectionEdgeInstance) instance, null);
					}
				...
				newInstance = (IXConnectionEdgeInstance) existingInstance;
			}
