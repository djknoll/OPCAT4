...
/**
 * <p>
 * Title:class InteractionWithOpcat
 * </p>
 * <p>
 * Description: This class contains all the interaction with opcat after
 * recognizing the shape that was drawn
 * </p>
 */
...
public class OpenDrawInOpcat {
	...
	/**
	 * static protected Rectangle shapeRectangle the enclosing rectangle of the
	 * draw
	 */
	static protected Rectangle shapeRectangle;// the enclosing rectangle of
	...
	/**
	 * static protected IXConnectionEdgeInstance openIn the Instance in which
	 * the draw should be open
	 */
	static protected IXConnectionEdgeInstance openIn = null;// the Instance in

	// which the draw
	// should be open

	/**
	 * static protected Point openAt the location in which the draw should be
	 * open
	 */
	static protected Point openAt = new Point(10, 10);// the location in which

	// the draw should be
	// open

	/**
	 * private OpdConnectionEdge my_OpdConnectionEdge the current
	 * OpdConnectionEdge in opcat
	 */
	...
	// private Graphics g;// the current graphics
	/**
	 * private int linkType holds the diagnose in case of a link or a general
	 * relation
	 */
	private int linkType = -1;// holds the diagnose in case of a link or a

	// general relation

	/**
	 * static protected int addToX an extension to a search point in case of a
	 * state Instance in x direction
	 */
	static protected int addToX = 0;// an extension to a search point in case of

	// a state Instance in x direction

	/**
	 * static protected int addToY an extension to a search point in case of a
	 * state Instance in y direction
	 */
	static protected int addToY = 0;// an extension to a search point in case of

	// a state Instance in y direction

	/**
	 * private IXConnectionEdgeInstance sourceThingInstance the source Instance
	 * for a link or a relation
	 */
	private IXConnectionEdgeInstance sourceThingInstance = null;// the source
	...
	/**
	 * private IXConnectionEdgeInstance destinationThingInstance the destination
	 * Instance for a link or a relation
	 */
	private IXConnectionEdgeInstance destinationThingInstance = null;// the
	 ...
	 *            the current project
	 */
	public OpenDrawInOpcat(OpdProject myProject_) {
	...
	/**
	 * the function openOpcatShape opens the draw in opcat according to the
	 * diagnose
	 * 
	 * @param shapeDiagnose
	 *            the diagnose
	 * @param wholeShapeRect
	 *            the enclosing rectangle of the draw
	 */
	public void openOpcatShape(final int shapeDiagnose, Rectangle wholeShapeRect) {
		LinkInstance link = null;// a new link
		this.diagnose = shapeDiagnose;
		shapeRectangle = wholeShapeRect;
		Point sourcePoint;// holds the sourcePoint incaes the diagnose is
		// link/relation
		Point destinationPoint;// holds the destinationPoint incaes the
		// diagnose is link/relation
		...
		if ((this.diagnose > 2) && (this.diagnose != 6) && (this.diagnose != 50)) {
			if (DrawAppNew.link != null) {// if diagnose is a link

				sourcePoint = new Point(DrawAppNew.link.getSourceEdgePoint());// the
				// point
				// around
				// it
				// we
				...
				// source
				// Instance
				destinationPoint = new Point(DrawAppNew.link
						.getDestinationEdgePoint());// the point around it we
				...
				addToX = this.addToX(sourcePoint, destinationPoint, sourceRange);
				addToY = this.addToY(sourcePoint, destinationPoint, sourceRange);

				Point[] source = { sourcePoint };// an array of points
				this.sourceThingInstance = nearestThingInstance(source, sourceRange);// find
				// the
				// valid
				// source
				// Instance
				// an extension to a search point in case of a state Instance
				addToX = this.addToX(destinationPoint, sourcePoint, destinationRange);
				addToY = this.addToY(destinationPoint, sourcePoint, destinationRange);

				Point[] destination = { destinationPoint };// an array of
				// points
				this.destinationThingInstance = nearestThingInstance(destination,
						destinationRange);// find the valid destination
				// Instance
				// in case didn't found sourceThingInstance or
				// destinationThingInstance open message window
				if ((this.sourceThingInstance == null)
						|| (this.destinationThingInstance == null)) {
		...
		switch (this.diagnose) {

		case DrawAppNew.rectangle:
			this.openThing(DrawAppNew.rectangle);
			break;

		case DrawAppNew.ellipse:
			this.openThing(DrawAppNew.ellipse);
			break;

		case DrawAppNew.fullRelation:
			this.showPopUp(DrawAppNew.relation.getDestinationEdgePoint(), 0);
			break;
		...
		case DrawAppNew.bidirectionalRelation:
			if (this.linkType == -1) {// if link type dosen't hold the diagnose
				// yet
				this.linkType = 206;
		...
		case DrawAppNew.openArrow:
			...
			GeneralRelationInstance relation = (GeneralRelationInstance) openGeneralRelation(
					this.linkType, this.sourceThingInstance, this.destinationThingInstance);
			this.enableGeneralRelation(relation);
			break;
		...
		case DrawAppNew.triangleArrow:
			if (this.linkType == -1) {// if link type dosen't hold the diagnose
				// yet
				if (((ConnectionEdgeInstance) this.sourceThingInstance) instanceof ProcessInstance) {
					this.linkType = 306;
				} else {
					this.linkType = 301; // source is state or object
				}
			}
		...
		case DrawAppNew.instrumentArrow:
			...
			link = this.openLink(this.linkType, this.sourceThingInstance,
					this.destinationThingInstance);
			if (link == null) { // in case open link fails: source or
				// destination are null
				break;
			}
		...
		default:
			// JOptionPane.showMessageDialog(myProject.getMainFrame(), "Cannot
			// Recognize The Shape, Please Try Again", "Message"
			// ,JOptionPane.ERROR_MESSAGE);//roni
			// System.out.println("System didn't recognized the draw");

			myProject.refresh(); // clean the drawing area screen from the
			// draw
	...
	/**
	 * the function nearestThingInstance find the valid Instance. if draw was
	 * process/object/state, finds the Instance inside it most of the draw was
	 * drawn. if draw was a link or relation find the Instance of the source or
	 * destination according to the given points
	 * 
	 * @param inPoints
	 *            array of points where to look for an Instance: 1 if its a
	 *            link/relation, 3 if its process/object/state
	 * @param radiusAroundPoint
	 *            the range from the point where an Instance is considered valid
	 * @return the valid Instance
	 */
	static protected IXConnectionEdgeInstance nearestThingInstance(
			Point[] inPoints, int radiusAroundPoint) {

		boolean isObject = false;// boolean which tells if it's an object
		IXConnectionEdgeInstance nearest = null;// holds the nearest instance to
		// the source/dest point
		IXStateInstance nearestState = null;
		myOpd = myProject.getCurrentOpd();
		IXSystemStructure mySystemStructure = myProject.getIXSystemStructure();// holds
		// the
		// current
		// system
		// Structure
		Enumeration enumOutsideZoom = mySystemStructure.getThingsInOpd(myOpd
				.getOpdId());// enumaration that go over all the Instances in
		// the current opd
		RectangularShape nearestShape;// holds the nearest shape
		Point thingEdgePoint = new Point(-1, -1);// holds the x,y param of a
		// checked thing
		int counter = 0;// counts the number of points found in the checked
		// instance
		// how many points are needed to be in an Instance for it to be valid
		int threshold = inPoints.length == 1 ? 1
				: (int) (inPoints.length * 0.5 + 1);

		if (inPoints == null) {
			return null;
		}

		// loop to find in which Instance we have at least as many points as
		// defined in threshold
		while (enumOutsideZoom.hasMoreElements()) {
			...
			if (((IXThingInstance) nearest).getParentIXThingInstance() != null) {
				if (((IXThingInstance) nearest).getParentIXThingInstance()
						.isZoomedIn()) {
					thingEdgePoint = SwingUtilities.convertPoint(
							((ThingInstance) ((IXThingInstance) nearest)
									.getParentIXThingInstance())
									.getConnectionEdge(), thingEdgePoint,
			...
			if (nearest instanceof ObjectInstance) {
				...
				isObject = true;
				// create an extended rectangle around the object Instance to
				// check if the point is inside it
				nearestShape = new Rectangle((int) thingEdgePoint.getX()
						- radiusAroundPoint, (int) thingEdgePoint.getY()
						- radiusAroundPoint, nearest.getWidth() + 2
						* radiusAroundPoint, nearest.getHeight() + 2
						* radiusAroundPoint);
			} else { // nearest is instanceof process
				// create an extended ellipse around the process Instance to
				// check if the point is inside it
			...
			// check how many points from the given array are inside the
			// Instance
			...
			// if found at least as many points as defined in threshold this is
			// the valid Instance
			if (counter >= threshold) {
				if ((inPoints.length == 1) && (isObject == true)) {
					// link/relation
					// check if it
					// relates to a
					// state
	 ...
	 * 
	 * @param searchPt
	 *            the point where to look for a state Instance
	...
	static protected IXStateInstance nearestStateInstance(Point searchPt) {
		Component inComponent = myProject.getCurrentOpd().getDrawingArea()
				.findComponentAt((int) searchPt.getX() + addToX / 2,
						(int) searchPt.getY() + addToY / 2);
		if (inComponent instanceof OpdState) {
			return (IXStateInstance) ((OpdConnectionEdge) inComponent)
					.getInstance();
		}
		return null;
	}
	 ...
	 * @param linkType
	 *            the link type
	 * @param sourceThingInstance
	 *            the source Instance of the link
	 * @param destinationThingInstance
	 *            the destination Instance of the link
	...
	private LinkInstance openLink(int linkType,
		...
		if (res.getResult() == IXCheckResult.RIGHT) {
			link = myProject.addLink(sourceThingInstance,
					destinationThingInstance, linkType);
			return (LinkInstance) link;
		} else if (res.getResult() == IXCheckResult.WRONG) {
			JOptionPane.showMessageDialog(myProject.getMainFrame(), res
					.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
		} else if (res.getResult() == IXCheckResult.WARNING) {
			JOptionPane.showConfirmDialog(Opcat2.getFrame(), res.getMessage()
	...
	static protected IXRelationInstance openRelation(int linkType) {
		Point[] source = { DrawAppNew.relation.getSourceEdgePoint() };// gets
		// the
		// edge
		// points
		Point[] destination = { DrawAppNew.relation.getDestinationEdgePoint() };// gets
		// the
		// Destination
		// points
		IXConnectionEdgeInstance sourceThingInstance;// holds the nearest
		// thing instance
		IXConnectionEdgeInstance destinationThingInstance;
		IXCheckResult res;// checks if the relation was opened or not
		IXRelationInstance relation = null;// holds the relation instance that
		// was opend

		sourceThingInstance = nearestThingInstance(source, DrawAppNew.relation
				.getSourceRange());

		destinationThingInstance = nearestThingInstance(destination,
				DrawAppNew.relation.getDestinationRange());
		relation = null;
		if ((sourceThingInstance == null) || (destinationThingInstance == null)) {
			return null;
		}
		...
		if (res.getResult() == IXCheckResult.RIGHT) {
			...
			int retValue = JOptionPane.showConfirmDialog(Opcat2.getFrame(), res
					.getMessage()
					+ "\n Do you want to continue?", "Opcat2 - Warning",
					JOptionPane.YES_NO_OPTION);
	...
	/**
	 * the function openGeneralRelation opens a bidirectional or unidirectional
	 * relation in opcat
	 * 
	 * @param linkType
	 *            the relation type
	...
	static protected IXRelationInstance openGeneralRelation(int linkType,
			IXConnectionEdgeInstance sourceThingInstance,
			IXConnectionEdgeInstance destinationThingInstance) {

		IXCheckResult res;// checks if the GeneralRelation was opened or not
		IXRelationInstance relation = null;// holds the GeneralRelation
		// instance that was opend
	...
	private void openObject() {
		...
		if (openIn != null) {// if object is opened in an Instance
			if (((IXThingInstance) openIn).isZoomedIn()
					&& (openIn instanceof ObjectInstance)) { // open object
				// or state in
				// object zoomed
				// in

				this.showPopUp(openAt, 1);
				return;

			} else if (((IXThingInstance) openIn).isZoomedIn()
					&& (openIn instanceof ProcessInstance)) { // open object
				// in process
				// zoomed in
				if (od.showDialog()) {
					ObjectInstance currObject = (ObjectInstance) myProject
							.addObject(od.getName(), (int) openAt.getX(),
									(int) openAt.getY(),
									(IXThingInstance) openIn);
					currObject.setLocation((int) openAt.getX(), (int) openAt
							.getY());
				...
				this.my_OpdConnectionEdge = ((ThingInstance) openIn)
						.getConnectionEdge();
				if ((this.my_OpdConnectionEdge.getInstance() instanceof ObjectInstance)) {
					openState();
					return;
				}
			}
	...
	private void openProcess() {
		...
		OpdProcess sampleProcOpd = (OpdProcess) sampleProcInstance
				.getConnectionEdge();
		ProcessPropertiesDialog pd1 = new ProcessPropertiesDialog(
		...
		if (openIn != null) {// if process is opened in an Instance
			if (((IXThingInstance) openIn).isZoomedIn()) { // if zoomed in
				if (pd1.showDialog()) {
					...
					currProcess.setLocation((int) openAt.getX(), (int) openAt
							.getY());
					this.enableProcess(currProcess, sampleProcOpm,
							sampleProcInstance);
				}
			}
		} else { // not zoomed in
			if (pd1.showDialog()) {
				ProcessInstance currProcess = (ProcessInstance) myProject
						.addProcess(pd1.getName(), (int) openAt.getX(),
								(int) openAt.getY(), myOpd.getOpdId());
				this.enableProcess(currProcess, sampleProcOpm, sampleProcInstance);
	...
	static protected void openState() {
		...
		StateInstance sampleSI = new StateInstance(sampleStateEntry,
				new OpdKey(0, 0), sampleObjOpd, myProject);

		parentInstance = (ObjectInstance) openIn;// the object in which we
		// open the state
		StatePropertiesDialog sd = new StatePropertiesDialog(sampleSI,
				myProject, false, BaseGraphicComponent.SHOW_OK
		...
		if (sd.showDialog()) {
			...
			for (Enumeration en = parentInstance.getEntry().getInstances(); en
					.hasMoreElements();) {
			...
			Opcat2.getUndoManager().undoableEditHappened(
					new UndoableEditEvent(myProject, new UndoableAddState(
							myProject, myEntry)));
	...
	/**
	 * the function enableFundamentalRelation enables undo/redo/delete after
	 * opening a relation
	 * 
	 * @param relation
	 *            the relevant relation
	 */
	static protected void enableFundamentalRelation(
			FundamentalRelationInstance relation) {
		if (relation == null) {
			return;
		}
		Opcat2.getUndoManager()
				.undoableEditHappened(
						new UndoableEditEvent(myProject,
								new UndoableAddFundamentalRelation(myProject,
										relation)));
		Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
		Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
	...
	/**
	 * the function enableGeneralRelation enables undo/redo/delete after opening
	 * a general relation
	 * 
	 * @param relation
	 *            the relevant relation
	...
	private void enableGeneralRelation(GeneralRelationInstance relation) {
		...
		Opcat2.getUndoManager().undoableEditHappened(
				new UndoableEditEvent(myProject,
						new UndoableAddGeneralRelation(myProject, relation)));
		Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
		Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
		...
		if (thingFlag == 0) { // object
			Opcat2.getUndoManager().undoableEditHappened(
					new UndoableEditEvent(myProject, new UndoableAddObject(
							myProject, (ObjectInstance) thing)));
		} else {
			Opcat2.getUndoManager().undoableEditHappened(
					new UndoableEditEvent(myProject, new UndoableAddProcess(
							myProject, (ProcessInstance) thing)));
	...
	private void enableLink(LinkInstance link) {
		Opcat2.getUndoManager().undoableEditHappened(
				new UndoableEditEvent(myProject, new UndoableAddLink(myProject,
						link)));
		Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
		Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
	...
	/**
	 * the function enableObject enables undo/redo/delete after opening an
	 * Object
	 * 
	 * @param currObject
	 *            the relevant object
	 * @param sampleObjOpm
	 *            the relevant sampleObjOpm
	 * @param sampleObjInstance
	 *            the relevant sampleObjInstance
	 */
	static protected void enableObject(ObjectInstance currObject,
			OpmObject sampleObjOpm, ObjectInstance sampleObjInstance) {
		...
		currObject.getConnectionEdge().setCursor(
	...
	/**
	 * the function enableProcess enables undo/redo/delete after opening an
	 * Process
	 * 
	 * @param currProcess
	 *            the relevant process
	 * @param sampleProcOpm
	 *            the relevant sampleProcOpm
	 * @param sampleProcInstance
	 *            the relevant sampleProcInstance
	 */
	private void enableProcess(ProcessInstance currProcess,
			OpmProcess sampleProcOpm, ProcessInstance sampleProcInstance) {
		...
		currProcess.setSize((int) shapeRectangle.getWidth(),
				(int) shapeRectangle.getHeight());
		currProcess.getConnectionEdge().setCursor(
				new Cursor(Cursor.DEFAULT_CURSOR));
	...
	private void openThing(int diagnose) {
		...
		IXConnectionEdgeInstance nearestThing = this.openThingIn();// where to open
		// the shape at

		openIn = nearestThing;

		if (nearestThing != null) {
			openAt = SwingUtilities.convertPoint(myProject.getCurrentOpd()
					.getDrawingArea(), openAt, ((ThingInstance) nearestThing)
					.getConnectionEdge());
		}
	...
	/**
	 * the function openThingIn find 3 points of shape in order to decide where
	 * to open the Thing that was drawn
	 * 
	...
	private IXConnectionEdgeInstance openThingIn() {
		...
		int pointsSize = DrawAppNew.points.size() - 1;// the number of points
		// the draw has
		for (int i = 0; i < 3; i++) {
			originalPoints[i] = (Point) DrawAppNew.points.elementAt(i
					* pointsSize / 3);
		}
		myOpd = myProject.getCurrentOpd();
	 ...
	 * 
	 * @param showPoint
	 *            where to open the popup menu
	 * @param menuKind
	 *            0 if its a relation menu, 1 if its an object/state menu
	 */
	private void showPopUp(Point showPoint, int menuKind) {

		JPopupMenu jpm = new HioPopUps(menuKind);// new popupmenu
		if (menuKind == 0) {
			jpm.show(myProject.getCurrentOpd().getDrawingArea(),
	...
	/**
	 * the function addToX determine the value to add/remove to the x parameter
	 * of a point in order to find the closest state Instance more accurately
	 * 
	 * @param addTo
	 *            the point to change the value
	 * @param otherEdge
	 *            the point in the other side of the link in order to calculate
	 *            the direction and the proportion of the change
	 * @param range
	 *            a parameter for calculating the value of the change
	 * @return the value of the change, minos if its for remove
	 */
	private int addToX(Point addTo, Point otherEdge, int range) {
		double xSide = Math.pow(addTo.getX() - otherEdge.getX(), 2);// who much
		// to add in
		// x side
		double ySide = Math.pow(addTo.getY() - otherEdge.getY(), 2);// who much
		// to add in
		// y side
		double hypotenuse = Math.sqrt(xSide + ySide);
		double addValue = range * Math.sqrt(xSide) / hypotenuse;// the
		// proportion to
		// be added
	...
	/**
	 * the function addToY determine the value to add/remove to the y parameter
	 * of a point in order to find the closest state Instance more accurately
	 * 
	 * @param addTo
	 *            the point to change the value
	...
	private int addToY(Point addTo, Point otherEdge, int range) {
		...
		// y side
		double hypotenuse = Math.sqrt(xSide + ySide);
		double addValue = range * Math.sqrt(ySide) / hypotenuse;// the
		// proportion to
		// be added
