...
public abstract class OpdThing extends OpdConnectionEdge implements
					...
					if (this.draggedTInstance.getTargetThingInstance() == null) {
						...
						this.draggedTInstance
								.setTargetBackgroundColor(this.targetInstance
										.getBackgroundColor());
						this.targetInstance.setBackgroundColor(new Color(255,
								0, 0));
		...
		if (StateMachine.getCurrentState() != StateMachine.IN_SELECTION) {
			...
			if (targetInstance != null) {
				if (!((targetInstance.getEntry().isLocked()) || this.getEntry()
					...
					try {
						command.commitMerge((ThingInstance) (this.getEntry()
								.getInstance(this.getOpdKey())),
								(ThingInstance) targetInstance, true);
	...
	// private BufferedImage scaleToSize(long nMaxWidth, long nMaxHeight,
	// BufferedImage imgSrc, Graphics2D g2) {
	...
	// .getScaleInstance(fScale, fScale), null);
	//		
	// BufferedImage dest = null;
	// return op.filter(imgSrc,dest);
	// }
