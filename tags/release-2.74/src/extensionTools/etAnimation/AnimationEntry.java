...
class AnimationEntry extends Object {
  ...
  public void activate(AnimationEvent aEvent)  {
    ...
    if (!currentEvent.getPath().equals("")) {
      AnimationAuxiliary.debug("Set path " + currentEvent.getPath() + " for " + iEntry.getName() + " execId = " + aEvent.getExecutionId());
    ...
    AnimationAuxiliary.debug("Activate " + iEntry.getName() + ", exec ID = " + aEvent.getExecutionId());
    AnimationAuxiliary.debug(", number of instances = " + aEvent.getNumberOfInstances());

    // store current event
    this.addCurrentEvent(new AnimationEvent(aEvent));

    this.currentEvent.setParentProcess(aEvent.getParentProcess());
    AnimationAuxiliary.debug("setParentProcess " + aEvent.getParentProcess());
    // remove all future events scheduled for it for this execution
    if (aEvent.getExecutionId() != 0) {
      AnimationAuxiliary.debug("Calling removeAllScheduledEvents from step " + (aEvent.getStep() - 1));
...
//    if (AnimationAuxiliary.isThing(iEntry) && !this.isFeature())  {
  ...
  public void deactivate(AnimationEvent aEvent)  {
    AnimationAuxiliary.debug("Starting deactivate for " + iEntry.getName());
    ...
    AnimationAuxiliary.debug("deactivate for " + iEntry.getName());
    AnimationAuxiliary.debug("exec id = " + aEvent.getExecutionId());
    AnimationAuxiliary.debug("getNumberOfInstances() " + getNumberOfInstances());
    AnimationAuxiliary.debug("aEvent.getNumberOfInstances() " + aEvent.getNumberOfInstances());
    AnimationAuxiliary.debug("numberOfSpecializations " + this.currentEvent.getNumberOfSpecializations());
  ...
  public boolean isActive() {
    AnimationAuxiliary.debug(iEntry.getName() + " is active = " +  this.currentEvent.getNumberOfInstances());
  ...
  public boolean isActive(long executionId) {
    AnimationAuxiliary.debug("isActive for " + iEntry.getName() + ", executionId = " +  executionId);
    AnimationAuxiliary.debug("return " + this.currentEvents.containsKey(new Long(executionId)));
  ...
  public String getPath(long execId) {
    ...
    AnimationAuxiliary.debug("getPath: could not find AnimationEvent for execId " + execId);
    return "";
  }

  public void setNumberOfSpecializations(int numberOfSpecialization)  {
    AnimationAuxiliary.debug("setNumberOfSpecializations for " + iEntry.getName());
  ...
  public AnimationEvent getCurrentEvent(long executionId) {
    AnimationAuxiliary.debug("getCurrentEvent for " + iEntry.getName() + ", executionId = " + executionId);
    AnimationAuxiliary.debug("return " + (AnimationEvent)currentEvents.get(new Long(executionId)));
  ...
  public void addCurrentEvent(AnimationEvent aEvent)  {
    ...
    try {
      ...
      AnimationAuxiliary.debug("ExecId: " + execId.longValue() + ", numb of ins = " +  aEvent.getNumberOfInstances());
    }
  ...
  public void removeCurrentEvent(AnimationEvent aEvent)  {
    ...
    try {
      aEventTemp = (AnimationEvent)this.currentEvents.get(execId);
      AnimationAuxiliary.debug("removeCurrentEvent for " + iEntry.getName());
      if (AnimationAuxiliary.isProcess(iEntry) && aEventTemp.getNumberOfInstances() > 1) {
        ...
        AnimationAuxiliary.debug("reduce number of instanced for exec id" + execId.longValue());
        AnimationAuxiliary.debug("new number of instances is " + aEventTemp.getNumberOfInstances());
      }
      else  {
        AnimationAuxiliary.debug("remove execId " + execId.longValue());
  ...
  public boolean futureActivationScheduled(long currentStep, long executionId)  {
    AnimationAuxiliary.debug("futureActivationScheduled for " + iEntry.getName() + ", executionId " + executionId);
    AnimationAuxiliary.debug(" returns " + actvScheduler.futureEventsScheduled(currentStep, executionId));
  ...
  public long getDuration() {
    ...
    if (AnimationAuxiliary.isProcess(iEntry)) {
      if (AnimationSettings.FIXED_PROCESS_DURATION) {
        ...
        //AnimationAuxiliary.debug("random int value " + i + " for " + this.getIXEntry().getName());
        return temp;
      }
    }
  ...
  public void animate(AnimationEvent aEvent) {
    ...
    for(; eTemp.hasMoreElements();) {
      ...
      if (AnimationAuxiliary.isLink(iEntry))  {
        ...
        if (AnimationAuxiliary.isEvent(iEntry)) {
          //AnimationAuxiliary.debug("Event: before animateAsFlash");
          iLinkInstance.animateAsFlash();
          //AnimationAuxiliary.debug("Event: after animateAsFlash");
        }
        else  {
          // link, but not event
          if (aEvent.getDuration() > 0) {
            //AnimationAuxiliary.debug("Link: before animate");
            iLinkInstance.animate(aEvent.getDuration());
            //AnimationAuxiliary.debug("Link: after animate");
          }
        }
      }
      else  {
        ...
        if (AnimationAuxiliary.isState(iEntry) || AnimationAuxiliary.isThing(iEntry)) {
          ...
          //AnimationAuxiliary.debug("getBackgroundColor " + iThingInstance.getBackgroundColor());
          //AnimationAuxiliary.debug("defaultColor " + defaultColor);
          AnimationAuxiliary.debug("animate thing " + iEntry.getName());
  ...
  public void stopAnimation(AnimationEvent aEvent) {
    ...
    for(; eTemp.hasMoreElements();) {
      ...
      if (AnimationAuxiliary.isThing(iEntry) || AnimationAuxiliary.isState(iEntry))  {
        ...
        //AnimationAuxiliary.debug("stopAnimation for " + iEntry.getName());
        //AnimationAuxiliary.debug("duration =  " + aEvent.getDuration());
        ...
        if (AnimationAuxiliary.isLink(iEntry))  {
          ...
          //AnimationAuxiliary.debug("stopAnimation for link " + iEntry.getName());
          //AnimationAuxiliary.debug("duration " + aEvent.getDuration());
          //i.stopAnimation(aEvent.getDuration());
          i.stopAnimation();
        }
  ...
  public long getNextLevelY(IXThingInstance parentInstance, long levelY)  {
    AnimationAuxiliary.debug("Starting getNextLevelY, parent = " + parentInstance.getIXEntry().getName());
    AnimationAuxiliary.debug(", levelY = " + levelY);
    ...
    AnimationAuxiliary.debug("after first loop, minY = " + minY);
    return minY;
  }

  public Enumeration getChildrenByLevel(IXThingInstance parentInstance, long levelY)  {
    AnimationAuxiliary.debug("Starting getChildrenByLevel, parent = " + parentInstance.getIXEntry().getName());
    AnimationAuxiliary.debug(", levelY = " + levelY);
    Enumeration en = parentInstance.getChildThings();
    Vector children = new Vector();
    long minY = java.lang.Long.MAX_VALUE;
    ...
    for(; en.hasMoreElements();) {
      ...
      if ((tempChildY <= (minY + 3)) && (tempChildY >= minY)) {
        AnimationAuxiliary.debug("Found process in the same level,  y = " + tempChildY + ",process = " + temp.getName());
        children.add(tempChildInstance);
      }
    }
  ...
  public boolean isLowLevel() {
    if (AnimationAuxiliary.isProcess(iEntry)) {
    AnimationAuxiliary.debug("isLowLevel for " + iEntry.getName());
    AnimationAuxiliary.debug("this.getChildrenProcesses().isEmpty() = " + this.getChildrenProcesses().isEmpty());
    /*AnimationAuxiliary.debug("this.getParents().hasMoreElements() = " + this.getParents().hasMoreElements());
    AnimationAuxiliary.debug("currentEvent.getParentProcess() = " + currentEvent.getParentProcess());
    AnimationAuxiliary.debug("isLowLevel returns " +
  ...
  public void restore(long fromStep, long toStep)  {
    AnimationAuxiliary.debug("restore for " + iEntry.getName() + ", from " + fromStep + ", to " + toStep)  ;
    if (AnimationAuxiliary.isLink(iEntry))  {
      if (fromStep > toStep)  {
    ...
    if (fromStep > toStep)  {
      ...
      AnimationAuxiliary.debug("Initial  numOfInstances " + numOfInstances);
      for(long step=fromStep; step>toStep;step--) {
        ...
        for(; enEvents.hasMoreElements();) {
          ...
          AnimationAuxiliary.debug("Get event for step " + step);
          AnimationAuxiliary.debug("aEvent.getEventType() " + aEvent.getEventType());
          ...
          if (aEvent.getEventType() == AnimationSettings.ACTIVATION_EVENT)  {
            ...
            AnimationAuxiliary.debug("minus " + numOfInstances);
            ...
            numOfInstances += aEvent.getNumberOfInstances();
            AnimationAuxiliary.debug("plus " + numOfInstances);
            ...
            if (actvEvent != null)  {
              AnimationAuxiliary.debug("Step of restored activation event = " + actvEvent.getStep());
      ...
      AnimationAuxiliary.debug("final " + numOfInstances);
