...
class AnimationEventsScheduler extends Hashtable {
  ...
  public void scheduleEvent(AnimationEvent aEvent)  {
    AnimationAuxiliary.debug("scheduleEvent getSchedulingStep = " + aEvent.getSchedulingStep());
  ...
  public void removeAllScheduledEvents(long schedulingStep, long executionId)  {
    AnimationAuxiliary.debug("Starting removeAllScheduledEvents");
      ...
      for(int i=v.size()-1; i>=0;i--) {
        ...
        if (aEvent.getSchedulingStep() > schedulingStep)  {
          if ((executionId == 0)  || executionId == aEvent.getExecutionId())  {
            AnimationAuxiliary.debug("remove event for step "  + aEvent.getStep());
        ...
        if (aEvent.getCancelStep() > schedulingStep)  {
          if ((executionId == 0)  || executionId == aEvent.getExecutionId())  {
            AnimationAuxiliary.debug("restore cancelled event for step "  + aEvent.getStep());
    ...
    AnimationAuxiliary.debug("After removeAllScheduledEvents");
    this.print();
  }
  ...
  public void print()  {
    ...
    for(; en.hasMoreElements();) {
      Vector v = (Vector)en.nextElement();
      if (!v.isEmpty()) {
        AnimationAuxiliary.debug("Step: " + ((AnimationEvent)v.firstElement()).getStep());
      }
      // print the events stored inside teh vector
      for(int i=0; i<v.size();i++) {
        AnimationEvent aEvent = (AnimationEvent)v.get(i);
        if (aEvent.getEventType()==AnimationSettings.ACTIVATION_EVENT)  {
          AnimationAuxiliary.debug("#" + i + ": Activation event");
        }
        else  {
          AnimationAuxiliary.debug("#" + i + ": Termination event");
        }
        AnimationAuxiliary.debug(" was issued on " + aEvent.getSchedulingStep());
      }
    }
  }
      ...
      if (key.longValue() >= currentStep)  {
        ...
        for(int i=0; i<v.size();i++) {
          ...
          if ((aEvent.getExecutionId() == executionId) &&
            ...
            AnimationAuxiliary.debug("found future event, step " + aEvent.getStep());
            return aEvent;
          }
        }
      }
