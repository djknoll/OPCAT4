...
public class ModelReflection extends Reflection {
	 ...
	 * <code>constraintMap</code>. For model-based reflection, the attached role is 
	 * role of the inspected thing. 
	 * The method sets a set of Finder algorithms, each of them implements the
	 * <code>NeighborFinder</code> interface. 
	 * @see	NeighborFinder
	 */
	public void addPair(IThingEntry thing) throws ValidationException {
		Enumeration allRoles;
		allRoles = thing.getRoles();
		while (allRoles.hasMoreElements()) {
			try	{
				Role role = (Role)allRoles.nextElement();
				ModelVRole vrole = new ModelVRole(role.getLibraryId(), role.getThingId(), thing);
				...
				ConstraintSet set = findConstraints(vThing, thing, vrole);
				constraintMap.put(vrole, set);
			}
	...
	/**
	 * Builds a {@link MultipleApplicant} and populates it with <code>VRole</code>
	 * objects. In a model, each thing can be mapped to multiple roles of things on 
	 * a single relation/link because each mapped thing can have multiple roles.
	 */
	protected MultipleApplicant createApplicant(IThingEntry thing)	{
	...
	protected Constraint createConstraintObject(
		...
		Case theCase = new Case((MultipleApplicant)applicant, connectionType, dir);
		return theCase;
	}
	 ...
	 * @return
	 */
	protected ConstraintSet findConstraints(Finder vThing, IThingEntry thingEntry, VRole role)	
	 ...
	 * each of them signifies an offence. Note that the offences have polymorphic
	 * behaviour, according to the offence type.
	 */
	public Vector justice(HashMap lawsSet) throws ValidationException 	{
		...
		try	{
			while (lawIt.hasNext())	{
				...
				offences.addAll(justiceMeta(meta));
			}
		}
			...
			throw new ValidationException("A validation exception had occured. "+ ex.getMessage());
		}
		return offences;
	}
	 ...
	 * The method runs on all the laws of the meta-library which are relevant
	 * to the role in the model, comparing them to every constraint in the model (case).
	 * @param role	A given role of a specific thing in the model.
	 * @param laws	The set of laws that reflect a specific meta-library.
	 * @return	A set of offences that represent the violation of <code>role</code>
	 * to the given <code>laws</code>.
	 * @throws ValidationException
	 * TODO: add hashing values to the applicants to speed up search
	 */
	private OffenceSet checkValidity(ModelVRole role, MetaReflection laws) throws ValidationException	{
		if (laws == null)	{
			throw new ValidationException("Laws object is null");
		}
		ConstraintSet modelConstraints = getStatements(role);
