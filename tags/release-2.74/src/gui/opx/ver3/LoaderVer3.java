...
public class LoaderVer3 extends LoaderVer2 {
	...
	/**
	 * Overloads the load method of version 2. Currently, does not provide any functionality.
	 * 
	 */
	...
	/**
	 * Overloads the system attributes loading, setting the version number of the 
	 * OPX to <code>myProject</code>.
	 * @see	OpdProject#setVersionString
	 */
	protected void _loadSystemAttributes(Element system)throws LoadException
	{
		myProject.setVersionString(getVersion());
		
		try	{
			super._loadSystemAttributes(system);
			if (system.getAttribute("global-id") != null)	{
				myProject.setGlobalID(system.getAttributeValue("global-id"));
			}
		}
		catch (LoadException lex)	{
			throw lex;
	...
	protected void _loadSystemProperties(Element props) throws JDOMException {
		try {
			...
			if (metaSection != null) {
				...
				if (metaLibs != null) {
					_loadMetaLibraries(metaLibs, myProject.getMetaManager());
				}
			}
	...
	protected void _loadThingAttr(ThingEntry thing, Element xmlThing)
        ...
        /**
		 * Marks whether the opd is locked, and sets the path to a referenced
		 * system if the opd is "open reused"
		 */
        protected Opd _loadOpd(Element xmlOpd, Opd parentOpd, int relationsWithParent) throws JDOMException
        {
              ...
              if(xmlOpd.getAttribute("reusePath") != null){
                ...
                _loadOpenReuseRelations(xmlOpd, myOpd);
              }
              else{
...
//LogicalState
        ...
        {
          Element xmlOpenReuseRelationsSection = xmlOpd.getChild("openReuseRelations");
          List relations = xmlOpenReuseRelationsSection.getChildren("openReuseRelation");
