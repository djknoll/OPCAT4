...
public class SaverVer2 implements SaverI {
	...
	public void save(OpdProject project, OutputStream os,
		...
		try {
			...
			doc.setRootElement(mainTag);
			mainTag.setAttribute("version", getVersion());
			Element system = new Element("OPMSystem");
			mainTag.addContent(system);
			//creationDate="2003-06-24T16:44:29.000+02:00" lastUpdate="2003-07-18T13:17:05.218+02:00"
			system
					.setAttribute("creationDate",
							"2003-06-24T16:44:29.000+02:00");
			system.setAttribute("lastUpdate", "2003-06-24T16:44:29.000+02:00");
			_saveSystemAttributes(system);
			system.addContent(_saveSystemProperties());
			system.addContent(_saveLogicalStructure());
			system.addContent(_saveVisualPart());
	...
	protected void _saveSystemAttributes(Element system) {
		system.setAttribute("author", myProject.getCreator());
		system.setAttribute("name", myProject.getName());
		
		SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");

		system.setAttribute("creationDate", df.format(myProject
				.getCreationDate()));
		GregorianCalendar cal = new GregorianCalendar();
		system.setAttribute("lastUpdate", df.format(cal.getTime()));
		Attribute attr = new Attribute("maxEntityEntry", Long
				.toString(myProject.getEntityMaxEntry()));
		system.setAttribute(attr);
		attr = new Attribute("maxOpdEntry", Long.toString(myProject
				.getOpdMaxEntry()));
		system.setAttribute(attr);
	}
	...
	protected Element _saveSystemProperties() throws JDOMException {
		...
		_saveGenericTable(cnfg, myProject.getConfiguration());
		props.addContent(cnfg);
		Element genInfo = new Element("GeneralInfo");
		_saveGenericTable(genInfo, myProject.getGeneralInfo());
		props.addContent(genInfo);

		return props;
	}

	//Gets cnfg - SystemConfiguration and adds to it list of "Property" elements
	...
	protected Element _saveLogicalStructure() throws JDOMException {
		...
		logicalStruct.addContent(_saveObjectSection());
		logicalStruct.addContent(_saveProcessSection());
		logicalStruct.addContent(_saveRelationSection());
		logicalStruct.addContent(_saveLinkSection());
	...
	protected Element _saveObjectSection() throws JDOMException {
		...
		for (Enumeration e = myProject.getComponentsStructure().getElements(); e
				.hasMoreElements();) {
			Entry currentEntry = (Entry) (e.nextElement());
			if (currentEntry instanceof ObjectEntry) {
				//reuse comment - don't save entries which are open reused
				if (currentEntry.isOpenReused())
					continue;
				//end reuse comment
				//Accepts list!!
				_saveLogicalObject((ObjectEntry) currentEntry, objectSect);
			}
	...
	protected Element _saveEntityAttr(Entry entity) throws JDOMException {
		...
		xmlEntity.addContent(name);
		//xmlEntity.setAttribute("name",entity.getName());
		//xmlEntity.setAttribute("description",entity.getDescription());
		xmlEntity.setAttribute("environmental", Boolean.toString(entity
				.isEnvironmental()));
		/*
		 * saves the url
		 * FIXME: move this url and project description into ver 3
		 * Raanan
		 */
	...
	protected Element _saveProcessSection() throws JDOMException {
		...
		for (Enumeration e = myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof ProcessEntry) {
				...
				_saveLogicalProcess((ProcessEntry) currentEntry, processSect);
			}
		...
		for (Enumeration e = myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof RelationEntry) {
				...
				if (currentEntry.isOpenReused())
					continue;
				//end reuse comment
				_saveLogicalRelation((RelationEntry) currentEntry, relationSect);
			}
		...
		for (Enumeration e = myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof LinkEntry) {
				...
				if (currentEntry.isOpenReused())
					continue;
				//end reuse comment
				_saveLogicalLink((LinkEntry) currentEntry, linkSect);
			}
	...
	protected Element _saveVisualPart() throws JDOMException {
		...
		Hashtable opdDependencies = _prepareOpdsDependencies();
		vpt.addContent(_saveOpd(ROOT_OPD, opdDependencies));
		return vpt;
	}

	/**
	 * Creates an OPX representation of an OPD. Uses <code>_appendOpdPropetries</code>
	 * in order to render the properties of the actual OPD, and 
	 * <code>_appendOpdPropetries</code> in order to render the subelements of the OPD.
	...
	protected Element _saveOpd(long opdNum, Hashtable opdDependencies)
		...
		Opd myOpd = myProject.getComponentsStructure().getOpd(opdNum);
		opd = _appendOpdPropetries(opd, myOpd);
		opd = _appendOpdChildren(opdNum, opdDependencies, opd, myOpd);
		return opd;
	}
	...
	protected Element _appendOpdChildren(long opdNum,
			...
			throws JDOMException {
		Hashtable parentChildren = _prepareParentChildrenDependencies(opdNum);

		if (opdNum != ROOT_OPD) {
			opd.addContent(_saveMainEntity(myOpd.getMainInstance(), opdNum,
					parentChildren));
		}

		opd.addContent(_saveThingSection(opdNum, parentChildren));
		opd.addContent(_saveFundamentalRelationSection(opdNum));
		opd.addContent(_saveGeneralRelationSection(opdNum));
		opd.addContent(_saveVisualLinkSection(opdNum));
		opd.addContent(_saveUnfolded(opdNum, opdDependencies));
		opd.addContent(_saveInZoomed(opdNum, opdDependencies));
	...
	protected Element _saveMainEntity(ThingInstance mainInstance, long opdNum,
		...
		mainEntity.addContent(_saveVisualThing(mainInstance, parentChildren));
	...
	protected Element _saveThingSection(long opdNum, Hashtable parentChildren)
		...
		for (Iterator it = rootList.iterator(); it.hasNext();) {
			//reuse comment - don't save instances which are from the reused system
			ThingInstance instance = (ThingInstance) it.next();
			if (instance.getEntry().isOpenReused())
				continue;
			//end reuse comment
			thingList.add(_saveVisualThing(instance, parentChildren));
		}
	...
	protected Hashtable _prepareParentChildrenDependencies(long opdNum) {
		...
		for (Enumeration e = myProject.getComponentsStructure().getThingsInOpd(
				opdNum); e.hasMoreElements();) {
	...
	protected Element _saveVisualThing(ThingInstance tInstance,
		...
		if (tInstance instanceof ProcessInstance) {
			tData.addContent(_saveVisualProcess((ProcessInstance) tInstance));
		} else {
			tData.addContent(_saveVisualObject((ObjectInstance) tInstance));
		}
		...
		if (childrenList != null) {
			...
			for (Iterator it = childrenList.iterator(); it.hasNext();) {
				xmlChildrenList.add(_saveVisualThing((ThingInstance) it.next(),
						parentChildren));
			}
	...
	protected Element _saveVisualProcess(ProcessInstance proc)
		...
		xmlProcess.addContent(_saveInstanceAttr(proc));
		xmlProcess.addContent(_saveConnectionEdgeAttr(proc));
		xmlProcess.addContent(_saveThingInstanceAttr(proc));
	...
	protected Element _saveVisualObject(ObjectInstance obj)
		...
		xmlObject.addContent(_saveInstanceAttr(obj));
		xmlObject.addContent(_saveConnectionEdgeAttr(obj));
		xmlObject.addContent(_saveThingInstanceAttr(obj));
		...
		for (Enumeration e = obj.getStateInstances(); e.hasMoreElements();) {
			statesList.add(_saveVisualState((StateInstance) e.nextElement()));
		}
	...
	protected Element _saveVisualState(StateInstance state)
		...
		xmlState.addContent(_saveInstanceAttr(state));
		xmlState.addContent(_saveConnectionEdgeAttr(state));
	...
	protected Element _saveFundamentalRelationSection(long opdNum)
		...
		for (Enumeration e = myProject.getComponentsStructure()
			...
			relList.add(_saveCommonPart((GraphicalRelationRepresentation) e
					.nextElement()));
		}
	...
	protected Element _saveCommonPart(GraphicalRelationRepresentation rel)
		...
		for (Enumeration e = rel.getInstances(); e.hasMoreElements();) {
			insList
					.add(_saveVisualFundamentalRelation((FundamentalRelationInstance) e
							.nextElement()));
		}
	...
	protected Element _saveVisualFundamentalRelation(
		...
		xmlRelation.addContent(_saveInstanceAttr(rel));
	...
	protected Element _saveGeneralRelationSection(long opdNum)
		...
		for (Enumeration e = myProject.getComponentsStructure()
			...
			relList.add(_saveVisualGeneralRelation((GeneralRelationInstance) e
					.nextElement()));
		}
	...
	protected Element _saveVisualGeneralRelation(GeneralRelationInstance rel)
		...
		xmlRelation.addContent(_saveInstanceAttr(rel));
		xmlRelation.addContent(_saveLineAttr(rel.getLine()));
		...
		if (!rel.getLine().isStraight()) {
			xmlRelation.addContent(_saveBreakPoints(rel.getLine()));
		}
		return xmlRelation;
	}
	...
	protected Element _saveVisualLinkSection(long opdNum) throws JDOMException {
		...
		for (Enumeration e = myProject.getComponentsStructure().getLinksInOpd(
				opdNum); e.hasMoreElements();) {
			linkList.add(_saveVisualLink((LinkInstance) e.nextElement()));
		}
		linkSect.addContent(linkList);
		_saveOrXor(linkSect, opdNum);
	...
	protected Element _saveVisualLink(LinkInstance link) throws JDOMException {
		...
		xmlLink.addContent(_saveInstanceAttr(link));
		xmlLink.addContent(_saveLineAttr(link.getLine()));

		if (!link.getLine().isStraight()) {
			xmlLink.addContent(_saveBreakPoints(link.getLine()));
		}
		return xmlLink;
	}
					...
					if (parentInstance != null) {
						...
						xmlOpd.setAttribute("entityInOpdId", Long
								.toString(parentInstance.getKey()
										.getEntityInOpdId()));
						xmlOpd.addContent(_saveOpd(currOpd.getOpdId(),
								opdDependencies));
		...
		if (children != null) {
			for (Iterator it = children.iterator(); it.hasNext();) {
				...
				if (currOpd.getMainEntry().getZoomedInOpd() == currOpd) {
					xmlChildrenList.add(_saveOpd(currOpd.getOpdId(),
							opdDependencies));
				}
			}
		}
