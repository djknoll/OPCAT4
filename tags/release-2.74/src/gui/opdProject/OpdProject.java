...
import gui.opmEntities.LinkPrecedence;
...
//					+ (System.currentTimeMillis() - startTime));
	...
	public void copy() {
		...
		_copy(getCurrentOpd(), clipBoard, 0, 0, clipBoard.getDrawingArea(),
				COPY);
		//		cutted = false;
		//		if (currentOpd == null) return;
		//		if (currentOpd.getSelectedItemsHash().size() == 0)
		//		{
		//			JOptionPane.showMessageDialog(Opcat2.getFrame(), "No item was
		// selected ", "Error" ,JOptionPane.ERROR_MESSAGE);
		//			return;
		//		}
		//
		//		if(currentOpd.getSelectedItemsHash().size() == 1)
		//		{
		//			Instance item = currentOpd.getSelectedItem();
		//			if (!(item instanceof ThingInstance))
		//			{
		//				JOptionPane.showMessageDialog(Opcat2.getFrame(), "Can't copy this
		// kind of entity ", "Error" ,JOptionPane.ERROR_MESSAGE);
		//				return;
		//			}
		//		}
		//		thingsToCopy = new Hashtable();
		//		thingsToCopy.putAll(currentOpd.getSelectedItemsHash());
		//        System.err.println(thingsToCopy);
		//		return;
	}
	...
	public void paste(int x, int y, Container parent) {
		...
		//		if (thingsToCopy == null || thingsToCopy.size() == 0)
		//		{
		//			JOptionPane.showMessageDialog(Opcat2.getFrame(), "No entity was
		// copied ", "Error" ,JOptionPane.ERROR_MESSAGE);
		//			return;
		//		}

		// separate to Things, Links and Relations
		//		Hashtable t = new Hashtable();
		//		Hashtable l = new Hashtable();
		//		Hashtable fr = new Hashtable();
		//		Hashtable gr = new Hashtable();
		//		Instance i;
		//		for(Enumeration e = thingsToCopy.elements(); e.hasMoreElements();)
		//		{
		//			i = (Instance)e.nextElement();
		//			if(i instanceof ProcessInstance || i instanceof ObjectInstance)
		//			{
		//				t.put(new Long(i.getKey().getEntityInOpdId()), i);
		//			}
		//			else if(i instanceof LinkInstance)
		//			{
		//				l.put(new Long(i.getKey().getEntityInOpdId()), i);
		//			}
		//			else if(i instanceof FundamentalRelationInstance)
		//			{
		//				fr.put(new Long(i.getKey().getEntityInOpdId()), i);
		//			}
		//			else if(i instanceof GeneralRelationInstance)
		//			{
		//				gr.put(new Long(i.getKey().getEntityInOpdId()), i);
		//			}
		//		}
		//
		//		// now select the most topleft point
		//		// so we could shift all things
		...
		//						new UndoableAddProcess(this, pi)));
		//				Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
		//				Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
		//
		//				orig2new.put(ti, pi);
		...
		//			{
		//				ObjectInstance oi;
		//				oi = this.addObject(theX,theY,parent, ti.getLogicalId(), -1, true);
		...
		//			newLi = this.addLink(newSrc.getConnectionEdge(),
		//								 origLi.getSourceDragger().getRelativeConnectionPoint(),
		//								 newDst.getConnectionEdge(),
		//								 origLi.getDestinationDragger().getRelativeConnectionPoint(),
		//								 newSrc.getConnectionEdge().getParent(),
		//								 Constants.getType4Link((OpmProceduralLink)origLi.getEntry().getLogicalEntity()),
		//								 origLi.getEntry().getId(),
		//								 -1);
		...
		//			origSrc =
		// (ConnectionEdgeInstance)origFri.getGraphicalRelationRepresentation().getSourceInstance();
		//			origDst = (ConnectionEdgeInstance)origFri.getDestinationInstance();
		//			newSrc = (ConnectionEdgeInstance)orig2new.get(origSrc);
		//			newDst = (ConnectionEdgeInstance)orig2new.get(origDst);
		//			newFri = this.addRelation(newSrc.getConnectionEdge(),
		...
		//	  if(cutted)
		//	  {
		//		  // have to set currentOpd to one that we copyed from
		//		  // cos delete() uses getCurrentOpd()
		//		  Instance inst = (Instance)thingsToCopy.elements().nextElement();
		//		  if(inst != null)
		//		  {
		//			  Opd tmpOpd = currentOpd;
		//			  currentOpd =
		// mainComponentsStructure.getOpd(inst.getKey().getOpdId());
		//			  currentOpd.getSelectedItemsHash().putAll(thingsToCopy);
		//			  this.delete();
		//			  cutted = false;
		//			  currentOpd = tmpOpd;
		//		  }
		...
		//	  if (thingToCopy instanceof ProcessInstance)
		//	  {
		//		ProcessInstance pi;
		//		pi = addProcess(x,y,parent, thingToCopy.getLogicalId(), -1);
		//		pi.setLocation(x,y);
		//		pi.getThing().fitToContent();
		//		pi.update();
		//
		...
		//	  }
		//
		//	  if (thingToCopy instanceof ObjectInstance)
		//	  {
		//		ObjectInstance oi;
		//		oi = this.addObject(x,y,parent, thingToCopy.getLogicalId(), -1,
		// true);
		//		oi.setLocation(x,y);
		//		oi.getThing().fitToContent();
		//		oi.update();
	...
	public void cut() {
		clipBoard.selectAll();
		_delete(clipBoard, DELETE);
		_copy(getCurrentOpd(), clipBoard, 0, 0, clipBoard.getDrawingArea(), CUT);
		_delete(getCurrentOpd(), CUT);
	}
