...
public class OpdExhibition extends OpdFundamentalRelation
{
	...
	public void paintComponent(Graphics g)
	{
        ...
        double dist = ((double)(getWidth()-dSelOffset))*9/32;
        double hDist = dist/2; // (sin 30)
        double wDist = dist*0.8660254;   // (cos 30)


        int xS1[] = {selOffset,(int)Math.round((double)(getWidth()-1)/2), getWidth()-selOffset-1};
        int yS1[] = {getHeight()-selOffset-1, selOffset,getHeight()-selOffset-1};


		int xS2[] = {(int)Math.round(selOffset+wDist+1),(int)Math.round(((double)(getWidth()-1))/2),(int)Math.round(getWidth()-selOffset-wDist-1)};
		int yS2[] = {(int)Math.round(getHeight()-selOffset-hDist-1), (int)Math.round(selOffset+dist), (int)Math.round(getHeight()-selOffset-hDist-1)};
	...
	{
		RelationPropertiesDialog rd = new RelationPropertiesDialog(graphicalEntry, "Featuring-Characterization", true);
		rd.showDialog();
	}
