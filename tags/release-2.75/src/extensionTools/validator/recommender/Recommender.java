...
public class Recommender {
	...
	public Recommender(IThingInstance _instance, IXSystem _system) {
		thing = (IThingEntry)_instance.getIEntry();
		instance = _instance;
		system = _system;
	}
	...
	public JMenu getAdvices() {
		...
		Enumeration libs = system.getMetaLibraries();
		Enumeration roles = thing.getRoles();
			...
			if (role.getState() == Role.LOADED)	{
				try	{
					...
					for (int i = 0; i < advises.size(); i++) {
						...
						JMenuItem item = createItem(neighbor, role);
						menu.add(item);
						numOfAdvices++;
					}
				}
	...
	protected JMenuItem createItem(Neighbor neighbor, Role role)	{
		...
		Icon icon = iconToType(neighbor.getConnectionType());
		String message = "New "+ cName + " to " + name;
		Action doAdvice = new PerformAction(neighbor, instance, system, role.getOntology());
