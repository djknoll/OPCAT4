 ...
 * @version 1.0
 */
public class OPLProcessFactor
    extends OPLFactor {

  public OPLProcessFactor(OPLTreeBuilder builder_, String systemName_,ObjectFactory ob) {
    super(builder_, systemName_,ob);
  }

  protected ProcessAggregationSentenceSet buildProcAggregSentenceSet(String
      objName) throws Exception {
  ...
  protected ProcessInheritanceSentenceSet buildProcInhSentenceSet(IRelationEntry rel) throws
  Exception {
ProcessInheritanceSentenceSet sentence;
IEntry destination = builder.myStructure.getIEntry(rel.getDestinationId());
...
try {
  ...
  sentence.getProcessInheritanceSentence().add(buildProcInhSentence(rel));
}
catch (Exception e) {
  throw e;
}
...
protected ProcessInheritanceSentence buildProcInhSentence(IRelationEntry rel) throws
Exception {
	ProcessInheritanceSentence particularSent;
String fatherName = builder.myStructure.getIEntry(rel.getSourceId()).
    getName();
try {
  ...
  protected ProcessInstanceSentence buildProcInstSentence(IRelationEntry rel) throws
      Exception {
  ...
  protected ProcessUniDirectionalRelationSentence buildProcUniDirSentence(
    ...
    String fatherName = builder.myStructure.getIEntry(rel.getSourceId()).
        getName();
    String destination = builder.myStructure.getIEntry(rel.getDestinationId()).
        getName();
    try {
  ...
  protected ProcessBiDirectionalRelationSentence buildProcBiDirSentence(
    ...
    try {
      ...
      if (rule == OPLGeneral.DESTINATION) {
        ...
        if((!OPLGeneral.isEmpty(rel.getForwardRelationMeaning())) && (!OPLGeneral.isEmpty(rel.getBackwardRelationMeaning())) )
         {
           fillProcBiDirSentence(sentence, destination, fatherName, meaning);
           //addCardinality(sentence, destinationCardinality, sourceCardinality);
           return sentence;
         }
      }
      else {
      ...
      fillProcBiDirSentence(sentence, fatherName, destination, meaning);
      //addCardinality(sentence, sourceCardinality, destinationCardinality);
    }
    catch (Exception e) {
      throw e;
    }
  ...
  protected ProcessExhibitionSentenceSet buildProcExhibSentenceSet(String
      objName) throws Exception {
  ...
  protected ProcessInvocationSentence buildProcessInvocationSentence(ILinkEntry link, String path) throws Exception{
    ProcessInvocationSentence agent= objFactory.createProcessInvocationSentence();
    String objName = builder.myStructure.getIEntry(link.getSourceId()).
        getName();
    agent.setProcessName(objName);
    fillPath(agent.getPathLabel(),path);
    agent.setLogicalRelation(setLogicBySource(link));
    agent.setMaxReactionTime(buildMaxReactionTime(link));
    agent.setMinReactionTime(buildMinReactionTime(link));
  return agent;
  }

  protected ProcessTimeoutSentence buildProcessTimeoutSentence(ILinkEntry link, String path) throws Exception{
    ProcessTimeoutSentence agent= objFactory.createProcessTimeoutSentence();
    IProcessEntry proc =(IProcessEntry)builder.myStructure.getIEntry(link.getSourceId());
    ...
    agent.setMaxReactionTime(buildMaxReactionTime(link));
    agent.setMinReactionTime(buildMinReactionTime(link));
    agent.setMaxTimeoutValue(buildMaxTimeoutValue(proc));
  return agent;
  }


  protected ChangingSentence buildChangingSentence(String procName, String logicRel, java.util.List path) throws
    Exception {
  ChangingSentence sentence = objFactory.createChangingSentence();
  sentence.setProcessName(procName);
  fillPath(sentence.getPathLabel(),path);
  sentence.setLogicalRelation(logicRel);
  return sentence;
 }

  protected ChangingClause buildChangingClause(ConsumptionClause cl,
                                               ResultClause res) throws
      Exception {
    ...
    cond.setSubjectAggregationFatherName(cl.getSubjectAggregationFatherName());
    cond.setSubjectExhibitionFatherName(cl.getSubjectExhibitionFatherName());
...
//What is logical relation?????
  protected ConditionClause buildConditionClause(ILinkEntry link) throws
      Exception {
    IStateEntry state = (IStateEntry) builder.myStructure.getIEntry(link.
        getSourceId());
    String stateName = state.getName();
    IObjectEntry obj = state.getParentIObjectEntry();
    ConditionClause cond = objFactory.createConditionClause();
    cond.setLogicalRelation(setLogicByDestination(link));
    cond.setMinimalCardinality(1);
    cond.setMaximalCardinality(1);
    ...
    }else cond.setObjectType("none");
    return cond;
  }

  protected EffectSentence buildEffectSentence(String procName, String logicRel, String path) throws
      Exception {
...
//What is logical relation?????
  protected EffectClause buildEffectClause(ILinkEntry link) throws Exception {
    IObjectEntry obj = (IObjectEntry) builder.myStructure.getIEntry(link.
        getSourceId());
...
//What is logical relation?????
  protected ConsumptionClause buildConsumptionClause(ILinkEntry link) throws
      Exception {
    IEntry entry = builder.myStructure.getIEntry(link.getSourceId());
    String stateName = "";
    IObjectEntry obj;
    ...
    String logicalRel = link.getPath();
...
//What is logical relation?????
  protected EnablingClause buildEnablingClause(ILinkEntry link) throws
      Exception {
    IEntry entry = builder.myStructure.getIEntry(link.getSourceId());
    String stateName = "";
    IObjectEntry obj;
  ...
  protected ResultSentence buildResultSentence(String procName, String logicRel, String path) throws
      Exception {
...
//What is logical relation?????
  protected ResultClause buildResultClause(ILinkEntry link) throws Exception {
    IEntry entry = builder.myStructure.getIEntry(link.getDestinationId());
    String stateName = "";
    IObjectEntry obj;
    if (entry instanceof IStateEntry) {
      IStateEntry state = (IStateEntry) entry;
    ...
    cond.setLogicalRelation(setLogicBySource(link));
    cond.setMinimalCardinality(1);
    cond.setMaximalCardinality(1);
    cond.setObjectName(obj.getName());
    if(OPLGeneral.isNotEmpty(obj.getType())){
  ...
  protected ProcessInZoomingSentenceSet buildProcessInZoomingSentenceSet(String
      procName) throws Exception {
  ...
  protected void fillProcBiDirSentence(ProcessBiDirectionalRelationSentence
                                       sentence,
                                       String fatherName,
 ...
 /* public void initStateObject(String stateName, IObjectEntry obj, IEntry entry) {
    if (entry instanceof IStateEntry) {
      IStateEntry state = (IStateEntry) entry;
      stateName = state.getName();
      obj = state.getParentIObjectEntry();
