...
public class OPLContainer {
  ...
  public OPLContainer(ISystem system,ObjectFactory ob_) {
    objFactory = ob_;
    myStructure = system.getISystemStructure();
  ...
  public OPLscript getOPLTree() throws Exception {
    if (nonUpdated) {
      treeBuilder = new OPLTreePerSystemBuilder(myStructure,
                                                systemName, objFactory);
      try {
        opl = treeBuilder.buildGeneralTree();
      }
      catch (Exception e) {
        throw e;
      }
  ...
  public OPLscript getOPLTreePerOPD(long opd) throws Exception {
    OPLscript tmpOpl;
    String opdID = Long.toString(opd);
    try {
      treeBuilder = new OPLTreePerOPDBuilder(myStructure,
                                             systemName, opd, objFactory);
      tmpOpl = treeBuilder.buildGeneralTree();
