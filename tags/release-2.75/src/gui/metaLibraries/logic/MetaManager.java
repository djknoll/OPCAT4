...
/**
 * Manages a collection of {@link MetaLibrary} objects, serving as a repository and 
 * a factory. 
 * @author Eran Toch
 * Created: 30/04/2004
 */
public class MetaManager implements Cloneable {
	 ...
	 * @return Enumeration  Contains MetaLibrary object, each of them represent
	 * a model of a domain-specific ontology.
	 * @author Eran Toch
	 */
	public Enumeration getMetaLibraries() {
		return metaLibraries.elements();
	}

	/**
	 * Retruns a <code>MetaLibrary</code> according to a given id.
	 * @param id	The ID of the library that should be retrieved.
	...
	public MetaLibrary getMetaLibrary(long id) {
		...
		for (int i = 0; i < metaLibraries.size(); i++) {
			meta = (MetaLibrary) metaLibraries.get(i);
	...
	/**
	 * Returns a copy of the vector of the meta libraries. Clones <code>metaLibraries</code>
	 * and returns it.
	 * @return	A Vector containing <code>MetaLibrary</code> objects. 
	 */
	public Vector getVectorClone() {
		Vector cloned = (Vector) metaLibraries.clone();
		return cloned;
	}

	/**
	 * Return a list of meta-libraries for display for users. The list is identical to
	 * the <code>metaLibraries</code> list, but does not contain libraries in a transient
	 * state (STATE_REMOVED, STATE_EDITED).
	 * @return	A Vector containing <code>MetaLibrary</code> objects, a subset of the
	 * current list.
	 */
	public final Vector getDisplayLibraries() {
		...
		MetaLibrary runner;
		for (int i = 0; i < metaLibraries.size(); i++) {
			runner = (MetaLibrary) metaLibraries.get(i);
	 ...
	 * 
	 * @param onto MetaLibrary The ontology to be added.
	 * @throws	MetaException	If the library already exists in the list.
	 */
	public void addMetaLibrary(MetaLibrary onto) throws MetaException {
		if (findByPath(onto.getPath()) != null) {
			throw new MetaException("MetaLibrary already exists", onto
					.getPath());
		}
		metaLibraries.add(onto);
	}

	/**
	 * Removes a <code>MetaLibrary</code> from the list, according to its path.
	 * 
	 * @param path	The path of the MetaLibrary to be removed.
	 * @throws	If no MetaLibrary exist with the given path.
	 */
	public void removeMetaLibrary(String path) throws MetaException {
		MetaLibrary lib = findByPath(path);
		if (lib == null) {
			throw new MetaException("MetaLibrary was not found", path);
		} else {
			removeMetaLibrary(lib);
		}
	}

	/**
	 * Removes a <code>MetaLibrary</code> from the list of libraries.
	 * @param dead	The <code>MetaLibrary</code> to be removed.
	 * @throws MetaException	If the list does not contain 
	 */
	private void removeMetaLibrary(MetaLibrary dead) throws MetaException {
		if (!metaLibraries.contains(dead)) {
			throw new MetaException("MetaLibrary was not found", dead.getPath());
		}
		metaLibraries.remove(dead);
	}

	/**
	 * Marks a <code>MetaLibrary</code> for removal, in the next <code>refresh()</code>
	 * call.
	 * 
	 * @param path	A path to the MetaLibrary to be marked. 
	 * @throws MetaException	If no MetaLibrary exist with the given path.
	 */
	 ...
	 */
	public void changeLibraryState(String path, String newPath, int newType) throws MetaException		{
		MetaLibrary theLib = findByPath(path);
		if (theLib == null)	{
	...
	/**
	 * Removes all the roles that belong to the given <code>MetaLibrary</code> from 
	 * the current <code>OpdProject</code>.
	 * @param lib	The <code>MetaLibrary</code> that should be cleaned out the project.
	 * @throws MetaException if any of the given inputs are null
	 */
	private int cleanRoles(MetaLibrary lib, OpdProject myProject)
			throws MetaException {
		try	{
			return goOverRoles(lib, myProject, true);
		}
	...
	/**
	 * Returns <code>true</code> if the project contains roles of the <code>lib</code>.
	 * @throws MetaException if any of the given inputs are null
	 */
	public boolean projectHasRolesOfLib(MetaLibrary lib, OpdProject myProject)	
		throws MetaException	{
		try	{
			if (goOverRoles(lib, myProject, false) > 0)	{
				return true;
			}
			else	{
				return false;
	 ...
	 * @param lib	The library that the roles belongs to.
	 * @param myProject	The project
	 * @param doRemove	If set to <code>true</code>, the method removes all the
	 * found roles. Otherwise, it only goes over the roles and return if it
	 * found any roles.
	 * @return	The number of roles found.
	 * @throws MetaException	If no project was found.
	 */
	private int goOverRoles(MetaLibrary lib, OpdProject myProject, boolean doRemove)
	throws MetaException {
		if (myProject == null) {
			throw new MetaException("While going over the roles, current project is null", lib.getPath());
		}
		if (lib == null)	{
			throw new MetaException("While going over the roles, MetaLibrary is null", lib.getPath());
		}
		int foundRolesNum = 0;
		long libID = lib.getReferenceID();
		IXSystem system = (IXSystem) myProject;
		IXSystemStructure oStructure = system.getIXSystemStructure();
		Enumeration enum = oStructure.getElements();
		IXEntry entry;
		IXThingEntry thing;
			...
			if (entry instanceof IXThingEntry) {
				...
				while (allRoles.hasMoreElements()) {
					...
					if (role.getLibraryId() == libID) {
						foundRolesNum++;
						//If the method is set for removing roles, we remove the role
						if (doRemove)	{
	...
	/**
	 * Gets a <code>MetaLibrary</code> from the list according to it's path name. If two
	 * libraries have the same path, the first one that appears on the list will be removed.
	 * @param path	The path of the library.
	 * @return	The first MetaLibrary that was found with the given path name, or <code>null</code>
	 ...
	 * @param id	The reference ID of the library.
	 */
	...
	/**
	 * Returns an avaliable unique ID for a MetaLibrary in the context
	 * of this <code>MetaManager</code>.
	 * Goes through the existing id's and return an ID which is 
	 * larger in 1 than the highest.
	 */
	...
	/**
	 * A factory method for creating a {@link MetaLibrary}, in an 
	 * unloaded state. The method automatically assigns an ID to 
	 * the library.  
	 * @param _path		The path to the library.
	 * @param _refType	The reference type (<code>TYPE_FILE</code>
	 *  or <code>TYPE_URL</code>). 
	...
	public MetaLibrary createNewMetaLibraryReference(String _path, int _refType) {
		return new MetaLibrary(getNextAvaliableID(), _path, _refType);
	}

	/**
	 * Returns a string representation of the MetaManager. Containing the path names of
	 * the MetaLibraries with line breaks between them.
	 */
	public String toString() {
		String output = "";
		MetaLibrary lib;
		for (int i = 0; i < metaLibraries.size(); i++) {
			lib = (MetaLibrary) metaLibraries.get(i);
			output += lib.getPath();
	 ...
	 * @param libraries	A Vector containing <code>MetaLibrary</code> objects.
	 */
	public void setLibrariesVector(Vector libraries) {
		metaLibraries = libraries;
	}
	...
	public Object clone() {
		...
		copy.setLibrariesVector(getVectorClone());
		return copy;
	}

	/**
	 * Refresh the library list, loading referenced libraries, removeing necessary
	 * libraries etc.<p>
	 ...
	 * <p>
	 * The method manages a progress bar which is incremented according to the loading
	 * process.
	 */
	public void refresh(OpdProject myProject, JProgressBar progressBar) {
		...
		pBar.setMinimum(0);
		pBar.setMaximum(this.size() + 1);
		pBar.setValue(0);
		try {
			removeMarkedLibraries(myProject);
			loadLibraries();
			refreshRoles(myProject);
	...
	/**
	 * Loads roles in the given <code>OpdProject</code> which are not loaded from an
	 * imported meta-library. The method is used after the libraries loading procedure 
	 * had ended, and assignes the identity of the unloaded roles 
	 * (that <code>role.getState() == Role.UNLOADED</code>).
	 * 
	 * @param myProject	The project to be refreshed.
	 */
	public void refreshRoles(OpdProject myProject) {
		Enumeration enum = myProject.getComponentsStructure().getElements();
		ThingEntry thing;
		Enumeration allRoles;
		Role role;
		Entry entry;
				...
				if (entry instanceof ThingEntry) {
					...
					while (allRoles.hasMoreElements()) {
						...
						if (role.getState() == Role.UNLOADED) {
							role.load(getMetaLibrary(role.getLibraryId()));
						}
					}
				}
			}
	...
	/**
	 * Checks if the manager contains <code>MetaLibrary</code> objects that failed
	 * to load. 
	 * @return	<code>true</code> if at least one library failed to load. <code>false</code>
	 * otherwise.
	 */
	...
	/**
	 * Returns a message that contains all the MetaLibraries that failed to load.
	 */
	...
	/**
	 * Removes all libraries which are marked as <code>STATE_REMOVED</code>. The method 
	 * cleans the roles of a removed <code>MetaLibrary</code> and removes the library.
	 * @throws MetaException	Whatever removeMetaLibrary or cleanRoles throws.
	 */
		...
		if (metaLibraries != null) {
			...
			for (int i = 0; i < metaLibraries.size(); i++) {
				...
				if (runner.getState() == MetaLibrary.STATE_REMOVED) {
					try {
						cleanRoles(runner, myProject);
						removeMetaLibrary(runner);
					} catch (MetaException E) {
						throw E;
					}
				}
	...
	/**
	 * Loads all libraries which are not loaded from the Web or the local file system.
	 * The method runs through the meta-libraries list and loads each library which has
	 * an <code>STATE_REFERENCE</code> state,
	 */
		...
		for (int i = 0; i < metaLibraries.size(); i++) {
			...
			if (metaLib.getState() == MetaLibrary.STATE_REFERENCE) {
				...
				if (metaLib.getReferenceType() == MetaLibrary.TYPE_FILE) {
					String fileName = (String) metaLib.getPath();
					...
					try {
						...
						//System.out.println(E.getMessage() +" file: "+ fileName);
						metaLib.setState(MetaLibrary.STATE_LOAD_FAILED);
					}
				}
				//Handling URL's
				...
				try {
					if (is != null) {
						OpdProject pr = _loadProject(ld, is);
						if (pr != null) {
							boolean doSetProject = true;
							//Checking if the global ID is equal. We deal gracefully with
							...
							if (metaLib.projectIsNull()) {
								//System.out.println("Error - Ontology loading returned a null project");
							}
					...
					//System.out.println("Error while loading meta-library:"+ E.getMessage());
					metaLib.setState(MetaLibrary.STATE_LOAD_FAILED);
				} finally {
					incrementProgressBar();
				}
	...
	/**
	 * Returns the number of meta-libraries managed by this <code>MetaManager</code>.
	 */
	public int size() {
		if (metaLibraries == null) {
			return 0;
		}
		return metaLibraries.size();
	}
