...
/**
 *  <p>This class graphicaly represents OPM State as defined in OPM documentation.
 *  State can be drawn inside <a href = "OpdObject.html"><code>OpdObject</code></a> or outside it (this option is not implemented).
 *  State also can be drawn as final or initial state according to OPD documentation.</p>
 */
...
public class OpdState extends OpdConnectionEdge
{
	...
	/**
	 *  <p>Constructs <code>OpdState</code> object and initialize all defaults.</p>
	 ...
	 *  @param <code>mState OpmState</code>, the logic representation of the state <a href = "../opmEntities/OpmState.html"><code>OpmState</code></a>.
	 ...
	 */
	public	OpdState(OpmState mState, OpdProject pProject, long pOpdId, long pEntityInOpdId)
	{
		super(pOpdId , pEntityInOpdId, pProject);
		myEntity = mState;

		GenericTable config = pProject.getConfiguration();
		borderColor = (Color)config.getProperty("StateColor");
		...
		myEntry = myProject.getComponentsStructure().getEntry(mState.getId());
	}
	 ...
	 *  <p>If state is initial paints it with bolder border. If state is final paints with double line border.</p>
	 */
	public void paintComponent(Graphics g)
	{
		...
		int stringX;
		int stringY;
		int extRound = Math.max((int)(getHeight()/4.0), (int)(MIN_EXTERNAL_ROUND*factor));
		int intRound = Math.max((int)((getHeight()-10.0*factor)/4.0), (int)(MIN_INTERNAL_ROUND*factor));
		...
		OpmState opmState = (OpmState)myEntity;
		Font currentFont = (Font)myProject.getConfiguration().getProperty("StateFont");
		currentFont = currentFont.deriveFont((float)(currentFont.getSize2D()*factor));
		g2.setFont(currentFont);
		currentMetrics = getFontMetrics(currentFont);

		g2.setColor(backgroundColor);
		g2.fillRoundRect(selOffset, selOffset,  getWidth()-1-dSelOffset, getHeight()-1-dSelOffset, extRound, extRound);
		g2.setColor(borderColor);
		if (opmState.isInitial())
		{
			g2.setStroke(new BasicStroke((float)factor*3.0f));
			g2.drawRoundRect(selOffset, selOffset,  getWidth()-1-dSelOffset, getHeight()-1-dSelOffset, extRound, extRound);
		}
		...
		if (opmState.isFinal())
		{
			...
			g2.drawRoundRect(dist, dist,  getWidth()-1-dist*2, getHeight()-1-dist*2, intRound, intRound);
		...
		st = new StringTokenizer(myEntity.getName(), "\n");

		stringY=(getHeight()-currentMetrics.getAscent()*numOfRows-selOffset)/2;
		int dY = 0;
		...
		while (st.hasMoreTokens())
		{
			...
			stringX=(getWidth()-currentMetrics.stringWidth(tString)-selOffset)/2;
	...
	/**
	 *  This method constructs and shows <a href = "StatePropertiesDialog.html"><code>StatePropertiesDialog</code></a>
	 *  <p>Updates changed State properties not only in this OpdState, but in <a href = "../opmEntities/OpmState.html"><code>OpmState</code></a> this <code>OpdState</code> represents.
	 */
	public  void callPropertiesDialog(int showTabs, int showButtons)
	{
		StateInstance si = (StateInstance)getEntry().getInstance(new OpdKey(getOpdId(), getEntityInOpdId()));
		StatePropertiesDialog sd = new StatePropertiesDialog(si, myProject, true, showButtons);
		sd.showDialog();
	}
	...
	public boolean inMove(int pX, int pY)
	{
		...
		if (pX>borderSensitivity && pY>borderSensitivity
			 && pX<getWidth()-borderSensitivity
			 && pY<getHeight()-borderSensitivity) return true;
		else return false;
	}
...
//	}
	...
	public boolean inResize(int pX, int pY)
	{
		...
		if (!(pX>borderSensitivity && pY>borderSensitivity &&
			pX<getWidth()-borderSensitivity && pY<getHeight()-borderSensitivity)) return true;
		else return false;
	}
...
//		if (parentObject.isStatesAutoarrange()) return NOT_IN_BORDER;
	...
	public  Point getAbsoluteConnectionPoint(RelativeConnectionPoint pPoint)
	{
		...
		switch (pPoint.getSide())
		{
			case OpcatConstants.N_BORDER:
				...
				if (pPoint.getParam() > 1) return new Point(getWidth() - 2 , 2);

				retPoint.setLocation(getWidth()*pPoint.getParam(), 2);
				break;
			case OpcatConstants.S_BORDER:

				if (pPoint.getParam() < 0) return new Point(2, getHeight() - 2);
				if (pPoint.getParam() > 1) return new Point(getWidth() - 2 , getHeight() - 2);

				retPoint.setLocation(getWidth()*pPoint.getParam(), getHeight()-2);
				break;
			case OpcatConstants.E_BORDER:

				if (pPoint.getParam() < 0) return new Point(getWidth() - 2, 2);
				if (pPoint.getParam() > 1) return new Point(getWidth() - 2 , getHeight() - 2);

				retPoint.setLocation(getWidth()-2, getHeight()*pPoint.getParam());
				break;
			case OpcatConstants.W_BORDER:

				if (pPoint.getParam() < 0) return new Point(2, 2);
				if (pPoint.getParam() > 1) return new Point(2, getHeight() - 2);

				retPoint.setLocation(2, getHeight()*pPoint.getParam());
				break;
		}
	...
	{
		JPopupMenu jpm = new StatePopup(this.myProject, getInstance());
		Point showPoint = GraphicsUtils.findPlace4Menu(this, new Point(pX, pY), jpm.getPreferredSize());
	...
	public void fitToContent()
	{
		...
		int width = Math.max((int)(stWidth.doubleValue()*factor), (int)Math.round(sw*1.2+extension));
		int height = Math.max((int)(stHeight.doubleValue()*factor), (int)Math.round(sh+extension));
