...
public abstract class OpdFundamentalRelation extends OpdBaseComponent
{
    ...
    {
        setSize((int)Math.round(d.getWidth()), (int)Math.round(d.getHeight()));
    }
	...
	{
		graphicalEntry = pGraphicalEntry;
	}
	...
	public Point getAbsoluteConnectionPoint(RelativeConnectionPoint pPoint)
	{
		...
		switch (pPoint.getSide())
		{
			case OpcatConstants.N_BORDER:
				retPoint.setLocation((getWidth()- 1)*pPoint.getParam(), selOffset);
				break;
			case OpcatConstants.S_BORDER:
				retPoint.setLocation((getWidth()- 1)*pPoint.getParam(), getHeight()-selOffset);
				break;
			case OpcatConstants.E_BORDER:
				retPoint.setLocation(getWidth()-selOffset, selOffset+(getHeight()-dSelOffset)*pPoint.getParam());
				break;
			case OpcatConstants.W_BORDER:
				retPoint.setLocation(selOffset, selOffset+(getHeight()-dSelOffset)*pPoint.getParam());
				break;
		}
	...
	public void mousePressed(MouseEvent e)
	{
        ...
        for (Enumeration enum = graphicalEntry.getInstances();enum.hasMoreElements();)
        {
            Instance currInst = (Instance)enum.nextElement();
