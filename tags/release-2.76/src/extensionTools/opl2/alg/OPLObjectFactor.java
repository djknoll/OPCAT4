...
public class OPLObjectFactor
    extends OPLFactor {

  public OPLObjectFactor(OPLTreeBuilder builder_, String systemName_,ObjectFactory ob) {
    super(builder_, systemName_,ob);
  }

  protected TypeDeclarationSentence buildTypeDeclSentence(IObjectEntry obj) throws
      Exception {
    ...
    try {
      ...
      else sentence.setObjectType(type);
      String fatherName = builder.findFatherName(obj);
...
//Should add role parameter
  protected ObjectAggregationSentenceSet buildObjAggregSentenceSet(String
...
//Added role
  protected ObjectInstanceSentence buildObjInstSentence(IRelationEntry rel) throws
     Exception {
...
//Should add parameter
  protected ObjectStateSentence buildObjStateSentence(String objName) throws
      Exception {
...
//Added role
  protected AgentSentence buildAgentSentence(ILinkEntry link, String path) throws Exception{
...
//Added role
  protected GeneralEventSentence buildGeneralEventSentence(ILinkEntry link, String path) throws Exception{
    GeneralEventSentence agent= objFactory.createGeneralEventSentence();
    IObjectEntry obj = (IObjectEntry)builder.myStructure.getIEntry(link.getSourceId());
...
//Added role
  protected StateTimeoutSentence buildStateTimeoutSentence(ILinkEntry link, String path) throws Exception{
    StateTimeoutSentence agent= objFactory.createStateTimeoutSentence();
    IStateEntry state = (IStateEntry)builder.myStructure.getIEntry(link.getSourceId());
...
//Added role
  protected StateEntranceSentence buildStateEntranceSentence(ILinkEntry link, String path) throws Exception{
  ...
  protected StateClause buildStateClause(IStateEntry state) throws Exception {
    ...
    try {
      ...
      if (!maxTime.equals("infinity"))
        clause.setMaxTimeValue(buildMaxTimeValue(maxTime));
      if (!minTime.equals("0;0;0;0;0;0;0"))
        clause.setMinTimeValue(buildMinTimeValue(minTime));
...
//Added role
  protected ObjectInheritanceSentenceSet buildObjInhSentenceSet(IRelationEntry rel) throws
      Exception {
    ...
    try {
      ...
      sentence.setObjectName(childName);
      sentence.getObjectInheritanceSentence().add(buildObjInhSentence(rel));
    }
    catch (Exception e) {
      throw e;
    }
  ...
  protected ObjectInheritanceSentence buildObjInhSentence(IRelationEntry rel) throws
  Exception {
...
//Added roles
  protected ObjectUniDirectionalRelationSentence buildObjUniDirSentence(
    ...
    try {
      ...
      if(!((IThingEntry)builder.myStructure.getIEntry(rel.getDestinationId())).getRole().equals(""));
        //sentence.setDestinationRole(((IThingEntry)builder.myStructure.getIEntry(rel.getDestinationId())).getRole());
      if(!((IThingEntry)builder.myStructure.getIEntry(rel.getSourceId())).getRole().equals(""));
  ...
  protected ObjectBiDirectionalRelationSentence buildObjBiDirSentence(
     ...
     try {
       ...
       if (rule == OPLGeneral.DESTINATION) {
         ...
         {
           fillObjBiDirSentence(sentence, destination, fatherName, meaning);
       ...
       fillObjBiDirSentence(sentence, fatherName, destination, meaning);
...
//Added role
  public AggregatedObject buildAggregatedObject(IEntry destination,
    ...
    int[] cardinality = OPLGeneral.getCardinality(rel.getDestinationCardinality());
    obj.setMaximalCardinality(cardinality[MAX]);
    obj.setMinimalCardinality(cardinality[MIN]);
    return obj;
  }

  protected ObjectExhibitionSentenceSet buildObjExhibSentenceSet(String objName) throws
      Exception {
...
//Should add role!!!
  protected ObjectInZoomingSentenceSet buildObjectInZoomingSentenceSet(String
  ...
  protected void addCardinality(ObjectBiDirectionalRelationSentence sentence,
    ...
    sentence.setSourceMaximalCardinality(sourceCardinality[MAX]);
    sentence.setSourceMinimalCardinality(sourceCardinality[MIN]);
    sentence.setDestinationMaximalCardinality(destinationCardinality[MAX]);
    sentence.setDestinationMinimalCardinality(destinationCardinality[MIN]);
  }

  protected void addCardinality(ObjectUniDirectionalRelationSentence sentence,
                                IRelationEntry rel) {
    //System.err.println(" Adding cardinality to "+builder.myStructure.getIEntry(rel.getSourceId()).getName());
    //System.err.println(" Adding cardinality to "+builder.myStructure.getIEntry(rel.getDestinationId()).getName());
    int[] sourceCardinality = OPLGeneral.getCardinality(rel.
        getSourceCardinality());
    int[] destinationCardinality = OPLGeneral.getCardinality(rel.
        getDestinationCardinality());
  ...
  protected void fillObjBiDirSentence(ObjectBiDirectionalRelationSentence
                                      sentence,
                                      String fatherName,
