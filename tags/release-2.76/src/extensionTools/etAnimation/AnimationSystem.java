...
//////////////////// deactivate ////////////////////////
  ...
  private void animationChecking(AnimationEvent tempEvent, AnimationEntry tempEntry, String animationAction ) {
	  ...
	  //first check the event validaty. if it is canceled then do nothing. 
	  if ( (tempEvent.getCancelStep() != 0) || (tempEvent.getNumberOfInstances() == 0 )) {
		  APILogger.Print(this, "Event with Zero Instances, Parent = " + tempEvent.getParentProcess() + ", entry = " + tempEntry.getIXEntry().getName()) ; 
		  return ; 
	  }
	  try {
...
//      AnimationGridAddRow(AnimationAuxiliary.ActionActivate, link, retVal,link.getName() + "(Validate) - " +  str);
  ...
  private AnimationGridPanel InitAnimationGridPanel () {
		...
		locPanel.GetColumnsModel().getColumn(4).sizeWidthToFit(); 
