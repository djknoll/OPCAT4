...
public class ConstraintBasedForceDirectedLayoutSystem
{
    ...
    {
        if (m_edges.containsKey(edge))
        {
            return;
        }

        NodeContext sourceNode      = getNodeContext(edge.getSourceIXNode());
        NodeContext destinationNode = getNodeContext(edge.getDestinationIXNode());
    ...
    public void removeAllAnchors()
    {
        ...
        for (Enumeration enumNodes = m_nodes.elements(); enumNodes.hasMoreElements(); /* inside loop */)
        {
    ...
    public Iterator addSyntacticConstraints()
    {
        ...
        for (Enumeration enumOuterNodes = m_nodes.elements(); enumOuterNodes.hasMoreElements(); /* inside loop */)
        {
            ...
            Enumeration enumInnerNodes = m_nodes.elements();
            ...
            while (enumInnerNodes.hasMoreElements())
            {
                ...
                {
                    addNodeToConstraintList(zoneConstraintNodesLists, outerNode, innerNode);
                }
                else if (innerNode.IsParentOf(outerNode))
                {
                    addNodeToConstraintList(zoneConstraintNodesLists, innerNode, outerNode);
                }
                else
                {
            ...
            //
            for (Enumeration enumEdges = m_edges.elements(); enumEdges.hasMoreElements(); /* inside loop */)
            {
    ...
    public void initArrange(boolean arrangeInTwoPhases)
    {
        ...
        //
        m_currentPhase = arrangeInTwoPhases ? 1 : 2;

        Debug.Print( 10,
                     "The system is ready for arrangment operation with {0} nodes, {1} edges and {2} constraints",
                     new Object[] { new Integer(m_nodes.size()),
                                    new Integer(m_edges.size()),
                                    new Integer(m_constraints.size())});
    ...
    public boolean arrange(int maximumIterations)
    {
        ...
        int startPhase = m_currentPhase;

        //
        // Execute the selected phases.
        //
        while (m_currentPhase <= 2)
        {
            ...
            for (;;)
            {
                ...
                Debug.Print( 10,
                             ...
                             new Object[] {new Integer(m_currentIteration)});
                ...
                if (m_currentIteration > maximumIterations)
                {
                    ++m_currentPhase;

                    Debug.Print( 20,
                                 "***** Phase raised to {0} after {1} iterations. *****",
                                 new Object[] {new Integer(m_currentPhase), new Integer(m_currentIteration)});
                    break;
                }
            }
        }
    ...
    public boolean step()
    {
        ...
        //
        boolean ignoreSyntacticConstraints = (1 == m_currentPhase);

        if (adjustPosition(ignoreSyntacticConstraints) || isLowEnergyState())
        {
            //
    ...
    {
        return new NodeNodeOverlapConstraint(getNodeContext(node1), getNodeContext(node2));
    }
    ...
    {
        return new NodeEdgeOverlapConstraint(getNodeContext(node), getEdgeContext(edge));
    }
    ...
    {
        return new SequenceConstraint(getNodeContext(node1), getNodeContext(node2), direction, false);
    }
    ...
    {
        ZoneConstraint constraint = new ZoneConstraint( getNodeContext(containerNode),
                                                        getNodesList(nodes).iterator());
        return constraint;
    }
    ...
    {
        TShapeConstraint constraint = new TShapeConstraint( getNodeContext(rootNode),
                                                            getNodeContext(relationNode),
                                                            getNodesList(childNodes).iterator());
        return constraint;
    }
    ...
    private NodeContext getNodeContext(IXNode node)
    {
        ...
        return (NodeContext)m_nodes.get(node);
    }
    ...
    private EdgeContext getEdgeContext(IXLine edge)
    {
        ...
        return (EdgeContext)m_edges.get(edge);
    }
    ...
    private void updateSprings()
    {
        ...
        // Create new springs.
        //
        for (ListIterator iterConstraints = m_constraints.listIterator(); iterConstraints.hasNext(); /* inside loop */)
        {
    ...
    private boolean adjustPosition(boolean ignoreSyntacticConstraints)
    {
        ...
        {
            NodeContext node = (NodeContext)enumNodes.nextElement();
            node.CalculateNewPosition(dampingConstant, m_timeInterval, ignoreSyntacticConstraints);
        }
    ...
    private boolean isLowEnergyState()
    {
        ...
        double energy        = 0;
        double allowedEnergy = m_nodes.size() * 0.01 * 2;
