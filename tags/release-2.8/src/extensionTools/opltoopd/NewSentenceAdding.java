...
public class NewSentenceAdding {
  ...
  public IXObjectInstance newObjectAdding(ObjectStruct newSent,long opdId,boolean newOne){
    ...
    Enumeration locEnum = getThingEntry(newSent.getObjectName()).getInstances();
  ...
  public IXProcessInstance newProcessAdding(ProcessStruct newSent,long opdId, boolean newOne){
    ...
    Enumeration locEnum = getThingEntry(newSent.getProcessName()).getInstances();
    ...
    while(stateIter.hasNext()){
      ...
      while (stateEnum.hasMoreElements()){
        ...
        if (stateStr.getStateName().equals(stateInst .getIXEntry().getName())){
          if (stateStr.getFrom()){
            newLinkUpdate(newSent, iXSystem.addLink(stateInst,proc,OpcatConstants.CONSUMPTION_LINK),"");
          }
          else{
            newLinkUpdate(newSent, iXSystem.addLink(proc,stateInst,OpcatConstants.CONSUMPTION_LINK),"");
          }
        }
      }
    }
 ...
 */
  public void addnewLink (IXConnectionEdgeInstance source, IXConnectionEdgeInstance dest,LinkStruct newSent,boolean newSource, boolean newDest){
    ...
    if (res.getResult()==IXCheckResult.WRONG) {
      if (iXSystem.checkDeletion(source).getResult()==IXCheckResult.RIGHT)
        if (newSource){
          iXSystem.delete(source);
        }
      if (iXSystem.checkDeletion(dest).getResult()==IXCheckResult.RIGHT)
        if(newDest){
 ...
 */
    ...
    public long getNumFromUser(HashSet screens)
     {
       ...
       String opdName = selectedValue.toString();
         ...
         Enumeration locEnum = iXSystem.getIXSystemStructure().getOpds();
         while(locEnum.hasMoreElements()){
  ...
  private IXEntry getThingEntry (String name){
    Enumeration locEnum = systStruct.getElements();
    IXEntry entry;
    while (locEnum.hasMoreElements()){
...
/*This function returns true if the thing is completely new*/
  private boolean ifNewThing(String newThingName ){
    ...
    Enumeration locEnum = iXSystem.getIXSystemStructure().getElements();
 ...
 */
  ...
  public void newRelationUpdate(RelationStruct rel, IXRelationInstance inst,String destCard){
   ...
   private  int getType(String type){
     int intType = 0;
     if (type.compareTo("exhibits")==0) return OpcatConstants.EXHIBITION_RELATION;
     if (type.compareTo("consists of")==0) return OpcatConstants.AGGREGATION_RELATION;
     if (type.compareTo("is a")==0) return OpcatConstants.SPECIALIZATION_RELATION;
     if (type.compareTo("is an instance of")==0) return OpcatConstants.INSTANTINATION_RELATION;
     if (type.compareTo("relates to")==0) return OpcatConstants.UNI_DIRECTIONAL_RELATION;
     if (type.compareTo("are related")==0) return OpcatConstants.BI_DIRECTIONAL_RELATION;
     if (type.compareTo("are equivalent")==0) return OpcatConstants.BI_DIRECTIONAL_RELATION;
     if (type.compareTo("handles")==0) return OpcatConstants.AGENT_LINK;
     if (type.compareTo("requires")==0) return OpcatConstants.INSTRUMENT_LINK;
     if (type.compareTo("consumes")==0) return OpcatConstants.CONSUMPTION_LINK;
     if (type.compareTo("affects")==0) return OpcatConstants.EFFECT_LINK;
     if (type.compareTo("yields")==0) return OpcatConstants.RESULT_LINK;
     if (type.compareTo("triggers")==0) return OpcatConstants.INVOCATION_LINK;
     if (type.compareTo("inst_event")==0) return OpcatConstants.INSTRUMENT_EVENT_LINK;
     if (type.compareTo("exception")==0) return OpcatConstants.EXCEPTION_LINK;
     if (type.compareTo("occurs")==0) return OpcatConstants.CONDITION_LINK;
     if (type.compareTo("cons_event")==0) return OpcatConstants.CONSUMPTION_EVENT_LINK;
  ...
  public IXThingInstance getThingInstance (String name, long opdId){
       Enumeration locEnum = systStruct.getThingsInOpd(opdId);
