 ...
 */
  ...
  private void jbInit() {
    ...
    DocSelP.setBorder(BorderFactory.createEtchedBorder());
    DocSelP.setBounds(new Rectangle(8, 6, 253, 202));
    DocSelP.setLayout(null);
    Anal.setText("Analysis Document");
    Anal.setBounds(new Rectangle(38, 28, 191, 25));
    Des.setText("Design Document");
    Des.setBounds(new Rectangle(38, 59, 191, 25));
    Cust.setText("Customized Document");
    Cust.setBounds(new Rectangle(38, 90, 191, 25));
    ok.setBounds(new Rectangle(74, 159, 79, 27));
    ok.setText("OK");
    ...
    Cancel.setBounds(new Rectangle(160, 159, 79, 27));
    ...
    this.getContentPane().add(DocSelP, null);
    DocSelP.add(Anal, null);
    DocSelP.add(Des, null);
    DocSelP.add(Cust, null);
    DocSelP.add(Cancel, null);
    DocSelP.add(ok, null);
    group.add(Anal);
    group.add(Des);
    group.add(Cust);
 ...
 */
  void ok_actionPerformed(ActionEvent e) {
    ...
    if (Cust.isSelected()) {
      DocGen new_doc = new DocGen(mySys);
      new_doc.setVisible(true);
    }// of cust
    //if Analysis
    if (Anal.isSelected()){
        Document doc = AnalDocCreate();//create analysis document
        String filename="";
        //saving the file
        ...
        if(returnVal == JFileChooser.APPROVE_OPTION) {
          ...
          if (flag==1){
            ...
            if (retval== JOptionPane.YES_OPTION){
              try{
                doc.PrintDocument(mySys,filename);//save the file
    ...
    //if Design Document
    if (Des.isSelected()){
        Document doc = DesDocCreate();//create document
        String filename="";
        //saving document
        DocSave doc_save = new DocSave(this);
        int returnVal = doc_save.SDFileChooser.showSaveDialog(this);
        if(returnVal == JFileChooser.APPROVE_OPTION) {
          ...
          if (flag==1){
            ...
            if (retval== JOptionPane.YES_OPTION){
              try{
                doc.PrintDocument(mySys,filename);//save the document
                this.dispose();//close this screen
              }// of try
              catch(IOException er){
                System.out.println("There was a problem saving the document");
