...
public class MetaReflection extends Reflection	{
	...
	public void addPair(IThingEntry thing) throws ValidationException {
		VRole vrole = new VRole(libraryID, thing.getId());
		...
		try	{
			constraintMap.put(vrole, findConstraints(vThing, thing));
		}
	...
	protected MultipleApplicant createApplicant(IThingEntry thing)	{
		...
		if (AlgorithmConfiguration.DO_IS_A) {
			app.addAllApplicants(getSpecializationRoles(vrole));
		}
		return app;
	}
	...
	protected Set getSpecializationRoles(VRole vrole)	{
		...
		IThingEntry thing = (IThingEntry)model.getISystemStructure().getIEntry(vrole.getThingID());
		...
		while (connections.hasMoreElements())	{
			...
			if (rel.getRelationType() == OpcatConstants.SPECIALIZATION_RELATION) {
				IThingEntry specialization =(IThingEntry)model.getISystemStructure().getIEntry(
													rel.getDestinationId());
				VRole role = new VRole(libraryID, specialization.getId());
				set.add(role);
				set.addAll(getSpecializationRoles(role));
			}					
		}
		return set;
	}
