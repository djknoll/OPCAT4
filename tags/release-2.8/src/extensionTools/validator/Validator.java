...
public class Validator implements OpcatExtensionTool {
	...
	public JPanel execute(ISystem opcatSystem, Opcat2 myOpcat2, JFrame parent) {
		if (algorithm == null)	{
			algorithm = new VAlgorithm();
		}
		return new ValidatorPanel(opcatSystem, myOpcat2, algorithm, parent);
	}
	...
	public JPanel execute(ISystem opcatSystem) {
		...
		return new ValidatorPanel(opcatSystem, null, algorithm, null);
	}
	...
	public JDialog validationResults(ISystem opcatSystem, Opcat2 myOpcat2, JFrame parent) {
		...
		dialog.getContentPane().add(
			new ValidatorPanel(opcatSystem, myOpcat2, algorithm, parent),
			BorderLayout.CENTER);
		return dialog;
	}
