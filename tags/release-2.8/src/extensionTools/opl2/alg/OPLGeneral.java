...
public class OPLGeneral {
  ...
  static public int[] getTime(String time) {
    int[] res = {
        -1, -1, -1, -1, -1, -1, -1};
    int i_new = time.length(), i_old = time.length();
    String tmp = "0";
  ...
  static public List sort2(Enumeration e, GeneralComparator comp) {
    ...
    java.util.List list = new java.util.LinkedList();
    GeneralComparatorByProcess proc = new GeneralComparatorByProcess();
    MyComparator id = new MyComparator();
    IEntry elem;
    if(e!=null)
    while(e.hasMoreElements()){
  ...
  static public boolean equalPaths(java.util.List sentencePath,java.util.List path){
    ...
    for(int i=0;i<sentencePath.size();i++){
      if(!((String)sentencePath.get(i)).equals((String)path.get(i)))
        return false;
    }
  return true;
  }
 ...
 return visit(min.getTimeValue());
  ...
  public static String visit(TimeValueType max){
   ...
   if(max.getDays()>1){
    ...
    }else if(number>2) bf.insert(last+2,"and ");
    return bf.toString();
 }
