...
public class OPLFactor {
  ...
  public class EmptySentenceException
      extends Exception {}
  ...
  protected void throwSpecialException(IEntry abstractEntry) throws
      EmptySentenceException {
    builder.VisitedThings.remove(abstractEntry);
    throw new EmptySentenceException();
  }

  protected ThingSentenceSet buildThingSentence(IEntry abstractEntry) throws
      Exception, EmptySentenceException {
    ThingSentenceSet thing;
    if (builder.VisitedThings.contains(abstractEntry))
      throw new Exception();
    //Has non visited zooming father
    if ( /*builder.hasNonVisitedInheritenceFather(abstractEntry)||*/
        builder.hasNonVisitedZoomingFather(abstractEntry))
      throw new EmptySentenceException();
    builder.VisitedThings.add(abstractEntry);
    try {
      thing = objFactory.createThingSentenceSet();
      thing.setSystemName(systemName);

      thing.setSubjectThingName(abstractEntry.getName());
      builder.addFatherName(abstractEntry, thing);
      if (abstractEntry instanceof IObjectEntry) {
        IObjectEntry obj = (IObjectEntry) abstractEntry;
        if (!builder.fillThingSentence(obj, thing)) {
        ...
        if (!builder.fillThingSentence(proc, thing)) {
          //HERE WE CAN MAKE EMPTY SENTENCE
          throw new EmptySentenceException();
        }
        thing.setMinTimeValue(buildMinTimeValue(proc.getMinTimeActivation()));
        thing.setMaxTimeValue(buildMaxTimeValue(proc.getMaxTimeActivation()));
...
java.util.List buildRoles(IThingEntry abstractEntry)throws JAXBException{
  ...
  java.util.Enumeration en = abstractEntry.getRoles();
...
protected MinTimeValue buildMinTimeValue(String sourceTime_)throws Exception{
  ...
  public IEntry getEntry(IRelationEntry rel) {
    return builder.myStructure.getIEntry(rel.getDestinationId());
  }
...
//while (st.hasMoreTokens()) {
 ...
 protected String setLogicBySource(ILinkEntry link){
   ...
   if(((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorSourceNeighbours(true)!=null)
     or = ((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorSourceNeighbours(true).hasMoreElements();
   if(this.builder.getIInstanceByOPD(link)!=null)
   if(((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorSourceNeighbours(false)!=null)
     xor = ((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorSourceNeighbours(false).hasMoreElements();
   if(or)
       return "or";
   else if(xor){
     return "xor";
 ...
 protected String setLogicByDestination(ILinkEntry link){
   ...
   boolean xor = false;
   if(this.builder.getIInstanceByOPD(link)!=null)
    if(((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorDestinationNeighbours(true)!=null)
       or=((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorDestinationNeighbours(true).hasMoreElements();
   if(this.builder.getIInstanceByOPD(link)!=null)
   if(((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorDestinationNeighbours(false)!=null)
     xor = ((ILinkInstance)this.builder.getIInstanceByOPD(link)).getOrXorDestinationNeighbours(false).hasMoreElements();
   if(or)
      return "or";
   else if(xor){
    return "xor";
