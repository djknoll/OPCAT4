...
public class AnimationSystem extends Object implements Animation  {
  ...
  public AnimationSystem(IXSystem mySys, AnimationStatusBar guiPanel) {
    ...
    animationPanel =  InitAnimationGridPanel() ;
	animationPanel.AddToExtensionToolsPanel(AnimationAuxiliary.AnimationPanelName) ;     
  }
  ...
  public void animationStop()  {
    ...
    animationPanel.RestoreThingOrigColor(); 
  ...
  public void animationActivate() {
	  if(animationTimer == null || animationTimer.isPause()) {
...
//////////////////// deactivate ////////////////////////
  ...
  private void animationChecking(AnimationEvent tempEvent, AnimationEntry tempEntry, String animationAction ) {  
	  try {
		if(debugEvents.size() > 0 &&  debugEvents.get(new Long(tempEvent.getExecutionId())) != null ) {
			AnimationGridAddRow(animationAction,tempEntry, tempEvent) ;     			
		}   
		else {
			debugEvents.put(new Long(tempEvent.getExecutionId()), tempEvent) ;
		}	
	  } catch (Exception e ) {
  ...
  { 	  
    ...
    { 
      ...
      if (aEntry.getIXEntry() instanceof IXLinkEntry ) {
		 	int linkType = ((IXLinkEntry) aEntry.getIXEntry()).getLinkType(); 
		    if (linkType == exportedAPI.OpcatConstants.AGENT_LINK ) {
		    	//AnimationGridAddRow(AnimationAuxiliary.ActionActivate,aEntry, AnimationAuxiliary.ActionResultUnknown, "Activate Manualy") ;     	  
		    }
      }
      ...
      if (!hasTerminations) hasTerminations = termEvents.hasMoreElements();
      for(; termEvents.hasMoreElements();)  {   	  
        ...
        if ((currentEvent.getCancelStep() == 0) &&
          ...
          //AnimationGridAddRow(AnimationAuxiliary.ActionStartingTask,aEntry, AnimationAuxiliary.ActionResultUnknown, "Trying to deactivate " + aEntry.getIXEntry().getName()) ; 
          deactivate(aEntry, currentEvent);
          //AnimationGridAddRow(AnimationAuxiliary.ActionFinishedTask,aEntry, AnimationAuxiliary.ActionResultUnknown, "Finished deactivate of " + aEntry.getIXEntry().getName()) ;          
          
        } else {
        	//check event if need to be printed in the log 
      	  if (currentEvent.getCancelStep() != 0) {
    		  //event cancled 
    		  aEntry.setTaxtualInfo("Event Cancled by Time Limits in Step - " + currentEvent.getCancelStep()) ; 
    	  }        	
      	  animationChecking(currentEvent,aEntry , AnimationAuxiliary.ActionTerminate ) ;
        }
      }
      // get scheduled activations
      Enumeration actvEvents = aEntry.getScheduledActivations(currentStep);
      if (!hasActivation) hasActivation = actvEvents.hasMoreElements() ; 
      for(; actvEvents.hasMoreElements();)  {
        AnimationEvent currentEvent = (AnimationEvent)actvEvents.nextElement();
//        
//        if (currentEvent.getParentProcess() != null && AnimationAuxiliary.isThing(currentEvent.getParentProcess().getIXEntry())) {
//        	try {
//        		String url = ((IXThingEntry) currentEvent.getParentProcess().getIXEntry()).getUrl() ; 
//        		if ((url != null || !url.equalsIgnoreCase("none")) && (url.endsWith("jpg") || url.endsWith("bmp") || url.endsWith("jpeg")))
...
//        }
          ...
          if ((currentEvent.getCancelStep() == 0) &&(currentEvent.getNumberOfInstances() > 0)&&
            ...
            activate(aEntry, currentEvent);            
        	  ...
        	  if (currentEvent.getNumberOfInstances() == 0) {
        		//zero instances. 
        		  if (AnimationAuxiliary.isProcess(aEntry.getIXEntry())) {
  				  	if(aEntry.getIXEntry().getId() != mySys.getCurrentIXOpd().getMainIXEntry().getId()) {
  				  		aEntry.setTaxtualInfo("Inconsistent Model, Parent Process not consistent with upper levels ?");
  				  	} else {
  				  		aEntry.setTaxtualInfo("Inconsistent Model, Process not consistent with upper levels ?") ;
  				  	}
        		  } else {
        			  aEntry.setTaxtualInfo("Inconsistent Model or Problem with Conditions") ;
        		  }
        	  }
            ...
            if (currentEvent.getRecurringInd()) {
				...
				aEntry.setTaxtualInfo(aEntry.getTaxtualInfo() + ", Will try again in step - " + (currentStep +  1) ) ;  				
            }  
          	animationChecking(currentEvent,aEntry , AnimationAuxiliary.ActionActivate ) ; 
          }
        } // end synchronized
      }
    }
    if(!hasActivation && !hasTerminations) {  
	    en = animationElements.elements();
	    for(; en.hasMoreElements();)
	    { 
	    	aEntry = (AnimationEntry)en.nextElement(); 
			if (AnimationAuxiliary.isProcess(aEntry.getIXEntry())) {
				  	if(aEntry.getIXEntry().getId() != mySys.getCurrentIXOpd().getMainIXEntry().getId()) {
				  		//aEntry.setTaxtualInfo("Inconsistent Model, Parent Process does nothing Or not consistent with upper levels ?");
				  	} else {
				  		aEntry.setTaxtualInfo("Inconsistent Model, Process does nothing Or Process not consistent with upper levels ?");
				  		AnimationGridAddRow (AnimationAuxiliary.ActionGlobel, aEntry, null) ;
				  	}
			}
	    }
    }    
  ...
  private boolean checkBeforeActivate(AnimationEntry aEntry, AnimationEvent aEvent) {
    ...
    if (AnimationAuxiliary.isLink(aEntry.getIXEntry())) { 
      return  checkLinkBeforeActivate(aEntry, (IXLinkEntry) aEntry.getIXEntry() ); 
    }
    return false;
  }
  ...
  private boolean checkProcessBeforeActivate(AnimationEntry aProcess, AnimationEvent aEvent)  {
    ...
    if(enParents.hasMoreElements())  {
      ...
      if (!aProcess.isEventLinkActivated() &&  !aParent.isActive())  {
    	  aProcess.setTaxtualInfo("Parent, " + aParent.getIXEntry().getName() + ", Is still Active"); 
    	  return false;
      }
...
//       	  AnimationGridAddRow(AnimationAuxiliary.ActionActivate, aProcess, false, "REASON TBD");  
          aProcess.setTaxtualInfo("Could not find scheduled activation") ; 
          return false;
        }
    ...
    if (aEvent != null) {
      if (aEvent.getEventLink() != null)  {
        ...
        if (!triggerEvent.getPath().equals("")) {
        	...
        	aProcess.setTaxtualInfo("Not valid Path ("+ triggerEvent.getPath() + ") of " + triggerEvent.getName()) ; 
            return false;
        }
      }
    }
...
//      AnimationGridAddRow(AnimationAuxiliary.ActionActivate, aProcess, retVal,iLinkEntry.getName() + " - " +  str);
      ...
      if (retVal)  {
        ...
        if (iLinkEntry.getPath().equals("")) {
          ...
          aProcess.setTaxtualInfo("Problem with link From ( " + mySys.getIXSystemStructure().getIXEntry(iLinkEntry.getSourceId()).getName()  + " )"); 
          return false ;
        }
...
//      AnimationGridAddRow(AnimationAuxiliary.ActionActivate, aProcess, retVal,iLinkEntry.getName() + " - " +  str);
  ...
  private boolean checkLinkBeforeActivate( AnimationEntry aEntry ,  IXLinkEntry iLinkEntry )  {  
    int linkType = iLinkEntry.getLinkType();  // link Type
    AnimationEntry aSrc= findEntryById(iLinkEntry.getSourceId()); // link source
    AnimationAuxiliary.debug(this, "checkLinkBeforeActivate, Src IS " + aSrc.getIXEntry().getName()) ; 
    ...
    if ((linkType == exportedAPI.OpcatConstants.AGENT_LINK) ||
        ...
        if (!aSrc.isActive() ) {
          ...
          if (aEntry != null ) aEntry.setTaxtualInfo("Source is not Active" ); 
          return false;
        }
    }
    ...
    if ((aSrc.getPath().equals(""))  ||
      ...
      if (aEntry != null ) aEntry.setTaxtualInfo("Source Path is not empty");       
      return false;
    }
    if (aEntry != null ) aEntry.setTaxtualInfo("");     
    return true;
  }
      ...
      if (AnimationAuxiliary.isProcess(aProcess.getIXEntry())) {
        if (activatedEvent.getDestinationId() == aProcess.getIXEntry().getId())  {
          ...
          try {
            ...
            if (currentEvent.getParentProcess() != null && currentEvent.getParentProcess().equals(aProcess)) {
        	  ...
        	  APILogger.Print(this, "currentEvent.getParentProcess().equals(aProcess)") ; 
          }
...
//              if (currentEvent.getExecutionId() == 0) {
            ...
            // store the link that tries to activate the process
            currentEvent.setEventLink(aEventLink);
            //currentEvent.setNumberOfInstances(currentEvent.getNumberOfInstances() + 1 ) ; 
  ...
  private void treatProcessActivationConsequences(AnimationEntry aProcess, AnimationEvent currentEvent) {
	  ...
	  //debug.Print(this, "Starting treatProcessActivationConsequences for " + aProcess.getIXEntry().getName(), "KEY") ;
    ...
    // termination of the last child
	  //debug.Print(this, aProcess.getIXEntry().getName() + " Is LowLevel - " + aProcess.isLowLevel(), "KEY") ;    
  ...
  private void treatObjectActivationConsequences(AnimationEntry aObject) {
    ...
    if ( !(hasElements)) {
    	AnimationAuxiliary.debug("Object has no instances " + activatedObject.getName());    	
    	return ;
  ...
  private boolean checkBeforeTerminate(AnimationEntry aEntry, AnimationEvent aEvent) {
    ...
    if (AnimationAuxiliary.isProcess(aEntry.getIXEntry())) {
      ...
      retVal = checkProcessBeforeTerminate(aEntry, aEvent);  
      return retVal;
    }
    return true;
  }
    ...
    for(; children.hasMoreElements();) {
      ...
      if (aChildEntry.isActive(aEvent.getExecutionId()) ||
    		  aChildEntry.futureActivationScheduled(currentStep, aEvent.getExecutionId())) {
    	  	  aProcess.setTaxtualInfo("Child - " + aChildEntry.getIXEntry().getName() + " Is/Will Active") ;  
    	  	  return false;
      }
    }
  ...
  private boolean findUnavailableCondition(AnimationEntry process)  {
    ...
    for(; en.hasMoreElements();)  {
      ...
      String str = null; 
      boolean retVal = checkLinkBeforeActivate(process, iLinkEntry );
//      AnimationGridAddRow(AnimationAuxiliary.ActionActivate, process, retVal,iLinkEntry.getName() + " - " +  str);
  ...
  private boolean checkPath(String path, Enumeration en)  {
    ...
    /*
      ...
      if (tempPath.equals("") || tempPath.equals(path))  {
          ...
          String str = null; 
          boolean retVal = checkLinkBeforeActivate(null, tempLink );
//          AnimationGridAddRow(AnimationAuxiliary.ActionActivate, tempLink, retVal,tempLink.getName() + "(Check Path) - " +  str);  
        if (!retVal) {
  ...
  private class DeactivateConsumedEntriesTask implements Task {
    ...
    public boolean validate(IXEntry iEntry, Enumeration enAllEntries ) {
      ...
      IXLinkEntry link = (IXLinkEntry) iEntry;
      boolean retVal = checkLinkBeforeActivate(aProcess, link);
//      AnimationGridAddRow(AnimationAuxiliary.ActionActivate, link, retVal,link.getName() + "(Validate) - " +  str);
      if (retVal)  {
      ...
      AnimationAuxiliary.debug("Running ActivateResultsTask");
       if ((aProcess.getPath().equals(""))  ||
        ...
        if (AnimationAuxiliary.isEvent(link))  {
          ...
          entry = findEntryById(link.getDestinationId()) ; 
          activate(entry);
        }
      }
    }
  ...
  private AnimationGridPanel InitAnimationGridPanel () {
		...
		locPanel =  new AnimationGridPanel(cols.size(), cols, mySys);
		locPanel.doLayout(); 
		return locPanel ; 
}

//  synchronized private void AnimationGridAddRow(String action, AnimationEntry  aEntry,boolean actionResult , String reason ) {
//	  //if(aEntry.currentEvents.size() == 0 ) _AnimationGridAddRow(action, aEntry.getIXEntry() ,actionResult , "Animation Events is null" ) ;
//	  if(reason == null ) reason = ""; 
//	  _AnimationGridAddRow(action, aEntry.getIXEntry() ,actionResult , reason ) ; 
//  }
  
//  synchronized private void AnimationGridAddRow(String action, IXEntry entry ,boolean actionResult , String reason ) {
//	  _AnimationGridAddRow(action, entry,actionResult , reason ) ; 
//}
  
  synchronized private void AnimationGridAddRow(String action, AnimationEntry aEntry,AnimationEvent aEvent ) {
	  ...
	  for (;insEnum.hasMoreElements();) {		  
	  		...
	  		Object[] row = new Object[animationPanel.GetGridData().getColumnCount()];
	  		
		  	ins= (IXInstance) insEnum.nextElement() ; 
		  	key = ins.getKey();
		  	if (mySys.getCurrentIXOpd().getOpdId() == key.getOpdId()) {
				String type = "None" ; 					
				if (AnimationAuxiliary.isProcess(entry)) type = "Process" ;
				if (AnimationAuxiliary.isObject(entry)) type = "Object" ;
				if (AnimationAuxiliary.isState(entry)) type = "State" ;
				if(entry instanceof IXLinkEntry) {
				 	int linkType = ((IXLinkEntry) entry).getLinkType(); 
				    if (linkType == exportedAPI.OpcatConstants.AGENT_LINK) type = "Agent Link" ; 
				    if (linkType == exportedAPI.OpcatConstants.CONDITION_LINK) type = "Condition Link" ; 
				    if     (linkType == exportedAPI.OpcatConstants.CONSUMPTION_EVENT_LINK) type = "Event Link";  
				    if     (linkType == exportedAPI.OpcatConstants.CONSUMPTION_LINK) type = "Consumption Link";  
				    if     (linkType == exportedAPI.OpcatConstants.EFFECT_LINK) type = "Effect Link" ; 
				    if     (linkType == exportedAPI.OpcatConstants.INSTRUMENT_EVENT_LINK) type = "Instrument Event Link" ; 
				    if     (linkType == exportedAPI.OpcatConstants.INSTRUMENT_LINK) type = "Instrument Link" ; 
				}
				row[0] = new Long(currentStep) ; 
			  	row[1] = type;	
			  	row[2] = mySys.getIXSystemStructure().getIXOpd(key.getOpdId()).getName(); 
			  	row[3] = entry.getName(); 
			  	row[4] = action ; 
			  	if (aEvent != null ) {
				  	if (aEvent.getParentProcess() == null ) {
					  	...
					  	row[5] = aEvent.getParentProcess().getIXEntry().getName();
				  	}
			  	} else {
			  		row[5] = "null" ; 
			  	}
			  	...
			  	int numOfInsertedRow = animationPanel.GetGridData().addRow(row, tag) ;
			  	animationPanel.GetGridData().Update(numOfInsertedRow,numOfInsertedRow) ; 
		  	}
	  }
}
