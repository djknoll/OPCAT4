...
public class NodeOrthogonalToLineSpring extends SingleNodeSpring
{
    ...
    public String toString()
    {
        return MessageFormat.format( "[{0} - {1} by constraint {2}]",
                                     new Object[] {m_nodeConnection.m_node, m_line, m_constraint});
    }
    ...
    protected Vector2D InternalGetForce(NodeContext node)
    {
        ...
        //
        double nodeX = m_nodeConnection.m_node.GetCenterX() + m_nodeConnection.m_xOffset;
        double nodeY = m_nodeConnection.m_node.GetCenterY() + m_nodeConnection.m_yOffset;

        //
        // Calculate the spring actual length.
        //
        double springActualLength = m_line.ptLineDist(nodeX, nodeY);
        ...
        //
        double forceModule = Math.abs((springActualLength - m_springRestLength) * GetSpringConstant());
        ...
        // Get the line angle.
        //
        double alpha = Math.atan((m_line.getY2() - m_line.getY1()) / (m_line.getX2() - m_line.getX1()));
        if (!Util.IsValidDouble(springActualLength))
        {
            return new Vector2D();
        }
        ...
        //
        int relativePosition = m_line.relativeCCW(nodeX, nodeY);
        Debug.Print( 25,
                     "Relative position for line ({0},{1})-({2},{3}) and point ({4},{5}) is {6}.",
                     new Object[] {new Double(m_line.x1),
                                   new Double(m_line.y1),
                                   new Double(m_line.x2),
                                   new Double(m_line.y2),
                                   new Double(nodeX),
                                   new Double(nodeY),
        ...
        if (springActualLength > m_springRestLength)
        {
            //
