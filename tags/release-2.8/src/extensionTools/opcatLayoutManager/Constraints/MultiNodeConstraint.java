...
public abstract class MultiNodeConstraint extends Constraint
{
    ...
    protected MultiNodeConstraint(int direction, NodeContext mainNode, Iterator nodes, Comparator comparator)
    {
        ...
        if (null != mainNode)
        {
            m_mainNodeConnection = new SpringConnectionPoint(mainNode, false);
        }
    ...
    public void Show()
    {
        ...
        {
            m_mainNodeConnection.m_node.Select(true);
        }
    ...
    public void Hide()
    {
        if (null != m_mainNodeConnection)
        {
            m_mainNodeConnection.m_node.Select(false);
        }
    ...
    {
        AddNode(new SpringConnectionPoint(node, true));
    }
    ...
    {
        if (null != m_mainNodeConnection && m_mainNodeConnection.m_node.equals(nodeConnection.m_node))
        {
            return;
        }
        ...
        if (null != m_comparator)
        {
            ...
            while(iterNodes.hasNext())
            {
                ...
                if (m_comparator.compare(nodeConnection.m_node, currentNode) <= 0)
                {
                    iterNodes.previous();
                    break;
                }
    ...
    protected void AddNodes(Iterator nodes)
    {
        if (null != nodes)
        {
            while (nodes.hasNext())
            {
                ...
                {
                    AddNode((NodeContext)element);
                }
                else if (element instanceof SpringConnectionPoint)
                {
                    AddNode((SpringConnectionPoint)element);
                }
            }
        }
    }
    ...
    {
        SpringConnectionPoint nodeConnection = GetNodeConnection(node);
        if (null != nodeConnection)
        {
            m_nodes.remove(nodeConnection);
        }
    }
