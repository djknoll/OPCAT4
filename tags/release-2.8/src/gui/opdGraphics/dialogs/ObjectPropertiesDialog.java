	...
	*/
	...
	public ObjectPropertiesDialog(
		...
		if ((showTabs & BaseGraphicComponent.SHOW_7) != 0) {
			tabs.add("Details", _constructDescriptionTab());
		}
	...
	private void _initVariables() {
		try {
			if ((showTabs & BaseGraphicComponent.SHOW_1) != 0) {
				...
				additionHelpON = new JRadioButton("Enable");
				additionHelpON.addActionListener(positionAction); 
				additionHelpOff = new JRadioButton("Disable"); 
				additionHelpOff.addActionListener(positionAction); 
				addBg = new ButtonGroup() ; 
				addBg.add(additionHelpOff) ; 
				addBg.add(additionHelpON) ; 
				additionHelpON.setSelected(true) ; 
				addHelpON = true ; 
			}
			...
			if ((showTabs & BaseGraphicComponent.SHOW_2) != 0) {
				...
				//description
				generaldescription = new JTextArea(object.getDescription(), 8, 30);
	...
	private JPanel _constructGeneralTab() {
		...
		p1 = new JPanel();
		p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "   Addition Helper   "));
		p1.setLayout(new GridLayout(2,1)); 
		p1.add(additionHelpON);
		p1.add(additionHelpOff);	
	...
	private boolean _updateObjectData() {
		...
		if ((showTabs & BaseGraphicComponent.SHOW_1) != 0) {
			...
			if (!object.equals(generaldescription.getText())) {
				object.setDescription(
					GraphicsUtils.capitalizeFirstLetters(
						generaldescription.getText().trim()));
			}
	...
	Action positionAction = new AbstractAction() {
		public void actionPerformed(ActionEvent e) {
			...
			addHelpON = additionHelpON.isSelected() ; 			
			(ObjectPropertiesDialog.this).sampleObject.repaint();
