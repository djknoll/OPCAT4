...
{
	private static long
		timeValues[] = {1,1000, 1000*60, 1000*60*60, 1000*60*60*24,
						1000*60*60*24*30, 1000*60*60*24*30*365};
...
//    public static RelativeConnectionPoint calculateConnectionPoint(OpdConnectionEdge mainItem, OpdConnectionEdge secondItem)
	public static RelativeConnectionPoint calculateConnectionPoint(Connectable mainItem, Connectable secondItem)
	{

		Point sItemCenter;

		Point tmpPoint = new Point(secondItem.getX(), secondItem.getY());
		tmpPoint = SwingUtilities.convertPoint(secondItem.getParent(), tmpPoint, mainItem.getParent());
		sItemCenter = new Point((int)((tmpPoint.getX()+(double)secondItem.getActualWidth()/2)-(mainItem.getX()+(double)mainItem.getActualWidth()/2)),
								  (int)((mainItem.getY()+(double)mainItem.getActualHeight()/2)-(tmpPoint.getY()+(double)secondItem.getActualHeight()/2)));
		...
		if (mainItem instanceof OpdProcess)
		{
			...
			{
			  return new RelativeConnectionPoint(OpcatConstants.N_BORDER, (x+a)/(double)mainItem.getActualWidth());
			}

			if (sItemCenter.getY()>=0 && sItemCenter.getX()<=0)
			{
			  return new RelativeConnectionPoint(OpcatConstants.N_BORDER, (a-x)/(double)mainItem.getActualWidth());
			}

			if (sItemCenter.getY()<=0 && sItemCenter.getX()>=0)
			{
			  return new RelativeConnectionPoint(OpcatConstants.S_BORDER, (x+a)/(double)mainItem.getActualWidth());
			}

			if (sItemCenter.getY()<=0 && sItemCenter.getX()<=0)
			{
			  return new RelativeConnectionPoint(OpcatConstants.S_BORDER, (a-x)/(double)mainItem.getActualWidth());
			}
			...
			if (sItemCenter.getY()>=0 && sItemCenter.getX()>=0)
			{
			  if (x1<=a)
			  {
				return new RelativeConnectionPoint(OpcatConstants.N_BORDER, (x1+a)/(2*a));
			  }
			  else
			  {
				return new RelativeConnectionPoint(OpcatConstants.E_BORDER, (b-y1)/(2*b));
			  }
			}
			...
			if (sItemCenter.getY()>=0 && sItemCenter.getX()<=0)
			{
			  if (x1<=a)
			  {
				return new RelativeConnectionPoint(OpcatConstants.N_BORDER, (a-x1)/(2*a));
			  }
			  else
			  {
				return new RelativeConnectionPoint(OpcatConstants.W_BORDER, (b-y1)/(2*b));
			  }
			}
			...
			if (sItemCenter.getY()<=0 && sItemCenter.getX()>=0)
			{
			  if (x1<=a)
			  {
				return new RelativeConnectionPoint(OpcatConstants.S_BORDER, (x1+a)/(2*a));
			  }
			  else
			  {
				return new RelativeConnectionPoint(OpcatConstants.E_BORDER, (b+y1)/(2*b));
			...
			if (sItemCenter.getY()<=0 && sItemCenter.getX()<=0)
			{
			  if (x1<=a)
			  {
				return new RelativeConnectionPoint(OpcatConstants.S_BORDER, (a-x1)/(2*a));
			  }
			  else
			  {
				return new RelativeConnectionPoint(OpcatConstants.W_BORDER, (b+y1)/(2*b));
	...
	public static String capitalizeFirstLetters(String str)
	{
		...
		while (st.hasMoreTokens())
		{
			...
			result += tString.substring(0,1).toUpperCase() + tString.substring(1);
		}
	...
	public static String convertCardinality2GrRepr(String s)
	{
		...
		if (str.length() == 4 && str.startsWith("0..") && Character.isLetter(str.charAt(3)))
		{
			return "*";
		}

		if (str.length() == 4 && str.startsWith("1..") && Character.isLetter(str.charAt(3)))
		{
			return "+";
		}
...
//		return false;
	...
	public static OpdConnectionEdge findAdjacentComponent(Container cn, int x, int y)
	{
		...
		for (int i = 0; i<components.length; i++)
		{
			if (components[i] instanceof OpdConnectionEdge)
			{
				...
				retEntity = findAdjacentComponent(temp, (int)convPoint.getX(), (int)convPoint.getY());
				...
				//System.err.println(temp + " "+(int)convPoint.getX()+" "+ (int)convPoint.getY());

				if (temp.isInAdjacentArea((int)convPoint.getX(), (int)convPoint.getY()))
				{
					return temp;
				}
			}
	...
	public static Point findPlace4Menu(Component cm, Point clickPlace, Dimension menuSize)
	{
		...
		if (onScreenPoint.getY()+menuSize.getHeight() > screenSize.getHeight())
		{
