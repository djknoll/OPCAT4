...
//    }
    ...
    public void refit() {
    	...
    	ThingInstance mainInstance = getMainInstance() ; 
    	ThingEntry mainThing = getMainEntry() ;
    	...
    	if ((mainThing != null) && (mainThing instanceof ProcessEntry) ) {     		 
    		...
    		_refit(thingsEnum , dArea); 
    	}
    	
    	//now refit the main frame.
    	removeSelection() ;
    	selectAll() ; 
    	thingsEnum = myProject.getSystemStructure().getInstancesInOpd(this.getOpdId());
    	_refit(thingsEnum, getDrawingArea());
    	removeSelection() ; 
    }
    ...
    private void _refit(Enumeration thingsEnum , Container container ) {
    	...
    	while(thingsEnum.hasMoreElements()) {
    		...
    		if(obj instanceof ThingInstance) {
    			...
    			if (ins.getX() + ins.getWidth()> maxX ) maxX = ins.getX() + ins.getWidth() ; 
    			if (ins.getY() + ins.getHeight() > maxY ) maxY = ins.getY() + ins.getHeight() ;
    		}
    		if (obj instanceof LinkInstance) {
    			LinkInstance ins = (LinkInstance) obj ;  
    			if (ins.getDestination().getX() + ins.getDestination().getWidth() > maxX ) maxX = ins.getDestination().getX() + ins.getDestination().getWidth() ; 
    			if (ins.getSource().getX() + ins.getSource().getWidth() > maxX ) maxX = ins.getSource().getX() + ins.getSource().getWidth() ;
    			if (ins.getDestination().getY() + ins.getDestination().getHeight()  > maxY ) maxY = ins.getDestination().getY() + ins.getDestination().getHeight() ;    			    			
    			if ( ins.getSource().getY() + ins.getSource().getHeight() > maxY ) maxY = ins.getSource().getY() + ins.getSource().getHeight() ;
    			if(obj instanceof RelationEntry) {
    				if(ins.getDestinationDragger().getX() + ins.getDestinationDragger().getWidth() > maxX ) maxX = ins.getDestinationDragger().getX() + ins.getDestinationDragger().getWidth();  
    				if(ins.getDestinationDragger().getY() + ins.getDestinationDragger().getHeight() > maxY ) maxY = ins.getDestinationDragger().getY() + ins.getDestinationDragger().getHeight()  ;
    			}
    		}
    	}
    	...
    	if (change) { 
    		...
    		if (container instanceof OpdConnectionEdge) {
    			((OpdConnectionEdge) container).setSize((int) (maxX * 1.3) , (int) (maxY * 1.3) ) ; 
    		}
    		container.repaint() ;
    	}    	
    }
