...
public class OpdProject implements ISystem, IXSystem {
	...
	public OpdProject(JDesktopPane parent, long pId) {
		...
		clipBoard = new Opd(this, "ClipBoard", CLIPBOARD_ID, null, false);
	...
	public IXLinkInstance getIXLinkBetweenInstances(
		...
		return  getLinkBetweenInstances(
	...
	public IXObjectInstance addObject(String name, int x, int y, long opdId) {
		...
		IXObjectInstance newIns =  addObject(x, y, tempOpd
	...
	public IXObjectInstance addObject(String name, int x, int y,
		...
		IXObjectInstance newIns =  addObject(x, y,
	...
	public IXProcessInstance addProcess(String name, int x, int y, long opdId) {
		...
		IXProcessInstance newIns =  addProcess(x, y, tempOpd
				.getDrawingArea(), -1, -1);
		newIns.getIXEntry().setName(name);
	...
	public IXProcessInstance addProcess(String name, int x, int y,
		...
		ThingInstance tempInstance = (ThingInstance) tempEntry
				.getInstance(parentInstance.getKey());
		IXProcessInstance newIns =  addProcess(x, y,
	...
	public IXLinkInstance addLink(IXConnectionEdgeInstance source,
		...
		return  addLink(sourceInstance.getConnectionEdge(),
	...
	public IXStateEntry addState(String name, IXObjectInstance parentObject) {
		...
		IXStateEntry newEntry =  addState((OpdObject) parentInstance
				.getThing());
	...
	public void paste(int x, int y, Container parent) {
		...
		if (!edit.IsCutPending()) {
			...
			if (doPaste) {
			    ...
			    Enumeration locEnum = clipBoard.getSelectedItems() ; 
	...
	public void delete() {
		 ...
		 */ 
		Enumeration deleteFromEnum = getCurrentOpd().getSelectedItems();
		if (!deleteFromEnum.hasMoreElements()) return; 
		...
		if (ask) {
		   	Object[] possibilities = {"Selected", "All", "Step by Step", "Consistency Checker"};
...
//    	If a string was returned, say so.
    	...
    	if ((s != null) && (s.length() > 0)) {
    		if (s.equalsIgnoreCase("Selected")) message = _delete(getCurrentOpd(), DELETE);
    		if (s.equalsIgnoreCase("All")) message = _deleteALL(getCurrentOpd(), DELETE, false);
    		if (s.equalsIgnoreCase("Step by Step")) message = _deleteALL(getCurrentOpd(), DELETE, true);
    		if (s.equalsIgnoreCase("Consistency Checker")) {
    			ConsistencyResult checkResult = new ConsistencyResult(); 
    			ConsistencyAbstractChecker checker = (ConsistencyAbstractChecker) (new ConsistencyFactory(this, ConsistencyAction.DELETION, checkResult)).create() ;
    			checker.check() ; 
    			if (checkResult.isConsistent()) {
    				message = _delete(getCurrentOpd(), DELETE); 
    			} else {
	...
	/**
	 * Delete instances from the Project. if a thing can not be deleted a String is 
	 * returned which holds the Error Message. If the deletion was succesfull then a null is
	 * returned. 
	 * 
	...
	private String _delete (Enumeration thingsToDelete , int action ) {
		...
		for (Enumeration e4 = selLinksRelationsInst.elements(); e4.hasMoreElements();) {
			undoAction.addAction(deleteInstance((Instance) e4.nextElement()));
		}
		if (action == CUT || action == DELETE) {
			Opcat2.getUndoManager().undoableEditHappened(
					new UndoableEditEvent(this, undoAction));
	...
	private String _deleteALL(Opd deletingFrom, int action, boolean stepBystep ) {
		...
		if (!deleteFromEnum.hasMoreElements()) return null ; 
		...
		if (!stepBystep) {
			retVal = _delete (vec.elements() ,action ) ;
		} else {
	    	...
	    	while (locEnum.hasMoreElements() && !cancel) {
	    		...
	    		showOPD(locIns.getKey().getOpdId()) ; 
    			...
    			removeSelection(locIns);  		    	
		    	switch(n) {
		    		case JOptionPane.OK_OPTION :
		    			Vector v = new Vector() ; 
		    			v.add(locIns); 
		    			retVal = _delete (v.elements() ,action ) ;
		    			break ; 
		...
		for (Enumeration e = getComponentsStructure().getThingsInOpd(childOpd); e
			...
			if (!isState) {
				...
				for (Enumeration e1 = oi.getStateInstances(); e1
					...
					if (currSi.getEntry().getId() == destination.getEntity().getId()) {
						currTi = currSi;
						destExist = true;
						break;
					}
	...
	public ThingInstance getThingInstanceByName(String name) {
		...
		while (e.hasMoreElements()) {
			...
			if (ins instanceof ThingInstance) {
				...
				if (returnThing.getEntry().getName().equals(name)) return returnThing;
				returnThing = null;
			}
		}
		return returnThing;
	...
	public boolean deleteOpd(Opd opdToDelete, boolean automaticDelete) {
		...
		Opd tempOpd;
		for (locEnum = mainComponentsStructure.getOpds(); locEnum.hasMoreElements();) {
		...
		// 1 delete all links
		for (locEnum = mainComponentsStructure.getLinksInOpd(opdToDelete); locEnum
				.hasMoreElements();) {
			deleteLink((LinkInstance) locEnum.nextElement());
		}
		// 2 delete all general relations
		for (locEnum = mainComponentsStructure
				.getGeneralRelationsInOpd(opdToDelete); locEnum.hasMoreElements();) {
			deleteGeneralRelation((GeneralRelationInstance) locEnum.nextElement());
		}
		...
		ThingInstance ti;
		for (locEnum = mainComponentsStructure.getThingsInOpd(opdToDelete); locEnum.hasMoreElements();) {
	...
	public ProcessInstance addProcess(int pX, int pY, Container container,
		...
		/**
		 * TODO: when adding a new process check consistency for it
		 * consistencey checks: 
		 * 1) search for new process name
		 * 
		 * do this if we are not in reuse ? if i do this in reuse then 
		 * the user could choose not to add existing things.  
		 */	
	...
	/**
	 * Used by the MouseRelesed action in <code> OPDBaseGraphics </code> 
	 * to add links to the project. If there is a posibility to add more then one link. the user
	 * will be asked. more then one link = more instances of the source and dest in the same OPD in the 
	 * project.
	 * 
	 * @param source
	 * @param pPoint1
	...
	public void  addLinks(OpdConnectionEdge source,
		...
		li = addLink/*addLinkConsistent*/(
				source, pPoint1, destination, pPoint2,
				container, StateMachine.getDesiredComponent(),
				-1, -1);
		...
		//isSourceMainEntityinOPD = (this.getOpdByID(sourceID).getMainEntry().getId() == sourceID);
		Enumeration sourceEnum = li.getSourceInstance().getEntry().getInstances() ;
		...
		while(sourceEnum.hasMoreElements() && !foundMoreOPD) {
			...
			//debug.Print(locSourceInstance, locSourceInstance.getKey().toString(),"1") ;			
			...
			while (destEnum.hasMoreElements() && !foundMoreOPD) {
				...
				//debug.Print(locDestInstance, "dest - " + locDestInstance.getKey().toString(),"1") ; 				
				long destID = locDestInstance.getKey().getOpdId() ;
				foundMoreOPD = !(addedDest.toString().equalsIgnoreCase(locDestInstance.getKey().toString()));
				foundMoreOPD = foundMoreOPD && !(addedSource.toString().equalsIgnoreCase(locSourceInstance.getKey().toString())); 
				foundMoreOPD =  foundMoreOPD &&  (destID == sourceID) && (destID != currentOpd.getOpdId());
			}
		}
		...
		if (foundMoreOPD){
    		...
    		boolean ask = false  ; 
    		if (s == null ) s = "" ; 
	    	if(s.equalsIgnoreCase("Step by Step")) ask = true ; 
	    	if(s.equalsIgnoreCase("Step by Step") || s.equalsIgnoreCase("All")) { 
				...
				while(sourceEnum.hasMoreElements() && !cancel) {
					...
					while (destEnum.hasMoreElements() && !cancel) {
						...
						if (currentOpd.isContaining(locSourceInstance) && 
							currentOpd.isContaining(locDestInstance) && 
							!currentOpd.isConnected(locSourceInstance, locDestInstance, li)) {
							...
							switch(n) {
								case JOptionPane.OK_OPTION :
										li = this.addLink/*addLinkConsistent*/(
												locSourceConnectionEdge 
												, new RelativeConnectionPoint (locSourceConnectionEdge.getX(), locSourceConnectionEdge.getY())
												, locDestConnectionEdge
												, new RelativeConnectionPoint (locDestConnectionEdge.getX(), locDestConnectionEdge.getY()),
