...
public class MetaManager implements Cloneable {
	...
	public void refresh(OpdProject myProject, JProgressBar progressBar) {
		_refresh(myProject, progressBar, false) ; 
	}
	
	public void refresh(OpdProject myProject, JProgressBar progressBar, boolean forceLoading) {
		_refresh(myProject, progressBar, forceLoading) ; 
	}	
	
	private void _refresh(OpdProject myProject, JProgressBar progressBar, boolean forceLoading) {
		...
		try {
			...
			this.loadLibraries(forceLoading);
	 ...
	 */
	private void loadLibraries(boolean forceLoading) {
		if (this.metaLibraries == null) {
			return;
		}
		MetaLibrary metaLib;
		for (int i = 0; i < this.metaLibraries.size(); i++) {
			...
			if (metaLib.getType() == Types.MetaLibrary) {
				this.loadOPMLibraries(metaLib, forceLoading);
			}
			if (metaLib.getType() == Types.RequirementsData) {
	...
	private void loadREQLibraries(MetaLibrary metaLib) {
		...
		if (metaLib.getState() == MetaLibrary.STATE_REFERENCE) {
			...
			try {
				...
				ReqProject repProject = new ReqProject(is, metaLib
						.getPath());		
	...
	private void loadOPMLibraries(MetaLibrary metaLib, boolean forceLoading) {
		...
		String fileName = metaLib.getPath();
		if ((metaLib.getState() == MetaLibrary.STATE_REFERENCE) || (forceLoading) ) {
			InputStream is = null;
			// Handling meta libraries which are on the local file systems
			if (metaLib.getReferenceType() == MetaLibrary.TYPE_FILE) {
				File f = new File(fileName);
	...
	public void loadDummyReqProject() {
		ReqProject repProject = new ReqProject("demo");
		repProject.loaddummy();
		MetaLibrary metaLib = new MetaLibrary(-999, "1", 1, "666");
		repProject.setName("test Reqs");
		metaLib.setPath("demo");
		metaLib.setType(Types.RequirementsData);
		metaLib.setModelProject(repProject);

		try {
			this.addMetaLibrary(metaLib);
		} catch (Exception e) {
