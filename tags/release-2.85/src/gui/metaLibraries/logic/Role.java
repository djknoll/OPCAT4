...
public class Role implements IXRole, IRole {
	...
	public void load(MetaLibrary onto) throws MetaException,
		...
		if (onto != null) {
			if ((onto.getType() == Types.MetaLibrary)) {
				...
				ThingEntry entry = (ThingEntry) metaStruct.getIEntry(this.thingID);
				if (entry != null) {
			...
			if ((onto.getType() == Types.RequirementsData)) {
				ReqProject reqProject = (ReqProject) onto.getProjectHolder() ; 
				Req req = reqProject.getReq(this.getThingId()) ;
				this.load (req, onto) ; 
			}
		}
	}
	...
	public IThingEntry getThingEntry() {
		ThingEntry entry = new ThingEntry((OpmThing) this.theThing.getDataInstance(), (OpdProject) this.theOntology
				.getProjectHolder());
		return entry;
	}
