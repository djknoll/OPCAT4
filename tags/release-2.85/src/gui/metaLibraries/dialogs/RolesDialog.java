...
public class RolesDialog extends JPanel {
	...
	private void build() {
		...
		this.avaliableScroll.setBorder(
	...
	private JPanel _constructMasTab() {
		...
		this.noOfInstancesText =
			new JTextField(String.valueOf(this.theThing.getNumberOfInstances()));
	...
	private void initLists() {
		this.selectedRoles = new Vector() ; 
		this.selectedRoles = ((RolesManager)this.theThing.getRolesManager().clone()).getRolesVector(Types.MetaLibrary); 
		
		this.selectedList.setModel(new RolesListModel(this.selectedRoles));
		this.selectedList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		Vector allRoles = new Vector();
		Enumeration ontologies = this.myProject.getMetaManager().getMetaLibraries(Types.MetaLibrary);
		try {
		...
		this.avaliableRoles = new Vector();
		for (int i = 0; i < allRoles.size(); i++) {
			Role role = (Role) allRoles.get(i);
			OpmThing thing = (OpmThing) (role.getThing()).getDataInstance();
	...
	private void addListeners() {
		...
		this.okButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				RolesDialog.this.selectedRoles =
					((RolesListModel) (RolesDialog.this.selectedList.getModel())).getRoles();
	...
	private void addPressed() {
		RolesListModel avaliableModel = (RolesListModel) (this.avaliableList.getModel());
		Role theRole = (Role) (this.avaliableList.getSelectedValue());
		if ((this.avaliableList.getSelectedValue() == null)
			|| (this.avaliableList.getSelectedIndex() == avaliableModel.getSize())) {
		...
		RolesListModel selectedModel = (RolesListModel) (this.selectedList.getModel());
	...
	private void removePressed() {
		...
		if ((this.selectedList.getSelectedValue() == null)
			|| (this.selectedList.getSelectedIndex() == selectedModel.getSize())) {
			JOptionPane.showMessageDialog(
				null,
