...
public class OpdView extends BaseView {
	...
	public OpdView(String invisibleRoot) {
		super(invisibleRoot);
		this.tip = new String("OPD view");
		this.viewName = "OPD View";
	...
	private void _scanOpdTreeDFS(MainStructure s, DefaultTreeModel model,
		...
		for (Iterator i = opdSorter.values().iterator(); i.hasNext();) {
			...
			this.nodesHash.put(new HashKey(parentNode.getUserObject()), parentNode);
			...
			this._scanOpdTreeDFS(s, model, ((Opd) tmpObj).getOpdId(), tmpOpdNode);
		}
