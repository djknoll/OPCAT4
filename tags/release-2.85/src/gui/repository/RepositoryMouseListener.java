...
class RepositoryMouseListener extends MouseAdapter {
		...
		if (e.isPopupTrigger() || e.isMetaDown()) {
			...
			if (node != null) {
				...
				if (obj instanceof Opd) {
					JPopupMenu jpm = new ROpdPopup(this.eventSender, this.repository
							.getProject());
					jpm.show(this.eventSender, e.getX(), e.getY());
				}
				if ((obj instanceof ThingInstance) && (eventSender.getType() != Repository.MetaVIEW))  {
					JPopupMenu jpm = new RThingPopup(this.eventSender, this.repository
							.getProject());
					jpm.show(this.eventSender, e.getX(), e.getY());
				}
				//Meta View 
				if ((obj instanceof ThingEntry) && (eventSender.getType() == Repository.MetaVIEW))  {
					JPopupMenu jpm = new RMetaThingPopup(this.eventSender, this.repository
							.getProject());
					jpm.show(this.eventSender, e.getX(), e.getY());
				}		
				
				if ((obj instanceof MetaLibrary) && (eventSender.getType() == Repository.MetaVIEW))  {
					JPopupMenu jpm = new RMetaPopup(this.eventSender, this.repository
							.getProject());
					jpm.show(this.eventSender, e.getX(), e.getY());
				}			
				//END Meta View 
				
				if ((obj instanceof OpmEntity) && (eventSender.getType() != Repository.MetaVIEW) ) {
					JPopupMenu jpm = new REntryPopup(this.eventSender, this.repository
							.getProject());
					jpm.show(this.eventSender, e.getX(), e.getY());
				}
				if (obj instanceof OpdProject) {
					JPopupMenu jpm = new RProjectPopup(this.eventSender, this.repository,
	...
	public void mouseClicked(MouseEvent e) {
		...
		TreePath selPath = this.eventSender.getPathForLocation(e.getX(), e.getY());
		if (selPath == null) {
			return;
		}
		this.eventSender.setSelectionPath(selPath);
		...
		if (e.getClickCount() == 2) {
			if (node != null) {
				...
				if (obj instanceof OpmThing) {
					ThingEntry.getEntryFromOpmThing((OpmThing) obj).ShowInstances(); 
				}
			}
		}
