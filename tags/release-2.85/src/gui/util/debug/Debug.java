...
public class Debug {
	...
	private Debug () {
		...
		if (LEVEL > 0 ) {                
            ...
            this.panel = new GridPanel(ColumnsNumber,this.ColumnsNames) ; 
            this.panel.GetColumnsModel().getColumn(0).setMaxWidth(10000); 
            this.panel.AddToExtensionToolsPanel(tabName); 			
		}
	...
	public void Print(Object obj, String Message, String Key ) {
		if (LEVEL > 0 ) {
			...
			tag[0] = Key ; 
			int numberOfInsertedRow = this.panel.getGrid().addRow(row, tag);
			this.panel.getGrid().Update(numberOfInsertedRow, numberOfInsertedRow) ; 
		}
	}
