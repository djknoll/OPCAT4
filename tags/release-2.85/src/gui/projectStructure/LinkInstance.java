...
*/
				...
				OpmProceduralLink pLink, OpdKey myKey, OpdProject project,Container container)
	{
		...
		this.line = new StyledLine(source, this.sourceDragger,
								destination, this.destinationDragger,
								this.myLink, this.getKey(), this.myProject);
	...
	private TransparentLinkDragger createTransparentDragger(Connectable edge, RelativeConnectionPoint pPoint)
	{
		...
		gDragger = new TransparentLinkDragger(edge, pPoint, this.myLink ,this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
	...
	private OpdLink createOpdLink(Connectable edge, RelativeConnectionPoint pPoint)
	{
		...
		{
			nLink = new OpdAgentLink(edge, pPoint, (OpmAgent)this.myLink ,this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmInstrument)
		{
			nLink = new OpdInstrumentLink(edge, pPoint, (OpmInstrument)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmConditionLink)
		{
			nLink = new OpdConditionLink(edge, pPoint, (OpmConditionLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmInstrumentEventLink)
		{
			nLink = new OpdInstrumentEventLink(edge, pPoint, (OpmInstrumentEventLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmResultLink)
		{
			nLink = new OpdResultLink(edge, pPoint, (OpmResultLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmConsumptionLink)
		{
			nLink = new OpdConsumptionLink(edge, pPoint, (OpmConsumptionLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}


		if (this.myLink instanceof OpmEffectLink)
		{
			nLink = new OpdEffectLink(edge, pPoint, (OpmEffectLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmExceptionLink)
		{
			nLink = new OpdExceptionLink(edge, pPoint, (OpmExceptionLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmInvocationLink)
		{
			nLink = new OpdInvocationLink(edge, pPoint, (OpmInvocationLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}

		if (this.myLink instanceof OpmConsumptionEventLink)
		{
			nLink = new OpdConsumptionEventLink(edge, pPoint, (OpmConsumptionEventLink)this.myLink , this.getKey().getOpdId(), this.getKey().getEntityInOpdId(), this.myProject);
		}
...
*/
    ...
    {
        return this.line.getRemainedAnimationTime();
    }

    public long getTotalAnimationTime()
    {
        return this.line.getTotalAnimationTime();
    }
...
*/
	...
	public void copyPropsFrom(LinkInstance origin)
	{
		...
		this.destinationDragger.setSide(origin.getDestinationDragger().getSide());
		this.destinationDragger.setParam(origin.getDestinationDragger().getParam());
	...
	public void add2Container(Container cn)
	{
			...
			if (neighbors.size()>0)
			{
				...
				if (this.destOr == null)
				{
					...
					if (this.destOr == null)
					{
						...
						this.destOr = new OrInstance(neighbors, false, this.myProject);
        ...
        public void updateSourceConnections()
        {
            ...
            if (neighbors.size()>0)
            {
                if (this.sourceOr == null)
                {
                    for (Enumeration e = neighbors.elements(); e.hasMoreElements();)
                    {
                        this.sourceOr = ((LinkInstance)e.nextElement()).getSourceOr();
                    }

                    if (this.sourceOr == null)
                    {
                        neighbors.put(this.getKey(), this);
                        this.sourceOr = new OrInstance(neighbors, true, this.myProject);
                        this.sourceOr.add2Container(this.sourceDragger.getParent());

                        for (Enumeration e = neighbors.elements(); e.hasMoreElements();)
                        {
                            ((LinkInstance)e.nextElement()).setSourceOr(this.sourceOr);
                        }
                    }
                }
                else
        ...
        {
          if (((LinkEntry)this.getEntry()).getLinkType()!=  ((LinkEntry)li.getEntry()).getLinkType())
          {
            return false;
          }
          GenericTable config = this.myProject.getConfiguration();
            ...
            if (li.getDestination() == this.destination)
            {
                ...
                RelativeConnectionPoint myPoint = this.destinationDragger.getRelativeConnectionPoint();

                if (this.isClose(this.destination.getAbsoluteConnectionPoint(nPoint),
                            this.destination.getAbsoluteConnectionPoint(myPoint), epsilon))
                {
                    return true;
                }
            }
        ...
        private boolean isSourceNeighbour(LinkInstance li)
        {
          ...
          GenericTable config = this.myProject.getConfiguration();
            ...
            if (li.getSource() == this.source)
            {
                ...
                RelativeConnectionPoint myPoint = this.sourceDragger.getRelativeConnectionPoint();
        ...
        private Hashtable findDestinationNeighbors() {
          ...
          ConnectionEdgeInstance dIns = (ConnectionEdgeInstance) this.destination.getEntry().
        ...
        private Hashtable findSourceNeighbors()
        {
            ...
            ConnectionEdgeInstance sIns = (ConnectionEdgeInstance)this.source.getEntry().getInstance(this.source.getOpdKey());
		...
		{
			return (ConnectionEdgeInstance)this.source.getEntry().getInstance(this.source.getOpdKey());
		}

		public ConnectionEdgeInstance getDestinationInstance()
		{
			return (ConnectionEdgeInstance)this.destination.getEntry().getInstance(this.destination.getOpdKey());
		}
