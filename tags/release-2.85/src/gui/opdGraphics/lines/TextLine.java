...
//        animationParametr = 0.75;
	...
	{
		this.animationParametr = parametr;
	}

	public double getAnimationParametr()
	{
		return this.animationParametr;
	}
	...
	public void mouseReleased(MouseEvent e)
	{
		...
		if (this.contains(SwingUtilities.convertPoint(this.getParent(), this.dragPoint, this)))
		{
			this.inDrag = false;
			super.mouseReleased(e);
			this.repaint();
	...
	public void mouseDragged(MouseEvent e )
	{
		...
		this.dragPoint = SwingUtilities.convertPoint(this, this.dragPoint, this.getParent());
		...
		p1.setLocation(this.edge1.getX()+p1.getX()
									,this.edge1.getY()+p1.getY());
		p1 = SwingUtilities.convertPoint(((JComponent)this.edge1).getParent(), p1, this.getParent());

		p2 = this.edge2.getAbsoluteConnectionPoint(this.cPoint2);
		p2.setLocation(this.edge2.getX()+p2.getX()
								,this.edge2.getY()+p2.getY());
		p2 = SwingUtilities.convertPoint(((JComponent)this.edge2).getParent(), p2, this.getParent());
		...
		int x = (int)(Math.min(this.dragPoint.getX(), Math.min(p1.getX(), p2.getX())) - shiftLine);
		int y = (int)(Math.min(this.dragPoint.getY(), Math.min(p1.getY(), p2.getY()))- shiftLine);

		int width = (int)(Math.max(this.dragPoint.getX(), Math.max(p1.getX(), p2.getX())) - x + shiftLine*2);
		int height = (int)(Math.max(this.dragPoint.getY(), Math.max(p1.getY(), p2.getY())) - y + shiftLine*2);
...
//        repaint();
	...
	public void paintComponent(Graphics g)
	{
		...
		int stringX;
		GenericTable config = this.myProject.getConfiguration();
		Font currentFont = (Font)this.myProject.getConfiguration().getProperty("LineFont");
		double currentSize = ((Integer)config.getProperty("CurrentSize")).doubleValue();
		double normalSize = ((Integer)config.getProperty("NormalSize")).doubleValue();
...
//        g2.setStroke(new BasicStroke(1.8f));
		...
		{
			Point tPoint = SwingUtilities.convertPoint(this.getParent(), this.dragPoint, this);
		...
		g2.rotate((angle+Math.PI/2)%Math.PI-Math.PI/120, this.getWidth()/2, this.getHeight()/2);

		if ( (angle > 0) && (angle < Math.PI/2))
		{
			g2.rotate(Math.PI-Math.PI/120, this.getWidth()/2, this.getHeight()/2);
		}
		...
		{
			stringX=(this.getWidth()-currentMetrics.stringWidth(this.upperText) - (int)((MIN_SIZE+10)*factor) )/2;
			g2.drawString(this.upperText, stringX, this.getHeight()/2-currentMetrics.getAscent()+2);
		}


		if (this.lowerText != null)
		{
			stringX=(this.getWidth()-currentMetrics.stringWidth(this.lowerText))/2;
			g2.drawString(this.lowerText, stringX, this.getHeight()/2+currentMetrics.getAscent());
		}
		...
		if (this.animated)
		{
			...
			sPoint = SwingUtilities.convertPoint((Container)this.edge1, sPoint, this);

			Point dPoint = this.edge2.getAbsoluteConnectionPoint(this.cPoint2);
			dPoint = SwingUtilities.convertPoint((Container)this.edge2, dPoint, this);

			Point resPoint = new Point();
			resPoint.setLocation(dPoint.getX()-sPoint.getX(), dPoint.getY()-sPoint.getY());
			resPoint.setLocation(resPoint.getX()*this.animationParametr+sPoint.getX(), resPoint.getY()*this.animationParametr+sPoint.getY());
...
//		p = getUpperPoint();
		...
		if (this.inDrag)
		{
			tail = SwingUtilities.convertPoint(this.getParent(), this.dragPoint, this);
		}
