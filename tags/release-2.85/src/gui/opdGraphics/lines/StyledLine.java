...
public class StyledLine implements MoveUpdatable {
	...
	public StyledLine(OpdConnectionEdge src, AroundDragger srcDragger,
		...
		TextLine newLine = this.createTextLine(this.sourceDragger, this.destinationDragger);
	...
	public void arrange() {
		if ((this.style == BREAKABLE) || (this.style == INVOCATION)) {
			...
			if (this.points.isEmpty()) {
				sourcePoint = GraphicsUtils.calculateConnectionPoint(this.source,
						this.destination);
				destinationPoint = GraphicsUtils.calculateConnectionPoint(
						this.destination, this.source);
			} else {
				sourcePoint = GraphicsUtils.calculateConnectionPoint(this.source,
						(DraggedPoint) this.points.get(0));
				destinationPoint = GraphicsUtils.calculateConnectionPoint(
			...
			this.destinationDragger.setRelativeConnectionPoint(destinationPoint);
	...
	public void setStyle(int style) {
		...
		if (style == INVOCATION) {
			this.points.clear();
			this.lines.clear();
			OpdInvocationLine newLine = this.createInvocationLine(this.sourceDragger,
	...
	public void add2Container(Container cn) {
		...
		for (int i = 0; i < this.points.size(); i++) {
			cn.add((DraggedPoint) this.points.get(i), JLayeredPane.PALETTE_LAYER);
		}
	...
	private OpdInvocationLine createInvocationLine(Connectable source,
		...
		gLine = new OpdInvocationLine(source, centerPoint, this.destinationDragger,
				centerPoint, this.myEntity, this.myKey, this.myProject);
	...
	private void breakLastLine(Point location) {
		...
		OpdLine newLine = this.createTextLine(newPoint, this.destinationDragger);
	...
	public synchronized void updateMove(Object origin) {
		if (origin instanceof OpdConnectionEdge) {
			if ((this.getStyle() == BREAKABLE) || (this.getStyle() == INVOCATION)) {
	...
	private void updatePoint(int pointToChangeIndex, int originIndex) {
		TextLine tLine = (TextLine) this.lines.get(Math.min(pointToChangeIndex,
				pointToChangeIndex));
		DraggedPoint tPoint = (DraggedPoint) this.points.get(pointToChangeIndex);
	...
	private boolean onLine(DraggedPoint point) {
		...
		if (index == 0) {
			src = new DoublePoint(this.sourceDragger.getX()
					+ this.sourceDragger.getWidth() / 2, this.sourceDragger.getY()
		...
		if (index == this.points.size() - 1) {
			dest = new DoublePoint(this.destinationDragger.getX()
					+ this.destinationDragger.getWidth() / 2, this.destinationDragger
					.getY()
	...
	public void setTextColor(Color clr) {
		...
		if (this.destinationDragger instanceof OpdRelationDragger) {
			((OpdRelationDragger) this.destinationDragger).getOpdCardinalityLabel()
					.setTextColor(clr);
		}
	}
	...
	private int getLongestLine() {
		int index = 0;
		TextLine line = (TextLine) this.lines.get(0);
	...
	private void setNumOfPoints(int num) {
		...
		for (int i = this.points.size() - 1; i < num - 1; i++) {
			TextLine lastLine = (TextLine) this.lines.get(this.lines.size() - 1);
			this.breakLastLine(new Point(lastLine.getX() + lastLine.getWidth() / 2,
					lastLine.getY() + lastLine.getHeight() / 2));
		}
	}
	...
	public BaseGraphicComponent[] getPointsArray() {
		BaseGraphicComponent[] bgcs = new BaseGraphicComponent[this.points.size()];
		for (int i = 0; i < this.points.size(); i++) {
	...
	public synchronized void animateAsFlash() {
		...
		this.repaint();
		this.animationTimer = new javax.swing.Timer(500, this.stopFlash);
	...
	public synchronized void animate(long time) {
		...
		if (this.style == INVOCATION) {
			...
			this.animationTimer = new javax.swing.Timer((int) time, this.stAnimation);
			this.animationTimer.setInitialDelay((int) time);
			this.animationTimer.setRepeats(false);
			this.animationTimer.start();
			return;
		}
		...
		this.animationTimer = new javax.swing.Timer(
				(int) (1000 / FRAMES_PER_SECOND), this.updateAnimation);
		this.startTime = (new GregorianCalendar()).getTime().getTime();
		this.totalTime = time;
		this.animationTimer.start();
	}
	...
	public synchronized void animate(long animationTime, long remainedTime) {
		...
		if (this.style == INVOCATION) {
			...
			;
			((OpdInvocationLine) this.lines.get(0)).repaint();
			this.animationTimer = new javax.swing.Timer((int) remainedTime,
					this.stAnimation);
			this.animationTimer.setInitialDelay((int) remainedTime);
			this.animationTimer.setRepeats(false);
			this.animationTimer.start();
			return;
		}
	...
	Action updateAnimation = new AbstractAction() {
		...
		public synchronized void actionPerformed(java.awt.event.ActionEvent e) {
			long timeLeft = StyledLine.this.totalTime + StyledLine.this.startTime
					- (new GregorianCalendar()).getTime().getTime();

			if (timeLeft <= 0) {
				StyledLine.this._stpAnimation();
				return;
			}

			double totalLength = StyledLine.this.getTotalLength();
			double currLength = totalLength * (StyledLine.this.totalTime - timeLeft)
