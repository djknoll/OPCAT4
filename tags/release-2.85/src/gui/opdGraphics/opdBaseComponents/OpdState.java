 ...
 * State can be drawn inside <a href = "OpdObject.html"><code>OpdObject</code></a>
 * or outside it (this option is not implemented). State also can be drawn as
 * final or initial state according to OPD documentation.
 * </p>
 */
...
public class OpdState extends OpdConnectionEdge {
	 ...
	 *            state <a href = "../opmEntities/OpmState.html"><code>OpmState</code></a>.
	...
	public OpdState(OpmState mState, OpdProject pProject, long pOpdId,
		...
		this.myEntry = this.myProject.getComponentsStructure().getEntry(mState.getId());
	}
	...
	public void paintComponent(Graphics g) {
		...
		int intRound = Math.max((int) ((this.getHeight() - 10.0 * factor) / 4.0),
		...
		Font currentFont = (Font) this.myProject.getConfiguration().getProperty(
				"StateFont");
		...
		g2.fillRoundRect(selOffset, selOffset, this.getWidth() - 1 - dSelOffset,
				this.getHeight() - 1 - dSelOffset, extRound, extRound);
		g2.setColor(this.borderColor);
		if (opmState.isInitial()) {
			g2.setStroke(new BasicStroke((float) factor * 3.0f));
			g2.drawRoundRect(selOffset, selOffset, this.getWidth() - 1 - dSelOffset,
					this.getHeight() - 1 - dSelOffset, extRound, extRound);
		}
		...
		if (opmState.isFinal()) {
			...
			g2.drawRoundRect(dist, dist, this.getWidth() - 1 - dist * 2, this.getHeight()
		...
		stringY = (this.getHeight() - this.currentMetrics.getAscent() * numOfRows - selOffset) / 2;
		int dY = 0;
		...
		while (st.hasMoreTokens()) {
			...
			stringX = (this.getWidth() - this.currentMetrics.stringWidth(tString) - selOffset) / 2;
	...
	public void callPropertiesDialog(int showTabs, int showButtons) {
		...
		StatePropertiesDialog sd = new StatePropertiesDialog(si, this.myProject,
				true, showButtons);
		sd.showDialog();
	}
	...
	public boolean inResize(int pX, int pY) {
		...
		if (!((pX > borderSensitivity) && (pY > borderSensitivity)
				&& (pX < this.getWidth() - borderSensitivity) && (pY < this.getHeight()
				- borderSensitivity))) {
			return true;
		} else {
			return false;
	...
	public Point getAbsoluteConnectionPoint(RelativeConnectionPoint pPoint) {
		...
		case OpcatConstants.S_BORDER:
			...
			retPoint.setLocation(this.getWidth() * pPoint.getParam(),
					this.getHeight() - 2);
			break;
	...
	public void showPopupMenu(int pX, int pY) {
		...
		Point showPoint = GraphicsUtils.findPlace4Menu(this, new Point(pX, pY),
