...
public abstract class OpdConnectionEdge extends OpdBaseComponent {
	 ...
	 * "../projectStructure/MainStructure.html" ><code>projectStructure.MainStructure</code></a>
	 */
	 ...
	 * a superclass <a href = "OpdBaseComponent.html"><code>OpdBaseComponent</code></a>,
	 * all subclasses have to call <code>super()</code> in their constructors
	 * to initialize the defaults and set parameters given as arguments
	 * </p>
	 * 
	 * @param <code>long pOpdId</code> uniqe identifier of the OPD this component
	 *            is added to.
	...
	public Instance getInstance() {
		return this.myEntry.getInstance(this.getOpdKey());
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		case StateMachine.USUAL_MOVE: {
			...
			Instance myInstance = this.myEntry.getInstance(new OpdKey(this
					.getOpdId(), this.getEntityInOpdId()));
			if (e.isShiftDown()) {
			...
			if (SwingUtilities.isMiddleMouseButton(e)
				...
				if (e.isShiftDown()) {
					if (myInstance.isSelected()) {
						...
						this.myProject.addSelection(myInstance, !e
								.isShiftDown());
					}
				} else {
					if (myInstance.isSelected()) {
...

		{
			...
			case StateMachine.ADD_LINK: {
				try {
					...
					this.myProject.addLinks/* addLinkConsistent */(this,
							sourcePoint, target, targetPoint,
							this.tempContainer, StateMachine
									.getDesiredComponent(), -1, -1);
					...
					ConsistencyAbstractChecker checker = (ConsistencyAbstractChecker) (new ConsistencyFactory(
							this.myProject, ConsistencyAction.ADDITION,
							checkResult)).create();
					checker.check();
	...
	private void addObject(int x, int y) {
		...
		if (od.showDialog()) {
			...
			if (od.isAddHelpON()) {
				ConsistencyResult checkResult = new ConsistencyResult();
				ConsistencyAbstractChecker checker = (ConsistencyAbstractChecker) (new ConsistencyFactory(
						this.myProject, ConsistencyAction.ADDITION, checkResult))
						.create();
				checker.check();
				if (!checkResult.isConsistent()) {
					checker.deploy(checkResult);
	...
	public synchronized void animateUrl(long time) {
		...
		String url = this.getEntity().getUrl();
		if (((url != null) || !url.equalsIgnoreCase("none"))
				&& (url.endsWith("jpg") || url.endsWith("bmp")
						|| url.endsWith("jpeg") || url.endsWith("mpa")
						|| url.endsWith("avi") || url.endsWith("qt")
						|| url.endsWith("mp4") || url.endsWith("mp3")
						|| url.endsWith("mpg") || url.endsWith("mpeg")
						|| url.endsWith("wmv") || url.endsWith("wma"))) {
			try {
				BrowserLauncher2.openURL(this.getEntity().getUrl());
			} catch (Exception e) {
				Debug debug = Debug.getInstance();
				debug.Print(this, e.getMessage(), "animation");
			}
		}
	}
	...
	public synchronized void animate(long time) {
		if (this.animated) {
			if (time == 0) {
				this.animatedDown = false;
				this.animatedUp = false;
				this.stopColorChangeImmediately(this.animationColor);
			}
			return;
		}
		...
		this.animationColor = this.muchBrighter(this.getBorderColor());
		this.originalColor = this.getBackgroundColor();

		if (this.isZoomedIn()) {
			this.animationColor = this.muchBrighter(this.animationColor);
		}

		this.animationTimer = new javax.swing.Timer(
				(int) (1000 / FRAMES_PER_SECOND), this.change2animationColor);

		if (time == 0) {
			this.setBackgroundColor(this.animationColor);
			this.repaint();
			return;
		}
	...
	public synchronized void stopAnimation(long time) {
		...
		this.startTime = (new GregorianCalendar()).getTime().getTime();
		this.totalTime = time;
		this.animationColor = this.getBackgroundColor();
		this.animationTimer.setInitialDelay(0);
		this.animationTimer.start();
	}
		...
		public synchronized void actionPerformed(java.awt.event.ActionEvent e) {
			...
			if (timeLeft <= 0) {
				OpdConnectionEdge.this
						.stopColorChangeImmediately(OpdConnectionEdge.this.animationColor);
			...
			OpdConnectionEdge.this
					.setBackgroundColor(new Color(
							OpdConnectionEdge.this.originalColor.getRed()
									+ (int) ((OpdConnectionEdge.this.animationColor
	...
	Action change2originalColor = new AbstractAction() {
		...
		public synchronized void actionPerformed(java.awt.event.ActionEvent e) {
			...
			OpdConnectionEdge.this
					.setBackgroundColor(new Color(
							OpdConnectionEdge.this.animationColor.getRed()
									+ (int) ((OpdConnectionEdge.this.originalColor
											.getRed() - OpdConnectionEdge.this.animationColor
											.getRed()) * param),
							OpdConnectionEdge.this.animationColor.getGreen()
									+ (int) ((OpdConnectionEdge.this.originalColor
											.getGreen() - OpdConnectionEdge.this.animationColor
											.getGreen()) * param),
							OpdConnectionEdge.this.animationColor.getBlue()
									+ (int) ((OpdConnectionEdge.this.originalColor
											.getBlue() - OpdConnectionEdge.this.animationColor
											.getBlue()) * param)));
			OpdConnectionEdge.this.repaint();
	...
	public void mouseEntered(MouseEvent e) {
		...
		if (this.myEntry.hasURL()) {
			...
			this.setToolTipText("Ctrl-LeftMouse to Activate URL");
