...
//	private OpdLink opdLink;
  ...
  public LinkPropertiesDialog(OpmProceduralLink link, OpdKey key,
    ...
    this.myEntry = (LinkEntry) this.myStructure.getEntry(this.opmLink.getId());
    this.myInstance = (LinkInstance) this.myEntry.getInstance(key);

    long sId, dId;

    sId = this.opmLink.getSourceId();
    dId = this.opmLink.getDestinationId();

    this.sourceName = this.myStructure.getEntry(sId).getLogicalEntity().getName().replace(
        '\n', ' ');
    this.destinationName = this.myStructure.getEntry(dId).getLogicalEntity().getName().
        replace('\n', ' ');
...
//		applyButton.setEnabled(false);
    ...
    KeyListener escapeAndEnterListener = new KeyAdapter() {
      public void keyReleased(KeyEvent e) {
        ...
        if ((e.getKeyCode() == KeyEvent.VK_ENTER) && !LinkPropertiesDialog.this.condition.hasFocus()) {
    ...
    this.setLocation(fX + Math.abs(pWidth / 2 - this.getWidth() / 2),
                fY + Math.abs(pHeight / 2 - this.getHeight() / 2));

    this.setResizable(false);

    //reuseComment
     if (this.myInstance.getEntry().isLocked()) {
...
//		tab.add(p1);
  ...
  private JPanel _constructTimingTab() {
    ...
    tab.setLayout(new BoxLayout(tab, BoxLayout.Y_AXIS));
    tab.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
    this.minReactionTime.setBorder(BorderFactory.createTitledBorder(BorderFactory.
        createEtchedBorder(), " Minimum Reaction Time "));
    this.maxReactionTime.setBorder(BorderFactory.createTitledBorder(BorderFactory.
...
//		tab.add(Box.createVerticalStrut(40));
  ...
  private boolean _updateLinkData() {
    ...
    if (GraphicsUtils.getMsec4TimeString(this.minReactionTime.getTime()) >
        GraphicsUtils.getMsec4TimeString(this.maxReactionTime.getTime())) {
...
public void commitLockForEdit()
  {
    this.condition.setEditable(false);
    this.conditionScrollPane.setEnabled(false);
    this.sourceThing.setEditable(false);
    this.destinationThing.setEditable(false);
