...
public class ProcessPropertiesDialog extends JDialog implements ComponentListener
{
	...
	private JRadioButton physical, informational, environmental, system, additionHelpOff, additionHelpON;
	...
	{ 
		...
		{
			this.tabs.add("Roles", this._constructRolesTab());
		}
		...
		KeyListener escapeAndEnterListener = new KeyAdapter()
		{
			public void keyReleased(KeyEvent e)
			{
				...
				if ((e.getKeyCode() == KeyEvent.VK_ENTER) && !ProcessPropertiesDialog.this.body.hasFocus() && !ProcessPropertiesDialog.this.description.hasFocus())
				{
	...
	private void _initVariables(){
		try{
			...
			if((this.showTabs & BaseGraphicComponent.SHOW_1) != 0)
			{
				...
				this.urlScrollPane = new JScrollPane(this.url, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
				...
				this.scope.setSelectedIndex(Character.digit((this.process.getScope()).charAt(0), 3));
			...
			if((this.showTabs & BaseGraphicComponent.SHOW_2) != 0)
			{
				...
				this.bodyScrollPane = new JScrollPane(this.body, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED, ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
				this.body.setLineWrap(true);
				this.body.setWrapStyleWord(true);

				this.description = new JTextArea(this.process.getDescription(), 4, 30);
			...
			if((this.showTabs & BaseGraphicComponent.SHOW_3) != 0)
			{
				this.maxTimeActivation = new TimeSpecifier(SwingConstants.HORIZONTAL, SwingConstants.TOP, 3, 3);
				this.maxTimeActivation.setTime(this.process.getMaxTimeActivation());
				this.minTimeActivation = new TimeSpecifier(SwingConstants.HORIZONTAL, SwingConstants.TOP, 3, 3);
				this.minTimeActivation.setTime(this.process.getMinTimeActivation());
			}
			//end activation time
...
//		p1.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(), BorderFactory.createEmptyBorder(10, 10, 10, 10)));
		this.p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "Process Name"));
...
//		p1.add(l1);
		...
		this.p2.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Essence "));
		...
		this.p2 = new JPanel();
		this.p2.setLayout(new BoxLayout(this.p2, BoxLayout.Y_AXIS));
		this.p2.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Origin "));
		...
		// scope ComboBox
		this.p1 = new JPanel();
		this.p1.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEtchedBorder(), BorderFactory.createEmptyBorder(8, 8, 8, 8)));
		this.p1.setLayout(new BoxLayout(this.p1, BoxLayout.X_AXIS));
		...
		this.p1 = new JPanel();
		this.p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), "   Addition Helper   "));
	...
	private JPanel _constructPreferences1Tab(){
		...
		this.p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " URL "));
		...
		// Process Body
		this.p1 = new JPanel();
		this.p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Body "));
	...
	private JPanel _constructTimingTab()
	{
		...
		tab.setLayout(new BoxLayout(tab, BoxLayout.Y_AXIS));
		tab.setBorder(BorderFactory.createEmptyBorder(8, 8, 8, 8));
		this.minTimeActivation.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Minimum Activation Time "));
		this.maxTimeActivation.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Maximum Activation Time "));
	...
	private JPanel _constructMiscTab(){
		...
		ProcessEntry sampleEntry = new ProcessEntry(new OpmProcess(0, "Sample"), this.myProject);
		...
		this.sampleProcess.setBackgroundColor(this.opdProcess.getBackgroundColor());
	...
	private boolean _updateProcessData()
	{
		...
		if((this.showTabs & BaseGraphicComponent.SHOW_1) != 0)
		{
			if(this.name.getText().equals(this.defaultName) || this.name.getText().trim().equals(""))
			{
			...
			if (GraphicsUtils.getMsec4TimeString(this.minTimeActivation.getTime()) > GraphicsUtils.getMsec4TimeString(this.maxTimeActivation.getTime()))
			{
...
//				}
			...
			this.process.setScope(String.valueOf(this.scope.getSelectedIndex()));
		}
		//end general
		...
		//by Eran Toch
		this.process.getRolesManager().setRoles(this.rolesTab.getRoles());
		//end roles
		...
		{
			Opcat2.getUndoManager().undoableEditHappened( new UndoableEditEvent(this.myProject,
					new UndoableChangeProcess(this.myProject, this.myEntry, oldProcess,this.process)));
		...
		public void actionPerformed(ActionEvent e){
			...
			if (newRef != null) {
				...
				if (type == MetaLibrary.TYPE_FILE ) {
					ProcessPropertiesDialog.this.url.setText("file:///".concat(path));
				}
				else {
	...
	Action positionAction = new AbstractAction(){
		...
		public void actionPerformed(ActionEvent e){
			...
			ProcessPropertiesDialog.this.addHelpON = ProcessPropertiesDialog.this.additionHelpON.isSelected() ; 
			(ProcessPropertiesDialog.this).sampleProcess.repaint();
		}
	};
