...
public class DrawingArea extends JDesktopPane implements MouseListener,
	...
	public void mouseReleased(MouseEvent e) {
		...
		case StateMachine.ADD_PROCESS: {
			...
			if (pd.showDialog()) {
				ProcessInstance pi = this.myProject.addProcess(e.getX(), e.getY(),
						this, -1, -1);
		...
		case StateMachine.ADD_OBJECT: {
			...
			ObjectInstance sampleObjInstance = new ObjectInstance(
					sampleObjEntry, new OpdKey(0, 0), null, this.myProject, false);
			...
			if (od.showDialog()) {
				ObjectInstance oi = this.myProject.addObject(e.getX(), e.getY(),
						this, -1, -1, false);
				// reuseComment
				if (oi == null) {
					return;
				...
				this.setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
				Opcat2.getUndoManager().undoableEditHappened(
						new UndoableEditEvent(this.myProject, new UndoableAddObject(
								this.myProject, oi)));
				Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
				Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
				this.myProject.removeSelection();
	...
	public void paint(Graphics g) {
		// reuseComment
		super.paint(g);
		if (this.myOpd.getReusedSystemPath() != null) {
			g.setColor(Color.black);
			int height = this.getHeight();
			int width = this.getWidth();
			if (this.myOpd.getOpdId() == 1) {
