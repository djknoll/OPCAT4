...
public class ExceptionHandler {
	...
	String PERMISSION_DOES_NOT_EXIST = "01008"; // The requested permission
	...
	String ENABLED_OPMMODELS_FOUND = "02004"; // Active OPM models found.
												// Disable all OPM models before
												// proceeding.
	// OPMmodels
	...
	String ENABLED_SESSIONS_FOUND = "03004"; // Enabled collaborative
												// sessions found. Disable all
												// collaborative sessions before
												// proceeding.
	// Sessions

	String INTERNAL_DB_ERROR4 = "04001"; // Internal Database Error.

	String COLLABORATIVESESSION_ALREADY_EXIST = "04002"; // CollaborativeSession
															// already exists.

	String COLLABORATIVESESSION_DOES_NOT_EXIST = "04003"; // CollaborativeSession
															// does not exist.

	String COLLABORATIVESESSION_TOKEN_TAKEN = "04004"; // The token is not
														// available.

	String USER_IS_NOT_TOKEN_HOLDER = "04005"; // You are not the token holder.

	String USER_ALREADY_LOGGED_INTO_SESSION = "04006"; // You are already
														// logged into the
														// session

	String USER_NOT_LOGGED_INTO_SESSION = "04007"; // You are not logged into
													// the session

	String SESSION_HAS_LOGGEDIN_USERS = "04008"; // Users are still logged
													// into the session. Active
													// sessions cannot be
													// disabled
	...
	public ExceptionHandler(Exception exception) {
		...
		if (exception instanceof AxisFault) {
			...
			if (index2 != -1) {
				// that means that the message is not internal to OPCATeam
				// server-
				// in such case - just present the message to the user, dont try
				// tp parse it.

				String str2 = str1.substring(index1, index2);
				switch (this.findExceptionNumber(str2)) {
				case 1008:
					return;
				}
			}

			JOptionPane.showMessageDialog(Opcat2.getFrame(), str1
					.substring(index1), "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}

		if (exception instanceof JMSException) {
			JOptionPane.showMessageDialog(Opcat2.getFrame(), exception
	...
	private int findExceptionNumber(String stringNum) {
		if (stringNum.equals(this.PERMISSION_DOES_NOT_EXIST)) {
			return 1008;
		}
		return 0;
