...
public class PrettyPrinter {
  ...
  public PrettyPrinter(ISystemStructure elems, ISystem opcatSystem_) {
    ...
    this.startHTML = "<HTML><HEAD></HEAD><BODY>"; /*+
  ...
  public void visit(ObjectUniDirectionalRelationSentenceType e) {
    this.bf.append(this.margin);
    boolean has_s = false;
    has_s = this.printCardinality(e.getSourceMinimalCardinality(),
                                     e.getSourceMaximalCardinality(),true);
    if(has_s){
      this.printAttr(OplColorScheme.OBJECT_STYLE, extensionTools.opl2.alg.
  ...
  public void visit(ProcessTimeoutSentenceType proc) {
    ...
    this.printAttr(OplColorScheme.DEFAULT_STYLE," when it lasts more than ");
    this.printAttr(OplColorScheme.DEFAULT_STYLE, OPLGeneral.visit(proc.getMaxTimeoutValue()));
    this.bf.append(this.endLine);
  }
  ...
  public boolean printCardinality(int min, int max, boolean isFirst) {
    if ((min == max) && (max > 1)) {
        this.printAttr(OplColorScheme.CARDINALITY_STYLE, Integer.toString(min)+" ");
      return true;
    }
    if ((min == 1) && (max == 1)) {
      return false;
    ...
    if ((min == 0) && (max == 1)) {
      if(isFirst){
        this.printAttr(OplColorScheme.CARDINALITY_STYLE, "An optional ");
      } else {
		this.printAttr(OplColorScheme.CARDINALITY_STYLE, "an optional ");
	}
      return false;
    ...
    if ((min == 1) && (max == -1)) {
      if(isFirst){
        this.printAttr(OplColorScheme.CARDINALITY_STYLE, "At least one ");
      } else {
		this.printAttr(OplColorScheme.CARDINALITY_STYLE, "at least one ");
	}
      return false;
  ...
  public void addExhibitedThings(boolean key_, java.util.List Objects) {
    ...
    for (java.util.ListIterator itr = Objects.listIterator(); itr.hasNext(); ) {
      if (key_) {
        obj = (ExhibitedObject) itr.next();
        hasS = this.printCardinality(obj.getMinimalCardinality(),
  ...
  public void addAggregationSentence(int key, java.util.List exhibits) {
    ...
    for (java.util.ListIterator itr = exhibits.listIterator(); itr.hasNext(); ) {
      if (isObj) {
        ...
        hasS = this.printCardinality(tmp.getMinimalCardinality(),
  ...
  public void visit(ProcessExhibitionSentenceType e) {
    ...
    for (int i = 0; i < objSize; i++) {
      ...
      this.printAttr(OplColorScheme.OBJECT_STYLE, exhib.getAttributeName());
    }
    this.bf.append(this.endLine);
  }
  ...
  public void visit1(extensionTools.opl2.generated.ObjectInheritanceSentenceSet e) {
    this.addInhFather(OplColorScheme.OBJECT_STYLE, e.getObjectName(),e.getObjectInheritanceSentence());
  }

  public void visit1(extensionTools.opl2.generated.ProcessInheritanceSentenceSet e) {
  	this.addInhFather(OplColorScheme.PROCESS_STYLE, e.getObjectName(),e.getProcessInheritanceSentence());
  }

  public void visit(ObjectInstanceSentenceType e) {
   this.addInstFather(OplColorScheme.OBJECT_STYLE, e.getObjectName(),
                e.getInstanceFatherName());
 }

 public void visit(ProcessInstanceSentenceType e) {
   this.addInstFather(OplColorScheme.PROCESS_STYLE, e.getProcessName(),
                e.getInstanceFatherName());
 }


  public void visit(ObjectEnvironmentalPhysicalSentenceType sentence) {
    this.addEnvPhysical(OplColorScheme.OBJECT_STYLE, sentence.getObjectName(),
                   sentence.isEnvironmental(),
                   sentence.isPhysical());
  }

  public void visit(ProcessEnvironmentalPhysicalSentenceType sentence) {
    this.addEnvPhysical(OplColorScheme.PROCESS_STYLE, sentence.getProcessName(),
                   sentence.isEnvironmental(),
                   sentence.isPhysical());
  }
  ...
  protected void addEnvPhysical(int key, String subj,
    ...
    if (isEnvir && isPhysical) {
      this.printAttr(OplColorScheme.DEFAULT_STYLE,"environmental and physical");
      this.bf.append(this.endLine);
      return;
    }
    if (isEnvir) {
  ...
  public void visit(ChangingSentenceType sentence) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      this.printAttr(OplColorScheme.DEFAULT_STYLE," to ");
      this.printAttr(OplColorScheme.STATE_STYLE, cl.getDestinationStateName());
    }
    this.bf.append(this.endLine);
  }
...
//    }
  ...
  public void visit(GeneralEventSentence sentence, boolean hasStates) {
    ...
    if (hasStates) {
		this.printAttr(OplColorScheme.DEFAULT_STYLE," when its state changes");
	}
    this.bf.append(this.endLine);
  }
...
//  }
  ...
  public void visitExistential(ThingSentenceSetType e){
	...
	if(e.equals("exists")){
		this.printAttr(OplColorScheme.PROCESS_STYLE, e.getSubjectThingName());
	}else{
  ...
  public void visit(ThingSentenceSetType e) {
    ...
    if (e.getObjectInheritanceSentenceSet() != null) {
      this.visit1((ObjectInheritanceSentenceSet)e.getObjectInheritanceSentenceSet());
    }
    ...
    for (i = 0; i < e.getGeneralEventSentence().size(); i++) {
      this.visit( (GeneralEventSentence) e.getGeneralEventSentence().get(i),
            hasStates);
    }
   // visitStateTriggers(e);
    for (i = 0; i < e.getStateEntranceSentence().size(); i++) {
		this.visit( (StateEntranceSentence) e.getStateEntranceSentence().get(i));
	}
    for (i = 0; i < e.getStateTimeoutSentence().size(); i++) {
		this.visit( (StateTimeoutSentence) e.getStateTimeoutSentence().get(i));
	}
    ...
    if (e.getProcessInheritanceSentenceSet() != null) {
		this.visit1((extensionTools.opl2.generated.ProcessInheritanceSentenceSet)e.getProcessInheritanceSentenceSet());
	}
    ...
    for (i = 0; i < e.getConsumptionSentence().size(); i++) {
		this.visit( (ConsumptionSentenceType) e.getConsumptionSentence().get(i));
	}
    ...
    for (i = 0; i < e.getProcessTimeoutSentence().size(); i++) {
		this.visit( (ProcessTimeoutSentenceType) e.getProcessTimeoutSentence().get(i));
	}
    for (i = 0; i < e.getProcessInvocationSentence().size(); i++) {
		this.visit( (ProcessInvocationSentenceType) e.getProcessInvocationSentence().
		        get(i));
	}
  ...
  public void printRoles(ThingSentenceSetType e){
	...
	for(int i = 0; i < e.getRole().size(); i++){
		  ...
		  this.printAttr(OplColorScheme.DEFAULT_STYLE," of "+rl.getRoleName());
		  if((rl.getLibrary()!=null) && !rl.getLibrary().equals("")){
			this.printAttr(OplColorScheme.DEFAULT_STYLE," of "+rl.getLibrary());
		  }
		}
	this.bf.append(this.endLine);
  }
  ...
  public void visit(TypeDeclarationSentenceType e) {
    ...
    if ( ('0' <= firstChar) && (firstChar <= '9')) {
      this.printAttr(OplColorScheme.DEFAULT_STYLE,"character string of legth " + e.getObjectType());
    } else {
    ...
    if (e.getObjectScope().equals("public")) {
      ...
      this.bf.append(this.margin);
      this.printAttr(OplColorScheme.OBJECT_STYLE, e.getObjectName());
      this.printAttr(OplColorScheme.DEFAULT_STYLE," is " + e.getObjectScope());
      this.bf.append(this.endLine);
    }
    ...
    this.bf.append(this.margin);
    this.printAttr(OplColorScheme.OBJECT_STYLE, e.getObjectName());
    this.printAttr(OplColorScheme.DEFAULT_STYLE," is "+e.getInitialValue()+" by default");
    this.bf.append(this.endLine);
  ...
  public void printAttr(int attributeName, String value) {
    ...
    this.bf.append(this.colorScheme.getAttribute(attributeName).openingHTMLFontTag());
    this.bf.append(value);
    this.bf.append(this.colorScheme.getAttribute(attributeName).closingHTMLFontTag());
  }
  ...
  protected void setEndLine() {
    this.endLine = this.colorScheme.getAttribute(OplColorScheme.DEFAULT_STYLE).
