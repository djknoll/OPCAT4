...
//Added role
  protected GeneralEventSentence buildGeneralEventSentence(ILinkEntry link, String path) throws Exception{
    ...
    IObjectEntry obj = (IObjectEntry)this.builder.myStructure.getIEntry(link.getSourceId());
...
//Added role
  protected StateTimeoutSentence buildStateTimeoutSentence(ILinkEntry link, String path) throws Exception{
    ...
    IStateEntry state = (IStateEntry)this.builder.myStructure.getIEntry(link.getSourceId());
...
//Added role
  protected ObjectInheritanceSentenceSet buildObjInhSentenceSet(IRelationEntry rel) throws
    ...
    try {
      ...
      sentence.setObjectName(childName);
      sentence.getObjectInheritanceSentence().add(this.buildObjInhSentence(rel));
    }
    catch (Exception e) {
      throw e;
    }
...
//Added roles
  protected ObjectUniDirectionalRelationSentence buildObjUniDirSentence(
    ...
    try {
      ...
      if(!((IThingEntry)this.builder.myStructure.getIEntry(rel.getDestinationId())).getRole().equals("")) {
		;
	}
        //sentence.setDestinationRole(((IThingEntry)builder.myStructure.getIEntry(rel.getDestinationId())).getRole());
      if(!((IThingEntry)this.builder.myStructure.getIEntry(rel.getSourceId())).getRole().equals("")) {
		;
	}
  ...
  protected ObjectBiDirectionalRelationSentence buildObjBiDirSentence(
     ...
     try {
       ...
       if (rule == OPLGeneral.DESTINATION) {
         ...
         {
           this.fillObjBiDirSentence(sentence, destination, fatherName, meaning);
           this.addCardinality(sentence, destinationCardinality, sourceCardinality);
           return sentence;
         }
       }
       else {
       ...
       this.fillObjBiDirSentence(sentence, fatherName, destination, meaning);
       this.addCardinality(sentence, sourceCardinality, destinationCardinality);
     }
     catch (Exception e) {
       throw e;
     }
...
//Should add role!!!
  ...
  protected void addCardinality(ObjectBiDirectionalRelationSentence sentence,
    ...
    sentence.setDestinationMaximalCardinality(destinationCardinality[this.MAX]);
    sentence.setDestinationMinimalCardinality(destinationCardinality[this.MIN]);
  }
