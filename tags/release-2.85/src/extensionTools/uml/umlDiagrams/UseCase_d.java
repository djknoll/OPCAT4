...
class UseCase_d {
	 ...
	 *            All OPD Levels-->op=2.
	 */
	private void PrintUsecase(ISystemStructure MyStructure,
			FileOutputStream file, IProcessEntry proc, Vector usecaseVec,
			Vector usecaseExtendVec, Vector usecaseIncludeVec,
			Vector usecaseInzoomIncludeVec, Vector generalizationVec,
			Vector usecasespecVec, Vector OpdVec, int op) throws IOException {
		try {
			String temp = "";
			...
			temp = "<UML:UseCase xmi.id=\"S."
					...
					+ "\" visibility=\""
					+ this.GetScope(proc)
					+ "\" isSpecification=\"false\" isRoot=\"false\" isLeaf=\"false\" isAbstract=\"false\" namespace=\"G.0000.1\"  ";
			file.write(temp.getBytes());
			...
			// ======INVOCATION_LINK AND
			// EXCEPTION_LINK_SUPPLIER==================== EXTEND
			list = proc.getLinksBySource();
			flag = 0;
			while (list.hasMoreElements()) {
				link = (ILinkEntry) list.nextElement();
	 ...
	 */
	private void CreateProcVec(ISystemStructure MyStructure,
			IProcessEntry proc, Vector OpdVec, int level) {
					...
					while (childList.hasMoreElements()) { // going throw all
						...
						if (new_level >= 0) {// number of level to go down
							...
							if (thing instanceof IProcessInstance) { // if
								...
								this.CreateProcVec(MyStructure,
	 ...
	 *            level - number of levels to "go down" in the Inzoomed process
	 */
	private void CreateActVec(ISystemStructure MyStructure,
			FileOutputStream file, IProcessEntry proc, Vector actVec, int level)
			throws IOException {
		try {
			int new_level;
			Enumeration e, childList;
				...
				while (e.hasMoreElements()) {
					...
					while (childList.hasMoreElements()) { // going throw all
						...
						if (thing instanceof IProcessInstance) {// if the
							...
							// -->again
							procInst = (IProcessInstance) thing;
							this.CreateActVec(MyStructure, file,
									(IProcessEntry) (procInst.getIEntry()),
									actVec, new_level); // recurtion
						}
					}
				}
			} else {
	 ...
	 *            between "actors"
	 */
	private void PrintActors(ISystemStructure MyStructure,
			FileOutputStream file, IObjectEntry obj, Vector actorTagsVec,
			Vector generalizationVec) throws IOException {
		try {
			String temp = "";
			int flag, flag2;
			...
			temp = "<UML:Actor xmi.id=\"S."
					...
					+ "\" visibility=\""
					+ this.GetScope(obj)
					+ "\" isSpecification=\"false\" isRoot=\"true\" isLeaf=\"true\" isAbstract=\"false\" namespace=\"G.0000.1\" ";
			file.write(temp.getBytes());
	 ...
	 */
	public void UseCaseDiagramCreate(ISystem sys, FileOutputStream file,
			Vector usecaseVec, Vector usecaseIncludeVec,
			Vector usecaseInzoomIncludeVec, Vector usecasespecVec,
		...
		try {
			...
			if (op == 2) { // all opd levels WERE SELECTED BY THE USER
				...
				while (e.hasMoreElements()) {
					...
					if (abstractEntry instanceof IProcessEntry) { // if
						...
						;
						if (this.isUsecase(proc, MyStructure)) { // if the proc is
							// use case
							this.PrintUsecase(MyStructure, file, proc, usecaseVec,
									usecaseExtendVec, usecaseIncludeVec,
									usecaseInzoomIncludeVec, generalizationVec,
									usecasespecVec, OpdVec, op);
						}// end of if(IsUsecase)
					}// end of if(proc)
					if (abstractEntry instanceof IObjectEntry) { // if object
						...
						if (obj.isEnvironmental()) {// if actor
							this.PrintActors(MyStructure, file, obj, actorTagsVec,
									generalizationVec);
			...
			new_level = levels - 1;
			if (op == 1) { // USER SELECTED THE OPD-ROOT OPTION OR UNTILL LEVEL
				// OPTION
				...
				while (e.hasMoreElements()) {
					...
					if (inst instanceof IProcessInstance) { // if it is a
						...
						this.CreateProcVec(MyStructure, (IProcessEntry) (procInst
								.getIEntry()), OpdVec, new_level);
						this.CreateActVec(MyStructure, file,
								(IProcessEntry) (procInst.getIEntry()), actVec,
								new_level);
					}
				}
						...
						if (obj.isEnvironmental()) {
							...
							while ((list.hasMoreElements()) && (flagPr == 0)) {
								...
								if (obj.getId() == obj2.getId()) {
									this.PrintActors(MyStructure, file, obj,
				...
				if (abstractEntry instanceof IRelationEntry) { // if relation
					...
					if (abstractEntry instanceof IObjectEntry) {// if source is
						...
						if (obj.isEnvironmental()) { // if an actor
							if (((op == 1) && (this.IsInVec(obj, actVec)))
									|| (op == 2)) {
								abstractEntry = MyStructure.getIEntry(rel
					...
					if (abstractEntry instanceof IProcessEntry) {// if source
						...
						// process
						proc = (IProcessEntry) abstractEntry;
						if (this.isUsecase(proc, MyStructure)) {// if source is a
							// usecase
							if (((op == 1) && (this.IsInVec(proc, OpdVec)))
									|| (op == 2)) {
								abstractEntry = MyStructure.getIEntry(rel
			...
			while (e.hasMoreElements()) {
				...
				temp = "<UML:Association xmi.id=\"G."
						+ tempProc.getId()
						+ ".3\" name=\"\" visibility=\"public\" isSpecification=\"false\" isRoot=\"false\" isLeaf=\"false\" isAbstract=\"false\">";
				file.write(temp.getBytes());
				temp = "<UML:Association.connection>";
				file.write(temp.getBytes());
				temp = "<!--  ================== [AssociationEnd] =================  --> ";
				...
				temp = "<UML:AssociationEnd xmi.id=\"G."
						+ tempProc.getId()
						+ ".3.2\" name=\"\" visibility=\"public\" isSpecification=\"false\" isNavigable=\"true\" ordering=\"unordered\" aggregation=\"none\" targetScope=\"instance\" changeability=\"changeable\" type=\"S."
