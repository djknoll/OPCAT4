...
ISystem mySys;
  ...
  private void jbInit() throws Exception  {
    ...
    act_d.GetAll_InZoomedProcess(this.mySys.getISystemStructure(), this.v_act);
    String data2[] = new String [this.v_act.size()];
    while (i<this.v_act.size()) {
      data2[i]=((IEntry)this.v_act.get(i)).getName().replace('\n',' ');
      i++;
    }
    ...
    while (i<this.v_obj.size()) {
      data1[i]=((IEntry)this.v_obj.get(i)).getName().replace('\n',' ');
      i++;
    }
    ...
    while (i<this.v_proc.size()) {
      data[i]=((IEntry)this.v_proc.get(i)).getName().replace('\n',' ');
      i++;
    }
    ...
    this.titledBorder1 = new TitledBorder(BorderFactory.createEtchedBorder(Color.white,new Color(148, 145, 140)),"Please select Processes for Sequence Diagrams");
    this.border2 = BorderFactory.createEtchedBorder(Color.white,new Color(148, 145, 140));
    this.titledBorder2 = new TitledBorder(BorderFactory.createEtchedBorder(Color.white,new Color(148, 145, 140)),"Please select Objects for Statechart Diagrams");
    this.border3 = BorderFactory.createEtchedBorder(Color.white,new Color(148, 145, 140));
    this.titledBorder3 = new TitledBorder(this.border3,"Please select levels of OPD for UseCase Diagram");
    this.border4 = BorderFactory.createEmptyBorder();
    this.titledBorder4 = new TitledBorder(this.border4,"Generate Class Diagram?");
    this.border5 = BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142));
    this.titledBorder5 = new TitledBorder(this.border5,"Generate Class Diagram?");
    this.border6 = new EtchedBorder(EtchedBorder.RAISED,Color.white,new Color(142, 142, 142));
    this.titledBorder6 = new TitledBorder(BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142)),"Generate Deployment Diagram?");
    this.border7 = BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142));
    this.titledBorder7 = new TitledBorder(this.border7,"Please select Processes and levels for Activity Diagrm");
    this.border8 = BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142));
    this.titledBorder8 = new TitledBorder(BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142)),"Use Case Diagram");
    ...
    this.titledBorder10 = new TitledBorder(this.border10,"Sequence Diagrams");
    this.border11 = new EtchedBorder(EtchedBorder.RAISED,Color.white,new Color(142, 142, 142));
    this.titledBorder11 = new TitledBorder(BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142)),"Statechart Diagrams");
    this.border12 = BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142));
    this.titledBorder12 = new TitledBorder(this.border12,"Deployment Diagram");
    this.border13 = BorderFactory.createEtchedBorder(Color.white,new Color(142, 142, 142));
    this.titledBorder13 = new TitledBorder(this.border13,"Activity Diagrams");
    ...
    this.UMLActListAll.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        UMLChooser.this.UMLActListAll_actionPerformed(e);
      }
    });
    this.UMLActListUntil.setBounds(new Rectangle(55, 240, 17, 25));
    this.UMLActListUntil.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
        UMLChooser.this.UMLActListUntil_actionPerformed(e);
      }
    });
    this.UMLActListNum.setBounds(new Rectangle(74, 243, 32, 21));
    this.UMLActListNum.addActionListener(new java.awt.event.ActionListener() {
      public void actionPerformed(ActionEvent e) {
 ...
 */
  private void UMLGenerate_actionPerformed(ActionEvent e) {
    ...
    try{
      ...
      if((this.UMLActListUntil.isSelected())&&(this.UMLActElse.isSelected())){
    ...
    if((this.UStateYes.isSelected())&&(!this.UClassYes.isSelected())){
      JOptionPane.showMessageDialog(this, "To create Statechart Diagram you need to choose:\n Class Diagram : Yes", "Error", JOptionPane.ERROR_MESSAGE);
      Sflag = true;
    }
    else
    if (this.UStateYes.isSelected()){
      ...
      if(selected.length<=0){
    	  JOptionPane.showMessageDialog(this, "Illegal input in statechart. Please select object.", "ERROR", JOptionPane.ERROR_MESSAGE);
        Sflag=true;
      }
    }
    ...
    if((this.UMLActElse.isSelected())&&(!Sflag)){
      ...
      if(selected.length<=0){
    	  JOptionPane.showMessageDialog(this, "Illegal input in activity. Please select process.", "ERROR", JOptionPane.ERROR_MESSAGE);
        Aflag=true;
      }
    }

    if((this.USeqYes.isSelected())&&((!this.UUAll.isSelected())||(!this.UClassYes.isSelected()))&&(!Sflag)&&(!Aflag)){
      JOptionPane.showMessageDialog(this, "To create Sequence Diagram you need to choose:\n Class Diagram : Yes\n Use Case Diagram : All OPD Levels", "Error", JOptionPane.ERROR_MESSAGE);
      Seqflag = true;
    }
    else
      if(this.USeqYes.isSelected() && (!Sflag) &&(!Aflag) ){
        int selected[] = this.USeqList.getSelectedIndices();
        if(selected.length <= 0){
        	JOptionPane.showMessageDialog(this,"Illigal input in sequence.Please select process.","ERROR",JOptionPane.ERROR_MESSAGE);
          Seqflag = true;
        }
      }
    ...
    if(!Aflag && !Sflag && !Seqflag){
      ...
      if(returnVal == JFileChooser.APPROVE_OPTION) {
        ...
        if (flag==1){
          int retval=JOptionPane.showConfirmDialog( this, "A file with this name already exists. Do you want to overwrite it?", "Overwrite Document", JOptionPane.YES_NO_OPTION);
          if (retval== JOptionPane.YES_OPTION){
           if ( (this.UClassYes.isSelected())||(this.UUNone.isSelected() == false)||(this.UStateYes.isSelected())
                        ||(this.UDepYes.isSelected())||(this.UMLActNone.isSelected()==false)||(this.USeqYes.isSelected())){
              try{
                this.UMLCreate(this.mySys, filename);
              }
    ...
    if(Uflag) {
		JOptionPane.showMessageDialog(this, "Illegal input in usecase: level is not valid.\n Please insert an integer.", "ERROR", JOptionPane.ERROR_MESSAGE);
	} else {
		JOptionPane.showMessageDialog(this, "Illegal input in activity: level is not valid.\n Please insert an integer.", "ERROR", JOptionPane.ERROR_MESSAGE);
	}
  }//end of catch
}//end of func
...
private void  UMLCreate(ISystem sys, String filename) throws IOException{
    try{
      ...
      int opD=-1;
        ...
        if(this.UUUntil.isSelected()){
          opU=1;
          levU=Integer.parseInt(this.UUUntilText.getText());
        }
        //-------statechart-------------
        if (this.UStateYes.isSelected()){
          opSt=1;
          ...
          while (i<selected.length){
            vecSt.addElement(this.v_obj.get(selected[i]));
            i++;
          }
        }
        //-------sequence-------------
        if (this.USeqYes.isSelected()){
          ...
          int i=0;
          while (i<selected.length){
            vecSq.addElement(this.v_proc.get(selected[i]));
            i++;
          }
        }
        //-------activity-------------
        if(this.UMLActTop.isSelected()){
          opA=1;
          levA=Integer.parseInt(this.UMLActTopText.getText());
        }
        if(this.UMLActElse.isSelected()){
          opA=2;// by processes
          int selected[] = this.UMLActList.getSelectedIndices();
          int i=0;
          while (i<selected.length){
            vecA.addElement(this.v_act.get(selected[i]));
            i++;
          }
          if (this.UMLActListAll.isSelected()) {
			levA=1000;
		} else {
			levA=Integer.parseInt(this.UMLActListNum.getText());
		}
        }
        //-------deployment-------------
        if(this.UDepYes.isSelected()) {
			opD=1;
		}
