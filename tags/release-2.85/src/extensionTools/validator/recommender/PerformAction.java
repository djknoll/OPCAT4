...
public class PerformAction extends AbstractAction {
	...
	public void actionPerformed(ActionEvent arg0) {
		try	{
			IThingEntry roleThing = this.neighbor.getConnectedThing();
			IXThingInstance newInstance = this.createThing(roleThing);
			if (newInstance == null)	{
				return;
			}
			this.createRole(newInstance, roleThing);
			...
			if (okPressed)	{
				...
				if (connection instanceof ILinkInstance)	{
					undoAction.addAction(new UndoableAddLink((OpdProject)this.system, (LinkInstance)connection));
				}
				else if (connection instanceof IRelationInstance)	{
					if (connection instanceof GeneralRelationInstance)	{
						undoAction.addAction(new UndoableAddGeneralRelation((OpdProject)this.system, (GeneralRelationInstance)connection));
					}
					else if (connection instanceof FundamentalRelationInstance)	{
						undoAction.addAction(new UndoableAddFundamentalRelation((OpdProject)this.system, (FundamentalRelationInstance)connection));
					}									
				}
				if (roleThing instanceof IObjectEntry)	{
					undoAction.addAction(new UndoableAddObject((OpdProject)this.system, (ObjectInstance)newInstance));
				}
				else if (roleThing instanceof IProcessEntry)	{
					undoAction.addAction(new UndoableAddProcess((OpdProject)this.system, (ProcessInstance)newInstance));
				}
				Opcat2.getUndoManager().undoableEditHappened(new UndoableEditEvent(this.system, undoAction));
	...
	protected IXThingInstance createThing(IThingEntry roleThing)	{
		...
		IXThingInstance parent = (IXThingInstance)this.instance.getParentIThingInstance();
		if (roleThing instanceof IObjectEntry)	{
			if (parent != null)	{
				newInstance = this.system.addObject(this.renderName(roleThing), x, y, parent);
			}
			else	{
				newInstance = this.system.addObject(this.renderName(roleThing), x, y, opdID);		   
			}
		}
		else if (roleThing instanceof IProcessEntry){
			if (parent != null)	{
				newInstance = this.system.addProcess(this.renderName(roleThing), x, y, parent);
			}
			else	{
				newInstance = this.system.addProcess(this.renderName(roleThing), x, y, opdID);
			}
		}
		return newInstance;
	...
	protected IXInstance createConnection(IXThingInstance newInstance)	{
		...
		if ((type < 300) && (type >= 200))	{
			if (direction == Finder.DESTINATION_DIRECTION)	{
				connection = this.system.addRelation((IXConnectionEdgeInstance)this.instance, (IXConnectionEdgeInstance)newInstance, this.neighbor.getConnectionType());
			}
			else if (direction == Finder.SOURCE_DIRECTION)	{
				connection = this.system.addRelation((IXConnectionEdgeInstance)newInstance, (IXConnectionEdgeInstance)this.instance, this.neighbor.getConnectionType());
			}
		}
		//Handling links
		if ((type >= 300) && (type < 400))	{
			if (direction == Finder.DESTINATION_DIRECTION)	{
				connection = this.system.addLink((IXConnectionEdgeInstance)this.instance, (IXConnectionEdgeInstance)newInstance, this.neighbor.getConnectionType());
			}
			else if (direction == Finder.SOURCE_DIRECTION)	{
				connection = this.system.addLink((IXConnectionEdgeInstance)newInstance, (IXConnectionEdgeInstance)this.instance, this.neighbor.getConnectionType());
			}
		}
		return connection;
	}
