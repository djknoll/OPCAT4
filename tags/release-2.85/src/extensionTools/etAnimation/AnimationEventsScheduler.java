...
class AnimationEventsScheduler extends Hashtable {
	...
	public void scheduleEvent(AnimationEvent aEvent) {
		AnimationAuxiliary.debug(this, "scheduleEvent getSchedulingStep = "
			...
			for (int i = v.size() - 1; i >= 0; i--) {
				...
				if (aEvent.getSchedulingStep() > schedulingStep) {
					if ((executionId == 0)
						...
						AnimationAuxiliary.debug(this, "remove event for step "
								+ aEvent.getStep());
						AnimationEvent aRemovedEvent = (AnimationEvent) v
								.remove(i);
	...
	public void print() {
		...
		for (; en.hasMoreElements();) {
			Vector v = (Vector) en.nextElement();
			if (!v.isEmpty()) {
				AnimationAuxiliary.debug(this, "Step: "
						+ ((AnimationEvent) v.firstElement()).getStep());
			}
			// print the events stored inside teh vector
			for (int i = 0; i < v.size(); i++) {
				AnimationEvent aEvent = (AnimationEvent) v.get(i);
				if (aEvent.getEventType() == AnimationSettings.ACTIVATION_EVENT) {
					AnimationAuxiliary.debug(this, "#" + i
							+ ": Activation event");
				} else {
					AnimationAuxiliary.debug(this, "#" + i
							+ ": Termination event");
				}
				AnimationAuxiliary.debug(this, " was issued on "
						+ aEvent.getSchedulingStep());
			}
		}
	}
	...
	public AnimationEvent futureEventsScheduled(long currentStep,
			long executionId) {
