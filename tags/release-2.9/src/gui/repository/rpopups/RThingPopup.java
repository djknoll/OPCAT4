...
public class RThingPopup extends RDefaultPopup {
	...
	Action showAction = new AbstractAction("Focus On Thing",
		...
		public void actionPerformed(ActionEvent e) {
			...
			Opd myOpd = RThingPopup.this.myProject.getComponentsStructure().getOpd(opdId);
			RThingPopup.this.showOpd(myOpd);
			RThingPopup.this.myProject.removeSelection();
			RThingPopup.this.myProject.addSelection(RThingPopup.this.myInstance, true);
	...
	Action deleteAction = new AbstractAction("Delete", StandardImages.DELETE) {
		...
		public void actionPerformed(ActionEvent e) {
			...
			CheckResult cr = CheckModule.checkDeletion(RThingPopup.this.myInstance, RThingPopup.this.myProject);
			...
			for (Enumeration e1 = RThingPopup.this.myInstance.getRelatedInstances(); e1
					.hasMoreElements();) {
				Instance currInstance = (Instance) e1.nextElement();
				undoAction.addAction(RThingPopup.this.myProject.deleteInstance(currInstance));
			}

			undoAction.addAction(RThingPopup.this.myProject.deleteInstance(RThingPopup.this.myInstance));
			...
			RThingPopup.this.myProject.getCurrentOpd().getDrawingArea().repaint();
	...
	Action zoomInAction = new AbstractAction("In-Zooming", StandardImages.EMPTY) {
		...
		public void actionPerformed(ActionEvent e) {
			try {
				Opd myOpd = RThingPopup.this.myProject.getComponentsStructure().getOpd(
						RThingPopup.this.myThing.getOpdId());
				RThingPopup.this.myProject.removeSelection();
				RThingPopup.this.myProject.addSelection(RThingPopup.this.myInstance, true);
				RThingPopup.this.myThing.setSelected(true);
				RThingPopup.this.centerView(myOpd, RThingPopup.this.myInstance);
	...
	Action propertiesAction = new AbstractAction("Properties",
		...
		public void actionPerformed(ActionEvent e) {
			RThingPopup.this.myThing.callPropertiesDialog(BaseGraphicComponent.SHOW_ALL_TABS,
					BaseGraphicComponent.SHOW_ALL_BUTTONS);
			RThingPopup.this.myProject.getComponentsStructure().getOpd(RThingPopup.this.myThing.getOpdId())
