...
public abstract class MetaDataAbstractItem implements MetaDataItemInterface {
	...
	public MetaDataAbstractItem(String name, long id) {
		this.id = id;
		this.name = name;
		this.extID = Long.toString(id) ;
	}
	
	public MetaDataAbstractItem(String name, String extID) {
		...
		while (iter.hasNext()) {
			...
			if ((entry instanceof Entry)) {
				...
				while (rolesIter.hasNext()) {
					...
					MetaDataAbstractItem tempReq = role.getThing(); 
					
					try {
						String roleProjectID = role.getOntology().getGlobalID();
						if (roleProjectID.toString().equalsIgnoreCase(
								myRole.getOntology().getGlobalID())) {
