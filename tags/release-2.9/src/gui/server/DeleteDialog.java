...
public class DeleteDialog extends JDialog {
	...
	public DeleteDialog(LoadParameters parameters) {
		super(Opcat2.getFrame(), "Delete Project", true);

		this.param = parameters;
		this.loadTable = new LoadTable(this.param.length());

		for (int i = 0; i < this.param.length(); i++) {
			this.loadTable.setValueAt(
					new ProjectNameId1(this.param.getProjectName(i), i), i, 0);
			this.loadTable.setValueAt(this.param.getProjectCreator(i), i, 1);
			this.loadTable.setValueAt(this.param.getCreationDate(i), i, 2);
		}
		this.loadButton = new JButton("Delete");
		this.cancelButton = new JButton("Cancel");
		this.loadButton.addActionListener(new DeleteListener());
		...
		KeyListener escapeAndEnterListener = new KeyAdapter() {
			public void keyReleased(KeyEvent e) {
				...
				if (e.getKeyCode() == KeyEvent.VK_ENTER) {
					try {
						DeleteDialog.this.param.setReturnedPId(DeleteDialog.this.loadTable.getSelectedRow());
						(DeleteDialog.this).dispose();
					} catch (Exception e1) {
						JOptionPane.showMessageDialog(DeleteDialog.this,
	...
	class DeleteListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			try {
				ProjectNameId1 pnid;
				pnid = (ProjectNameId1) DeleteDialog.this.loadTable.getValueAt(DeleteDialog.this.loadTable
						.getSelectedRow(), 0);
				DeleteDialog.this.param.setReturnedPId(pnid.getId());
				(DeleteDialog.this).dispose();
			} catch (Exception e1) {
				JOptionPane.showMessageDialog(DeleteDialog.this,
						"Choose the project to delete");
	...
	public class LoadTable extends JPanel {
		...
		public LoadTable(int myRows) {
			...
			Action emptyAction = new AbstractAction() {
				 ...
				 * 
				 */
				private static final long serialVersionUID = 3098390408750664416L;
		...
		class LoadTableModel extends AbstractTableModel {
			...
			final String[] columnNames = { "Project Name", "Author", "Created" };
...
class ProjectNameId1 {
	private String name;

	private int id;

	public ProjectNameId1(String projectName, int projectId) {
