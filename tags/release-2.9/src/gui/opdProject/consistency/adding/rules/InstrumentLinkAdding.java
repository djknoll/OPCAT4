...
public class InstrumentLinkAdding extends ConsistencyAbstractChecker implements ConsistencyCheckerInterface {
	...
	public InstrumentLinkAdding (ConsistencyOptions options, ConsistencyResult results) {
	...
	public void check() {
		...
		while (addedEnum.hasNext()) {
			...
			if((entry instanceof OpmInstrument) || (entry instanceof OpmConditionLink) || (entry instanceof OpmConsumptionLink) || (entry instanceof OpmEffectLink)) {
				OpmProceduralLink addedLink = null ; 
				if (entry instanceof OpmInstrument) {
					addedLink = (OpmInstrument) addedIns.getEntry().getLogicalEntity() ;
				}
				if (entry instanceof OpmConditionLink) {
					addedLink = (OpmConditionLink) addedIns.getEntry().getLogicalEntity() ;
				}
				if(entry instanceof OpmConsumptionLink) {
					addedLink = (OpmConsumptionLink) addedIns.getEntry().getLogicalEntity() ;
				}
				if(entry instanceof OpmEffectLink) {
					addedLink = (OpmEffectLink) addedIns.getEntry().getLogicalEntity() ;
				}				
				LinkInstance linkIns = (LinkInstance) addedIns ; 
				
				if(addedLink != null ) { 
					OpdConnectionEdge source = linkIns.getSource() ;
					if ((linkIns.getSourceInstance() instanceof StateInstance) && !((StateEntry) source.getEntry()).isInitial() ) {
						Enumeration sourceEnum  = source.getEntry().getInstances() ;
						while (sourceEnum.hasMoreElements() && !found) {
							ConnectionEdgeInstance connectionEdgeInstance = (ConnectionEdgeInstance) sourceEnum.nextElement() ;
							ConnectionEdgeEntry connectionEdgeEntry = (ConnectionEdgeEntry) connectionEdgeInstance.getEntry() ;
							Enumeration linksEnum = connectionEdgeEntry.getDestinationLinks() ; 
							while (linksEnum.hasMoreElements() && !found) {
								OpmProceduralLink link = (OpmProceduralLink) linksEnum.nextElement() ; 							
						...
						if (!found ) {
							...
							insArray[0] = addedIns ; 
							ConsistencyAction action = new ConsistencyAction(ConsistencyAction.INSTRUMENT_LINKS_ADDITION, 
															linkIns.getSourceInstance().getOpd(), 
															linkIns.getSourceInstance(), 
															insArray) ;    
	...
	public void deploy(ConsistencyResult checkResult) {
		...
		for(int i = 0 ; i < results.size(); i++) {
			...
			if (action.getMyType() == ConsistencyAction.INSTRUMENT_LINKS_ADDITION) { 		
				JOptionPane.showMessageDialog(this.getMyOptions().getProject().getMainFrame(),
