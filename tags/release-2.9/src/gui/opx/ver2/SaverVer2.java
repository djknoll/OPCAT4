...
public class SaverVer2 implements SaverI {
	...
	protected Element _saveObjectSection() throws JDOMException {
		...
		for (Enumeration e = this.myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof ObjectEntry) {
				...
				//end reuse comment
				//Accepts list!!			
				//OpcatLogger.logError(currentEntry.getName()) ; 
	...
	protected Element _saveProcessSection() throws JDOMException {
		...
		for (Enumeration e = this.myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof ProcessEntry) {
				...
				this._saveLogicalProcess((ProcessEntry) currentEntry, processSect);
			}
	...
	protected Element _saveRelationSection() throws JDOMException {
		...
		for (Enumeration e = this.myProject.getComponentsStructure().getElements(); e
			...
			if (currentEntry instanceof RelationEntry) {
				...
				//end reuse comment
				this._saveLogicalRelation((RelationEntry) currentEntry, relationSect);
			}
	...
	protected Element _appendOpdChildren(long opdNum,
			...
			throws JDOMException {
		Hashtable parentChildren = this._prepareParentChildrenDependencies(opdNum);

		if (opdNum != ROOT_OPD) {
			opd.addContent(this._saveMainEntity(myOpd.getMainInstance(), opdNum,
					parentChildren));
		}
	...
	protected Element _saveMainEntity(ThingInstance mainInstance, long opdNum,
		...
		mainEntity.addContent(this._saveVisualThing(mainInstance, parentChildren));
	...
	protected Hashtable _prepareParentChildrenDependencies(long opdNum) {
		...
		for (Enumeration e = this.myProject.getComponentsStructure().getThingsInOpd(
	...
	protected Element _saveVisualThing(ThingInstance tInstance,
		...
		if (tInstance instanceof ProcessInstance) {
			tData.addContent(this._saveVisualProcess((ProcessInstance) tInstance));
		} else {
			tData.addContent(this._saveVisualObject((ObjectInstance) tInstance));
		}
		...
		if (childrenList != null) {
			...
			for (Iterator it = childrenList.iterator(); it.hasNext();) {
				xmlChildrenList.add(this._saveVisualThing((ThingInstance) it.next(),
						parentChildren));
			}
	...
	protected Element _saveVisualObject(ObjectInstance obj)
		...
		for (Enumeration e = obj.getStateInstances(); e.hasMoreElements();) {
			statesList.add(this._saveVisualState((StateInstance) e.nextElement()));
		}
	...
	protected Element _saveInstanceAttr(Instance ins) throws JDOMException {
		...
		xmlInstance.setAttribute("textColor", Integer.toString(ins
				.getTextColor().getRGB()));
		xmlInstance.setAttribute("isVisible", Boolean.toString(ins.isVisible()));		
		return xmlInstance;
	}
	...
	protected Element _saveFundamentalRelationSection(long opdNum)
		...
		for (Enumeration e = this.myProject.getComponentsStructure()
			...
			relList.add(this._saveCommonPart((GraphicalRelationRepresentation) e
					.nextElement()));
		}
	...
	protected Element _saveCommonPart(GraphicalRelationRepresentation rel)
		...
		for (Enumeration e = rel.getInstances(); e.hasMoreElements();) {
			insList
					.add(this._saveVisualFundamentalRelation((FundamentalRelationInstance) e
							.nextElement()));
		}
	...
	protected Element _saveVisualFundamentalRelation(
		...
		xmlRelation.setAttribute("isVisible", Boolean.toString(rel.isVisible())) ; 			
	...
	protected Element _saveGeneralRelationSection(long opdNum)
		...
		for (Enumeration e = this.myProject.getComponentsStructure()
			...
			relList.add(this._saveVisualGeneralRelation((GeneralRelationInstance) e
					.nextElement()));
		}
	...
	protected Element _saveVisualLinkSection(long opdNum) throws JDOMException {
		...
		for (Enumeration e = this.myProject.getComponentsStructure().getLinksInOpd(
				opdNum); e.hasMoreElements();) {
		...
		if (children != null) {
			for (Iterator it = children.iterator(); it.hasNext();) {
				...
				if (currOpd.getParentOpd().getOpdId() == opdNum) {
					ThingInstance parentInstance = this._getParentInstance(currOpd);
