...
public class OpcatSkinManager {
	...
	private class a implements Runnable {
		...
		public synchronized void run() {
			SwingUtilities.updateComponentTreeUI(_fldif);
		}
...
//	        b._mthif();
	...
	public String[] getThemes() {
		j j1 = new j();
		for (Iterator iterator = this.a.keySet().iterator(); iterator.hasNext(); j1
				.a(iterator.next().toString())) {
			;
		}
		for (Iterator iterator1 = this._fldbyte.keySet().iterator(); iterator1
				.hasNext(); j1.a(iterator1.next().toString())) {
			;
		}
		return j1.a();
	}
	...
	private void a() {
		...
		for (int i = 0; i < alookandfeelinfo.length; i++) {
			try {
				Class class1 = Class
						.forName(alookandfeelinfo[i].getClassName());
		...
		try {
			...
			this.a("mmMagra-X", "mmMagra-Xthemepack.zip");
	...
	private boolean a(String s) {
		...
		try {
			if (s.startsWith("skin")) {
				...
				if (class1 != null) {
					LookAndFeel lookandfeel1 = (LookAndFeel) class1
							.newInstance();
 ...
 * 
 * DECOMPILED FROM: /home/raanan/My
 * Documents/Projects/Opcat/Opcat/trunk/lib/tablelib.jar
 * 
 * 
 * TOTAL TIME: 23 ms
 * 
