	...
	public void load(MetaLibrary onto) throws MetaException,
		...
		if (onto != null) {
			if ((onto.getType() == LibraryTypes.MetaLibrary)) {
				...
				Entry entry = (Entry) metaStruct
						.getIEntry(this.getThingId());
				if (entry != null) {
					MetaDataAbstractItem thing = (MetaDataAbstractItem) entry.getLogicalEntity();
					this.load(thing, onto);
			...
			if ((onto.getType() == LibraryTypes.MetaData)) {
				MetaDataProject reqProject = (MetaDataProject) onto
						.getProjectHolder();
				MetaDataAbstractItem req = reqProject.getMetaDataItem(this.getThingId());
				if (req != null) {
					this.load(req, onto);
				} 
			}
