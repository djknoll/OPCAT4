...
class UseCaseDiagram {
	...
	// -------------------------------- U S E C A S E - D I A G R A M - C R E A
	// T E -----------------------------------
	/**
	 ...
	 *            file
	 * @param Vector
	 *            usecaseVec - Vector which contains all the Diagram Elements of
	 *            usecase for outside printing in XML
	 * @param Vector
	 *            usecaseIncludeVec - Contains all associations need to be
	 *            printed as an Include
	 * @param Vector
	 *            usecaseInzoomIncludeVec - Contains all Inzoom associations
	 *            need to be printed as an Include
	 * @param Vector
	 *            usecasespecVec - Contains all Specialization Relations that
	 *            need to be printed as an associations
	 * @param Vector
	 *            usecaseassVec - Contains all associations to be printed
	 * @param Vector
	 *            usecaseassEndVec - Contains all associations End to be printed
	 * @param Vector
	 *            usecaseSterIncludeVec - Contains the printing of "Stereotype"
	 *            Include in XML
	 * @param Vector
	 *            actorTagsVec - Contains all the TaggedValue of usecase for
	 *            outside printing in XML
	 * @param Vector
	 *            usecaseExtendVec - Contains all associations need to be
	 *            printed as an Extend
	 * @param int
	 *            op - represents the choice of a user: Root Level Only-->op=1;
	 *            All OPD Levels-->op=2.
	 * @param int
	 *            levels - number of levels that the user have chosen.
	 */
	public void createUseCaseDiagram(ISystem sys, XmiWriter writer, Vector<String> usecaseVec, Vector<IRelationEntry> usecaseIncludeVec,
			Vector<IProcessEntry> usecaseInzoomIncludeVec, Vector<String> usecasespecVec, Vector<String> usecaseassVec,
			Vector<String> usecaseassEndVec, Vector<String> usecaseSterIncludeVec, Vector<String> actorTagsVec, Vector<ILinkEntry> usecaseExtendVec,
			int op, int levels) throws IOException {
		try {
			this.mySys = sys;
			this.writer = writer;
			...
			Vector<IObjectEntry> actorsList = new Vector<IObjectEntry>(4, 2); // list of "actors"
			Vector<IRelationEntry> generalizationVec = new Vector<IRelationEntry>(4, 2);
			Vector<IEntry> OpdVec = new Vector<IEntry>(4, 2);

			// ---------------------------------------------------------------------
			// printing of the beggining of the Use Case Diagram
			temp = "<!--  ===============[UseCase Model] ====================  --> \n";
			temp += "<UML:Model xmi.id=\"G.0000.1\" name=\"Use Case Model\" visibility=\"public\" isSpecification=\"false\" isRoot=\"false\" isLeaf=\"false\" isAbstract=\"false\" namespace=\""+ DiagramsCreator.MAIN_MODEL_NAME +"\">\n";
			...
			if (op == 2) { // all opd levels WERE SELECTED BY THE USER
				...
				while (e.hasMoreElements()) {
					...
					if (abstractEntry instanceof IProcessEntry) { // if
						...
						;
						if (this.isUsecase(proc, MyStructure)) { // if the
							// proc is
							// use case
							this
									.generateUseCase(MyStructure, writer, proc, usecaseVec, usecaseExtendVec, usecaseIncludeVec, usecaseInzoomIncludeVec, generalizationVec, usecasespecVec, OpdVec, op);
						}// end of if(IsUsecase)
					}// end of if(proc)
					if (abstractEntry instanceof IObjectEntry) { // if object
						...
						if (obj.isEnvironmental()) {// if actor
							this.generateActor(MyStructure, writer, obj, actorTagsVec, generalizationVec);
			...
			new_level = levels - 1;
			if (op == 1) { // USER SELECTED THE OPD-ROOT OPTION OR UNTIL LEVEL
				// OPTION
				...
				while (e.hasMoreElements()) {
					...
					if (inst instanceof IObjectInstance) {
						...
						actorsList.addElement((IObjectEntry) objInst.getIEntry());
					}
				}
				e = MyStructure.getInstancesInOpd(root.getOpdId()); // all
				// things in root opd
				while (e.hasMoreElements()) {
					...
					if (inst instanceof IProcessInstance) { // if it is a process instance
						...
						createProcessVector(MyStructure, (IProcessEntry) (procInst.getIEntry()), OpdVec, new_level);
						CreateActorsVector(MyStructure, writer, (IProcessEntry) (procInst.getIEntry()), actorsList, new_level);
					}
				}
				...
				if (abstractEntry instanceof IRelationEntry) { // if relation
					...
					if (abstractEntry instanceof IObjectEntry) {// if source is
						...
						if (obj.isEnvironmental()) { // if an actor
							if (((op == 1) && (this.IsInVec(obj, actorsList))) || (op == 2)) {
								...
								if (abstractEntry instanceof IProcessEntry) {// if
									...
									proc = (IProcessEntry) abstractEntry;
									if (this.isUsecase(proc, MyStructure)) {
										if (((op == 1) && (this.IsInVec(proc, OpdVec))) || (op == 2)) {
											if (this.assoc_needed(rel, proc, MyStructure, OpdVec, op)) {
												usecaseassVec.addElement("" + rel.getId() + "");
												usecaseassEndVec.addElement("" + rel.getId() + ".1");
												usecaseassEndVec.addElement("" + rel.getId() + ".2");
												generateAssociation(rel.getId(), rel.getName(), obj.getId(), proc.getId(), rel.getSourceCardinality(), rel
					...
					if (abstractEntry instanceof IProcessEntry) {// if source
						// is a
						// process
						proc = (IProcessEntry) abstractEntry;
							...
							if (((op == 1) && (this.IsInVec(proc, OpdVec))) || (op == 2)) {
								...
								if (abstractEntry instanceof IObjectEntry) {// if
									...
									if (obj.isEnvironmental()) { // if an //
										// actor
										if (((op == 1) && (this.IsInVec(obj, actorsList))) || (op == 2)) {
											if (this.assoc_needed(proc, rel, MyStructure, OpdVec, op)) {
												usecaseassVec.addElement("" + rel.getId() + "");
												usecaseassEndVec.addElement("" + rel.getId() + ".1");
												usecaseassEndVec.addElement("" + rel.getId() + ".2");
												// printing
												generateAssociation(rel.getId(), rel.getName(), proc.getId(), obj.getId(), rel.getSourceCardinality(), rel
														.getDestinationCardinality());
											}
										}
						...
						if (obj.isEnvironmental()) { // if an actor
							if (((op == 1) && (this.IsInVec(obj, actorsList))) || (op == 2)) {
								...
								if (abstractEntry instanceof IProcessEntry) {// if
									...
									if (this.isUsecase(proc, MyStructure)) {
										if (((op == 1) && (this.IsInVec(proc, OpdVec))) || (op == 2)) {
											if (this.assoc_needed(link, proc, MyStructure, OpdVec, op)) {
												usecaseassVec.addElement("" + link.getId() + "");
												usecaseassEndVec.addElement("" + link.getId() + ".1");
												usecaseassEndVec.addElement("" + link.getId() + ".2");
												// printing
												generateAssociation(link.getId(), link.getName(), obj.getId(), proc.getId(), "1", "1");
											}
										}
									}// end of if(destination is a usecase)
								}// end of if(destinatin is a process)
							...
							if (((op == 1) && (this.IsInVec(proc, OpdVec))) || (op == 2)) {
								...
								if (abstractEntry instanceof IObjectEntry) {// if destination is an object
									...
									if (obj.isEnvironmental()) { // if an
										// actor
										if (((op == 1) && (this.IsInVec(obj, actorsList))) || (op == 2)) {
											if (this.assoc_needed(proc, link, MyStructure, OpdVec, op)) {
												usecaseassVec.addElement("" + link.getId() + "");
												usecaseassEndVec.addElement("" + link.getId() + ".1");
												usecaseassEndVec.addElement("" + link.getId() + ".2");
												// printing
												generateAssociation(link.getId(), link.getName(), proc.getId(), obj.getId(), "1", "1");
											}
										}
									}// end if(destination is an actor)
								}// end if(destination is an object)
			...
			while (e.hasMoreElements()) {
				...
				temp = "<!--  ==================  [Association] ====================  --> ";
				writer.write(temp.getBytes());
				temp = "<UML:Association xmi.id=\"G." + tempRel.getId()
			...
			while (e.hasMoreElements()) {
				...
				temp = "<!--  ==================  [Association] ====================  --> ";
				writer.write(temp.getBytes());
				temp = "<UML:Association xmi.id=\"G." + tempProc.getId()
			...
			while (e.hasMoreElements()) {
				...
				temp = "<!--  ==================  [Association] ====================  --> ";
				writer.write(temp.getBytes());
				temp = "<UML:Association xmi.id=\"G." + tempLink.getId() + "\" name=\"" + tempName
	...
	private boolean assoc_needed(IRelationEntry rel, IProcessEntry proc, ISystemStructure MyStructure, Vector OpdVec, int op) {
		...
		while (e.hasMoreElements()) {
			...
			if (abstractEntry instanceof IRelationEntry) {// if relation
				...
				if ((rel.getSourceId() == rel_1.getSourceId()) && (rel.getRelationType() == rel_1.getRelationType())
						&& (rel.getId() != rel_1.getId())) {
					// the same source, the same relation type, but different
					// relation
	...
	private boolean assoc_needed(IProcessEntry proc, IRelationEntry rel, ISystemStructure MyStructure, Vector OpdVec, int op) {
		...
		while (e.hasMoreElements()) {
			...
			if (abstractEntry instanceof IRelationEntry) {
				rel_1 = (IRelationEntry) abstractEntry;
				if ((rel.getDestinationId() == rel_1.getDestinationId()) && (rel.getRelationType() == rel_1.getRelationType())
						&& (rel.getId() != rel_1.getId())) {
					e1 = proc.getInstances();
					while (e1.hasMoreElements()) {
						procInst = (IProcessInstance) e1.nextElement();
	...
	private boolean assoc_needed(IProcessEntry proc, ILinkEntry link, ISystemStructure MyStructure, Vector OpdVec, int op) {
		...
		while (e.hasMoreElements()) {
			...
			if (abstractEntry instanceof ILinkEntry) {
				link_1 = (ILinkEntry) abstractEntry;
				if ((link.getDestinationId() == link_1.getDestinationId()) && (link.getLinkType() == link_1.getLinkType())
	 ...
	 * @param Vector
	 *            usecaseVec - responsible for outside printing of usecases
	 *            (Diagram Elements) in XML file
	 * @param Vector
	 ...
	 *            associations in XML file
	 * @param Vector
	 *            usecaseInzoomIncludeVec - responsible of printing associations
	 *            created by Inzoom, in XML file
	 * @param Vector
	 *            generalizationVec - responsible of printing associations
	 *            created by Generalization in XML file
	 * @param Vector
	 *            usecasespecVec - responsible of printing associations created
	 *            by Specialization in XML file
	 * @param Vector
	 *            OpdVec - includes all the process that should be converted to
	 *            usecase
	 * @param int
	 *            op - represents the choice of a user: Root Level Only-->op=1;
	 *            All OPD Levels-->op=2.
	 */
	private void generateUseCase(ISystemStructure MyStructure, XmiWriter file, IProcessEntry proc, Vector<String> usecaseVec,
			Vector<ILinkEntry> usecaseExtendVec, Vector<IRelationEntry> usecaseIncludeVec, Vector<IProcessEntry> usecaseInzoomIncludeVec,
			Vector<IRelationEntry> generalizationVec, Vector<String> usecasespecVec, Vector OpdVec, int op) throws IOException {
		...
		temp = "<UML:UseCase xmi.id=\"S." + proc.getId() + "\" name=\"" + proc.getName() + "\" visibility=\"" + this.GetScope(proc)
		...
		if (flag == 1) {
			...
			while (g_list.hasMoreElements()) {
				...
				if (relx.getDestinationId() == proc.getId()) {
					temp = "<UML:Generalization xmi.id=\"G." + relx.getId()
							+ "\" name=\"\" visibility=\"public\" discriminator=\"\" child=\"S." + relx.getDestinationId()
							+ "\" parent=\"S." + relx.getSourceId() + "\" /> ";
					file.write(temp.getBytes());
				}
			}
			temp = "</UML:Namespace.ownedElement>";
	...
	// ------------------------------------- C R E A T E - P R O C - A C T - V E
	// C ---------------------------------------------------------
	/**
	 * Recursive function that creates a Vector which contains all the "UseCase"
	 * in the system
	 * 
	 * @param ISystemStructure
	 *            MyStructure
	 ...
	 * @param int
	 *            level - number of levels to "go down" in the Inzoomed process
	 */
	private void createProcessVector(ISystemStructure MyStructure, IProcessEntry proc, Vector<IEntry> OpdVec, int level) {
		...
		OpdVec.addElement(proc); // the process is added to the opdVec->it
					...
					while (childList.hasMoreElements()) { // going throw all
						...
						if (new_level >= 0) {// number of level to go down
							...
							if (thing instanceof IProcessInstance) { // if
								// the "child" is a process--> it is an use case (will be added to the vector in the next step)
								procInst = (IProcessInstance) thing;
								this.createProcessVector(MyStructure, (IProcessEntry) (procInst.getIEntry()), OpdVec, new_level); // recurtion
							}
						}
					}
				} else {
	...
	// C-------------------------------------------
	/**
	 * Recursive function that creates a Vector which contains all the "Actors"
	 * in the system
	 * 
	 * @param ISystemStructure
	 *            MyStructure
	 ...
	 *            level - number of levels to "go down" in the Inzoomed process
	 */
	private void CreateActorsVector(ISystemStructure MyStructure, XmiWriter file, IProcessEntry proc, Vector<IObjectEntry> actorsList, int level)
			throws IOException {
			...
			while (e.hasMoreElements()) {
				...
				while (childList.hasMoreElements()) { // going throw all
					...
					if (thing instanceof IProcessInstance) {// if the
						...
						// -->again
						procInst = (IProcessInstance) thing;
						this.CreateActorsVector(MyStructure, file, (IProcessEntry) (procInst.getIEntry()), actorsList, new_level); // recurtion
					}
				}
			}
		} else {
	 ...
	 *            file
	 * @param IObjectEntry
	 *            obj - an object which represent an "Actor" in USECASE DIAGRAM
	 * @param Vector
	 *            actorTagsVec - used for outside printing of the usecase
	 *            diagram part in XML file
	 * @param Vector
	 *            generalizationVec - cointains all generalization relations
	 *            between "actors"
	 */
	private void generateActor(ISystemStructure MyStructure, XmiWriter file, IObjectEntry obj, Vector<String> actorTagsVec,
		...
		temp += "<UML:Actor xmi.id=\"S." + obj.getId() + "\" name=\"" + obj.getName() + "\" visibility=\"" + this.GetScope(obj)
	...
	// -----------------------------------------
	/**
	 * Functions returns true if the given IEntry entry is already inside the
	 * given Vector vec.
	 * 
	 * @param IEntry
	 *            entry
	 ...
	 */
	private void generateAssociation(long associationId, String associationName, long sourceId, long destId, String sourceCardinality,
		...
		temp = "<!--  ==================  [Association] ====================  --> \n";
		temp += "<UML:Association xmi.id=\"G." + associationId + 
			"\" name=\"" + associationName	+ "\" visibility=\"public\" isRoot=\"false\" isLeaf=\"false\" isAbstract=\"false\">\n";
		temp += "<UML:Association.connection>\n";
		temp += "<!--  ================== [AssociationEnd] =================  --> \n";
		temp += "<UML:AssociationEnd xmi.id=\"G."
				+ associationId
		...
		temp += "<UML:Multiplicity>\n";
		temp += "<UML:Multiplicity.range>\n";
		temp += "<UML:MultiplicityRange lower=\"1\" upper=\"" + sourceCardinality + "\" />\n";
		temp += "</UML:Multiplicity.range>\n";
		temp += "</UML:Multiplicity>\n";
		temp += "</UML:AssociationEnd.multiplicity>\n";
		temp += "</UML:AssociationEnd>\n";
