...
public class TestingSystem extends Object implements Testing {
    ...
    private TestingEntry getTestingElementByIXEntryID(long ixEntryID) {
	...
	if (!hasActivation && !hasTerminations) {
		...
		if (this.mySys.getCurrentIXOpd().getMainIXEntry() != null) {
			...
			if (aEntry.getIXEntry().getId() != this.mySys
			    ...
			    // aEntry.setTaxtualInfo("Inconsistent Model,
			    // Parent
			    // Process does nothing Or not consistent with
			    // upper
			    // levels ?");
			} else {
			    aEntry.setTaxtualInfo(
				    "Waiting for time limits or events", 1);
			    this
				    .TestingGridAddRow(
