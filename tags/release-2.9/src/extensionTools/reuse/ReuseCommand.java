...
public class ReuseCommand {
    ...
    public ReuseCommand(OpdProject _current, OpdProject _reused, int _reuseType) {
        ...
        this.currentProject.reusedProject = _reused;
    ...
    public void commitReuse() throws OpcatException {
        ...
        this.addReusedOpd(this.reusedBaseOpd, this.currentProject.getCurrentOpd(), designatedContainer, forceContainer);    	
        ...
        this.currentProject.reusedProject = null;
        ...
        try	{
	        ...
	        while (e.hasMoreElements()) {
	            ...
	            if ((generalInstance instanceof ObjectInstance)
	                ...
	                if (generalInstance instanceof ObjectInstance) {
	                    addedReusedInstance = this.addReusedObject(generalInstance, x, y, targetOpd, theContainer);
	                    //Adding process
	                } else if (generalInstance instanceof ProcessInstance) {
	                    addedReusedInstance = this.addReusedProcess(generalInstance, x, y, targetOpd, theContainer);
	                } else {
	                ...
	                if ((myEntry.getZoomedInOpd() != null)
	                    ...
	                    this.createZoominLevels((ThingInstance) generalInstance, addedReusedInstance, targetOpd);
	                }
	                //Creates unfolding levels
	                if (thingInstance.getUnfoldingOpd() != null) {
	                    this.createUnfoldingLevels((ThingInstance) generalInstance, addedReusedInstance, targetOpd);
	                }
	            }
	        }
	        // end of first loop
	        ...
	        e = this.reusedProject.getSystemStructure().getInstancesInOpd(reusedOpd
	                .getOpdId());
	        this.currentProject.setCurrentOpd(targetOpd);
	        while (e.hasMoreElements()) {
	            generalInstance = (Instance) e.nextElement();
        ...
        try {
            ...
            //Adding things for the first time
                ...
                if (container != null) {
                    ...
                    addedObjectInstance =  this.currentProject
                            .addObject(x, y,  container.getThing(), -1, -1, false);
                } else {
                ...
                this.copyReusedObjectEntryProperties((ObjectEntry) reusedObjectInstance
            ...
            addedObjectInstance.setSize(i.getConnectionEdge().getWidth(), i.getConnectionEdge().getHeight());            
            addedObjectInstance.setLocation(i.getConnectionEdge().getX(), i.getConnectionEdge().getY());
            //addedObjectInstance.update();
            
            //Adding the states            
            this.addReusedStates(addedObjectInstance, (ObjectInstance)reusedObjectInstance, targetOpd);            
            ...
            //if (addedObjectInstance.getStateInstances().hasMoreElements() &&  addedObjectInstance.isStatesAutoArranged())	{
    		//	oce.fitToContent();
            //}
    ...
    protected ProcessInstance addReusedProcess(Instance reusedProcessInstance, int x, int y, Opd current_opd, ThingInstance container) throws OpcatException {
        ...
        try {
            ...
            this.copyReusedProcessEntryProperties(  (ProcessEntry)reusedProcessInstance.getEntry() , (ProcessEntry) addedProcessInstance.getEntry());            
    ...
    protected void addReusedStates(ObjectInstance addedObject, ObjectInstance reusedFrom, Opd targetOpd) {
        ...
        while (e.hasMoreElements()) {
            ...
            if (!this.existReusedEntry(addedReusedState.getEntry())) {
                addedState = (Entry) this.currentProject.addState(addedReusedState
                        .getEntry().getName(), addedObject);
                
                this.reusedEntriesTable.put(addedReusedState.getEntry(), addedState);
                //Copy entry properties
                this.copyReusedStateEntryProperties((StateEntry) addedReusedState
            ...
            // if this state has been already entered
                ...
                instance = this.currentProject
                        .addStateInstance(targetOpd, addedObject, (StateEntry) this.reusedEntriesTable
            ...
            targetStateInstance.getConnectionEdge().setSize(addedReusedState.getConnectionEdge().getWidth(), addedReusedState.getConnectionEdge().getHeight());
            targetStateInstance.getConnectionEdge().setLocation(addedReusedState.getConnectionEdge().getX(), addedReusedState.getConnectionEdge().getY());
    ...
    protected void createZoominLevels(ThingInstance reusedInstance, ThingInstance addedInstance, Opd target) throws OpcatException 
    {
        ...
        try {
            ...
            Opd newOpd = this.currentProject.createNewOpd(true, addedInstance, target);
...
//        reusedProject.setCurrentOpd(zoomIn);
            ...
            this.currentProject.getParentDesktop().getDesktopManager().deiconifyFrame(
    ...
    protected void createUnfoldingLevels(ThingInstance reusedInstance, ThingInstance addedInstance, Opd target) throws OpcatException {
        ...
        try {
            ...
            if (!this.isOpdReused(unfold)) {
                //Performing the creation of inner levels
                Opd newOpd = this.currentProject.createNewOpd(false, addedInstance, target);
                ...
                if (mainReusedIns instanceof ObjectInstance) {
                    newMainIns = this.addReusedObject(mainReusedIns, x, y, newOpd, null);
                } else {
                    if (mainReusedIns instanceof ProcessInstance) {
                        newMainIns = this.addReusedProcess(mainReusedIns, x, y, newOpd, null);
                    }
                }
    ...
    protected LinkInstance addReusedLink(Instance reusedLinkInstance, Opd target) throws OpcatException {
        ...
        try {
            ...
            myLinkInstance =  this.currentProject.addLink(n_source, n_destination, lEntry.getLinkType(), target);
    ...
    protected void addReusedRelation(Instance reusedRelationInstance, boolean isGeneral) throws OpcatException {
        ...
        try {
            ...
            try {
	            n_source = (ConnectionEdgeInstance) this.reusedInstancesTable.get(source);
            ...
            if (isGeneral) {
            	...
            	GraphicalRelationRepresentation repr = locMyRel.getGraphicalRelationRepresentation();
            	RelativeConnectionPoint locsourcePoint = new RelativeConnectionPoint(locReuseRel.getSourceConnectionPoint().getSide(),locReuseRel.getSourceConnectionPoint().getParam() );

            	repr.getSourceDragger().setRelativeConnectionPoint(locsourcePoint);
            	repr.getSourceDragger().updateDragger();

            	OpdFundamentalRelation triangle = repr.getRelation();
            	OpdFundamentalRelation reusetriangle = locReuseRel.getGraphicalRelationRepresentation().getRelation() ;
            	
            	triangle.setLocation(reusetriangle.getX(), reusetriangle.getY());
            	triangle.setSize(reusetriangle.getWidth(), reusetriangle.getHeight());
            	repr.arrangeLines();
    ...
    protected void copyReusedProcessEntryProperties(ProcessEntry reused, ProcessEntry target) {    
    ...
    protected void emptyAndLockReusedInstancesGlobalList() {
        ...
        while (!this.reusedInstancesGlobalList.isEmpty()) {
            Instance aInstance = (Instance) this.reusedInstancesGlobalList
                    .getFirst();
            this.reusedInstancesGlobalList.removeFirst();
