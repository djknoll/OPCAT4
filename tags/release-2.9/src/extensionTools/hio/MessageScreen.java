...
public class MessageScreen {
 ...
 /**
  ...
  public void showMessage(OpdProject myProject_)
     {
       ...
       this.myMsgTimer = new MessageTimer();
       this.myMsgTimer.start();
  ...
  private void createMsgWindow()
  {
    ...
    this.msgScreen.setLocation(this.myProject.getMainFrame().getWidth()  -
                             this.msgScreen.getSize().width - 10,
                             this.myProject.getMainFrame().getHeight()  -
                             this.msgScreen.getSize().height - 35 );

    //open the window
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        MessageScreen.this.msgScreen.setVisible(true);
      }
    });
  	}
       ...
       private void hideWaitMessage()
       {
         ...
         SwingUtilities.invokeLater(new Runnable() {
           public void run() {
             MessageScreen.this.msgScreen.dispose();
           }
         });
          ...
          class RemindTask
            ...
            public void run() {
              //if the time expired close the window
                if ( MessageScreen.this.startTime + MessageScreen.this.showTime <= System.currentTimeMillis()) {
                    MessageScreen.this.hideWaitMessage();                  
                }
                MessageTimer.this.time.cancel(); //Terminate the timer thread
                     ...
                     //run the thread
                       public void run() {
                       		//create the timer
                       		this.time.schedule(new RemindTask(), System.currentTimeMillis() + MessageScreen.this.showTime - MessageScreen.this.startTime);                         
                         
                       		//if timer is done then kill the thread
                         try {
                         	MessageScreen.this.myMsgTimer.join();                         	
                         }
        ...
        */
        private void buildScreen() {
        	...
        	//create a window
          this.msgScreen = new JWindow(this.myProject.getMainFrame());
          ...
          mainPanel.add(this.buildMsg());
          mainPanel.add(this.buildButtonsPanel());
          mainPanel.add(this.buildCheckbox());

          this.msgScreen.getContentPane().add(mainPanel, BorderLayout.CENTER);
         ...
         */
         ...
         private JPanel buildButtonsPanel(){
           ...
           this.openTips = new Button("Show Tips");

           this.closeMsg = new Button("Close Window");

           ButtonsPanel.add(this.openTips);
           ButtonsPanel.add(this.closeMsg);
         ...
         private void addListeners(){
           ...
           this.closeMsg.addActionListener(
               new ActionListener() {
             ...
             this.openTips.addActionListener(
               new ActionListener() {
                    public void actionPerformed(ActionEvent e) {
                      MessageScreen.this.openTips();
                    }
                  }
             );

             //add a listener for the chackbox
             this.dontShow.addItemListener(new java.awt.event.ItemListener() {
        ...
        showMsg = false;
          ...
          */
          private Checkbox buildCheckbox(){          	 
            this.dontShow = new Checkbox("Don't show this message again");
