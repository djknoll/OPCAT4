    ...
    for (int i = 0; i < segments.size(); i++)
      for (int j = i + 1; j < segments.size(); j++) {
        ...
        if (!DrawAppNew.separateByLines(edgeval, mySegs)) {
          ...
          Point source = new Point(p1start.distance( midX, midY) <
                             p1end.distance( midX, midY) ?
                             p1end : p1start);
    setSourceEdgePoint(source);

    Point destination = new Point(p2start.distance( midX, midY) <
                                  p2end.distance( midX, midY) ?
                                  p2end : p2start);
