...
/**
 * This class is used to check the consistency of deleting a Instance of 
 * for now the only rule implemented is this - 
 * 
 * rule 1) when deleting, see if the deleted object is outside an zoomed-in opd if yes then return all the OPD's
 * 		   in which the pair (deleted, in-zoomed) are. and state a deletion action for the deleted instance in each of this opd's   
 * 
 * 
 * @author raanan
 *
...
public class DeletionChecker extends ConsistencyAbstractChecker implements ConsistencyCheckerInterface{
	...
	public void check() {	
		...
		ConsistencyAbstractChecker checker = new InzoomedDeletion (this.getMyOptions(), this.getResults()) ;
		checker.check() ; 	
		this.checkers.add(checker); 	
		//checkResults will collect all the actions and every deploy will use it's own ?
	...
	public void deploy(ConsistencyResult checkResult) {
		...
		for (int i = 0 ; i < this.checkers.size(); i++) {
			ConsistencyAbstractChecker checker = (ConsistencyAbstractChecker) this.checkers.get(i) ; 
			if(checker.isDeploy()) {
