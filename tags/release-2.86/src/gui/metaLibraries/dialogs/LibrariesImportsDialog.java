...
public class LibrariesImportsDialog extends JPanel {
	...
	public LibrariesImportsDialog(MetaManager meta, OpdProject _project, JFrame parent, String type) throws Exception {
		this.metaManager = (MetaManager)meta.clone();
		this.currentProject = _project;
		this.parentFrame = parent;
		if (this.metaManager == null) {
	...
	public void add_actionPerformed(ActionEvent e) {
		LibraryLocationDialog addLocation =
			new LibraryLocationDialog(this.parentFrame, "", MetaLibrary.TYPE_FILE);
		...
		if (newRef != null) {
			...
			MetaLibrary newLib = this.metaManager.createNewMetaLibraryReference(path, type);
			try	{
			...
			PathObject[] references = this.getPathsArray(this.metaManager.getDisplayLibraries());
			this.importedList.setListData(references);
	...
	public void edit_actionPerformed(ActionEvent e) {
		if (this.importedList.getSelectedValue() != null) {
			PathObject selectedPathValue = (PathObject)this.importedList.getSelectedValue();
			MetaLibrary edited = this.metaManager.findByPath(selectedPathValue.getFullPath());		
			if (edited == null)	{
				return;
			}
			LibraryLocationDialog addLocation =
				new LibraryLocationDialog(this.parentFrame,
					edited.getPath(),
			...
			if (ref != null) {
				...
				if ((!edited.getPath().equals(path)) || (edited.getReferenceType() != type))	{
					try	{
						this.metaManager.changeLibraryState(edited.getPath(), path, type);
					}
					catch (MetaException E)	{
						JOptionPane.showMessageDialog(this, E.getMessage(), "Message",
																		  JOptionPane.ERROR_MESSAGE);
...
//			String selectedValue = (String)importedList.getSelectedValue();
			try	{
				if (this.metaManager.projectHasRolesOfLib(candidate, this.currentProject))	{
					int retValue = 0;
					retValue =
