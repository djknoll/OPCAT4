...
//	public static final int STATE_EDITED = 5;
	...
	*/
	...
	private int CheckDataTypeByFileType(String path) {
		...
		if (path.endsWith(".csv") || path.endsWith(".txt") || path.endsWith("req")) {
...
//	}
	...
	public void setModelProject(Object projectHolder) throws NullPointerException	{
		...
		if (projectHolder instanceof OpdProject) {
			this.holder = new DataHolder(projectHolder , LibraryTypes.MetaLibrary);
		}
		
		if (projectHolder instanceof MetaDataProject) {
			this.holder = new DataHolder(projectHolder , LibraryTypes.MetaData);
		}
	...
	*/
	...
	public Collection getRolesCollection() throws MetaException	{
		...
		if(this.getType() == LibraryTypes.MetaData) {
			...
			while (iter.hasNext()) {
				MetaDataItem req = (MetaDataItem) iter.next() ; 
