...
/**
	...
	public RelationPropertiesDialog(GraphicalRelationRepresentation graphicalRelation, String dialogName, boolean isCardinalityEditable)
	{
		...
		{
			this.relationInstances[i] = (FundamentalRelationInstance)e.nextElement();
		}

		if(this.opdRelation instanceof OpdAggregation) {
			dialogName = "Aggregation-Particulation";
		}
...
//		applyButton.setEnabled(false);
		...
		KeyListener escapeAndEnterListener = new KeyAdapter()
		{
			public void keyReleased(KeyEvent e)
			{
				...
				{
					if(!RelationPropertiesDialog.this.destinationsTable.hasFocus())
					{
...
//			description.setEditable(false);
			...
			this.sourceName = (this.opdSourceThing.getEntity()).getName().replace('\n',' ');
			...
			this.sourceCardinality.setHorizontalAlignment(SwingConstants.CENTER);


			this.destinationsTable = new DestinationsTable(this.relationInstances.length, this.isCardinalityEditable);
			for (int j = 0 ; j < this.relationInstances.length; j++)
			{
				...
				this.destinationsTable.setValueAt(((this.relationInstances[j].getDestination()).getEntity()).getName().replace('\n',' '), j, 0);
				this.tmpCardinality = (this.relationInstances[j].getLogicalRelation()).getDestinationCardinality();
				if (this.tmpCardinality.compareTo("1") == 0)
								{
				  this.destinationsTable.setValueAt(this.tmpCardinality, j, 1);
				}
				...
				else
				{
					...
					this.destinationsTable.setValueAt(this.tmpCardinality, j, 2);
				}
			}
...
//		tab.add(p1);
		...
		this.p1.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Source "));
		//source
		this.p2 = new JPanel();
		...
		//destination
		this.destinationsTable.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(), " Destinations "));
	...
	private boolean _updateRelationData()
	{
		...
		{
			this.tmpCardinality = (this.destinationsTable.getValueAt(k, 1)).toString();
			if (this.tmpCardinality.compareTo("1") == 0)
			{
				(this.relationInstances[k].getLogicalRelation()).setDestinationCardinality(this.tmpCardinality);
			}
			else if (this.tmpCardinality.compareTo("many") == 0)
			{
				(this.relationInstances[k].getLogicalRelation()).setDestinationCardinality("m");
			}

			else if (this.tmpCardinality.compareTo("custom") == 0)
			{
				String str = (this.destinationsTable.getValueAt(k, 2)).toString();

				if(GraphicsUtils.checkCustomCardinalityLegality(str))
				{
					(this.relationInstances[k].getLogicalRelation()).setDestinationCardinality(str);
				}
				else
				{
		...
		this.myGraphicalRelation.setBackgroundColor(this.bgColor.getBackground());
		this.myGraphicalRelation.setTextColor(this.textColor.getBackground());
		this.myGraphicalRelation.setBorderColor(this.borderColor.getBackground());
