...
public class XmlMerger {
 ...
 public IXSystem mergeOpdTree(IXOpd parentOpd){
   ...
   Enumeration serverOpds = this.serverSystem.getIXSystemStructure().getOpds();
    while(serverOpds.hasMoreElements()){
     ...
     else if (tmpServerOpd.getParentIXOpd() == null) {
		continue;
	} else if (this.getOpdName(tmpServerOpd.getParentIXOpd()).compareTo(this.getOpdName(parentOpd))
         == 0) {
       curNodeChildrenOpds.addElement(tmpServerOpd);
     }

   }
   if(curNodeChildrenOpds.isEmpty()) {
	return this.clientSystem;
}
   /* merge children */
   this.merge( curNodeChildrenOpds.elements(),parentOpd);
   for (int i = 0; i< curNodeChildrenOpds.size() ; i++) {
     /* Recursive call for every children (already merged)*/
     this.mergeOpdTree((IXOpd)curNodeChildrenOpds.elementAt(i));
   }
   return this.clientSystem;
 }
  ...
  public IXSystem merge( Enumeration serverOpds, IXOpd parentOpd){
    ...
    IXSystemStructure clientStruct = this.clientSystem.
	...
	/* check if client has this opd*/
        ...
        if (this.getOpdName(tmpClientOpd).compareTo(this.getOpdName(tmpServerOpd)) == 0) {

            /* Client has this OPD, just merge inner structures of OPD*/
            boolean rc = this.copyOpd(tmpServerOpd,tmpClientOpd) ;
            if(rc == false) {
				return null;
			}
      ...
      if (!opd_exist_in_client) {
        ...
        this.createOpd(tmpServerOpd,parentOpd);
      }

    }
    this.clientSystem.updateChanges();
    return this.clientSystem;
  }
  ...
  public boolean createOpd(IXOpd serverOpd, IXOpd parentOpd)
  {
   ...
   // Add opd to client
   IXOpd tmpClientOpd = this.addOpd(this.clientSystem,this.serverSystem,serverOpd);
   if(tmpClientOpd == null) {
	return false;
}
    // copy internal structure
    this.copyOpd(serverOpd,tmpClientOpd);
    return true;
 ...
 /** Merge internal stucture of server into client server
  ...
  public boolean copyOpd( IXOpd serverOpd, IXOpd clientOpd) {
    ...
    IXSystemStructure serverStruct = this.serverSystem.getIXSystemStructure();
    ...
    while(tenum.hasMoreElements() == true){
      ...
      if (tObj instanceof IXThingInstance) {
        ...
        if (this.checkThingExisting(clientOpd,
       ...
       /* insert thing from server*/
        if (tObj instanceof IXObjectInstance) {
          this.insertObj(clientOpd,(IXObjectInstance)tObj, newEntry);

        } else {
          this.insertPrc(clientOpd, tObj, newEntry);
	  ...
	  // Merge states in inserted objects
         ...
         if(entry.serverEntry instanceof IXObjectEntry) {
            Vector newStates  =
             this.insertStatesOfObject((IXObjectInstance)entry.serverEntry.getIXInstance(entry.serverKey),
                              this.clientSystem,(IXObjectInstance)entry.clientEntry.getIXInstance(entry.clientKey));
             if (newStates != null) {
      ...
      for (int i=0; i<objectList.size(); i++) {
         ...
         // Merge links and relations in opd
          ...
          this.insertOpdLnks( entry.serverEntry,
                        clientOpd, serverOpd,
                        entry.clientEntry);
          ...
          if (!entry.existsInClient) {
            this.copyEntry(entry.serverEntry,entry.clientEntry);
  ...
  /** Check the existance of thing in client
    ...
    */
    ...
    private boolean checkThingExisting(
      ...
      IXSystemStructure clientStruct = this.clientSystem.getIXSystemStructure();
      String name = serverEntry.getName();
      Enumeration locenum = clientStruct.getThingsInOpd(clientOpd.getOpdId());
      while(locenum.hasMoreElements()==true){
        IXThingInstance ins = (IXThingInstance)locenum.nextElement();
  ...
  /** Add process to client
    ...
    */
	...
	/* process is done on opd */
        newProc = this.clientSystem.addProcess(tmp.getIXEntry().getName(),
                                      tmp.getX(),
                                      tmp.getY(),
                                      clientOpd.getOpdId());
     } else {
       ...
       newProc = this.clientSystem.addProcess(tmp.getIXEntry().getName(),
                                      tmp.getX(),
                                      tmp.getY(),
                                      this.clientSystem.getIXSystemStructure().getIXOpd(clientOpd.getOpdId()).getMainIXInstance());
 ...
 /** Add object to client
  ...
  private boolean insertObj(IXOpd clientOpd,
	...
	//object is done on opd
      newObjectInstance = this.clientSystem.addObject(objEntry.getName(),
                                             obj.getX(),
                                             obj.getY(),
                                             clientOpd.getOpdId());
    } else {
      newObjectInstance =
          this.clientSystem.addObject(objEntry.getName(),
                             obj.getX(),
                             obj.getY(),
                             this.clientSystem.getIXSystemStructure().getIXOpd(clientOpd.getOpdId()).getMainIXInstance());
  ...
  private boolean insertOpdLnks (IXConnectionEdgeEntry serverEntry,
...

   {
    ...
    rcLinks = this.insertLnksOfThing(serverLinks,newObject, clientOpd,serverOpd);
    ...
    rcRels = this.insertRelationsOfThing(serverRelations,this.clientSystem,this.serverSystem,newObject,clientOpd,serverOpd);
   ...
   private boolean insertLnksOfThing(Enumeration thingLinks,
      ...
      while(thingLinks.hasMoreElements()){
        ...
        if (!lnk.hasInstanceInOpd(serverOpd.getOpdId())) {
			continue;
		}
        long destId = lnk.getDestinationId();
	// destination entry in server
        IXEntry destinationEntry = this.getEntryById(destId,clientOpd,serverOpd);
        boolean rc = this.insertLnk(sourceEntry,destinationEntry,lnk, clientOpd);
        if(rc == false) {
			return false;
		}
       ...
       // find instances of source and destination entries in current opd
         ...
         while (tnum.hasMoreElements() == true) {
           ...
           if((sid==cid) && (cid == clientOpd.getOpdId())) { // link belongs to opd
             ...
             for(Enumeration e =(xSrcIns).getRelatedInstances();
                 e.hasMoreElements(); ) {
             ...
             IXLinkInstance newLinkIns = this.clientSystem.addLink(xSrcIns, xDstIns, type);
             if (newLinkIns != null) {
               /* copy properties */
               this.copyEntry (lnk,newLinkIns.getIXEntry());
               rc = true;
             } else {
				rc = false;
			}
    ...
    */
    private boolean insertRelationsOfThing(Enumeration thingRelations,
     ...
     while (thingRelations.hasMoreElements()) {
       ...
       if (!rel.hasInstanceInOpd(serverOpd.getOpdId())) {
		continue;
	}
       long destId = rel.getDestinationId();
       IXEntry destinationEntry = this.getEntryById(destId,clientOpd,serverOpd);

       boolean rc = this.insertRel(mySystem, sourceEntry, destinationEntry,
     ...
     while (snum.hasMoreElements() == true) {
       ...
       while (tnum.hasMoreElements() == true) {
         ...
         if((sid==cid) && (cid == clientOpd.getOpdId())) {
           ...
           if (newRelIns != null) {
             this.copyEntry (rel,newRelIns.getIXEntry());
             rc = true;
           }
           else  {rc = false; }
           }
 ...
 private IXEntry getEntryById(long destId, IXOpd clientOpd,IXOpd serverOpd){
   Enumeration lenum = this.serverSystem.getIXSystemStructure().getInstancesInOpd(serverOpd.getOpdId());
   ...
   while(lenum.hasMoreElements()){
     ...
     if (myObj.getIXEntry().getId() == destId){
       ...
       IXEntry myObjEntry = this.findInClient(this.clientSystem, myObj.getIXEntry(),clientOpd);
       return myObjEntry;
  ...
  public IXOpd addOpd(IXSystem clientSystem, IXSystem serverSystem, IXOpd serverOpd) {
   ...
   /* Search for parent of new opd in client */
    while(opds.hasMoreElements()){
     ...
     if (this.getOpdName(curClientOpd).compareTo(this.getOpdName(serverOpd.getParentIXOpd()))==0) {
       break;
    }
   ...
   while(tenum.hasMoreElements()){
     ...
     if (ins.getIXEntry().getName().compareTo(mainInstance.getIXEntry().getName())==0)  {
       ...
       while(instances.hasMoreElements()) {
         ...
         if (!(parentIns instanceof IXThingInstance)) {
			continue;
		}
         if (parentIns.getIXEntry().getName().compareTo(ins.getIXEntry().getName())==0) {
           if(((IXThingInstance)parentIns).getUnfoldingIXOpd() == null) {
			continue;
		}
       ...
       if (unfold) {
         clientOpd =  ins.createUnfoldedOpd(true, false);
         break;
 ...
 void getName(IXOpd opd) {
  ...
  public IXSystem mergeProject(String sClientFileName,String sServerFileName) throws gui.opx.LoadException,IOException,Exception
  {
    ...
    try{
      ...
      // fix_me - message
      if ((isC == null) || (isS == null)) {
		return null;
	}

      Loader ldC = new gui.opx.Loader(fC.getPath());
      Loader ldS = new gui.opx.Loader(fS.getPath());
      JProgressBar pBar = new JProgressBar();
      this.serverSystem = ldS.load(this.myOpcat2.getDesktop(), isS, pBar);
      this.clientSystem = ldC.load(this.myOpcat2.getDesktop(), isC, pBar);

      isS.close();;
      isC.close();
      this.myOpcat2.setCurrentProject(this.serverSystem);
      //test(serverSystem);
      mergedProject = this.mergeOpdTree ( null);
      //test( mergedProject);
      if(mergedProject == null){
    	  Opcat2.getCurrentProject().refresh();
      ...
      Opcat2.getCurrentProject().close();
      this.myOpcat2.setCurrentProject(mergedProject);
      Opcat2.getCurrentProject().refresh();
      this.myOpcat2.setProjectOpened(true);
      Opcat2.getCurrentProject().showRootOpd();
      ...
      (this.myOpcat2.getRepository()).setProject((OpdProject)mergedProject);
      this.myOpcat2.getRepository().rebuildTrees(true);
      Opcat2.getCurrentProject().updateChanges();
      Opcat2.getCurrentProject().refresh();
      /*JOptionPane.showMessageDialog(Opcat2.getFrame(),
                                    "Merge is done",
                                    "Message",
