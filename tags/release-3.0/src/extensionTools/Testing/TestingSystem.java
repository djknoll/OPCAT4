...
public class TestingSystem extends Object implements Testing {
    ...
    private Vector<TestingEntry> testingElements = new Vector<TestingEntry>();
	...
	if (testingTimer == null) {
	    lifeSpanGrid = new TestingLifeSpanGrid(mySys, this);
    ...
    public TestingEntry getTestingElementByIXEntryID(long ixEntryID) {
	...
	for (; en.hasMoreElements();) {
		...
		synchronized (currentEvent) {
			...
			if (currentEvent.getNumberOfInstances() == 0) {
			    ...
			    // }
			    // }
			    if (currentEvent.getEventType() == TestingSettings.ACTIVATION_EVENT) {
	...
	if (TestingSettings.STOP_AT_AGENT) {
	...
	if ((linkType == exportedAPI.OpcatConstants.AGENT_LINK)
		&& TestingSettings.STOP_AT_AGENT && !bManualActivated) {
	    return false;
	}
	...
	for (; en.hasMoreElements();) {
		...
		if (activatedEvent.getDestinationId() == aProcess.getIXEntry()
			...
			if (this.checkProcessBeforeActivate(aProcess,
				currentEvent)) {
			    /*
                                 * // copy parent and execution id from the
                                 * process
