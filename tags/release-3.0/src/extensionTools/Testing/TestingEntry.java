...
class TestingEntry extends Object {
	...
	if (TestingAuxiliary.isProcess(this.iEntry)) {
	    if (TestingSettings.MIN_TIME__PROCESS_DURATION
		    && this.isLowLevel()
		    && (java.lang.Math.round(java.lang.Math.ceil(getMinTime())) > 0)) {
		return java.lang.Math.round(java.lang.Math.ceil(getMinTime())) - 1;
	    } else if (!TestingSettings.FIXED_PROCESS_DURATION
		    && !TestingSettings.RANDOM_PROCESS_DURATION) {
	...
	for (; en.hasMoreElements();) {
	    ...
	    if ((Opd) FileControl.getInstance().getCurrentProject().getOpdByID(
		    i.getKey().getOpdId()) == null)
		continue;
	    if (!((Opd) FileControl.getInstance().getCurrentProject()
		    .getOpdByID(i.getKey().getOpdId())).isView()) {
    ...
    /**
         * sets the text info, if sevirity is less then current and if there is
         * no info the same we try to set
         * 
         * @param taxtualInfo
         * @param sevirityIndex
         */
