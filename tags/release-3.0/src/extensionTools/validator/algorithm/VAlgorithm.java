...
public class VAlgorithm {
	...
	Enumeration metaLibs = sys.getMetaLibraries(LibraryTypes.MetaLibrary);
	...
	while (metaLibs.hasMoreElements()) {
		...
		if (!lib.isPolicyLibrary())
		    continue;
		try {
		    flagedMetas.add(new Long(lib.getReferenceID()));
		    this.metaLibraries.put(new Long(lib.getReferenceID()), lib);
		    MetaReflection laws = new MetaReflection(lib.getISystem(),
			    lib.getReferenceID());
		    laws.compile();
		    this.metas.put(new Long(lib.getReferenceID()), laws);
		} catch (ValidationException ve) {
    ...
    /**
         * Creates the warnings data set from a given <code>Offence</code>
         * objects <code>Vector</code>.
         * 
         * @param offences
         * @param sys
         * @return
