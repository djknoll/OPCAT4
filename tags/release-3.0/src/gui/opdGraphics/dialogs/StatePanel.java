...
//          }
            ...
            public void  lockForEdit()
            {
              ...
              this.viewBox.setEnabled(false);
              this.autoarrange.setEnabled(false);
            }
		...
		private void _createListModel()
		{
		  ...
		  for(int i = 0; i < comps.length; i++)
		  {
			...
			{
			  OpdState tmpState = (OpdState)comps[i];
			  this.listModel.addElement(new StateListCell(tmpState, tmpState.isVisible()));
			}
		  }
	...
	class AddButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			ObjectEntry sampleObjEntry = new ObjectEntry(sampleObj, StatePanel.this.myProject);
			OpdObject sampleObjOpd = new OpdObject(sampleObjEntry, StatePanel.this.myProject, 0, 0);

			OpmState sampleOpmState = new OpmState(0, "");
			StateEntry sampleStateEntry = new StateEntry(sampleOpmState,sampleObj, StatePanel.this.myProject);
			StateInstance sampleSI = new StateInstance(sampleStateEntry, new OpdKey(0,0),
													sampleObjOpd, StatePanel.this.myProject);

			StatePropertiesDialog sd = new StatePropertiesDialog(sampleSI, StatePanel.this.myProject, false,
												 BaseGraphicComponent.SHOW_OK | BaseGraphicComponent.SHOW_CANCEL);

			if(sd.showDialog())
			{
				StateEntry myEntry = StatePanel.this.myProject.addState(StatePanel.this.parentOpdObject);
				...
				ObjectEntry parentEntry = (ObjectEntry)StatePanel.this.myProject.getComponentsStructure().getEntry(StatePanel.this.parentOpdObject.getEntity().getId());
				...
				for (Enumeration en = parentEntry.getInstances() ; en.hasMoreElements();)
				{
					...
					if (!pObj.getInstance().getKey().toString().equalsIgnoreCase(
							parentOpdObject.getInstance().getKey().toString())) {
						
						StateInstance stateInstance =  ((ObjectInstance) pObj.getInstance()).getState(
								nState.getName()) ; 
				...
				Opcat2.getUndoManager().undoableEditHappened(new UndoableEditEvent(StatePanel.this.myProject, new UndoableAddState(StatePanel.this.myProject, myEntry)));
				...
				for(Enumeration en = myEntry.getInstances(); en.hasMoreElements();)
				{
					...
					if(stateInst.getKey().getOpdId() == StatePanel.this.parentOpdObject.getOpdId())
					{
	...
	class RemoveButtonListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e)
		{
			...
			MainStructure ms = StatePanel.this.myProject.getComponentsStructure();
			...
			CheckResult cr = CheckModule.checkDeletion(si, StatePanel.this.myProject);
			...
			UndoableDeleteState ud = new UndoableDeleteState(StatePanel.this.myProject, se);

			StatePanel.this.myProject.deleteState(si);

			Opcat2.updateStructureChange(Opcat2.LOGICAL_CHANGE);
			Opcat2.getUndoManager().undoableEditHappened(new UndoableEditEvent(StatePanel.this.myProject, ud));
			Opcat2.setUndoEnabled(Opcat2.getUndoManager().canUndo());
			Opcat2.setRedoEnabled(Opcat2.getUndoManager().canRedo());
...
class CheckListCellRenderer extends JCheckBox implements ListCellRenderer
{
	...
	public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
		...
		this.setFont(list.getFont()); this.setBorder((cellHasFocus) ? UIManager.getBorder("List.focusCellHighlightBorder") : this.m_noFocusBorder);
		return this;
	}
}
...
class CheckListener implements MouseListener, KeyListener
{
	...
	protected void doCheck()
	{
		...
		StateListCell choosedState = (StateListCell)this.m_list.getModel().getElementAt(index);
		if(choosedState.isSelected())
		{
			OpdState opdState = (((StateListCell)(this.parentWin).listModel.get(index)).getOpdState());
			MainStructure ms = this.myProject.getComponentsStructure();
			StateEntry se = (StateEntry)ms.getEntry(opdState.getEntity().getId());
			StateInstance si = (StateInstance)(se.getInstance(new OpdKey(opdState.getOpdId(), opdState.getEntityInOpdId())));
			if(si.isRelated())
			{
				JOptionPane.showMessageDialog(this.parentWin, "You can't hide this state instance\nbecause it related to another thing.", "Opcat2 - Error" ,JOptionPane.ERROR_MESSAGE);
				return;
			}
		}
