...
public class GuiControl {
	...
	public void setCursor4All(final int cursorType) {
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				Opcat2.getFrame().setCursor(Cursor.getPredefinedCursor(cursorType));
				Opcat2.getOplPane().setCursor(Cursor.getPredefinedCursor(cursorType));

				if (Opcat2.getCurrentProject()!= null) {
					for (Enumeration e =
						Opcat2.getCurrentProject().getComponentsStructure().getOpds();
						e.hasMoreElements();
						) {
	...
	public void showMetaWaitMessage(String message, JProgressBar pBar) {
		if(pBar != null) {
			...
			this.handleWaitScreen(this.myOpcat.getMetaWaitScreen(), message, pBar);			
		} 
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
