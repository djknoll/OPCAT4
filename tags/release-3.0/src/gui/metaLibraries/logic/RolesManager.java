...
public class RolesManager {
    ...
    /**
         ...
         * 
         * @return <code>true</code> if there are any roles,
    ...
    /**
         * Sets an ontology role for the thing. A thing might have several
         * roles, and calling the method multiple times will result in multiple
         * roles being added. If the role is already in the list, it will not be
         * added again.
         * 
         * @param role
    ...
    /**
         * Adds an {@link IXRole} to the list. The method converts the role to
         * the implemented <code>Role</code> or <code>RoleEntry</code> form
         * and performs the original method.
         * 
         * @param _role
         *                The role to be added.
         */
    ...
    /**
         * Removes an {@link IXRole} from the list. The method converts the role
         * to the implemented <code>Role</code> form and performs the original
         * method.
         * 
         * @param role
         *                The role to be removed.
    ...
    /**
         ...
         * 
         * @return <code>Vector</code> containing Role objects. If no roles
         *         exist, the method returns null. The roles are ordered
         *         according to the order of their insertion.
         * @author Eran Toch
         */
    ...
    public Vector<Role> getRolesVector(int ontoType, String ontGlobID) {
	this.lazyInit();
	Vector<Role> selectedRoles = new Vector<Role>();
	Iterator<Role> iter = this.ontoRoles.iterator();
	while (iter.hasNext()) {
	    ...
	    if ((role.getOntology() != null)
		    && (role.getOntology().getGlobalID()
			    .equalsIgnoreCase(ontGlobID))
    ...
    public Vector<Role> getRolesVector(String ontGlobID) {
	this.lazyInit();
	Vector<Role> selectedRoles = new Vector<Role>();
	Iterator<Role> iter = this.ontoRoles.iterator();
	while (iter.hasNext()) {
	    ...
	    try {
		if ((role.getOntology().getGlobalID()
			.equalsIgnoreCase(ontGlobID))) {
		    selectedRoles.add(role);
		}
	    } catch (Exception ex) {
	...
	while (iter.hasNext()) {
	    Role role = (Role) iter.next();
	    if (role.isLoaded()) {
		if ((role.getOntology().isPolicyLibrary())
			&& (role.getOntology().getType() == LibraryTypes.MetaLibrary)) {
		    selectedRoles.add(role);
		}
	    }
	}
    ...
    /**
         ...
         * situation in which there are multiple roles. In this case, the name
         * of the meta-library is added to the name of each thing.
         * 
         * @author Eran Toch
         */
    ...
    /**
         * Returns a a <code>Vector</code> object containing pairs of Thing
         * name / Library Name.
         * 
         * @return A <code>Vector</code> object holding <code>Property</code>
         *         object, in which the thing name has the key of
         *         <code>RolesManager.THING_NAME</code> and the meta-library
         *         name is keyed under <code>RolesManager.LIBRARY_NAME</code>.
         */
    ...
    /**
         * Checks if two role managers are equals. Two role managers are equal
         * if each of their roles are equals and the order in which the roles
         * are arranged is the same.
         */
