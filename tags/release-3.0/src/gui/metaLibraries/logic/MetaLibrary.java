...
public class MetaLibrary implements Comparable {
    ...
    /**
         * Create a new ObjectInstance in targetOpd from the Role. (i.e. insert
         * from metalib action) if keepConected is true the new object will be
         * assigned the insertedRole.
         * 
         * @param insertedRole
         * @param targetOpd
         * @param keepConnected
	...
	if (roleThing instanceof ObjectEntry) {
	    ...
	    if (parent != null) {
		newInstance = Opcat2.getCurrentProject().addObject(
			x,
			y,
	    ...
	    try {
		newInstance.getEntry()
			.addRole(
				new Role(roleThing.getId(), insertedRole
					.getOntology()));
	    } catch (Exception ex) {
		OpcatLogger.logError(ex);
	    }
	    ...
	    /**
                 * ugly but does the job move those to metalib methods -
                 * insertObject(Role, Opd) ; insertProcess(Role, Opd)
                 */
	    ...
	    if (parent != null) {
		newObjectInstance = Opcat2.getCurrentProject().addObject(
			x,
			y,
			parent.getGraphicalRepresentation() , -1, -1, true);
	    } else {
	    ...
	    ObjectEntry locObject = (ObjectEntry) newObjectInstance
		    .getIXEntry();
	    try {
	    ...
	    newInstance = (ConnectionEdgeInstance) mystate
		    .getInstanceInOPD(targetOpd);

	} else {

	    if (parent != null) {
		newInstance = (ConnectionEdgeInstance) Opcat2
			.getCurrentProject().addProcess(roleThing.getName(), x,
				y, (ThingInstance) parent);
	    } else {
		newInstance = (ConnectionEdgeInstance) Opcat2
			.getCurrentProject().addProcess(roleThing.getName(), x,
				y, targetOpd.getOpdId());
	    }
